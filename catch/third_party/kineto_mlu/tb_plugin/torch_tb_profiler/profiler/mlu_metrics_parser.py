...
class MLUMetricsParser(object):
    ...
    def get_mlu_metrics(self):
        ...
        def build_trace_counter_sm_efficiency(mlu_id, start_time, counter_value):
            util_json = ("{{\"ph\":\"C\", \"name\":\"MLU {} Est. SM Efficiency\", \"pid\":{}, \"ts\":{}, "
                         "\"args\":{{\"Est. SM Efficiency\":{}}}}}").format(mlu_id, mlu_id, start_time, counter_value)
            return util_json
        ...
        def add_trace_counter_sm_efficiency(mlu_id, start_time, end_time, value, counter_json_list: List):
            efficiency_json_start = build_trace_counter_sm_efficiency(mlu_id, start_time, value)
            efficiency_json_finish = build_trace_counter_sm_efficiency(mlu_id, end_time, 0)
        ...
        for mlu_id, ranges in enumerate(self.approximated_sm_efficiency_ranges):
            buckets = self.mlu_util_buckets[mlu_id]
            if len(ranges) > 0 and len(buckets) > 0:
                # Adding 1 as baseline. To avoid misleading virtualization when the max value is less than 1.
                add_trace_counter_sm_efficiency(mlu_id, buckets[0][0], buckets[0][0], 1, counter_json_list)
            for r in ranges:
                add_trace_counter_sm_efficiency(mlu_id, r[0], r[1], r[2], counter_json_list)
