...
void CnpapiActivityProfiler::processTraceInternal(ActivityLogger& logger) {
  ...
  if (!cpuOnly_) {
    ...
    if (traceBuffers_->mlu) {
      const auto count_and_size = cnpapi_.processActivities(
          *traceBuffers_->mlu,
          std::bind(&CnpapiActivityProfiler::handleCnpapiActivity, this, std::placeholders::_1, &logger));
      LOG(INFO) << "Processed " << count_and_size.first
                << " MLU records (" << count_and_size.second << " bytes)";
      LOGGER_OBSERVER_ADD_EVENT_COUNT(count_and_size.first);

      // TODO(SYSTOOL-3625): Workaround for records with unexpected order.
      for (auto record : noLinkedMluRecords_) {
        handleCnpapiActivity(record, &logger);
      }
      noLinkedMluRecords_.clear();
...
inline void CnpapiActivityProfiler::handleCorrelationActivity(
    const cnpapiActivityExternalCorrelation* correlation) {
...
void CnpapiActivityProfiler::handleRuntimeActivity(
    ...
    ActivityLogger* logger) {
  if (activity->end - activity->start < 1000) {
    return;
  }
  ...
  // TODO(SYSTOOL-3625): Workaround for records with unexpected order.
  if (!linked) {
    auto result_pair = noLinkedMluRecords_.insert(reinterpret_cast<const cnpapiActivity*>(activity));
...
inline void CnpapiActivityProfiler::handleMluActivity(
  ...
  // TODO(SYSTOOL-3625): Workaround for records with unexpected order.
  if (!linked) {
    auto result_pair = noLinkedMluRecords_.insert(reinterpret_cast<const cnpapiActivity*>(act));
    if (result_pair.second) {
      return;
    }
  }
...
void CnpapiActivityProfiler::handleCnpapiActivity(const cnpapiActivity* record, ActivityLogger* logger) {
  switch (record->type) {
    case CNPAPI_ACTIVITY_TYPE_EXTERNAL_CORRELATION:
      handleCorrelationActivity(
          reinterpret_cast<const cnpapiActivityExternalCorrelation*>(
              record));
      break;
    ...
    case CNPAPI_ACTIVITY_TYPE_CNRT_API:
      handleRuntimeActivity(
          reinterpret_cast<const cnpapiActivityAPI*>(record), logger);
      break;
    ...
    case CNPAPI_ACTIVITY_TYPE_OVERHEAD:
      handleOverheadActivity (
          reinterpret_cast<const cnpapiActivityOverhead*>(record), logger);
      break;
    default:
