...
PYTORCH_BRANCH="pytorch_1.13.1"
CATCH_BRANCH='catch_1.13.1_develop'
VISION_BRANCH='vision_0.14.1'
PYTORCH_MODELS_BRANCH="v0.24.0-torch1.13.1"
DRIVER_VERSION=NULL
...
#IMAGE_DOCKER_NAME=base_image_yy2
...
while getopts "r:b:c:p:n:x:y:z:o:v:w:d:t:f:a:" opt
do
  case $opt in
    r)
	      RELEASE_TYPE=$OPTARG;;
...
fetch_cn_dep_func() {
  ...
  if [ -f "/etc/os-release" ]; then
    ...
    if [ ${OS_TYPE} == "ubuntu" ] || [ ${OS_TYPE} == "debian" ] || [ ${OS_TYPE} == "centos" ]; then
      ...
      do
        ...
        if [ ${arr_modules[$i]} == "cntoolkit" ]; then
          ...
          if [ ${arr_modules[$i]} == "cnpapi" ]; then
            if [[ ${GLIBCXX_USE_CXX11_ABI} == "1" ]]; then
              PACKAGE_FILE=${arr_modules[$i]}"_"${arr_vers[$i]}".abinew."${OS_TYPE}${OS_VERSION}"_${PLATFORM}.deb"
            else
              PACKAGE_FILE=${arr_modules[$i]}"_"${arr_vers[$i]}".abiold."${OS_TYPE}${OS_VERSION}"_${PLATFORM}.deb"
            fi
...
pack_src_func(){
  ...
  cp ${PYTORCH_SRC_PACKAGE}/catch/scripts/release/env_pytorch.sh .
  cp ${PYTORCH_SRC_PACKAGE}/catch/scripts/release/build_cambricon_pytorch_catch.sh .
