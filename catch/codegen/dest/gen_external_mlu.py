...
class GenExternalMLU:
    ...
    def gen_unstructured(self, f: NativeFunction):
        ...
        postfix = "_autograd" if self.aux[f.func.name].get(
            'custom_autograd', None) else ""
        if self.target == Target.CNNL_KERNEL_DECLARATION:
            if derived_type == 'cnnl':
                return f"""\
{returns_type} {derived_type}_{impl_name}{postfix}({args_str});
"""
...
"""
    ...
    def gen_structured(self, g: NativeFunctionsGroup) -> List[str]:
        ...
        # NB: If metadata is not None, we will reuse declarations generated by Pytorch.
        # If metadata is None, we generate our own declarations.
...
"""
            ...
            postfix = "_autograd" if self.aux[f.func.name].get(
            'custom_autograd', None) else ""
            impl_name = f"torch_mlu::ops::{derived_type}_{impl_name}{postfix}"
...
"""
            ...
            # TensorIterator is used and MLU reuse the name.
            # 2. If metadata is not, we generate MLU unique structured kernel
