...
def run(options) -> None:
    ...
    # parse vision/audio/custom/custom_autograd functions in mlu_functions.yaml
    ...
    aux_custom_autograd, custom_autograd_functions = parse_mlu_custom_yaml(
        options.source_yaml, tags_yaml_path, 'custom_autograd', options)

    functions = aten_functions + vision_functions + \
        custom_functions + custom_autograd_functions + audio_functions
    aux = {
        **aux_aten,
        **aux_vision,
        **aux_audio,
    ...
    fm.write('RegisterMLU.cpp', lambda: {
        ...
        'dispatch_custom_autograd_registrations': list(concatMap(
            RegisterMLU(Target.REGISTRATION, selector,
                        aux, symint=True), custom_autograd_functions
        )),
