...
static PyObject * THMPEvent_pynew(
  ...
  unsigned int flags = (enable_timing ? CNRT_NOTIFIER_DEFAULT : CNRT_NOTIFIER_DISABLE_TIMING) |
      (interprocess ? CNRT_NOTIFIER_INTERPROCESS | CNRT_NOTIFIER_DISABLE_TIMING_ALL : CNRT_NOTIFIER_DEFAULT);

  new (&self->mlu_event) torch_mlu::Notifier(flags);
...
static PyObject* THMPEvent_from_ipc_handle(
  ...
  TORCH_CHECK(
      device.type() == at::kMLU,
      "Notifier can only be created on "
  ...
  new (&self->mlu_event) torch_mlu::Notifier(device.index(), &handle);
...
static void THMPEvent_dealloc(THMPEvent *self) {
  self->mlu_event.~Notifier();
  Py_TYPE(self)->tp_free((PyObject*)self);
}

static PyObject * THMPEvent_get_mlu_event(THMPEvent *self, void *unused) {
  HANDLE_TH_ERRORS
  return PyLong_FromVoidPtr(self->mlu_event.notifier());
  END_HANDLE_TH_ERRORS
}
...
static PyMethodDef THMPEvent_methods[] = {
  ...
  {(char*)"synchronize", THMPEvent_synchronize,
    METH_NOARGS, nullptr},
...
PyTypeObject THMPEventType = {
  ...
  "torch_mlu._MLUC._MLUEventBase",             /* tp_name */
...
void THMPEvent_init(PyObject *module) {
  ...
  if (PyModule_AddObject(
      module, "_MLUEventBase", (PyObject *)&THMPEventType) < 0) {
    throw python_error();
  }
}
