...
std::tuple<int, int, int> get_cncl_version() {
  ...
  C10D_CNCL_CHECK(cnclGetLibVersion(&major, &minor, &patch));
...
void init_internal(PyObject *module) {
  ...
  auto torch_c10d_module = THPObjectPtr(PyImport_ImportModule("torch.distributed"));
  ...
  // register cncl backend
  py::object backend = torch_c10d_m.attr("Backend");
  if (!py::hasattr(backend, "CNCL")) {
    py::object register_backend = backend.attr("register_backend");
    // The first parameter is the backend name used by user in invoking
    register_backend("cncl", py::cpp_function(ProcessGroupCNCL::createProcessGroupCNCL));
  }

  if (py::hasattr(torch_c10d_m, "ProcessGroup")) {
    auto processGroupCNCL =
        intrusive_ptr_no_gil_destructor_class_<ProcessGroupCNCL>(
            module, "ProcessGroupCNCL", torch_c10d_m.attr("ProcessGroup"))
            .def(
                py::init<
            ...
            .def_property_readonly(
                "options", &ProcessGroupCNCL::getOptions);
