...
void resize_bytes_mlu(
  ...
  if (size_bytes == 0) {
    storage->set_data_ptr(at::DataPtr(nullptr, at::Device(at::DeviceType::MLU, device)));
    ...
    if (storage->data_ptr()) {
      ...
      auto queue = torch_mlu::getCurrentQueue();
...
static PyObject* THMPStorage_nbytes(PyObject* _self, PyObject* noargs) {
  HANDLE_TH_ERRORS
...
static PyObject* THMPStorage_dataPtr(PyObject* _self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
...
static PyObject* THMPStorage_isPinned(PyObject* _self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
  return PyBool_FromLong(
      torch_mlu::isPinnedPtr(self->cdata->data<uint8_t>()));
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THMPStorage_new(PyObject* _self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
...
static PyObject* THMPStorage_resize_(PyObject* _self, PyObject* number_arg) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
  THPUtils_assert(
  ...
  if (device_type == at::kCPU) {
    ...
    resize_bytes_mlu(self->cdata, size_bytes);
  } else {
    TORCH_CHECK(
        false,
...
static PyObject* THMPStorage_fill_(PyObject* _self, PyObject* number_arg) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
  THPUtils_assert(
...
static PyObject* THMPStorage_fromBuffer(
  ...
  if (offset < 0 || offset > buffer.len) {
    std::string msg = "offset must be non-negative and no greater than buffer length ("
  ...
  if (count < 0) {
    if ((buffer.len - offset) % element_size != 0) {
      std::string msg = "buffer size ( " + std::to_string(buffer.len) +
          ") must be a multiple of element size (" + std::to_string(element_size) + ")";
      PyErr_SetString(PyExc_ValueError, msg.c_str());
      PyBuffer_Release(&buffer);
      return nullptr;
    }
  ...
  if (offset + (count * (Py_ssize_t)element_size) > buffer.len) {
    std::string msg = "buffer has only " + std::to_string(buffer.len - offset) +
        " elements after offset " + std::to_string(offset) + ", but specified a size of " +
        std::to_string(count);
    PyErr_SetString(PyExc_ValueError, msg.c_str());
    PyBuffer_Release(&buffer);
    return nullptr;
...
PyObject* THMPStorage_writeFile(PyObject* _self, PyObject* args) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
  ...
  THPUtils_assert(
      ...
      "from given object");
  if (self->cdata->device_type() == at::kMLU) {
    THMPStorage_writeFileRaw(self->cdata, fd, save_size, element_size);
  } else {
...
static PyObject* THMPStorage_setFromFile(PyObject* _self, PyObject* args) {
  HANDLE_TH_ERRORS
  auto self = (THMPStorage*)_self;
  ...
  if (!is_real_file) {
    ...
    auto storage = self_storage->device_type() == at::kMLU ?
        THMPStorage_readFileRaw<PyObject*>(file, std::move(self_storage), element_size) :
        THPStorage_readFileRaw<PyObject*>(file, std::move(self_storage), element_size);
    if (!storage.defined()) {
      return nullptr;
    }
    Py_INCREF(self);
  ...
  auto self_storage =
      c10::intrusive_ptr<c10::StorageImpl>::reclaim_copy(self->cdata);
  auto storage = self_storage->device_type() == at::kMLU ?
      THMPStorage_readFileRaw<int>(fd, self_storage, element_size) :
      THPStorage_readFileRaw<int>(fd, self_storage, element_size);
  if (!storage.defined())
    return nullptr;
  Py_INCREF(self);
...
static PyMethodDef THMPStorage_methods[] = {
    ...
    {"is_pinned", THMPStorage_isPinned, METH_NOARGS, nullptr},
