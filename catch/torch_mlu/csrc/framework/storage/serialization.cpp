...
c10::intrusive_ptr<c10::StorageImpl> THMPStorage_readFileRaw(
  ...
  if (!storage.defined()) {
    storage = c10::make_intrusive<at::StorageImpl>(
        ...
        nbytes,
        torch_mlu::getMLUCachingAllocator(),
        /*resizable=*/true);
  } else {
  ...
  TORCH_CNRT_CHECK(cnrtMemcpy(storage->data<uint8_t>(),
                   data, nbytes, cnrtMemcpyHostToDev));
  return storage;
}
template c10::intrusive_ptr<c10::StorageImpl> THMPStorage_readFileRaw<int>(
    ...
    c10::intrusive_ptr<c10::StorageImpl> storage,
    uint64_t element_size);
template c10::intrusive_ptr<c10::StorageImpl> THMPStorage_readFileRaw<PyObject*>(
    PyObject* fd,
    c10::intrusive_ptr<c10::StorageImpl> storage,
    uint64_t element_size);
