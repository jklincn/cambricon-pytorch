...
bool hasPrimaryContext(int64_t device_index) {
  TORCH_MLU_CHECK(device_index >= 0 && device_index < device_count(),
    "hasPrimaryContext expects a valid device index, but got device_index=", device_index);
  unsigned int flag = 0;
  int state = 0;
  TORCH_CNDRV_CHECK(cnSharedContextGetState(static_cast<int64_t>(device_index), &flag, &state));
...
c10::optional<int64_t> getDevceIndexWithSharedContext() {
  ...
  if (hasPrimaryContext(current_device_index)) return current_device_index;
  ...
  for (int16_t device_index = 0; device_index < num_devices; ++device_index) {
    ...
    if (hasPrimaryContext(device_index)) return device_index;
  }
  return c10::nullopt;
}
...
at::Device MLUHooks::getDeviceFromPtr(void* ptr) const {
  ...
  TORCH_MLU_CHECK(attr.type == cnrtMemTypeUnregistered,
    "Memeory is not registered in MLU side.");
  return {at::kMLU, static_cast<c10::DeviceIndex>(attr.device)};
}

void MLUHooks::initCUDA() const {
  const auto num_devices = device_count();
  TORCH_CHECK(num_devices, "No MLUs are available");
  torch_mlu::init(num_devices);
}
...
bool MLUHooks::isPinnedPtr(void* data) const {
  return torch_mlu::isPinnedPtr(data);
}

int64_t MLUHooks::cuFFTGetPlanCacheMaxSize(int64_t device_index) const {
  return ops::detail::cnfft_get_plan_cache_max_size_impl(device_index);
}

void MLUHooks::cuFFTSetPlanCacheMaxSize(int64_t device_index, int64_t max_size) const {
  ops::detail::cnfft_set_plan_cache_max_size_impl(device_index, max_size);
}

int64_t MLUHooks::cuFFTGetPlanCacheSize(int64_t device_index) const {
  return ops::detail::cnfft_get_plan_cache_size_impl(device_index);
}

void MLUHooks::cuFFTClearPlanCache(int64_t device_index) const {
  ops::detail::cnfft_clear_plan_cache_impl(device_index);
}

bool MLUHooks::hasPrimaryContext(int64_t device_index) const {
