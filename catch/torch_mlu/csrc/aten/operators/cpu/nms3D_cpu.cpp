...
void nms3D_cpu_kernel(float* output_data,
                      int& output_box_num,
                      float* input_data,
                      int input_box_num,
                      float thresh_iou) {
...
at::Tensor nms3D_cpu(const at::Tensor & dets, double iou_threshold) {
  ...
  TORCH_CHECK(dets_contiguous.dim() == 2,
              "boxes should be a 2D tensor, got ", dets_contiguous.dim(), "D");
  TORCH_CHECK(dets_contiguous.size(1) == 7,
              "boxes sizes should be (batch_size, 7).");
  TORCH_CHECK(dets.scalar_type() == at::ScalarType::Float,
             "dets dtype should be float");
  int input_box_num = (int)dets.size(0);
  auto dtype = dets.scalar_type();
  auto output = at::empty({input_box_num},
                          dets_contiguous.options().dtype(dtype));
