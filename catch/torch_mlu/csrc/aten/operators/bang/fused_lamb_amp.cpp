...
void _fused_lamb_amp_common(
  ...
  TORCH_CHECK(global_grad_norm.scalar_type() == at::ScalarType::Float,
      "MultiTensorLAMBAMP: The data type of global_grad_norm must be float");
  TORCH_CHECK(learning_rate.scalar_type() == at::ScalarType::Float,
      "MultiTensorLAMBAMP: The data type of learning_rate must be float");
  TORCH_CHECK(step.scalar_type() == at::ScalarType::Int,
      "MultiTensorLAMBAMP: The data type of step must be int32")
  TORCH_CHECK(noop_flag.scalar_type() == at::ScalarType::Int,
      "MultiTensorLAMBAMP: The data type of overflow must be int32")
  TORCH_CHECK(exp_avgs[0].scalar_type() == at::ScalarType::Float,
      "MultiTensorLAMBAMP: The data type of exp_avgs must be float");
  TORCH_CHECK(exp_avg_sqs[0].scalar_type() == at::ScalarType::Float,
  ...
  auto bias_correction_cvt = c10::checked_convert<int, int64_t>(bias_correction, "int");
  auto weight_decay_cvt = c10::checked_convert<float, double>(weight_decay, "float");
  auto grad_averaging_cvt = c10::checked_convert<int, int64_t>(grad_averaging, "int");
  ...
  int* noop_flag_ptr = static_cast<int*>(getMluTensorImpl(noop_flag)->mlu_data_ptr());
  float* lr_ptr = static_cast<float*>(getMluTensorImpl(learning_rate)->mlu_data_ptr());
  int* step_ptr = static_cast<int*>(getMluTensorImpl(step)->mlu_data_ptr());
  float* ggn_ptr = static_cast<float*>(getMluTensorImpl(global_grad_norm)->mlu_data_ptr());
  float* inv_scale_ptr = static_cast<float*>(getMluTensorImpl(inv_scale)->mlu_data_ptr());
  float* max_grad_norm_ptr = static_cast<float*>(getMluTensorImpl(max_grad_norm)->mlu_data_ptr());

  // compute workspace
  auto tensor_num = grads.size();
  auto workspace_size = sizeof(float) * (tensor_num * 2 + 1);
  at::Tensor workspace = \
      at::zeros(workspace_size, at::TensorOptions().dtype(at::kChar).device(at::kMLU));
  float* fp_workspace = static_cast<float*>(getMluTensorImpl(workspace)->mlu_data_ptr());
  ...
  for (size_t tensor_id = 0; tensor_id < tensor_num; ++tensor_id) {
    ...
    g.addresses[tensor_count] = getMluTensorImpl(grad_contiguous)->mlu_data_ptr();
    m.addresses[tensor_count] = getMluTensorImpl(exp_avg_contiguous)->mlu_data_ptr();
    v.addresses[tensor_count] = getMluTensorImpl(exp_avg_sq_contiguous)->mlu_data_ptr();
    p.addresses[tensor_count] = getMluTensorImpl(param_contiguous)->mlu_data_ptr();
    if (is_amp) {
      auto dst_param_contiguous = contiguous_tensors_list[tensor_id][0];
      half_p.addresses[tensor_count] = getMluTensorImpl(dst_param_contiguous)->mlu_data_ptr();
    }
    ...
    if (tensor_count == MAX_TENSOR_NUM) {
      bang_fused_lamb_amp_internal(
          g, p, m, v, half_p,
          sizes, tensor_count, noop_flag_ptr, beta1_cvt, beta2_cvt, grad_averaging_cvt,
          step_ptr, bias_correction_cvt, epsilon_cvt, mode_cvt, weight_decay_cvt, ggn_ptr,
          max_grad_norm_ptr, fp_workspace + tensor_offset,
          fp_workspace + tensor_offset + tensor_num, inv_scale_ptr, lr_ptr,
          use_nvlamb_python, k_dim, k_type, queue,
          grad_cnrt_type, param_cnrt_type);
      tensor_count = 0;
  ...
  for (size_t i = 0; i < tensor_num; ++i) {
    if (is_copy_necessary(exp_avgs[i],
       contiguous_tensors_list[i][n_tensors_list - 3])) {
      exp_avgs[i].copy_(contiguous_tensors_list[i][n_tensors_list - 3]);
    }
    if (is_copy_necessary(exp_avg_sqs[i],
       contiguous_tensors_list[i][n_tensors_list - 2])) {
      exp_avg_sqs[i].copy_(contiguous_tensors_list[i][n_tensors_list - 2]);
    }
    if (is_copy_necessary(params[i],
       contiguous_tensors_list[i][n_tensors_list - 1])) {
...
bool bang_fused_lamb_amp(
  ...
  TORCH_CHECK(grads[0].scalar_type() == params[0].scalar_type(),
      "MultiTensorLAMBAMP: The data type of grads and params must be the same")

  std::vector<at::Tensor> sub_tensors_list;
  std::vector<std::vector<at::Tensor>> contiguous_tensors_list(grads.size(), sub_tensors_list);
...
bool bang_fused_lamb_amp(
  ...
  TORCH_CHECK(grads[0].scalar_type() == at::ScalarType::Half,
      "MultiTensorLAMBAMP: The data type of grad should be half.")
  TORCH_CHECK(dst_param_fp16s[0].scalar_type() == at::ScalarType::Half,
      "MultiTensorLAMBAMP: The data type of dst_param_fp16s should be half.")
  TORCH_CHECK(params[0].scalar_type() == at::ScalarType::Float,
  ...
  for (size_t i = 0; i < tensor_num; ++i) {
    if (is_copy_necessary(dst_param_fp16s[i],
        contiguous_tensors_list[i][0]))
