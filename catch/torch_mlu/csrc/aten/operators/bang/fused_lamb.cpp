...
bool bang_fused_lamb(
  ...
  float* global_grad_norm_ptr = static_cast<float*>(getMluTensorImpl(global_grad_norm)->mlu_data_ptr());
  int32_t* overflow = static_cast<int32_t*>(getMluTensorImpl(_dummy_overflow_buf)->mlu_data_ptr());

  // compute workspace
  auto workspace_size = sizeof(float) * (tensor_num * 2 + 1);
  at::Tensor workspace = at::zeros(workspace_size, at::TensorOptions().dtype(at::kChar).device(at::kMLU));
  ...
  float correction_rate = std::sqrt(beta2_correction_recip) / beta1_correction_recip;
  ...
  for (int tensor_id = 0; tensor_id < tensor_num; ++tensor_id) {
    ...
    if (tensor_count == MAX_TENSOR_NUM) {
      bang_fused_lamb_internal(
          g, p, m, v,
          sizes, global_grad_norm_ptr, tensor_count, learning_rate, beta1, beta2,
          epsilon_correction, weight_decay, correction_rate, max_grad_norm,
          mode, grad_averaging, fp_workspace + tensor_offset,
          fp_workspace + tensor_offset + tensor_num,
          overflow, use_nvlamb_python, k_dim, k_type, queue, cnrt_type);
      tensor_count = 0;
