...
bool bang_fused_sgd(const at::Tensor& _dummy_overflow_buf,
                    at::TensorList grads,
  ...
  cnrtDataType_t in_type = cnnlType2CnrtType(getCnnlType(getMluTensorImpl(grads[0])));
  cnrtDataType_t out_type = cnnlType2CnrtType(getCnnlType(getMluTensorImpl(params_in[0])));
  ...
  TORCH_CHECK(_dummy_overflow_buf.device() == grads[0].device(),
  ...
  for (int tensor_id = 0; tensor_id < tensor_num; ++tensor_id) {
    ...
    TORCH_CHECK(is_contiguous_tensor(grad) && is_contiguous_tensor(in) &&
                is_contiguous_tensor(mt), "Found param with index=", tensor_id,
                                          " was not contiguous.");

    int64_t num_elements = grad.numel();
    TORCH_CHECK(in.numel() == num_elements,
                "Size mismatch. The numel of grad and param should be equal."
                " but grad is ", num_elements, ". param is ", in.numel());
    TORCH_CHECK(mt.numel() == num_elements,
                "Size mismatch. The numel of grad and momentums should be equal."
                " but grad is ", num_elements, ". momentums is ", mt.numel());
    if (num_elements == 0) {
    ...
    if (tensor_count == MAX_TENSOR_NUM) {
      bang_fused_sgd_internal(g, i, m, o, s, tensor_count, overflow,
          weight_decay, momentum, dampening, learning_rate,
          nesterov, first_run, wd_after_momentum, scale,
          k_dim, k_type, queue, in_type, out_type, N);
      tensor_count = 0;
    }
  }
