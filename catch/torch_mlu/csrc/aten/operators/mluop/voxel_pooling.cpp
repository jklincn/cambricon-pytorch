...
bool mluop_voxel_pooling(int64_t batch_size,
                         int64_t num_points,
                         ...
                         at::Tensor & pos_memo) {
  TORCH_MLU_CHECK((batch_size > 0 && num_points > 0 && num_channels > 0 &&
    ...
    num_voxel_y, ", ",
    num_voxel_z);
  TORCH_MLU_CHECK(geom_xyz.is_mlu(), "geom_xyz must be a MLU tensor.");
  TORCH_MLU_CHECK(input_features.is_mlu(), "input_features must be a MLU tensor.");
  TORCH_MLU_CHECK(geom_xyz.scalar_type() == at::ScalarType::Int &&
    pos_memo.scalar_type() == at::ScalarType::Int,
    "geom_xyz and pos_memo must be int tensor.");
  TORCH_MLU_CHECK(input_features.scalar_type() == at::ScalarType::Float &&
  ...
  mluop_voxel_pooling_internal(geom_xyz_contiguous, input_features_contiguous,
                               batch_size, num_points, num_channels, num_voxel_x,
                               num_voxel_y, num_voxel_z, output_features, pos_memo);
