...
static inline detail::CnFFTParamsLRUCache &cnfft_get_plan_cache(int64_t device_index) {
...
int64_t cnfft_get_plan_cache_max_size_impl(int64_t device_index) {
  TORCH_MLU_CHECK(0 <= device_index && device_index < device_count(),
    "cnfft_get_plan_cache_max_size: expected 0 <= device_index < ",
    device_count(), "], but got device_index=", device_index);
...
const at::Tensor& cnnl_fft_internal(at::Tensor& out, const at::Tensor& self,
  c10::IntArrayRef out_sizes, c10::IntArrayRef dim, bool forward, const float scale_factor) {
  ...
  auto fft_type = detail::get_cnfft_transformtype(input.is_complex(), out.is_complex());
  detail::CnFFTParams params(input.strides(), out.strides(), signal_size, fft_type, value_type);
  detail::CnFFTParamsLRUCache& plan_cache = cnfft_get_plan_cache(input.device().index());
  ...
  TORCH_CNNL_CHECK(cnnlSetFFTReserveArea(handle, plan, config->get_reservespace_ptr()));

  // run
  auto ws = at::empty({config->workspace_size()}, at::device(at::kMLU).dtype(at::kByte));
  TORCH_CNNL_CHECK(cnnlExecFFT(
    /* handle       */ handle,
    /* fft_plan     */ plan,
    /* input        */ getMluTensorImpl(input)->mlu_data_ptr(),
    /* scale_factor */ scale_factor,
    /* workspace    */ getMluTensorImpl(ws)->mlu_data_ptr(),
