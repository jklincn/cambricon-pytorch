...
namespace torch_mlu { namespace ops { namespace detail {

constexpr int kMaxRank = 1;  // currently only support 1-d RFFT because of CNNL limitation
...
struct CnFFTParams {
  ...
  CnFFTParams(c10::IntArrayRef in_strides, c10::IntArrayRef out_strides,
      c10::IntArrayRef signal_sizes, CnFFTTransformType fft_type, at::ScalarType value_type) {
...
inline CnFFTTransformType get_cnfft_transformtype(bool complex_input, bool complex_output) {
...
public:
  ...
  ::cnnlFFTPlan_t & get() { return plan_; }
  const ::cnnlFFTPlan_t & get() const { return plan_; }
...
class CnFFTConfig {
 public:
  ...
  explicit CnFFTConfig(const CnFFTParams& params, const at::Tensor& input,
                       const at::Tensor& output)
      : CnFFTConfig(c10::IntArrayRef(params.sizes_, params.signal_ndim_ + 1),
                    input, output, params.value_type_) {}

  // sizes are for the full signal, including batch size and always two-sided
  CnFFTConfig(c10::IntArrayRef sizes, const at::Tensor& input,
              const at::Tensor& output, at::ScalarType value_type) {
    ...
    input_desc.set_onchip_dtype(value_type == at::ScalarType::Half
                                      ? CNNL_DTYPE_HALF
                                      : CNNL_DTYPE_FLOAT);

    size_t ws_size_t, rs_size_t;
    TORCH_CNNL_CHECK(cnnlMakeFFTPlanMany(
        getCurrentHandle(), plan(), input_desc.desc(), output_desc.desc(),
        signal_ndim, signal_sizes.data(), &rs_size_t, &ws_size_t));
  ...
  const cnnlFFTPlan_t &plan() const { return plan_ptr.get(); }
...
public:
  ...
  using map_t = typename std::unordered_map<std::reference_wrapper<CnFFTParams>,
                                            typename std::list<kv_t>::iterator,
                                            torch_mlu::ParamsHash<CnFFTParams>,
                                            torch_mlu::ParamsEqual<CnFFTParams>>;
  using map_kkv_iter_t = typename map_t::iterator;

  // Different from cuFFT, CNNL FFT currently do not have the max plan number limitation.
  CnFFTParamsLRUCache() : CnFFTParamsLRUCache(std::numeric_limits<int64_t>::max()) {}
  ...
  // created.
  const CnFFTConfig &lookup(CnFFTParams params, const at::Tensor& input,
                            const at::Tensor& output) {
...
private:
  ...
  void _set_max_size(int64_t new_size) {
    TORCH_CHECK(new_size >= 0,
             "cnFFT plan cache size must be non-negative, but got ", new_size);
...
}}}  // namespace torch_mlu::ops::detail
