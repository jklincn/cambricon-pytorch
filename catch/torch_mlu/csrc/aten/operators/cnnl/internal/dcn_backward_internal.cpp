...
cnnl_dcn_backward_internal(
  ...
  dcn_desc.set(input.dim(), padding, stride,
               dilation, deformable_group, conv_group,
               im2col_step, CNNL_DTYPE_FLOAT);
  // set onchip dtype
  cnnlSetTensorDescriptorOnchipDataType(grad_desc.desc(), getCnnlDataType(grad.dtype()));
  cnnlSetTensorDescriptorOnchipDataType(input_desc.desc(), getCnnlDataType(input.dtype()));
  cnnlSetTensorDescriptorOnchipDataType(weight_desc.desc(), getCnnlDataType(weight.dtype()));
  // prepare mask desc
  void* mask_ptr = nullptr;
  void* grad_mask_ptr = nullptr;
  at::Tensor grad_mask;
  if (use_mask) {
    grad_mask = at::empty(mask.sizes(), mask.options().dtype(input.scalar_type()), memory_format);
  ...
  if (bias.defined() && bias.dim() == 1 && bias.size(0) == grad.size(1)) {
    grad_bias = at::empty(bias.sizes(), bias.options().dtype(input.scalar_type()));
  ...
  if (grad_input_mask[0]) {
    ...
    auto data_workspace_impl = getMluTensorImpl(data_workspace);
  ...
  if (grad_input_mask[1]) {
    ...
    weight_workspace = at::empty(weight_workspace_size,
                          input.options().dtype(at::ScalarType::Char));
    auto weight_workspace_impl = getMluTensorImpl(weight_workspace);
  ...
  auto output = std::make_tuple(grad_input, grad_weight, grad_offset, grad_mask, grad_bias);
  return output;
}
