...
at::Tensor cnnl_dcn_forward_internal(
  ...
  cnnlSetTensorDescriptorOnchipDataType(offset_desc.desc(), getCnnlDataType(offset.dtype()));
  ...
  if (bias.defined() && bias.dim() == 1 && bias.size(0) == output_sizes[1]) {
    TORCH_MLU_CHECK(bias.dim() == 1, "Currently only support 1-dim bias in dcn_forward_internal "
                    "when bias.dim() != 0, but got ", bias.dim(), " dim.");
