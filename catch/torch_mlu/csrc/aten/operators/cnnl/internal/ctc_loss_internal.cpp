...
std::tuple<at::Tensor, at::Tensor> cnnl_ctc_loss_internal(
  ...
  TORCH_CHECK((int64_t) il.size() == batch_size, "input_lengths must be of size batch_size");
  TORCH_CHECK((int64_t) tl.size() == batch_size, "target_lengths must be of size batch_size");
  ...
  auto workspace = at::empty(space_size, probs.options().dtype(at::kByte));
  auto workspace_impl = getMluTensorImpl(workspace);
  auto workspace_ptr = workspace_impl->mlu_data_ptr();

  // calculate
  TORCH_CNNL_CHECK(cnnlCTCLoss(handle,
                               ctc_loss_desc.desc(),
                               probs_desc.desc(),
                               probs_ptr,
                               targets_desc.desc(),
