...
at::Tensor& cnnl_roi_align_internal(
  ...
  int pool_mode = 1;
  desc_roialign.set(pooled_height, pooled_width, sampling_ratio,
  ...
  // compute ops
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "mlu_roi_align_forward",[&] {
                                      TORCH_CNNL_CHECK(
                                      cnnlRoiAlign_v2(handle, desc_roialign.desc(),
                                      desc_self.desc(), self_ptr,
                                      desc_rois.desc(), rois_ptr,
...
at::Tensor& cnnl_roi_align_backward_internal(
  ...
  // compute ops
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.scalar_type(),"mlu_roi_align_backward", [&]{
                                      TORCH_CNNL_CHECK(
                                      cnnlRoiAlignBackward(handle, spatial_scale, sampling_ratio,
                                      aligned, desc_grad.desc(),
                                      grad_ptr, desc_rois.desc(), rois_ptr,
                                      desc_grad_input.desc(), grad_input_ptr));
                                      });
