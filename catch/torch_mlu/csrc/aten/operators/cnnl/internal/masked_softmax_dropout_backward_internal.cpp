...
at::Tensor& cnnl_masked_softmax_dropout_backward_internal(at::Tensor& diff_x,
    ...
    const at::Tensor& dropout_mask, const int axis, const float p) {
  ...
  TORCH_CNNL_CHECK(cnnlMaskedScaleSoftmaxBackward(handle, axis, 1.0, desc_softmax_out.desc(),
      softmax_out_ptr, desc_diff_y.desc(), diff_y_ptr, desc_dropout_mask.desc(),
      static_cast<const uint8_t *> (dropout_mask_ptr), p, desc_diff_x.desc(), diff_x_ptr));
