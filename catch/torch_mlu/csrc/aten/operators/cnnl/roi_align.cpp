...
at::Tensor cnnl_roi_align(
  ...
  TORCH_CHECK(rois.is_mlu(), "rois must be a MLU tensor");
  ...
  at::Tensor output = at::empty({num_rois, channels, pooled_height, pooled_width},
                                input.options(), memory_format);
  auto rois_ = cnnl_contiguous(rois);

  if (output.numel() == 0) {
    output = at::zeros({num_rois, channels, pooled_height, pooled_width}, input.options());
    return output;
  }

  cnnl_roi_align_internal(output, input_, rois_, spatial_scale, pooled_height,
...
at::Tensor cnnl__roi_align_backward(
  ...
  TORCH_CHECK(grad.is_mlu(), "grad must be a MLU tensor");
  TORCH_CHECK(rois.is_mlu(), "rois must be a MLU tensor");
  ...
  if (grad.numel() == 0) {
    grad_input = at::zeros({batch_size, channels, height, width}, grad.options());
    return grad_input;
  }

  cnnl_roi_align_backward_internal(grad_, rois_, spatial_scale, pooled_height,
                                   pooled_width, batch_size, channels, height,
                                   width, sampling_ratio, aligned, grad_input);
