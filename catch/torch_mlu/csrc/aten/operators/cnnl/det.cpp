...
std::set<at::ScalarType> support_types{at::ScalarType::Double, at::ScalarType::Float, at::ScalarType::Half};
...
at::Tensor reverse_dim(const at::Tensor& t, int64_t dim) {
  at::Tensor index = at::arange(t.size(dim) - 1, -1, -1, t.options().dtype(at::kLong));
...
at::Tensor prod_safe_zeros_backward(const at::Tensor &grad, const at::Tensor& inp, int64_t dim) {
  ...
  at::Tensor exclusive_normal_nocp = at::cat({ones, inp.narrow(dim, 0, inp.size(dim) - 1)}, dim);
  at::Tensor exclusive_normal = exclusive_normal_nocp.cumprod(dim);

  at::Tensor narrow_reverse = reverse_dim(inp.narrow(dim, 1, inp.size(dim) - 1), dim);
  at::Tensor exclusive_reverse_nocp = at::cat({ones, narrow_reverse}, dim);
  at::Tensor exclusive_reverse = reverse_dim(exclusive_reverse_nocp.cumprod(dim), dim);
...
at::Tensor& cnnl_linalg_det_out(const at::Tensor& self, at::Tensor& out) {
  TORCH_CHECK(self.dim() >= 2,
              "torch.det: input must have at least 2 dimensions.");
  bool req_grad = torch::autograd::compute_requires_grad(self);
  TORCH_CHECK(!req_grad, "_linalg_det(): functions with out=... arguments "
  ...
  TORCH_CHECK(self.dtype() == out.dtype(),
    "Expected out tensor to have dtype ", self.dtype(), ", but got ", out.dtype(), " instead");
  TORCH_MLU_CHECK(support_types.find(self.scalar_type()) != support_types.end(),
                  "cnnl_det not implemented for '", self.scalar_type(), "'");
  ...
  if (self.numel() == 0) {
    ...
    } else out.fill_(1); //self.sizes() like (5,5,0,0)
  } else {
...
at::Tensor cnnl_linalg_det_backward(const at::Tensor & grad, const at::Tensor& self, const at::Tensor& det) {
  auto singular_case_backward = [&](const at::Tensor& grad, const at::Tensor& self,
  ...
  auto nonsingular_case_backward = [&](const at::Tensor& grad, const at::Tensor& self,
                                       const at::Tensor& det) -> at::Tensor {
...
at::Tensor cnnl_linalg_det(const at::Tensor& self) {
  // TODO(PYTORCH-9646): The native det operator requires cnnl to support lu_factor.
...
TORCH_LIBRARY_IMPL(aten, AutogradMLU, m) {
  m.impl("linalg_det",TORCH_FN(cnnl_linalg_det_autograd));
}
