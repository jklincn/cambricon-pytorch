...
at::Tensor cnnl_deform_conv2d(const at::Tensor& input,
  ...
  TORCH_CHECK(areSameType, "Expected input, weight, offset, mask, bias have same dtype");
  TORCH_MLU_CHECK(input.is_floating_point(), "only support real floating point");
  TORCH_CHECK(input.is_mlu(), "input must be a MLU tensor");
  // check input sizes
  TORCH_CHECK(input.dim() == 4, "deform_conv2d op input's dim must equal to 4.");
  TORCH_CHECK(weight.dim() == 4, "deform_conv2d op weight's dim must equal to 4.");
  TORCH_CHECK(offset.dim() == 4, "deform_conv2d op offset's dim must equal to 4.");
  TORCH_CHECK(!use_mask || mask.dim() == 4,
  ...
  TORCH_CHECK(weight_h > 0 && weight_w > 0,
    "dcn_forward: 2 and 3 dim of weight should be greater than zero, but got 2 dim: ",
              weight_h, ", 3 dim: ", weight_w);
  TORCH_CHECK(stride_h > 0 && stride_w > 0,
              "dcn_forward: stride should be greater than zero,but got sH: ",
              stride_h, ", sW: ", stride_w);
  TORCH_CHECK(pad_h >=0 && pad_w >= 0,
              "dcn_forward: pad should be greater than or equal zero, but got padLeft: ",
              pad_h, ", padRight: ", pad_w);
  TORCH_CHECK(dilation_h > 0 && dilation_w > 0,
              "dcn_forward: dilation should be greater than zero, but got dH: ",
              dilation_h, " dW: ", dilation_w);
  TORCH_CHECK(weight.size(1) * groups == n_in_channels);
  TORCH_CHECK(weight.size(0) % groups == 0);
  TORCH_CHECK((offset.size(1) == offset_groups * 2 * weight_h * weight_w),
              "offset.shape[1] is not valid: got: ", offset.size(1),
              " expected: ", offset_groups * 2 * weight_h * weight_w);
  TORCH_CHECK((!use_mask || mask.size(1) == offset_groups * weight_h * weight_w),
              "mask.shape[1] is not valid: got: ", mask.size(1),
              " expected: ", offset_groups * weight_h * weight_w);
  ...
  TORCH_CHECK((offset.size(2) == out_h && offset.size(3) == out_w),
              "offset output dims: (", offset.size(2),
              ", ", offset.size(3),
              ") - computed output dims: (", out_h,
              ", ", out_w, ")");
  TORCH_CHECK((mask.size(0) == input.size(0)),
              "invalid batch size of mask");
  TORCH_CHECK((!use_mask || (mask.size(2) == out_h && mask.size(3) == out_w)),
              "offset output dims: (", mask.size(2),
              ", ", mask.size(3),
              ") - computed output dims: (", out_h,
              ", ", out_w, ")");
  ...
  auto out = cnnl_dcn_forward_internal(
                input_contiguous, offset_contiguous,
                mask_contiguous, weight_contiguous, bias_contiguous,
                output_size, padding_t, stride_t,
                dilation_t, offset_groups,
                groups, im2col_step, bitwidth_value, use_mask);
  return out;
}
...
cnnl__deform_conv2d_backward(
  ...
  auto output = cnnl_dcn_backward_internal(
                   grad_contiguous, input_contiguous,
                   offset_contiguous, mask_contiguous,
                   weight_contiguous, bias_contiguous,
                   padding_t, stride_t, dilation_t,
                   offset_groups, groups, im2col_step,
                   grad_input_mask, use_mask, bitwidth_value);
  return output;
}
...
at::Tensor deform_conv2d_autocast(
  ...
  return deform_conv2d(
             at::autocast::cached_cast(at::kFloat, input, DeviceType::MLU),
             at::autocast::cached_cast(at::kFloat, weight, DeviceType::MLU),
             at::autocast::cached_cast(at::kFloat, offset, DeviceType::MLU),
             at::autocast::cached_cast(at::kFloat, mask, DeviceType::MLU),
             at::autocast::cached_cast(at::kFloat, bias, DeviceType::MLU),
             stride_h,
             stride_w,
             pad_h,
             pad_w,
...
TORCH_LIBRARY_IMPL(torchvision, AutocastMLU, m) {
  m.impl(
