...
const Tensor& _fft_apply_normalization(const Tensor& self, int64_t normalization,
                                       IntArrayRef sizes, IntArrayRef dims) {
  auto scale = _fft_normalization_scale(normalization, sizes, dims);
  return (scale == 1.0) ? self : self.mul_(scale);
}

Tensor& _fft_apply_normalization_out(Tensor& out, const Tensor& self, int64_t normalization,
                                     IntArrayRef sizes, IntArrayRef dims) {
  auto scale = _fft_normalization_scale(normalization, sizes, dims);
  return at::mul_out(out, self, c10::scalar_to_tensor(scale));
}

at::Tensor cnnl__fft_r2c(const at::Tensor& self, IntArrayRef dim, int64_t normalization,
                         bool onesided) {
  TORCH_MLU_CHECK(self.numel() > 0, "currently do not support empty Tensor input");
  TORCH_MLU_CHECK(self.is_floating_point(), "only support real floating point input");
  ...
  double scale_factor = _fft_normalization_scale(normalization, input_sizes, dim);
  auto scale_factor_float = c10::checked_convert<float, double>(scale_factor, "float");
  ...
  if (sorted_dims.empty()) {
    cnnl_fft_internal(output, working_tensor, onesided_sizes, last_dim, /*forward=*/true,
        scale_factor_float);
  } else {
    cnnl_fft_internal(output, working_tensor, onesided_sizes, last_dim, /*forward=*/true, 1.0);
  }
  if (dim.size() > 1) {
  ...
  while (!sorted_dims.empty()) {
    ...
    const auto max_dims = std::min(static_cast<size_t>(cnfft_max_ndim), sorted_dims.size());
    auto last_dims = IntArrayRef(sorted_dims).slice(sorted_dims.size() - max_dims, max_dims);

    if (sorted_dims.size() > 1) {
      cnnl_fft_internal(output, working_tensor, onesided_sizes, last_dims, /*forward=*/true, 1.0);
    } else {
      cnnl_fft_internal(output, working_tensor, onesided_sizes, last_dims, /*forward=*/true,
          scale_factor_float);
    }
...
Tensor cnnl__fft_c2r(const Tensor& self, IntArrayRef dim, int64_t normalization, int64_t lastdim) {
  TORCH_MLU_CHECK(self.numel() > 0, "currently do not support empty Tensor input");
  ...
  Tensor tmp = self;
  if (dim.size() > 1) {
    tmp = cnnl__fft_c2c(self, dim.slice(0, dim.size() - 1),
        static_cast<int64_t>(at::native::fft_norm_mode::none), /*forward=*/false);
  }
...
at::Tensor& cnnl__fft_c2r_out(const at::Tensor& self, at::IntArrayRef dim,
  ...
  auto result = cnnl__fft_c2r(
      ...
      last_dim_size);
  return _fft_apply_normalization_out(out, result, normalization,
                                      result.sizes(), dim);
}

// n-dimensional complex to complex FFT/IFFT
Tensor cnnl__fft_c2c(const Tensor& self, IntArrayRef dim, int64_t normalization, bool forward) {
  TORCH_MLU_CHECK(self.numel() > 0, "currently do not support empty Tensor input");
  TORCH_MLU_CHECK(self.is_complex(), "only support complex dtype input");
  if (dim.empty()) {
    return self.clone();
  ...
  auto scale_factor_ = c10::checked_convert<float, double>(scale_factor, "float");
  ...
  while (true) {
    ...
    if (sorted_dims.size() > 1) {
      cnnl_fft_internal(output, working_tensor, out_sizes, first_dims, forward, 1.0);
    } else {
      cnnl_fft_internal(output, working_tensor, out_sizes, first_dims, forward, scale_factor_);
    }
    sorted_dims.resize(sorted_dims.size() - max_dims);
