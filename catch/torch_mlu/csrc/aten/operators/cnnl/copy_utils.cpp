...
void direct_copy_kernel_mlu(at::TensorIterator& iter) {
  ...
  if (dst.dtype() == src.dtype()) {
    auto memory_format = src.suggest_memory_format();
    bool is_contiguous_copy = ((dst.is_contiguous(memory_format) &&
      src.is_contiguous(memory_format)) || is_same_format_tensor({dst, src}));
    if (!is_contiguous_copy && dst.is_non_overlapping_and_dense()) {
      non_overlapping_and_dense_out(dst, src);
    } else {
...
void copy_device_to_device(at::TensorIterator& iter, bool non_blocking) {
  ...
  if (memcpy_eligible && src_device != dst_device) {
    ...
    // 1. when notifier and queue are not on the same device.
    //    Wait and place operation can't be performed. See CNTOOLKIT-3871.
    // 2. cnrtMemcpyAsync can't cross-card copy. See CNTOOLKIT-3872.
    // 3. CNRT doesn't implement the same functions as cudaDeviceEnablePeerAccess. See CNTOOLKIT-3873.
    ...
    auto dst_dtype_onchip = c10::scalarTypeToTypeMeta(cnnlType2ScalarType(getCnnlType(dst_impl)));
  ...
  auto dst_contig = dst_tensor.is_contiguous() ? dst_tensor : at::empty_like(dst_tensor, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  auto memory_format = dst_contig.suggest_memory_format();
  auto src_contig = cnnl_contiguous(src_tensor.to(iter.dtype(0)), memory_format);
