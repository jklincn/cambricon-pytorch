...
std::tuple<at::Tensor&, at::Tensor&> cnnl_linalg_slogdet_out(const at::Tensor& A, at::Tensor& sign, at::Tensor& logabsdet) {
  TORCH_CHECK(A.dim() >= 2,
              "torch.slogdet: input must have at least 2 dimensions.");
  ...
  TORCH_CHECK(A.dtype() == sign.dtype(), "Expected out tensor to have dtype ",
              A.dtype(), ", but got ", sign.dtype(), " instead");
  TORCH_CHECK(A.dtype() == logabsdet.dtype(), "Expected out tensor to have dtype ",
              A.dtype(), ", but got ", logabsdet.dtype(), " instead");
  at::native::squareCheckInputs(A, "torch.slogdet");
  auto dtype = A.scalar_type();
  TORCH_MLU_CHECK(at::isFloatingType(dtype), "linalg.slogdet",
                   ": Expected a floating point as input. Got ", dtype);

  TORCH_MLU_CHECK(dtype == at::kFloat || dtype == at::kDouble, "linalg.slogdet",
                  ": Low precision dtypes not supported. Got ", dtype);
...
// TODO(zhiguangda): [JIRA-9646] Delete this backward function when cnnl supports LU factor
// operator, and use backward function of origin
Tensor cnnl_linalg_slogdet_backward(const Tensor& grad_logabsdet, const Tensor& self,
  ...
  if (self.dim() == 2) {
    ...
    grad_slogdet.index_put_(/*indices=*/nonzero_signdet_indices,
                            ...
                            /*value=*/nonsingular_case_backward(
...
class SlogdetFunction : public torch::autograd::Function<SlogdetFunction> {
 public:
  static torch::autograd::variable_list forward(
      torch::autograd::AutogradContext* ctx, const at::Tensor& A
  ) {
...
std::tuple<at::Tensor, at::Tensor> cnnl_linalg_slogdet(const Tensor& A) {
  ...
  auto sign = at::zeros(shape_outputs, A.options());
  auto logabsdet = at::zeros(shape_outputs, A.options());

  auto result = cnnl_linalg_slogdet_out(A, sign, logabsdet);
  return result;
}

std::tuple<at::Tensor, at::Tensor> cnnl_linalg_slogdet_autograd(const Tensor& A) {
  TORCH_CHECK(A.dim() >= 2,
              "torch.slogdet: input must have at least 2 dimensions.");
...
TORCH_LIBRARY_IMPL(aten, AutogradMLU, m) {
  m.impl("linalg_slogdet",TORCH_FN(cnnl_linalg_slogdet_autograd));
}
