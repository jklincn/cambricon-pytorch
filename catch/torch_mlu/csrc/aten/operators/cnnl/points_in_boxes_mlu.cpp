...
at::Tensor cnnl_points_in_boxes_mlu(const at::Tensor& points, const at::Tensor& boxes) {

  TORCH_MLU_CHECK(points.scalar_type() == at::ScalarType::Float,
                  "Only float type is supported for points, in points_in_boxes_mlu");
  TORCH_MLU_CHECK(boxes.scalar_type() == at::ScalarType::Float,
  ...
  auto point_indices = at::empty({points_shape[0], points_shape[1]},
  ...
  cnnl_points_in_boxes_mlu_internal(point_indices, points_contiguous, boxes_contiguous);
