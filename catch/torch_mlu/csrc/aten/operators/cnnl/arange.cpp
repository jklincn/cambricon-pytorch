...
at::Tensor& cnnl_arange_out(const at::Scalar& start, const at::Scalar& end,
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      "arange_mlu", [&]() {
        ...
        TORCH_MLU_CHECK(size_d <= std::numeric_limits<int32_t>::max(),
        ...
        switch (out_contiguous.scalar_type()) {
          ...
          case at::ScalarType::Long:
            cnnl_arange_internal(out_contiguous, &start_internal, &step_internal);
            break;
          default:
            // All floating type is supported, so just handle integral type.
            auto output_internal = at::empty(out_contiguous.sizes(),
                                             out_contiguous.options().dtype(at::kInt));
            cnnl_arange_internal(output_internal, &start_internal, &step_internal);
