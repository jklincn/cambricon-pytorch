...
TEST(CnnlDescriptorDeleter, CnnlDescriptordeleter) {
  ...
  TORCH_CHECK_EQ(data_ptr->data, 0);
...
TEST(CnnlDescriptor, CnnlDescriptor) {
  ...
  TORCH_CHECK_EQ(mut_desc_ptr->data, 1);

  TestData * desc_ptr = test_desc.desc();
  TORCH_CHECK_EQ(desc_ptr->data, 1);
...
TEST(CnnlCommondesc, CnnlGetcnnlstrides) {
  ...
  for (int i = 0; i < dim_; i++) {
      if (self.sizes()[i] != 1) {
          TORCH_CHECK_EQ(res[i], self.strides()[i]);
      } else {
          TORCH_CHECK_EQ(res[i], 1);
      }
  }
}
