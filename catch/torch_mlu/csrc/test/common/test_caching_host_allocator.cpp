...
TEST(HostMemoryAllocator, check_pin_memory_free_after_record) {
  ...
  at::Tensor result;
  at::Tensor left = at::ones({N, N}).to("mlu");
  at::Tensor right = at::ones({N, N}).to("mlu");
  at::Tensor bias = at::zeros({N}).to("mlu");
...
TEST(HostMemoryAllocator, check_empty_cache) {
  ...
  void* ctx{nullptr};
  {
    ...
    ASSERT_TRUE(torch_mlu::MLUCachingHostAllocator_recordEvent(
        ptr, ctx, torch_mlu::getCurrentQueue()));
  }
