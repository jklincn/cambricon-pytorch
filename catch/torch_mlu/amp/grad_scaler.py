    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(self,
        ...
        if enabled and not torch.mlu.is_available():
            warnings.warn("torch_mlu.amp.GradScaler is enabled, but MLU is not available.  Disabling.")
            enabled = False
    ...
    def scale(self, outputs):
        ...
        stash: List[_MluMultiDeviceReplicator] = []  # holds a reference that can be overwritten by apply_scale
        ...
        with torch.no_grad():
            ...
            for device, per_dtype_grads in per_device_and_dtype_grads.items():
                for grads in per_dtype_grads.values():
                    per_device_found_inf._per_device_tensors[device] = \
                        torch.ops.torch_mlu.amp_unscale(grads,
