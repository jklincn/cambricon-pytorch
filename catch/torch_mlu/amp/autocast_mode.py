...
class autocast(torch.amp.autocast_mode.autocast):
    ...
    def __init__(self, enabled : bool = True, dtype : torch.dtype = torch.float16, cache_enabled : bool = True):
        if torch._jit_internal.is_scripting():
            ...
            return
        super().__init__("mlu", enabled=enabled, dtype=dtype, cache_enabled=cache_enabled)
...
def _cast(value, dtype):
    if isinstance(value, torch.Tensor):
        is_eligible = (value.is_floating_point() and value.is_mlu and (value.dtype is not torch.float64))
...
def custom_fwd(fwd=None, *, cast_inputs=None):
    ...
    def decorate_fwd(*args, **kwargs):
        if cast_inputs is None:
            args[0]._fwd_used_autocast = torch.is_autocast_mlu_enabled()
            return fwd(*args, **kwargs)
        else:
            autocast_context = torch.is_autocast_mlu_enabled()
