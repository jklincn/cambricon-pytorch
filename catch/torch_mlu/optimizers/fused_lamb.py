...
class FusedLAMBAMP(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-3, step=0, bias_correction=True,
        ...
        defaults = dict(lr=torch.tensor(lr, dtype=torch.float32, device=torch.mlu.current_device()),
                        step=torch.tensor([step], dtype=torch.int, device=torch.mlu.current_device()),
        ...
        for gid, (group, fp32_group) in enumerate(zip(self.param_groups, self.param_groups_fp32)):
            ...
            if len(g_16) > 0:
                torch.ops.torch_mlu.fused_lamb_amp(
                    ...
                    global_grad_norm, max_grad_norm, self.use_nvlamb, inv_scale)
            if len(g_32) > 0:
                torch.ops.torch_mlu.fused_lamb_amp(
                    self._dummy_overflow_buf,
                    g_32, p_32, m_32, v_32,
