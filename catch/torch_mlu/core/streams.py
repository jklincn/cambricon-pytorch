...
if not hasattr(torch_mlu._MLUC, '_MLUQueueBase'):
    # Define dummy base classes
    torch_mlu._MLUC.__dict__['_MLUQueueBase'] = _dummy_type('_MLUQueueBase')

#define Stream
class Stream(torch_mlu._MLUC._MLUQueueBase):
    ...
    def __repr__(self):
        return ('<torch.mlu.Stream device={0} mlu_stream={1:#x}>'
                .format(self.device, self.mlu_stream))
...
def current_stream(device: Optional[_device_t] = None) -> Stream:
    ...
    return Stream(_cdata=torch_mlu._MLUC._mlu_getCurrentStream(
...
def default_stream(device: Optional[_device_t] = None) -> Stream:
    ...
    """
    torch.mlu._lazy_init()
    return Stream(_cdata=torch_mlu._MLUC._mlu_getDefaultStream(
        _get_device_index(device, optional=True)))
...
def set_stream(stream: Stream):
    ...
    if stream is None:
        return
    torch_mlu._MLUC._mlu_setStream(stream._cdata)
...
class StreamContext(object):
    ...
    def __init__(self, stream: Optional['torch.mlu.Stream']):
        ...
        self.src_prev_stream = None if not torch.jit.is_scripting() else torch.mlu.default_stream(None)
        self.dst_prev_stream = None if not torch.jit.is_scripting() else torch.mlu.default_stream(None)
...
interfaces = [Stream, ExternalStream, current_stream, default_stream, set_stream,
                     StreamContext, stream,
              Event]

for interface in interfaces:
    torch.mlu.__setattr__(interface.__name__, interface)
