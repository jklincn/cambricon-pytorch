...
def _mlu(self, *args, **kwargs):
    return torch_mlu._MLUC.mlu(self, *args, **kwargs)

torch.Tensor.mlu = _mlu

device_interfaces = [device, set_device, current_device, device_count, get_device_properties,
                     get_device_capability, get_device_name, device_of, synchronize, can_device_access_peer]

for device_interface in device_interfaces:
    torch.mlu.__setattr__(device_interface.__name__, device_interface)
