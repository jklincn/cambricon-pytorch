...
def _find_neuware_home() -> Optional[str]:
    ...
    if neuware_home is None:
        ...
        try:
            with open(os.devnull, 'w') as devnull:
                cncc = subprocess.check_output(
                    ['which', 'cncc'], stderr=devnull).decode().rstrip('\r\n')
    ...
    if neuware_home and not torch.mlu.is_available():
        print(
            f"No NEUWARE runtime is found, using neuware_home='{neuware_home}'"
        )
    return neuware_home
...
def include_paths() -> List[str]:
    ...
    paths.append(
        os.path.join(torch_mlu_path, 'csrc', 'api', 'include', 'torch_mlu'))
...
def _write_ninja_file(
    ...
    if with_bang:
        ...
        bang_compile_rule.append(
            f'  command = $cncc -c $in -o $out $bang_post_cflags')
...
def _get_bang_arch_flags(cflags: Optional[List[str]] = None) -> List[str]:
    ...
    if not _arch_list:
        ...
        arch_list = [arch for arch in _arch_list.strip(" ").split(
            ";") if arch in supported_arches and "--bang-arch=compute_{}".format(cc_to_arch[arch]) in valid_arch_strings]

    flags = []
    for arch in arch_list:
        if not any([
                len(re.findall(r"compute_{}".format(
                    cc_to_arch[arch]), valid)) > 0
