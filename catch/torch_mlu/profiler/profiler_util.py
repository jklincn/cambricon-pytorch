        ...
        if self.device_type == DeviceType.CPU:
            if not self.is_legacy:
                ...
                return (sum(kinfo.duration for kinfo in self.kernels) +
                        sum(ch.mlu_time_total for ch in self.cpu_children))
            else:
                # each legacy cpu events has a single (fake) kernel
                return sum(kinfo.duration for kinfo in self.kernels)
        else:
            assert self.device_type == DeviceType.MLU
    ...
    def self_mlu_time_total(self):
        ...
        if self.device_type == DeviceType.CPU:
            return self.mlu_time_total - \
                sum([child.mlu_time_total for child in self.cpu_children])
        else:
            assert(self.device_type == DeviceType.MLU)
            return self.mlu_time_total
...
def _build_table(
    ...
    if sort_by is not None:
        ...
        ), use_cuda=has_mlu_time, profile_memory=profile_memory, with_flops=with_flops)
    ...
    for evt in events:
        ...
        elif evt.device_type == DeviceType.MLU:
    ...
    for evt in events:
        ...
        if has_mlu_time:
            row_values.extend([
                ...
                # MLU time total %
                _format_time_share(evt.self_mlu_time_total, sum_self_mlu_time_total),
