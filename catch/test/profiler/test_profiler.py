...
from torch.profiler import (
    profile, record_function, supported_activities,
...
class TestProfiler(TestCase):
    ...
    def test_flops_memory_recordshapes(self):
        ...
        for evt in kineto_profiler.events():
            if "aten::conv2d" in evt.name:
                if [40, 16, 18, 260] in evt.input_shapes and [33, 16, 18, 18] in  evt.input_shapes \
    ...
    def test_profiler_multi_mlu(self):
        ...
        for evt in p.events():
            ...
            if "add" in evt.name:
                found_add = True
            if  evt.device_type == DeviceType.MLU:
    ...
    def test_high_level_trace(self):
        ...
        with profile(
                ...
                schedule=torch.profiler.schedule(
                        ...
                        active=10),#实际最大采集active个steps数据
                profile_memory=True,
    ...
    def test_profiler_autograd(self):
        ...
        with _profile(use_mlu=False, use_kineto=True) as p:
            y = x * 2 + 4  # pylint: disable=W0612
    ...
    def test_profiler_autograd_aggregation(cls):
        total_time_s = 0
        with _profile(use_mlu=True, record_shapes=True, profile_memory=True, use_kineto=True) as prof:
            start = time.time()
            x = torch.randn((3, 6, 334, 334), requires_grad=True,
                        dtype=torch.float32).to(ct.mlu_device())
            y = torch.pow(x, 3)
            y = y * 3 - 2
            y = y + 1.0
            y = y * 2.0
        ...
        print(
            prof.key_averages(group_by_input_shape=True).table(sort_by="self_cpu_time_total",
                                                               row_limit=10))
