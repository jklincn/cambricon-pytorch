...
TEST_LARGETENSOR = args.large or os.environ.get('TEST_LARGETENSOR', 'FALSE').upper() in ['TRUE', 'YES', 'ON']
SEED = args.seed
...
def read_card_info():
    try:
        result = subprocess.run(['cnmon', 'info', '-c', '0', '-t'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def shell(command, cwd=None, env=None, fail_log=None):
    ...
    p = subprocess.Popen(command, universal_newlines=True, cwd=cwd, env=env,
                         ...
                         stderr=subprocess.STDOUT if fail_log else None)  # pylint: disable= R1732
...
def _has_sufficient_memory(device, size):
    if torch.device(device).type == 'mlu':
        ...
        return torch.mlu.get_device_properties(device).total_memory - torch.mlu.memory_allocated(device) >= size
...
def largeTensorTest(size, device=None):
    ...
    def inner(fn):
        ...
        def dep_fn(self, *args, **kwargs):
            ...
            _device = device if device is not None else 'mlu:{0}'.format(torch.mlu.current_device())
            # large tensor test not support in 300 series
            if "MLU370" in torch.mlu.get_device_name():
                device_meg = f'large tensor test not support in {torch.mlu.get_device_name()}'
    ...
    for name in dir(cls):
        ...
        if name[:4] == "test" and name not in known_error_lst:
            ...
            if out != "":
                ...
                passed += 1
            row = pd.DataFrame({name_lst[0]:[has_error], name_lst[1]:[name], name_lst[2]:[out]})
