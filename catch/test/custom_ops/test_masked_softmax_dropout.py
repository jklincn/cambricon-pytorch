...
class FusedMaskSoftmaxDropoutFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, mask, batch, seqlen, heads, dropout_prob, stream, sync, is_training):
        output, dropout_mask = torch.ops.torch_mlu.mask_softmax_dropout_fprop(input, mask, batch,
          seqlen, heads, dropout_prob, stream, sync, is_training)
    ...
    def backward(ctx, grad_output):
        output, dropout_mask, seqlen, = ctx.saved_tensors
        batch = ctx.batch
        heads = ctx.heads
        dropout_prob = ctx.dropout_prob

        torch.ops.torch_mlu.mask_softmax_dropout_bprop_(output, grad_output, dropout_mask, batch,
          seqlen, heads, dropout_prob, ctx.stream, ctx.sync)
...
class TestFusedMaskSoftmaxDropoutOp(TestCase):
    ...
    def test_mask_softmax_dropout(self):
        ...
        for batch, heads, prob, dtype in product(batch_list, heads_list, prob_list, dtype_list):
            seqlen = torch.randint(1, 6, (batch,))
            input = torch.randn(seqlen.square().sum().item() * heads, dtype=dtype, device="mlu",
              requires_grad=True)
            ...
            input.grad.zero_()
            torch.mlu.manual_seed(10000)
            out_fused = FusedMaskSoftmaxDropoutFunction.apply(input, mask, batch, seqlen, heads,
    ...
    def test_mask_softmax_dropout_not_contiguous(self):
        ...
        heads_list = [1, 16]
        for heads, func0, func1, func2, func3 in product(heads_list, func_list, func_list,
          func_list, func_list):
            seqlen = func0(torch.randint(1, 513, (1,)))
            input = torch.randn(seqlen.square().sum().item() * heads, device="mlu",
              requires_grad=True)
            ...
            input.grad.zero_()
            torch.mlu.manual_seed(10000)
            out_fused = FusedMaskSoftmaxDropoutFunction.apply(func1(input), mask, 1, seqlen,
              heads, 0.5, False, False, True)
            out_fused.backward(grad)
            grad_fused = input.grad
            self.assertTensorsEqual(out_base.cpu(), out_fused.cpu(), 0.003, use_MSE=True)
            self.assertTensorsEqual(grad_base.cpu(), grad_fused.cpu(), 0.003, use_MSE=True)
    ...
    def test_mask_softmax_dropout_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            o = torch.ops.torch_mlu.mask_softmax_dropout_fprop(input, mask, 10, seqlen,
              1, 0.5, False, False, False)   # pylint: disable=W0612
        ...
        ref_msg = "currently do not support multiple queues acceleration"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            o = torch.ops.torch_mlu.mask_softmax_dropout_bprop_(input, output,
              mask, 10, seqlen, 1, 0.5, True, True)   # pylint: disable=W0612
