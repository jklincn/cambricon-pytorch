...
git clone -b release/0.13 https://github.com/pytorch/audio.git \
&& pushd audio && python setup.py bdist_wheel \
&& pip install dist/torchaudio-0.13.1+a23bc80-cp37-cp37m-linux_x86_64.whl && popd
```
...
class TestRNNTLossOp(TestCase):
    ...
    def rnnt_loss_input_with_params(self,
                                    batch,
                                    max_seq_length,
                                    max_target_length,
                                    class_nums,
                                    ...
                                    device='cpu'):
        logits_shape = [
            batch, max_seq_length, max_target_length + 1, class_nums
        ]
        targets_shape = [batch, max_target_length]
        logit_lengths_shape = [batch]
        logits = torch.rand(logits_shape,
                            dtype=dtype,
                            device=device,
                            requires_grad=True)
        targets = torch.randint(
            2, size=targets_shape, dtype=torch.int32, device=device) + 1
        logit_lengths = torch.randint(1,
                                      max_seq_length + 1,
                                      size=logit_lengths_shape,
                                      dtype=torch.int32,
                                      device=device)
        target_lengths = torch.randint(1,
                                       max_target_length + 1,
                                       size=logit_lengths_shape,
                                       dtype=torch.int32,
                                       device=device)
        ...
        assert logits.size()[1] == logit_lengths.max().item(
        ), "logits size[1] => {} != {}".format(logits.size()[1],
                                               logit_lengths.max().item())
        ...
        assert logits.size()[2] == target_lengths.max().item(
        ) + 1, "logits size[2] => {} != {}".format(
            logits.size()[2],
            target_lengths.max().item() + 1)
        return logits, targets, logit_lengths, target_lengths

    def forward_and_compare(self,
                            batch,
                            max_seq_length,
                            max_target_length,
                            class_nums,
                            ...
                            dtype=torch.float,
                            cmp_device='mlu'):
        logits, targets, logit_lengths, target_lengths = self.rnnt_loss_input_with_params(
    ...
    def backward_and_compare(self,
        ...
        if cmp_device == 'mlu':
            logits_ = logits.mlu()
            forward_mlu = transform(logits_, self.to_mlu(targets),
    ...
    def test_backward_with_shape(self):
        ...
        for param in params:
            for dtype, blank, reduction, clamp in itertools.product(
                [torch.float, torch.half], range(5), ['mean', 'sum'],
                    map(lambda x: float(x), range(-5, 5, 1))):
                param['dtype'] = dtype
                param['transform'] = transforms.RNNTLoss(blank=blank,
                                                         reduction=reduction,
                                                         clamp=clamp)
                cpu_forward, mlu_forward = self.backward_and_compare(**param)
                self.assertTensorsEqual(cpu_forward.float(),
                                        mlu_forward.cpu().float(),
                                        0.003,
                                        use_MSE=True)
    ...
    def test_forward_with_shape(self):
        ...
        for param in params:
            for dtype, blank, reduction, clamp in itertools.product(
                [torch.float, torch.half], range(5), ['mean', 'sum', 'none'],
    ...
    def test_rnnt_loss_forward_exception(self):
        logits, targets, logit_lengths, target_lengths = self.rnnt_loss_input_with_params(
            self.batch, self.max_seq_length, self.max_target_length,
            self.class_nums)
        ...
        with self.assertRaisesRegex(RuntimeError, input_length_msg):
            transform(logits.to('mlu'), targets.to('mlu'),
        ...
        with self.assertRaisesRegex(RuntimeError,
                                    r"^targets must be contiguous$"):
            transform(logits.to('mlu'),
                      targets.to('mlu').transpose(0, 1),
                      logit_lengths.to('mlu'), target_lengths.to('mlu'))

        with self.assertRaisesRegex(RuntimeError,
                                    r"^targets must be contiguous$"):
            transform(logits.to('mlu'), targets_illegal.to('mlu'),
                      logit_lengths.to('mlu'), target_lengths.to('mlu'))

        with self.assertRaisesRegex(
                RuntimeError,
                r"Expected all tensors to be on the same device"):
            transform(logits.to('mlu'), targets, logit_lengths.to('mlu'),
                      target_lengths.to('mlu'))

        with self.assertRaisesRegex(
                RuntimeError,
                r"Expected all tensors to be on the same device"):
            transform(logits.to('mlu'), targets.to('mlu'), logit_lengths,
                      target_lengths.to('mlu'))

        with self.assertRaisesRegex(
                RuntimeError,
                r"^logits must be float32 or float16 \(half\) type$"):
            transform(
                logits.type(torch.int).to('mlu'), targets.to('mlu'),
                logit_lengths.to('mlu'), target_lengths.to('mlu'))

        with self.assertRaisesRegex(RuntimeError,
                                    r"^targets must be int32 type$"):
            transform(logits.to('mlu'),
                      targets.type(torch.float).to('mlu'),
                      logit_lengths.to('mlu'), target_lengths.to('mlu'))

        with self.assertRaisesRegex(RuntimeError,
                                    r"^logit_lengths must be int32 type$"):
            transform(logits.to('mlu'), targets.to('mlu'),
        ...
        with self.assertRaisesRegex(
                RuntimeError,
                r"^logits must be 4-D \(batch, time, target, class\)$"):
            transform(
                logits.reshape(-1).to('mlu'), targets.to('mlu'),
                logit_lengths.to('mlu'), target_lengths.to('mlu'))

        with self.assertRaisesRegex(
                RuntimeError,
                r"^targets must be 2-D \(batch, max target length\)$"):
            transform(logits.to('mlu'),
                      targets.reshape(-1).to('mlu'), logit_lengths.to('mlu'),
                      target_lengths.to('mlu'))

        with self.assertRaisesRegex(RuntimeError,
                                    r"^logit_lengths must be 1-D$"):
            transform(logits.to('mlu'), targets.to('mlu'),
                      logit_lengths.reshape(2, 2).to('mlu'),
                      target_lengths.to('mlu'))

        with self.assertRaisesRegex(RuntimeError,
                                    r"^target_lengths must be 1-D$"):
            transform(logits.to('mlu'), targets.to('mlu'),
                      logit_lengths.to('mlu'),
        ...
        with self.assertRaisesRegex(
                RuntimeError,
                r"^batch dimension mismatch between logits and logit_lengths$"
        ):
            transform(
                logits.to('mlu'), targets.to('mlu'),
        ...
        with self.assertRaisesRegex(
            ...
            transform(
                logits.to('mlu'), targets.to('mlu'), logit_lengths.to('mlu'),
                torch.ones(logits.size(0) + 1, dtype=torch.int).to('mlu'))

        with self.assertRaisesRegex(
                RuntimeError,
                r"^batch dimension mismatch between logits and targets$"):
            transform(
                logits.to('mlu'),
                torch.ones([self.batch + 1, self.max_target_length],
                           dtype=torch.int).to('mlu'), logit_lengths.to('mlu'),
                target_lengths.to('mlu'))
