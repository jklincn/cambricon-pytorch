...
def boxes_iou3d_test(boxes_a, boxes_b, flag):
    ...
    # bev overlap
    if flag:
        overlaps_bev = torch.ops.torch_mlu.boxes_overlap_bev(boxes_a.contiguous(), \
                       boxes_b.contiguous())
    else:
        overlaps_bev = torch.ops.torch_mlu.boxes_overlap_bev_cpu(boxes_a.contiguous(), \
                       boxes_b.contiguous()).mlu()
...
class TestBoxesOverlapBevOp(TestCase):
    ...
    def test_boxes_overlap_bev(self):
        for i in range(1, 6):
            input = torch.Tensor(np.load(data_dir + "boxes_overlap_a_" \
                    + str(i) + ".npy")).mlu()
            other = torch.Tensor(np.load(data_dir + "boxes_overlap_b_" \
                    + str(i) + ".npy")).mlu()
            output_mlu = torch.ops.torch_mlu.boxes_overlap_bev(input, other)
            output_cuda = torch.Tensor(np.load(data_dir + "overlaps_bev_" \
                    + str(i) + ".npy"))
    ...
    def test_boxes_overlap_bev_not_dense(self):  # pylint: disable=R0201
        shape_list = (((279, 14), (279, 14)), ((279, 14), (200, 14)),
                      ((54, 14), (50, 14)), ((60, 14), (140, 14)))
