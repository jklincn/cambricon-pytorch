...
from common_utils import TestCase, testinfo, largeTensorTest, TEST_LARGETENSOR  # pylint: disable=C0411,C0413
...
class TestAmpOP(TestCase):
    ...
    def test_grad_unscale_found_inf_op(self, dtype=torch.float):
        ...
        for grads, has_inf in cases:
            found_inf.zero_()
            found_inf = torch.ops.torch_mlu.amp_unscale(grads,found_inf,inv_scale)
            if has_inf:
                self.assertEqual(found_inf, 1.0)
            else:
                self.assertEqual(found_inf, 0.0)
                for grad in grads:
                    self.assertTrue(torch.allclose(grad.cpu(),
                                                   torch.ones_like(grad.cpu()), atol=1e-7))
    ...
    def test_amp_update_scale_op(self, device="mlu", dtype=torch.float):
        ...
        scale = torch._amp_update_scale_(scale,growth_tracker,
                                        found_inf,
                                        growth,
                                        backoff,
                                        growth_interval)
    ...
    def test_overlength_tensors(self):
        ...
        found_inf_out = torch.ops.torch_mlu.amp_unscale(grads_mlu, found_inf, _scale)
        amp_unscale_cpu_(grads_cpu, _scale.cpu())
        found_inf_real = torch.FloatTensor([0.0])
        self.assertTrue(torch.allclose(found_inf_out.cpu(), found_inf_real, atol=0.0))
        for i in range(length):
            grad_cpu = grads_cpu[i]
            grad_mlu = grads_mlu[i]
            self.assertTensorsEqual(grad_cpu, grad_mlu.cpu(), 3e-3, allow_inf=True, use_MSE=True)
    ...
    def test_amp_unscale_boundary(self):
        ...
        initial_scale = 0.25
        _scale = torch.mlu.FloatTensor([initial_scale]).double().reciprocal().float()
        found_inf_out = torch.ops.torch_mlu.amp_unscale(inputs, found_inf, _scale)
    ...
    def test_amp_unscale_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.ops.torch_mlu.amp_unscale([input], found_inf, _scale)
