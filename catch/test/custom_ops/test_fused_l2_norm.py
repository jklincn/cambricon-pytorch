...
class TestFusedL2Norm(TestCase):
    ...
    def fused_l2norm_dtype(self,
        ...
        fused_out1, fused_out2 = op(self._dummy_overflow_buf,
                                    tensors_mlu, per_tensor)

        reference = torch.cat(tensors_cpu).norm().reshape(1)
        if per_tensor:
            referenceb = torch.cat(
                [t.flatten().norm().reshape(1) for t in tensors_cpu])

        self.assertTensorsEqual(fused_out1.cpu().float(),
                                reference,
                                0.003,
                                use_MSE=True)
        if per_tensor:
            self.assertTensorsEqual(fused_out2.cpu().float(),
                                    referenceb,
                                    0.003,
                                    use_MSE=True)
    ...
    def test_fused_l2norm(self):
        shape_list = [(0), (1), (15, 20), (2, 3, 4), (8, 1, 2, 3), (2, 1, 2, 1, 4),
                      (33333), (555), (2048 * 32 + 1)]
        op_list = [torch.ops.torch_mlu.fused_l2_norm_amp, torch.ops.torch_mlu.fused_l2_norm]
    ...
    def test_fused_l2norm_exception(self):
        msg_ref_list = ["\"bang_fused_l2_norm\" not implemented for 'Long'",
                        "\"bang_fused_l2_norm_amp\" not implemented for 'Long'"]
        op_list = [torch.ops.torch_mlu.fused_l2_norm, torch.ops.torch_mlu.fused_l2_norm_amp]
