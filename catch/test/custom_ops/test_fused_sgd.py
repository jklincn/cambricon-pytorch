...
class TestFusedSGD(TestCase):
    ...
    def fused_sgd_dtype(self, test_type=torch.float, sz=[40,40]):
        ...
        for size in sizes:
            tensors.append((torch.rand(size, dtype=torch.float)).to('mlu').to(test_type))
        for tensor in tensors:
            fused_opt_params.append(torch.nn.Parameter(tensor.clone()))
            opt_params.append(torch.nn.Parameter(tensor.clone()))
        fused_optimizer = self.fused_optimizer(fused_opt_params,
                                                     **self.options)
        origin_optimizer = self.origin_optimizer(opt_params,
                                     **self.options)
    ...
    def test_fused_sgd_fp32(self):
        shape_list = [[1], [300], [4096], [278011], [10,10],
                      [100,100], [4096, 1024], [4096, 2048], [32320, 1024]]
    ...
    def test_fused_sgd_fp16(self):
        shape_list = [[1], [300], [4096], [278011], [10,10], [100,100],
                      [4096, 1024], [4096, 2048], [32320, 1024]]
        for sz in shape_list:
