...
class RefLAMB(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3, bias_correction=True,
        ...
        # Skip buffer
        self._dummy_overflow_buf = torch.tensor([0], dtype=torch.int).to(self.param_groups[0]["params"][0].device)
    ...
    def step(self, closure=None):
        ...
        if len(g_all_32) > 0:
            g_norm_32 = torch.ops.torch_mlu.fused_l2_norm(
                self._dummy_overflow_buf, g_all_32, False)[0]
        if len(g_all_16) > 0:
            g_norm_16 = torch.ops.torch_mlu.fused_l2_norm(
                self._dummy_overflow_buf, g_all_16, False)[0]
...
class TestFusedLAMB(TestCase):
    ...
    def fused_lamb_dtype(self, test_type=torch.float, sz=[40,40], len=2, is_mixed=False):
        tensors = []
        fused_opt_params = []
        opt_params = []
        ...
        origin_optimizer = self.origin_optimizer(
            opt_params, **self.options)
    ...
    def test_fused_lamb_amp(self):
        ...
        shape_list = [(4096, 1024), (1), (15,20), (2,3,4), (8,1,2,3), (2,1,2,1,4)]
        wd_list = [0, 0.01]
        len_list = [2, 40]
        ...
        for sz, wd, len, is_mixed in product(*list_list):
            self.options['weight_decay'] = wd
            self.fused_lamb_dtype(test_type=torch.float, sz=sz, len=len, is_mixed=is_mixed)
