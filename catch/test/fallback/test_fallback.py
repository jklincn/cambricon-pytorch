...
class TestFallback(TestCase):
    ...
    def test_ops_not_implemented_with_fallback_on(self):
        ...
        self.assertEqual(
            input_cpu.grad, input_mlu.grad.cpu())
        self.assertEqual(
            output_cpu, output_mlu.cpu())
...
"""
    ...
    def test_ops_no_fallback_implementation(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "warp_ctc_loss only support sum mode"):
            torch.ops.torch_mlu.warp_ctc_loss(
                probs.mlu(), target.mlu(), input_lengths.mlu(),
                target_lengths.mlu(), 0, 2, True, 0)
