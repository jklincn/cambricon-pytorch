...
class TestAmp(TestCase):
    ...
    def test_grad_scaling_scale(self):
        ...
        self.assertTrue(outputs[0] == 8.0 and outputs[1][0] == 8.0 and
                        outputs[1][1] == 8.0 and outputs[2][0] == 8.0 and
                        outputs[2][1][0] == 8.0 and outputs[2][1][1] == 8.0)
    ...
    def test_grad_scaling_unscale_sparse(self, device="mlu", dtype=torch.float):
        ...
        p.grad = torch.sparse_coo_tensor(i, v, torch.Size([2, 3]),
                                         device="mlu", dtype=torch.float16)
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf, True)[cur]
        self.assertEqual(found_inf, 1.0)
    ...
    def test_grad_scaling_state_dict(self):
        for lazy_init_scale in True, False:
            s0 = torch.mlu.amp.GradScaler(init_scale=3.,
                                          growth_factor=4.,
                                          backoff_factor=.5,
                                          growth_interval=2)
            s1 = torch.mlu.amp.GradScaler(init_scale=6.,
                                          growth_factor=7.,
                                          backoff_factor=.8,
                                          growth_interval=1)
    ...
    def _create_scaling_models_optimizers(self, device="mlu"):
        mod_control = torch.nn.Sequential(torch.nn.Linear(8, 8),
                                          torch.nn.Linear(8, 8)).to(device=device)
    ...
    def _create_scaling_case(self, device="mlu", dtype=torch.float):
        data = [(torch.randn((8, 8), dtype=dtype, device=device),
    ...
    def _run_scaling_case(self, run, unskipped, skipped, atol=1e-5):
        ...
        for enabled in True, False:
            ...
            scaler = torch.mlu.amp.GradScaler(init_scale=128.,
                                              growth_factor=2.0,
                                              enabled=enabled,
                                              growth_interval=1)
                ...
                for i, (input, target) in enumerate(data):
                    ...
                    if try_scaling_api:
                        ...
                        if scaler.is_enabled():
                            ...
                            self.assertTrue(scaler._found_inf_per_device(optimizer0)[dev0].item()
                                            == 0.)
    ...
    def _run_autocast(self, op, args, run_as_type, out_type=None, module=torch, add_kwargs=None):
        # helper to cast args
        ...
        with torch.mlu.amp.autocast():
            ...
            if hasattr(torch.Tensor, op):
                ...
                if isinstance(out_method, torch.Tensor):
                    self.assertTrue(out_type == out_method.dtype,
                                    "autocast for torch.{} produced {}, should produce torch.{}"
                                    .format(op, out_method.dtype, out_type))

            self.assertTrue((output is not None) or (out_method is not None),
            ...
            if (output is not None) and (out_method is not None):
                ...
                comparison = torch.equal(output, out_method) if isinstance(output, torch.Tensor) \
            ...
            with torch.mlu.amp.autocast(enabled=False):
                ...
                if module is not None and hasattr(module, op):
                    ...
                    control = getattr(args[0].to(run_as_type), op)(*cast(args[1:],
                                                                   run_as_type),
                                                                   **add_kwargs)
                self.assertTrue(type(compare_) == type(control))
                comparison = torch.equal(compare_, control) if isinstance(control, torch.Tensor) \
    ...
    def test_autocast_custom_cast_inputs(self):
        ...
        y = (0, {0: torch.randn((8, 8), device="mlu",
    ...
    def test_amp_bfloat16(self):
        ...
        self.assertTensorsEqual(result_mlu.cpu().float(),
                                result.float(),
                                0.003,
                                use_MSE=True)
