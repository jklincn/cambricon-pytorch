...
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, onlyCPU, onlyNativeDeviceTypes, skipMeta)
...
class TestViewOps(TestCase):
    ...
    def test_view_dtype_new(self, device, dtype):
        ...
        #TODO(PYTORCH-9192): 64-bit view is not supported yet
        del dtypes[torch.int64]
        del dtypes[torch.float64]
        del dtypes[torch.complex128]
        def generate_inputs():
        ...
        for a in generate_inputs():
            ...
            for view_dtype, np_view_dtype in dtypes.items():
                ...
                if not equal_element_size and a.stride(-1) != 1:
                    # TODO(PYTORCH):uncomment the following 2 lines when fill, uniform and random are ready
    ...
    def test_view_as_complex(self, device):
        ...
        # Remove the following comments on error check code after all requirements are ready
    ...
    # TODO(PYTORCH-9187): view as is not support now.
...
class TestOldViewOps(TestCase):
    ...
    def test_view_all_dtypes_and_devices(self, device):
        # TODO(PYTORCH-9185):uncomment the following line when 64-bit view is ready
        # for dt in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
        for dt in (torch.uint8, torch.int8, torch.int16, torch.int32, torch.float16, 
                   torch.float32, torch.complex64, torch.bool):
            x = torch.tensor([[1, 2], [3, 4], [5, 6]], dtype=dt, device=device)
