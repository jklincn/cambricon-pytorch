...
from torch.testing._internal.common_device_type import (
    ...
    onlyCPU, largeTensorTest, precisionOverride,
    onlyCUDA, skipCPUIf, dtypesIfCUDA, skipMeta)
from _internal.mlu_common import dtypes, onlyMLU, dtypesIfMLU
...
class TestTensorCreation(TestCase):
    ...
    def test_cat_all_dtypes_and_devices(self, device):
        # Depends on bfloat16 supported and cast support complex.
        #for dt in all_types_and_complex_and(torch.half, torch.bool, torch.bfloat16, torch.chalf):
        for dt in all_types_and(torch.half, torch.bool):
    ...
    def test_zeros(self, device):
        ...
        # zeros op does not support bfloat16 on mlu
        ...
        # zeros op does not support complex64 on mlu
        ...
        # zeros op does not support complex32 on mlu
    ...
    def test_eye(self, device):
        for dtype in all_types_and_complex_and(torch.half, torch.bool, torch.bfloat16):
            # TODO(PYTORCH-8910): MLU eye don't support tensor dtype ComplexFloat.
            if dtype in (torch.bfloat16, torch.complex64, torch.complex128) :
                continue
    ...
    @unittest.skipIf(not TEST_SCIPY, "Scipy not found")
    @dtypesIfMLU(torch.float, torch.double, torch.bfloat16, torch.half, torch.long)
    ...
    @onlyMLU
    def test_tensor_factory_mlu_type_inference(self, device):
    ...
    @onlyMLU
    def test_tensor_factory_mlu_type(self, device):
        saved_type = torch.tensor([]).type()
        torch.set_default_tensor_type(torch.cuda.FloatTensor)
        x = torch.zeros((5, 5))
    ...
    @dtypesIfMLU(*all_types_and_complex_and(torch.bfloat16))
    ...
    @dtypesIfMLU(*integral_types_and(torch.half, torch.bfloat16, torch.float32, torch.float64) if TEST_WITH_ROCM else
...
class TestRandomTensorCreation(TestCase):
    ...
    @dtypesIfMLU(torch.float, torch.double, torch.half, torch.bfloat16)
