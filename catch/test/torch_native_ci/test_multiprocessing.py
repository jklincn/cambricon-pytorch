...
TEST_MLU_IPC = torch.mlu.is_available() and \
    sys.platform != 'darwin' and \
    sys.platform != 'win32'
TEST_MULTIMLU = TEST_MLU_IPC and torch.mlu.device_count() > 1
...
class TestMultiprocessing(TestCase):
    ...
    @unittest.skipIf(not TEST_MLU_IPC, 'MLU IPC not available')
    ...
    @unittest.skipIf(NO_MULTIPROCESSING_SPAWN, "Disabled for environments that \
                     don't support multiprocessing with spawn start method")
    @unittest.skipIf(not TEST_MLU_IPC, 'MLU IPC not available')
    def test_mlu_send_many(self, name=None, size=5, count=100000):
        ctx = mp.get_context('spawn')
        q1 = ctx.Queue()
        q2 = ctx.Queue()
        q3 = ctx.Queue()
    ...
    @unittest.skipIf(NO_MULTIPROCESSING_SPAWN, "Disabled for environments that \
                     don't support multiprocessing with spawn start method")
    @unittest.skipIf(not TEST_MLU_IPC, 'MLU IPC not available')
    @unittest.skipIf(not TEST_MULTIMLU, 'found only 1 GPU')
...
""")
    ...
    @unittest.skipIf(not torch.cuda.is_available(), 'CUDA not available')
    def test_empty_tensor_sharing_mlu(self):
    ...
    def _test_autograd_sharing(self, var, ctx=mp, is_parameter=False):
        device = 'mlu' if var.is_mlu else 'cpu'
    ...
    # (Issue #11422)
    def _test_mixed_types_mlu_sharing(self, ctx=mp):
    ...
    def test_leaf_variable_sharing(self):
        devices = ['cpu']
        if torch.cuda.is_available() and not NO_MULTIPROCESSING_SPAWN and TEST_MLU_IPC:
            devices.append('cuda')
    ...
    def test_mixed_types_mlu_sharing(self):
        self._test_mixed_types_mlu_sharing(mp.get_context('spawn'))
    ...
    def test_is_shared_mlu(self):
        t = torch.randn(5, 5).mlu()
        self.assertTrue(t.is_shared())
