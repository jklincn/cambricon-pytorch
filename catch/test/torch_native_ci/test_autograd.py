    ...
    def test_type_conversions(self):
        ...
        if torch.mlu.is_available():
            ...
            if torch.mlu.device_count() >= 2:
                x2 = x.float().mlu(1)
                self.assertIsInstance(x2, torch.mlu.FloatTensor)
                ...
                self.assertIsInstance(x2, torch.mlu.FloatTensor)
                self.assertIs(x2.get_device(), 1)
                y = Variable(torch.randn(5).mlu(1), requires_grad=True)
                y.cpu().sum().backward()
        ...
        for t in [torch.DoubleTensor, torch.FloatTensor, torch.IntTensor, torch.ByteTensor]:
            for y_var in (True, False):
                ...
                if torch.mlu.is_available():
                    for x_mlu in (True, False):
                        for y_mlu in (True, False):
                            x_c = x.mlu() if x_mlu else x
                            y_c = y.mlu() if y_mlu else y
                            _, y_type = y_c.type().rsplit('.', 1)
                            y_typestr = ('torch.mlu.' if y_mlu else 'torch.') + y_type
                            self.assertEqual(y_c.type(), x_c.type(y_typestr).type())
                            self.assertIs(y_c.dtype, x_c.type(y_c.dtype).dtype)
                            self.assertEqual(y_c.data_ptr(), y_c.mlu().data_ptr() if y_mlu else y_c.data_ptr())

        self._test_type_conversion_backward(lambda x: x)
        if torch.mlu.is_available():
            self._test_type_conversion_backward(lambda x: x.mlu())
            if torch.mlu.device_count() >= 2:
                # one of these has to be the non-default device
                self._test_type_conversion_backward(lambda x: x.mlu(0))
                self._test_type_conversion_backward(lambda x: x.mlu(1))
...
"""
        ...
        # The autograd engine creates worker threads only when MLU devices are present.
        # So make sure that we do shutdown threads when we're testing mlu and make sure
        # that there is no thread to shutdown when we're not using mlu.
        if TEST_MLU or torch.backends.mps.is_available():
    ...
    @unittest.skipIf(
        not torch.mlu.is_available(),
        "Test requires MLU bf16 support"
    )
    def test_checkpointing_non_reentrant_autocast_mlu(self):
        """
        Test that autocast args/kwargs such as the dtype are preserved during
        non-reentrant checkpoint recomputation on GPU.
        """
        self._test_checkpointing_non_reentrant_autocast(device_type='mlu')

    @unittest.skipIf(not torch.mlu.is_available(), "Test requires MLU")
    @slowTest
    def test_checkpointing_without_reentrant_memory_savings(self):
        ...
        model_no_checkpoint = MyModel(8, use_checkpoint=False, use_reentrant=False).mlu()
        model_reentrant_checkpoint = MyModel(8, use_checkpoint=True, use_reentrant=True).mlu()
        model_no_reentrant_checkpoint = MyModel(8, use_checkpoint=True, use_reentrant=False).mlu()

        x = torch.randn(100, 256, requires_grad=True, device='mlu')

        torch.mlu.reset_peak_memory_stats()
        loss = model_no_checkpoint(x.clone()).sum()
        loss.backward()
        mem_no_checkpoint = torch.mlu.max_memory_allocated()

        torch.mlu.reset_peak_memory_stats()
        loss = model_reentrant_checkpoint(x.clone()).sum()
        loss.backward()
        mem_reentrant_checkpoint = torch.mlu.max_memory_allocated()

        torch.mlu.reset_peak_memory_stats()
        loss = model_no_reentrant_checkpoint(x.clone()).sum()
        loss.backward()
        mem_no_reentrant_checkpoint = torch.mlu.max_memory_allocated()
    ...
    def test_graph_save_on_cpu(self):
        def test(get_input, mlu, pin_memory):
        ...
        for mlu in [False] + ([True] if torch.mlu.is_available() else []):
            for pin_memory in [True, False]:
                # FloatTensor
                test(lambda: torch.randn(5, requires_grad=True), mlu, pin_memory)
                # DoubleTensor
                test(lambda: torch.randn(5, requires_grad=True, dtype=torch.double), mlu, pin_memory)
                # Sparse tensor
                x = torch.sparse_coo_tensor(torch.tensor([[1, 1]]).long(), torch.tensor([1., 1.]), requires_grad=True)
                test(lambda: x, mlu, pin_memory)

    @unittest.skipIf(not TEST_MLU, "test requires MLU")
...
class TestAutogradDeviceType(TestCase):
    ...
    @dtypesIfMLU(torch.half, torch.float, torch.double, torch.int8, torch.int16, torch.int32, torch.int64)
    ...
    def test_reentrant_parent_error_on_cpu(self, device):
        def _get_mlu_memory_usage():
            # we don't need MLU synchronize because the statistics are not tracked at
            # actual freeing, but at when marking the block as free.
            num_devices = torch.mlu.device_count()
            gc.collect()
            return tuple(torch.mlu.memory_allocated(i) for i in range(num_devices))

        before = _get_mlu_memory_usage()
    ...
    def test_free_unneeded_tensor(self, device):
        ...
        z = x.sum()
        base_mem = torch.mlu.memory_allocated()
        z = ((x + 2) * m).sum()
        end_mem = torch.mlu.memory_allocated()
    ...
    def test_profiler_emit_nvtx(self, device):
        ...
        with torch.mlu.profiler.profile():
            with emit_nvtx():
                a.add(1.0)
    ...
    def test_gradcheck_input_output_different_device(self, device):
        x = torch.ones((1,), dtype=torch.double, device="mlu", requires_grad=True)
        gradcheck(lambda x: x.to("cpu"), (x,))

        x = torch.ones((1,), dtype=torch.double, device="cpu", requires_grad=True)
        gradcheck(lambda x: x.to("mlu"), (x,))
    ...
    def test_warning_in_backward(self, device):
        ...
        # NOTE: For device=mlu, warning gets propagated from a worker thread
...
class TestAutogradMultipleDispatch(TestCase):
    ...
    def test_foward_mode_AD(self, device):
        ...
        # dispatch for _test_autograd_multiple_dispatch.fullcoverage and not AutogradMLU
    ...
    def test_view_copy(self, device):
        ...
        if 'mlu' in device:
            # gradient registered to AutogradMLU is grad.reshape_as(self) + 1
...
# e.g., TestAutogradDeviceTypeCPU and TestAutogradDeviceTypeMLU
