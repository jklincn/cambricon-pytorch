...
from torch.testing._internal.common_device_type import (
    instantiate_device_type_tests, onlyCPU, onlyCUDA, onlyNativeDeviceTypes,
...
class TestShapeOps(TestCase):
    ...
    def generate_clamp_baseline(self, device, dtype, *, min_vals, max_vals, with_nans):
        ...
        X = X.to(dtype)
        if with_nans:
            # torch.randint for mlu doesn't support bool.
            # mask = torch.randint(0, 2, X.shape, dtype=torch.bool, device=device)
            mask = torch.randint(0, 2, X.shape, dtype=torch.long, device=device)
            X[mask] = nan
