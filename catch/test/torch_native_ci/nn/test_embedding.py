...
from _internal.mlu_common import TEST_MLU, dtypesIfMLU, skipMLUIf
from _internal.common_nn import NNTestCase
...
class TestEmbeddingNN(NNTestCase):
    ...
    def test_embedding_max_norm_unsorted_repeating_indices(self):
        ...
        ix = ix.to('mlu')
        out = create_embedding('mlu')(ix)
        self.assertEqual(out.cpu(), out_cpu)

    @unittest.skipIf(TEST_MLU, "MLU not support sparse")
...
class TestEmbeddingNNDeviceType(NNTestCase):
    ...
    @unittest.skipIf(TEST_MLU, "MLU not support aten::_indices op and sparse")
    ...
    @dtypesIfMLU(torch.float32, torch.half)
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation, not support padding_idx")
    ...
    @skipMLUIf(True, "no out-of-bounds check on MLU for perf.")
    ...
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation,backward not support per_sample_weights")
    ...
    @skipMeta
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation, not support include_last_offset")
    @dtypes(*itertools.product((torch.int, torch.long), (torch.int, torch.long), (torch.float, torch.double, torch.half)))
    ...
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation, not sparse and backward only support sum mode")
    @dtypesIfCUDA(*itertools.product((torch.int, torch.long), (torch.half, torch.float, torch.double)))
    @dtypesIfMLU(*itertools.product((torch.int, torch.long), (torch.half, torch.float, torch.double)))
    ...
    @skipMeta
    @dtypes(*itertools.product((torch.int, torch.long), (torch.int, torch.long), (torch.float, torch.double, torch.half)))
    # PYTORCH-10088 embedding_bag not support sparse and backward only support sum mode
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation, not support sparse, and backward only support sum mode")
    ...
    @unittest.skipIf(TEST_MLU, "PYTORCH-10088: MLU EmbeddingBag limitation, not support sparse.")
    @dtypes(*itertools.product((torch.int, torch.long), (torch.int, torch.long)))
