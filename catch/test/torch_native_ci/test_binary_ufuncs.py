...
from _internal.mlu_common_methods_invocations import (
    binary_ufuncs,
...
class TestBinaryUfuncs(TestCase):
    ...
    def _test_reference_numerics(self, dtype, op, gen, equal_nan=True):
        ...
        for sample in gen:
            ...
            # TODO(ops not support): MLU reshape_alias is not support now,
            # so convert to cpu first.
            def _convert_mlu_tensor_to_cpu_tensor(x: torch.Tensor):
                if isinstance(x, torch.Tensor) and x.device.type == "mlu":
                    return x.cpu()
                return x

            if _numel(l) <= 100 and _numel(r) <= 100:
                l_cpu = _convert_mlu_tensor_to_cpu_tensor(l)
                r_cpu = _convert_mlu_tensor_to_cpu_tensor(r)
                actual_cpu = _convert_mlu_tensor_to_cpu_tensor(actual)
                msg = (
                    "Failed to produce expected results! Input lhs tensor was"
                    " {0}, rhs tensor was {1}, torch result is {2}, and reference result is"
                    " {3}."
                ).format(l_cpu, r_cpu, actual_cpu, expected)
            else:
                msg = None
    ...
    # TODO(PYTORCH-9114): skip 64bit and complex in allowed dtypes
    @unittest.skipIf(TEST_WITH_ASAN, "Skipped under ASAN")
    @ops(
    ...
    def test_contig_size1_large_dim(self, device, dtype, op):
        # TODO(PYTORCH-9120): CNNL Only support 8-dims.
        # shape = (5, 2, 3, 1, 4, 5, 3, 2, 1, 2, 3, 4)
        shape = (5, 2, 3, 1, 4, 5, 3, 2)
        lhs = make_tensor(
            shape, dtype=dtype, device=device, **op.lhs_make_tensor_kwargs
        )
        rhs = make_tensor(
    ...
    def test_type_promotion(self, device, op):
        ...
        if _supported((torch.int16, torch.int32, torch.int64)):
            ...
            if op.supports_out:
                ...
                # TODO(PYTORCH-9114): bf16 or complex is not support.
    ...
    # TODO(PYTORCH-9106): cnnlCreate handle error.
    ...
    # torch.pow for mlu only support float32 and float16.
    ...
    def _test_pow(self, base, exponent, np_exponent=None):
        ...
        try:
            ...
            if isinstance(base, torch.Tensor):
                ...
                actual = base.clone()
                # When base is a 0-dim cpu tensor and exp is a mlu tensor, we exp `pow` to work but `pow_` to fail, since
                # `pow` will try to create the output tensor on a mlu device, but `pow_` needs to use the cpu tensor as the output
                if (
                    isinstance(exponent, torch.Tensor)
                    and base.dim() == 0
                    and base.device.type == "cpu"
                    and exponent.device.type == "mlu"
                ):
                    regex = "Expected all tensors to be on the same device, but found at least two devices, mlu.* and cpu!"
    ...
    @onlyMLU
    def test_mlu_tensor_pow_scalar_tensor(self, device):
        cuda_tensors = [
    ...
    @onlyMLU
    def test_cpu_tensor_pow_mlu_scalar_tensor(self, device):
        mlu_tensors = [
            torch.tensor(5.0, device="mlu"),
            torch.tensor(-3, device="mlu"),
        ]
        for exp in mlu_tensors:
            base = torch.randn((3, 3), device="cpu")
            regex = "Expected all tensors to be on the same device, but found at least two devices, mlu.* and cpu!"
            self.assertRaisesRegex(RuntimeError, regex, torch.pow, base, exp)
        for exp in mlu_tensors:
            # Binary ops with a cpu + mlu tensor are allowed if the cpu tensor has 0 dimension
            base = torch.tensor(3.0, device="cpu")
            self._test_pow(base, exp)

    @onlyMLU
    @dtypes(torch.complex64, torch.complex128)
    def test_pow_mlu_complex_extremal_failing(self, device, dtype):
    ...
    def test_binary_op_scalar_device_unspecified(self, devices):
        ...
        for default_device in devices:
            with torch.mlu.device(default_device):
    ...
    # TODO(PYTORCH-9105): complex and bfloat16 is not support.
    @onlyMLU
    ...
    def test_divmul_scalar(self, device, dtype):
        # CTR-4559 MLU do not handle cases when value exceed half max value
    ...
    def test_floor_divide_tensor(self, device, dtype):
        # TODO(CNNLCORE-2677): cnnl true div have precision problem on 370 platform
        # This problem will not fix so set a random seed to avoid getting error
    ...
    def test_muldiv_scalar(self, device, dtype):
        ...
        #TODO(xxx): div is not support now.
        # self.assertEqual(x / s, x / y)
        # self.assertEqual(s / x, y / x)
    ...
    def test_add(self, device):
        # TODO(PYTORCH-9105): CATCH add is not support complex.
    ...
    # TODO(CNNLCORE-12770): type promote is different, need cnnl align with cuda.
    @onlyMLU
    ...
    # TODO(PYTORCH-9105): CATCH OpTensor not support complex.
    @onlyMLU
    @dtypes(
        torch.chalf,
    )
