...
from _internal.mlu_common import onlyMLU
from _internal.common_modules import module_db, modules, TrainEvalMode
...
class TestModule(TestCase):
    ...
    def test_multiple_device_transfer(self, device, dtype, module_info, training):
        ...
        for module_input_device, module_input_cpu in zip(module_inputs_device, module_inputs_cpu):
            ...
            with freeze_rng_state():
                ...
                # === Move back to MLU and forward pass ===
                m.mlu()
                m(*input_device_args, **input_device_kwargs)
                self._assert_module_parameters_and_buffer_are(m, device, dtype)
                ...
                if torch.mlu.device_count() >= 2:
                    ...
                    self._assert_module_parameters_and_buffer_are(m, torch.device("mlu:1"), dtype)
    ...
    @modules(module_db)
    def test_cpu_mlu_parity(self, device, dtype, module_info, training):
    ...
    def test_memory_format(self, device, dtype, module_info, training):
        ...
        def _check_conv_out_mem_format(output, input_mem_format, module_mem_format, no_batch_dim):
            def inner_check_out_mem_format(output):
                d = output.dim()
                if output.device.type == "mlu":
                    # TODO(PYTORCH-10345): CATCH Conv only support CL. and no batch dim is true
                    # batch dim is deleted, so we need to add this dim back.
                    if no_batch_dim:
                    ...
                    if (d == 5):
                        ...
                        view4d_output = output.unsqueeze(2)
                        self.assertTrue(view4d_output.is_contiguous(memory_format=torch.channels_last))
                else:
            ...
            with freeze_rng_state():
                ...
                for input_mem_format in input_mem_formats:
                    ...
                    for module_mem_format in module_mem_formats:
                        ...
                        if "CONV" in module_cls.__name__.upper():
                            no_batch_dim = False if module_input.desc == "" else True
                            _check_conv_out_mem_format(outputs, input_mem_format, module_mem_format, no_batch_dim)
                        else:
