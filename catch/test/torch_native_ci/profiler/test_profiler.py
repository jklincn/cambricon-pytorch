...
from torch_mlu.profiler import _utils
...
# TODO(PYTORCH-9415): Early registration for MLU
with _profile(use_mlu=True, use_kineto=kineto_available()) as p:
    pass

@unittest.skipIf(not HAS_PSUTIL, "Requires psutil to run")
@unittest.skipIf(not torch.mlu.is_available(), "MLU is required")
class TestProfilerMLU(TestCase):

    def test_mem_leak(self):
        """Checks that there's no memory leak when using profiler with MLU
        """
        t = torch.rand(1, 1).mlu()
        p = psutil.Process()
        last_rss = collections.deque(maxlen=5)
        for outer_idx in range(10):
            with _profile(use_mlu=True):
                for _ in range(1024):
                    t = torch.mm(t, t)
        ...
        # with MLU events leaking the increase in memory was ~7 MB between
...
class TestProfiler(TestCase):
    ...
    def test_kineto(self):
        use_mlu = torch.profiler.ProfilerActivity.MLU in supported_activities()
        with _profile(use_mlu=use_mlu, use_kineto=True):
            self.payload(use_mlu=use_mlu)

        # rerun to avoid initial start overhead
        with _profile(use_mlu=use_mlu, use_kineto=True) as p:
            self.payload(use_mlu=use_mlu)
        output = p.key_averages().table(
            sort_by="self_mlu_time_total" if use_mlu else "self_cpu_time_total", row_limit=-1)
        # print(output)
        found_Gepb = False
        found_memcpy = False
        found_mm = False
    ...
    def test_kineto_multimlu(self):
        with profile(
            activities=[
                ProfilerActivity.CPU,
                ProfilerActivity.MLU]) as prof:
            for mlu_id in [0, 1]:
                x = torch.randn(10, 10).mlu(mlu_id)
                y = torch.randn(10, 10).mlu(mlu_id)
                z = x.matmul(y)
        ...
        for evt in prof.events():
            if "gepb" in evt.name.lower() and evt.device_type == DeviceType.MLU:
                if evt.device_index == 0:
                    found_gepb_0 = True
                elif evt.device_index == 1:
                    found_gepb_1 = True

            if "cnMemcpy" in evt.name and evt.device_type == DeviceType.CPU:
                found_mlu = True

        self.assertTrue(found_gepb_0)
        self.assertTrue(found_gepb_1)
        self.assertTrue(found_mlu)
    ...
    def test_memory_profiler(self):
        ...
        def create_mlu_tensor():
            return torch.rand(10, 10).mlu()
        ...
        if torch.mlu.is_available():
            create_mlu_tensor()
            stats = run_profiler(create_mlu_tensor)
            check_metrics(
                stats,
                "mlu_memory_usage",
                allocs=[
    ...
    def test_oom_tracing(self):
        ...
        def create_mlu_tensor_oom():
            device = torch.device("mlu:0")
            # Tried to allocate 120.00 GiB
            return torch.empty(1024, 1024, 1024, 30, dtype=torch.float32, device=device)
        ...
        if torch.mlu.is_available():
            with TemporaryFileName(mode="w+") as fname:
                prof = run_profiler(create_mlu_tensor_oom)
                check_trace(fname)
    ...
    def test_flops(self):
        ...
        if not (kineto_available() and torch.mlu.is_available()):
            return
    ...
    def test_export_stacks(self):
        with _profile(use_mlu=True, with_stack=True, use_kineto=kineto_available()) as p:
            x = torch.randn(10, 10).to('mlu')
            y = torch.randn(10, 10).to('mlu')
            z = torch.mm(x, y)
            z = z + y
    ...
    def test_profiler_metadata(self):
        t1, t2 = torch.ones(1).to('mlu'), torch.ones(1).to('mlu')
        with profile(use_mlu=True) as prof:
            torch.add(t1, t2)
    ...
    def _test_profiler_tracing(self, use_kineto):
        ...
        # Same test but for mlu.
        use_mlu = torch.profiler.ProfilerActivity.MLU in supported_activities()
        if not use_mlu:
            return

        device = torch.device("mlu:0")
        with _profile(use_mlu=True, use_kineto=use_kineto) as prof:
...
class MockKinetoEvent():
    ...
    def device_type(self) -> DeviceType:
        return DeviceType.MLU if self._device_type == 1 else DeviceType.CPU
...
class TestExperimentalUtils(TestCase):
    ...
    def generate_mock_profile():
        mlu_events = [
            MockKinetoEvent("cnInvokeKernel", 400, 100, 1, 0),
            MockKinetoEvent("cnInvokeKernel", 500, 100, 2, 0),
            MockKinetoEvent("cnInvokeKernel", 600, 100, 3, 0),
            MockKinetoEvent("cnInvokeKernel", 700, 100, 4, 0),
            MockKinetoEvent("cnInvokeKernel", 800, 100, 5, 0),
            MockKinetoEvent("cnInvokeKernel", 1500, 100, 6, 0),
            MockKinetoEvent("MLU", 900, 100, 1, 1),
            MockKinetoEvent("MLU", 1000, 100, 2, 1),
            MockKinetoEvent("MLU", 1100, 100, 3, 1),
            MockKinetoEvent("MLU", 1200, 100, 4, 1),
            MockKinetoEvent("MLU", 1300, 100, 5, 1),
            MockKinetoEvent("MLU", 1700, 100, 6, 1)
        ]
        cpu_events = [
            MockProfilerEvent("CPU (Before cnInvokeKernel)", 1, 0, 100000),
            MockProfilerEvent("CPU (Before cnInvokeKernel)", 2, 100000,
                              100000),
            MockProfilerEvent("CPU (Before cnInvokeKernel)", 3, 200000,
                              100000),
            MockProfilerEvent("CPU (Before cnInvokeKernel)", 4, 300000,
                              100000),
            MockProfilerEvent("CPU (After cnInvokeKernel)", 5, 400000,
                              100000),
            MockProfilerEvent("CPU (After cnInvokeKernel)", 6, 500000,
                              100000),
            MockProfilerEvent("CPU (After cnInvokeKernel)", 7, 600000,
                              100000),
            MockProfilerEvent("CPU (After cnInvokeKernel)", 8, 700000,
                              100000),
            MockProfilerEvent("CPU (After MLU)", 9, 800000, 100000),
            MockProfilerEvent("CPU (After MLU)", 10, 900000, 100000),
            MockProfilerEvent("CPU (After MLU)", 11, 1100000, 100000),
            MockProfilerEvent("CPU (After MLU)", 12, 1200000, 500000),
        ]

        profiler = unittest.mock.Mock()
        profiler.kineto_results = unittest.mock.Mock()
        profiler.kineto_results.events = unittest.mock.Mock(
            return_value=mlu_events)
    ...
    def load_mock_profile():
        ...
        if accept and torch.mlu.is_available():

            def garbage_code(x):
                for i in range(5):
                    x[0, i] = i

            x = torch.ones((4096, 4096), device="mlu")
            x = x @ x
            with profile(
            ...
            kineto_events = [{
                ...
                '_device_type':
                1 if e.device_type() == DeviceType.MLU else 0
            ...
            with open(json_file_path, "w") as f:
                json.dump([kineto_events, profiler_events], f, indent=2)
...
0 [CPU (Before cnInvokeKernel)]
0 [CPU (Before cnInvokeKernel)]
0 [CPU (Before cnInvokeKernel)]
0 [CPU (Before cnInvokeKernel)]
1 [CPU (After cnInvokeKernel)]
2 [CPU (After cnInvokeKernel)]
3 [CPU (After cnInvokeKernel)]
4 [CPU (After cnInvokeKernel)]
5 [CPU (After MLU)]
4 [CPU (After MLU)]
2 [CPU (After MLU)]
1 [CPU (After MLU)]
""")
    ...
    def test_utils_compute_queue_depth_when_no_mlu_events(self):
    ...
    def test_utils_compute_idle_time(self):
        ...
        self.assertExpectedInline(
            expected_output, """\
100000 [CPU (Before cnInvokeKernel)]
100000 [CPU (Before cnInvokeKernel)]
100000 [CPU (Before cnInvokeKernel)]
100000 [CPU (Before cnInvokeKernel)]
0 [CPU (After cnInvokeKernel)]
0 [CPU (After cnInvokeKernel)]
0 [CPU (After cnInvokeKernel)]
0 [CPU (After cnInvokeKernel)]
0 [CPU (After MLU)]
...
#         self.assertExpectedInline(
#             expected_output, """\
# <built-in method _synchronize>
# pytest(8): <module>""")
        # MLU outputs are different from cuda.
        # Further analysis in PYTORCH-9419.
        self.assertIn('<built-in method _synchronize', expected_output)
        self.assertIn('pytest(8): <module>', expected_output)
...
aten::mm""")
    ...
    @unittest.skipIf(not torch.mlu.is_available(), "MLU is required")
    def test_profiler_extra_mlu_copy_pattern(self):
        cases = (
            (0, lambda: torch.ones((100, 100), device="mlu")),
            (1, lambda: torch.ones((100, 100)).to("mlu")),
            (1, lambda: torch.zeros((100, 100)).to("mlu")),
            (1, lambda: torch.empty((100, 100)).fill_(5).to("mlu")),
            (1, lambda: torch.ones((100, 100)).mlu()),
            (1, lambda: torch.zeros((100, 100)).mlu()),
            (1, lambda: torch.empty((100, 100)).fill_(5).mlu()),
            (1, lambda: torch.rand((100, 100)).mlu()),
            (1, lambda: torch.randn((100, 100)).mlu()),
            (1, lambda: torch.full((100, 100), 10).mlu()),
            (0, lambda: torch.rand((100, 100)).to(dtype=torch.float16)),
            (0, lambda: torch.rand((100, 100)).half()),
            (0, lambda: torch.rand((100, 100), device="mlu").half()),
        )
        num_matched = []
        for _, fn in cases:
            with profile(with_stack=True, record_shapes=True) as prof:
                fn()
            pattern = ExtraMLUCopyPattern(prof)
    ...
    def test_profiler_fp32_matmul_pattern(self):
        x = torch.ones((100, 100), device="mlu")
        with profile(with_stack=True, record_shapes=True) as prof:
            x = x @ x
    ...
    @unittest.skipIf(not torch.mlu.is_available(), "MLU is required")
    def test_profiler_extra_mlu_copy_pattern_benchmark(self):
        with profile(with_stack=True, record_shapes=True) as prof:
            x = torch.ones((100, 100)).to("mlu")
            x = torch.ones((50, 50)).to("mlu")
        pattern = ExtraMLUCopyPattern(prof)
    ...
    def test_profiler_matmul_dim_fp16_pattern(self):
        cases = (
            (1, torch.randn((201, 201), device='mlu', dtype=torch.float16)),
            (1, torch.randn((3, 97, 97), device='mlu', dtype=torch.float16)),
            (0, torch.randn((200, 200), device='mlu', dtype=torch.float16)),
            (0, torch.randn((3, 200, 200), device='mlu', dtype=torch.float16))
        )
        num_matched = []
        for _, x in cases:
            with profile(with_stack=True, record_shapes=True) as prof:
