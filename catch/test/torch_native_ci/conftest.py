...
from mlu_exclude_caseid_list import MLU_EXCLUDE_CASEID_LIST
from mlu_fixed_exclude_uts import MLU_FIXED_EXCLUDE_UTS
def pytest_collection_modifyitems(items):
    for item in items:
        caseid = item.nodeid
        caseid = caseid[caseid.rfind('/')+1:]
        ut_name = caseid[:caseid.find('::')]
        ut_name_no_suffix = ut_name.split('.')[0]
        caseid = caseid.replace('::', '_')
        caseid = caseid.replace('.', '_')
        def match_fixed_exclude_ut():
            for fixed_exclude in MLU_FIXED_EXCLUDE_UTS:
                if fnmatch(ut_name, fixed_exclude) or \
                   fnmatch(ut_name_no_suffix, fixed_exclude):
                    return True
            return False

        if match_fixed_exclude_ut():
            item.add_marker(pytest.mark.skip('Skip on MLU.'))
        elif caseid in MLU_EXCLUDE_CASEID_LIST:
            item.add_marker(pytest.mark.skip('MLU not support now.'))
        else:
            item.add_marker(caseid)
