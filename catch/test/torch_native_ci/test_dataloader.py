...
# We cannot import TEST_MLU from torch.testing._internal.common_cuda here, because if we do that,
...
if TEST_MLU:
    dev_name = torch.mlu.get_device_name(torch.mlu.current_device()).lower()
...
class MLUCountingDataset(Dataset):
    def __init__(self, n):
        super(MLUCountingDataset, self).__init__()
        self.n = n

    def __getitem__(self, i):
        return torch.as_tensor(i, device='mlu')
...
"""])
    ...
    def test_bulk_loading_nobatch(self):
        ...
        for num_workers in [0, 4]:
            dl = self._get_data_loader(ds, num_workers=num_workers, batch_size=None, sampler=sampler, pin_memory=TEST_MLU)
            self.assertFalse(dl._auto_collation)
            samples = list(dl)
            self.assertEqual(samples[0].is_pinned(), TEST_MLU)
    ...
    def test_timeout(self):
        if TEST_MLU and not NO_MULTIPROCESSING_SPAWN:
    ...
    def test_multiprocessing_contexts(self):
        ...
        counting_ds_n = 11
        dl_common_args = dict(num_workers=3, batch_size=3, pin_memory=(not TEST_MLU))
        for ctx in supported_multiprocessing_contexts:
            # windows and jetson devices don't support sharing cuda tensor; ROCm does not yet fully support IPC
            if ctx in ['spawn', 'forkserver'] and TEST_MLU and not IS_WINDOWS and not IS_JETSON:
                ds_cls = MLUCountingDataset
            else:
    ...
    def test_multiprocessing_iterdatapipe(self):
        ...
        dl_common_args = dict(num_workers=2, batch_size=2, shuffle=True, pin_memory=(not TEST_MLU))
        for ctx in supported_multiprocessing_contexts:
    ...
    def test_proper_exit(self):
        ...
        for is_iterable_dataset, use_workers, pin_memory, hold_iter_reference in \
            ...
            if pin_memory and (not TEST_MLU or NO_MULTIPROCESSING_SPAWN or IS_WINDOWS):
                # This test runs in a subprocess, which can only initialize CUDA with spawn.
...
class TestNamedTupleDataLoader(TestCase):
    ...
    def test_dataloader_with_namedtuple(self):
        # auto-collation
        loader = DataLoader(self.dataset, batch_size=2, pin_memory=TEST_MLU)
        for batch in loader:
            self.assertIsInstance(batch, NamedTupleDataset.Batch)
            self.assertEqual(batch.random_tensor.is_pinned(), TEST_MLU)
            self.assertIsInstance(batch.data, NamedTupleDataset.Data)
            self.assertIsInstance(batch.data.positive, torch.Tensor)
            self.assertEqual(batch.data.positive.is_pinned(), TEST_MLU)
        # no auto-collation
        loader = DataLoader(self.dataset, batch_size=None, pin_memory=TEST_MLU)
        for batch in loader:
            self.assertIsInstance(batch, NamedTupleDataset.Batch)
            self.assertEqual(batch.random_tensor.is_pinned(), TEST_MLU)
            self.assertIsInstance(batch.data, NamedTupleDataset.Data)
