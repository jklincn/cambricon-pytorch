...
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, has_cusolver,
...
class TestLinalg(TestCase):
    ...
    def test_addr_type_promotion(self, device):
        # TODO(PYTORCH-8811): MLU addr only support floating types.
        for dtypes0, dtypes1, dtypes2 in product(floating_types_and(torch.bfloat16, torch.float16), repeat=3):
    ...
    @dtypesIfMLU(torch.float, torch.cfloat)
    ...
    @dtypesIfMLU(torch.float, torch.complex64)  # Integer matmul just supported on CPU
    ...
    @dtypesIfMLU(*floating_and_complex_types_and(
...
'''.format(tol, eq_err, eq_err_general, iters1, eq_err_scipy, eq_err_general_scipy, iters2, m, k, niter))
    ...
    @dtypesIfMLU(*floating_types_and(torch.bfloat16))
    @dtypes(torch.float, torch.double)
    ...
    @precisionOverride({torch.double: 1e-8, torch.float: 1e-4, torch.bfloat16: 0.6,
                        torch.half: 1e-1, torch.cfloat: 1e-4, torch.cdouble: 1e-8})
    @dtypesIfMLU(*floating_types_and(
                  *[torch.bfloat16] if TEST_WITH_ROCM or (CUDA11OrLater and SM53OrLater) else []))
    ...
    @dtypes(torch.float, torch.double)
    @dtypesIfMLU(*floating_and_complex_types())
    ...
    @dtypesIfMLU(torch.float32, torch.float64, torch.cfloat, torch.cdouble)
    @tf32_on_and_off(0.01)
    ...
    def test_slogdet_errors_and_warnings(self, device, dtype):
        ...
        with self.assertRaisesRegex(RuntimeError, r'must have at least 2 dimensions'):
            torch.linalg.slogdet(a)
        #TODO: [PYTORCH-10242]  
        a = torch.randn(2, 2, dtype=torch.bfloat16).to('mlu')
