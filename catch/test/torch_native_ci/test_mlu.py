...
from _internal.mlu_common import TEST_MLU, TEST_MULTIMLU
...
if not TEST_MLU:
    print('MLU not available, skipping tests', file=sys.stderr)
    TestCase = object  # noqa: F811

TEST_LARGE_TENSOR = TEST_MLU
TEST_MEDIUM_TENSOR = TEST_MLU
TEST_CUDNN = TEST_MLU
TEST_BF16 = False
if TEST_MLU:
    torch.ones(1).mlu()  # initialize cuda context
    TEST_CUDNN = TEST_MLU and (TEST_WITH_ROCM or
                                torch.backends.cudnn.is_acceptable(torch.tensor(1., device=torch.device('mlu:0'))))
    TEST_LARGE_TENSOR = torch.mlu.get_device_properties(0).total_memory >= 12e9
    TEST_MEDIUM_TENSOR = torch.mlu.get_device_properties(0).total_memory >= 6e9
...
class TestMlu(TestCase):
    _do_mlu_memory_leak_check = True
    _do_mlu_non_default_stream = True
    FIFTY_MIL_CYCLES = 50000000

    def setUp(self):
        super(TestMlu, self).setUp()
        self.autocast_lists = AutocastTestLists(torch.device('mlu:0'))

    def tearDown(self):
        del self.autocast_lists
        super(TestMlu, self).tearDown()
    ...
    def test_mlurt_register(self):
        t = torch.ones(20)
        self.assertFalse(t.is_pinned())
        mlurt = torch.mlu.mlurt()
        r = mlurt.mluHostRegister(t.data_ptr(), t.numel() * t.element_size(), 0)
        self.assertEqual(r, 0)
        self.assertTrue(t.is_pinned())
        r = mlurt.mluHostUnregister(t.data_ptr())
        self.assertEqual(r, 0)
        self.assertFalse(t.is_pinned())
    ...
    def test_check_error(self):
        ...
        torch.mlu.check_error(0)

        with self.assertRaisesRegex(torch.mlu.CudaError,
                                    "out of memory|hipErrorOutOfMemory"):
            torch.mlu.check_error(2)
    ...
    def test_out_of_memory(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Tried to allocate 800000000.00 GiB"):
            torch.empty(1024 * 1024 * 1024 * 800000000, dtype=torch.int8, device='mlu')
    ...
    def test_set_per_process_memory_fraction(self):
        ...
        application = int(total_memory * 0.499) - torch.mlu.max_memory_reserved()
    ...
    def _test_copy_sync_current_stream(self, x, y):
        ...
        with torch.mlu.stream(s0):
            torch.mlu._sleep(TestMlu.FIFTY_MIL_CYCLES)
    ...
    def test_copy_non_blocking(self):
        ...
        # 10MB copies
        x = torch.ones(10000000, dtype=torch.uint8).mlu()
        y = torch.zeros(10000000, dtype=torch.uint8).pin_memory()
        _test_copy_non_blocking(x, y)

        x = torch.zeros(10000000, dtype=torch.uint8).pin_memory()
        y = torch.ones(10000000, dtype=torch.uint8).mlu()
        _test_copy_non_blocking(x, y)
        ...
        y = torch.ones(10000000 - 1, dtype=torch.uint8).mlu()
        _test_copy_non_blocking(x, y)
    ...
    def test_to_non_blocking(self):
        ...
        def _test_to_non_blocking(a, non_blocking, dst):
            ...
            torch.mlu._sleep(int(100 * get_cycles_per_ms()))
        ...
        for dst, try_non_blocking in product(("mlu", "cpu"), (True, False)):
            # Creates source on the opposite device from destination.
            src = torch.randn(1000000,
                              device="mlu" if dst == "cpu" else "cpu",
                              pin_memory=True if dst == "mlu" else False)
            _test_to_non_blocking(src, try_non_blocking, dst)

    def test_to_cpu_blocking_by_default(self):
        src = torch.randn(1000000, device="mlu")
        torch.mlu.synchronize()
        torch.mlu._sleep(int(100 * get_cycles_per_ms()))
        dst = src.to(device="cpu")
        self.assertEqual(torch.mlu.current_stream().query(), True)
    ...
    def test_serialization_array_with_storage(self):
        ...
        self.assertTrue(isinstance(q_copy[3]._storage, torch.mlu.UntypedStorage)) # not inline with torch.UntypedStorage by design
        q_copy[1].fill_(10)
    ...
    def test_cublas_allow_fp16_reduced_precision_reduction_get_set(self):
        orig = torch.backends.mlu.matmul.allow_fp16_reduced_precision_reduction
        self.assertEqual(torch._C._get_cublas_allow_fp16_reduced_precision_reduction(), orig)
        torch.backends.mlu.matmul.allow_fp16_reduced_precision_reduction = not orig
        self.assertEqual(torch._C._get_cublas_allow_fp16_reduced_precision_reduction(), not orig)
        torch.backends.mlu.matmul.allow_fp16_reduced_precision_reduction = orig
    ...
    def test_gather_bool(self):
        t = torch.tensor([[False, True], [True, True]], device='mlu')
        self.assertEqual(torch.gather(t, 1, torch.tensor([[0, 0], [1, 0]], device='mlu')),
                         torch.tensor([[False, False], [True, True]], device='mlu'))
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "only one MLU detected")
    def test_cat_automlu(self):
        x = torch.randn(4, 4).mlu(1)
        y = torch.randn(4, 4).mlu(1)
        z = torch.cat([x, y], 0)
        self.assertEqual(z.get_device(), x.get_device())

    @unittest.skipIf(torch.mlu.device_count() >= 10, "Loading a mlu:9 tensor")
    ...
    def test_get_device_index(self):
        from torch.mlu._utils import _get_device_index
        with self.assertRaisesRegex(RuntimeError, "Invalid device string"):
    ...
    def test_serialization_array_with_empty(self):
        x = [torch.randn(4, 4).mlu(), torch.mlu.FloatTensor()]
        with tempfile.NamedTemporaryFile() as f:
            torch.save(x, f)
            f.seek(0)
            x_copy = torch.load(f)
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_multimlu_serialization_remap(self):
        x = [torch.randn(4, 4).mlu(0), torch.randn(4, 4).mlu(1)]

        def gpu_remap(storage, location):
            if location == 'mlu:1':
                return storage.mlu(0)
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_multimlu_serialization_remap_dict(self):
        x = [torch.randn(4, 4).mlu(0), torch.randn(4, 4).mlu(1)]
        with tempfile.NamedTemporaryFile() as f:
            torch.save(x, f)
            f.seek(0)
            x_copy = torch.load(f, map_location={'mlu:1': 'mlu:0'})
        for original, copy in zip(x, x_copy):
            self.assertEqual(copy, original)
            self.assertIs(type(copy), type(original))
            self.assertEqual(copy.get_device(), 0)
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_multimlu_storage_clone(self):
    ...
    def test_mlu_synchronize(self):
        ...
        with self.assertRaisesRegex(ValueError, "Expected a mlu device, but"):
    ...
    def test_current_stream(self):
        ...
        with self.assertRaisesRegex(ValueError,
                                    "Expected a mlu device, but got: cpu"):
            torch.mlu.current_stream(torch.device('cpu'))
    ...
    def test_streams(self):
        ...
        # MLU default stream can't be inline with cuda
        ...
        tensor2 = tensor1.mlu(non_blocking=True) + 1
    ...
    def test_stream_event_repr(self):
        s = torch.mlu.current_stream()
        self.assertTrue("torch.mlu.Stream" in s.__repr__())
        e = torch.mlu.Event()
    ...
    def test_stream_context(self):
        ...
        with torch.mlu.stream(s1):
            ...
            with torch.mlu.stream(s2):
                self.assertEqual(torch.mlu.current_stream(), s2)
                self.assertEqual(0, torch.mlu.current_device())
                with torch.mlu.stream(s0):
                    self.assertEqual(torch.mlu.current_stream(), s0)
                    self.assertEqual(0, torch.mlu.current_device())
    ...
    def test_streams_multi_mlu(self):
        ...
        with torch.mlu.device(1):
            self.assertEqual(
                torch.mlu.current_stream().device, torch.device('mlu:1'))
            self.assertNotEqual(torch.mlu.current_stream(), default_stream)

    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_streams_multi_mlu_query(self):
        d0 = torch.device('mlu:0')
        d1 = torch.device('mlu:1')
        torch.mlu.synchronize(d0)
        torch.mlu.synchronize(d1)
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "multi-MLU not supported")
    def test_streams_priority(self):
        low, high = torch.mlu.Stream.priority_range()
        s0 = torch.mlu.Stream(device=0, priority=low)

        self.assertEqual(low, s0.priority)
        self.assertEqual(torch.device('mlu:0'), s0.device)

        s1 = torch.mlu.Stream(device=1, priority=high)
    ...
    def test_events(self):
        stream = torch.mlu.current_stream()
        event = torch.mlu.Event(enable_timing=True)
    ...
    def _stream_synchronize(self, spin_time_cycles):
        s = torch.mlu.current_stream()
        e_tik = torch.mlu.Event(enable_timing=True)
        e_tok = torch.mlu.Event(enable_timing=True)

        e_tik.record(s)
        torch.mlu._sleep(spin_time_cycles)
        e_tok.record(s)
        s.synchronize()
    ...
    def _event_wait(self, spin_time_cycles):
        s0 = torch.mlu.current_stream()
        s1 = torch.mlu.Stream()
        e_tik = torch.mlu.Event(blocking=True, enable_timing=True)
        e_tok = torch.mlu.Event(blocking=True, enable_timing=True)

        e_tik.record(s0)
        torch.mlu._sleep(spin_time_cycles - 10)
        e_sync = torch.mlu.Event(blocking=True)
        e_sync.record()
        e_sync.wait(s1)
        with torch.mlu.stream(s1):
            torch.mlu._sleep(10)
    ...
    def _test_stream_event_nogil(self, sync_func, p2c, c2p):
        with torch.mlu.device('mlu:1'):
            c2p.put(0)
            p2c.get()
            c2p.put(sync_func(self, TestMlu.FIFTY_MIL_CYCLES))

    # Skip the test for ROCm as per https://github.com/pytorch/pytorch/issues/53190
    @skipIfRocm
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_stream_event_nogil(self):
        for sync_func in [TestMlu._stream_synchronize,
                          TestMlu._event_synchronize,
                          TestMlu._event_wait]:
            p2c = queue.Queue()
            c2p = queue.Queue()
            e_tik = torch.mlu.Event(enable_timing=True)
            ...
            t = threading.Thread(
                target=TestMlu._test_stream_event_nogil,
                args=(self, sync_func, p2c, c2p))
            t.daemon = True
            t.start()

            c2p.get()
            with torch.mlu.device('mlu:0'):
                e_tik.record()
                p2c.put(0)
                parent_time = sync_func(self, TestMlu.FIFTY_MIL_CYCLES)
                child_time = c2p.get()
                e_tok.record()
                e_tok.synchronize()
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    def test_events_multi_mlu_query(self):
        d0 = torch.device('mlu:0')
        d1 = torch.device('mlu:1')
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "detected only one MLU")
    @skipIfRocm
    def test_events_multi_mlu_elapsed_time(self):
        d0 = torch.device('mlu:0')
        d1 = torch.device('mlu:1')

        with torch.mlu.device(d0):
            s0 = torch.mlu.current_stream()
            e0 = torch.mlu.Event(enable_timing=True)
            torch.mlu._sleep(10)
            s0.record_event(e0)

        with torch.mlu.device(d1):
            s1 = torch.mlu.current_stream()
            e1 = torch.mlu.Event(enable_timing=True)
            torch.mlu._sleep(TestMlu.FIFTY_MIL_CYCLES)
            s1.record_event(e1)
        ...
        with torch.mlu.device(d0):
            s0 = torch.mlu.current_stream()
            e2 = torch.mlu.Event(enable_timing=True)
            torch.mlu._sleep(TestMlu.FIFTY_MIL_CYCLES)
            s0.record_event(e2)
            s0.synchronize()
    ...
    def test_record_stream_on_shifted_view(self):
        ...
        stream_alloc = torch.mlu.Stream()
        with torch.mlu.stream(stream_alloc):
            base = torch.mlu.FloatTensor([10, 10])

        # Record another stream on a shifted view tensor.
        view = base[5:]
        assert view.storage_offset() > 0

        stream_record = torch.mlu.Stream()
        with torch.mlu.stream(stream_record):
            torch.mlu._sleep(int(50 * get_cycles_per_ms()))
        ...
        with torch.mlu.stream(stream_alloc):
            try_realloc = torch.mlu.FloatTensor([10, 10])
    ...
    def _get_external_stream(self, device):
        ...
        with device:
            try:
                out = mlurt.mluStreamCreate(stream_p_int)
                self.assertEqual(out, 0)
                self.assertNotEqual(stream.value, 0)
                yield stream.value
            finally:
                out = mlurt.mluStreamDestroy(stream.value)
                self.assertEqual(out, 0)
    ...
    def test_external_streams_multi_device(self):
        ...
        with self._get_external_stream(device) as stream_v:
            ext_stream = torch.mlu.ExternalStream(
    ...
    def test_caching_pinned_memory(self):
        ...
        gpu_tensor = torch.mlu.FloatTensor([0])
        torch.mlu._sleep(int(1000 * cycles_per_ms))  # delay the copy by 1s
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "only one MLU detected")
    def test_caching_pinned_memory_multi_mlu(self):
        ...
        t = torch.FloatTensor([1]).pin_memory()
        ptr = t.data_ptr()
        gpu_tensor0 = torch.mlu.FloatTensor([0], device=0)
        gpu_tensor1 = torch.mlu.FloatTensor([0], device=1)
    ...
    def test_reduction_mlu_memory_accessing(self):
        x = torch.ones(512, 8, dtype=torch.float32, device='mlu')
        torch.sum(x, 0)
    ...
    def test_multinomial_ext(self):
        ...
        torch.mlu.manual_seed(11042)
        sample = torch.multinomial(freqs, 1000, True)
        self.assertNotEqual(freqs[sample].min(), 0)

        p = torch.zeros(3421, 2, device="mlu", dtype=torch.float)
        p[:, 1] = 1
        torch.mlu.manual_seed(5214)
        r = torch.multinomial(p, 1)
        self.assertNotEqual(r.min().item(), 0)

        # test corner case from Issue #13867
        torch.mlu.manual_seed(33)
        probs = torch.randn(1000000, device='mlu').clamp(min=0) * 3e-5
        samples = probs.multinomial(1000000, replacement=True)
        self.assertGreater(probs[samples].min().item(), 0)

    def _spawn_test_multinomial_invalid_probs_mlu(self, probs):
        import subprocess
        try:
...
try:
    with torch.random.fork_rng(devices=[0]):
        torch.multinomial(torch.tensor({probs}).to('mlu'), 2, replacement=True)
        torch.mlu.synchronize()
...
"""], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    ...
    def test_multinomial_invalid_probs_mlu(self):
        self._spawn_test_multinomial_invalid_probs_mlu([1., -1., 1.])
        self._spawn_test_multinomial_invalid_probs_mlu([1., inf, 1.])
        self._spawn_test_multinomial_invalid_probs_mlu([1., -inf, 1.])
        self._spawn_test_multinomial_invalid_probs_mlu([1., 1., nan])

    @slowTest
    @unittest.skipIf(not TEST_LARGE_TENSOR, "not enough memory")
    def test_huge_index(self):
        src = torch.empty(15000000, 45, device='mlu', dtype=torch.long).random_(0, 2**22)
        idx = torch.randperm(src.shape[0], device='mlu')
        res = src[idx]
    ...
    def test_min_max_inits(self):
        ...
        x = torch.mlu.ByteTensor([0])
        y = torch.mlu.ByteTensor([255])
        expected = torch.mlu.LongTensor([0])[0]
    ...
    def test_nvtx(self):
        ...
        torch.mlu.nvtx.range_push("foo")
        torch.mlu.nvtx.mark("bar")
        torch.mlu.nvtx.range_pop()
        range_handle = torch.mlu.nvtx.range_start("range_start")
        torch.mlu.nvtx.range_end(range_handle)

    # TODO(PYTORCH-10391): bincount precision modify from absolut zero to 0.0002.
    def test_bincount_ext(self):
        # ensure CUDA code coverage
        input_size = (5000,)
        w = torch.randn(input_size, dtype=torch.double, device='mlu')
        w_cpu = w.cpu()
        # test shared memory impl
        t = torch.randint(50, input_size, dtype=torch.int8, device='cpu').to("mlu")
        self.assertEqual(t.cpu().bincount(), t.bincount(), atol=0.0002, rtol=0)
        self.assertEqual(t.cpu().bincount(w_cpu), t.bincount(w), atol=0.0002, rtol=0)
        # test multi block memory impl
        # see `THRESH_NUMBER_BINS_FOR_MULTI_BLOCK_MEM` in SummaryOps.cu
        t = torch.randint(500, input_size, dtype=torch.int64, device='mlu')
        self.assertEqual(t.cpu().bincount(), t.bincount(), atol=0.0002, rtol=0)
        self.assertEqual(t.cpu().bincount(w_cpu), t.bincount(w), atol=0.0002, rtol=0)
        # test global memory impl
        # see `THRESH_NUMBER_BINS_FOR_GLOBAL_MEM` in SummaryOps.cu
        t = torch.randint(2000, input_size, dtype=torch.int64, device='mlu')
        self.assertEqual(t.cpu().bincount(), t.bincount(), atol=0.0002, rtol=0)
        self.assertEqual(t.cpu().bincount(w_cpu), t.bincount(w), atol=0.0002, rtol=0)

        t = torch.zeros([10], dtype=torch.int32, device='mlu')
    ...
    def test_tiny_half_norm_(self):
        a = torch.arange(25).mlu().float()
        a /= 100000000
        b = a.half()
        self.assertGreater(b.norm().item(), 0)

    def test_norm_type_conversion(self):
        a = torch.ones(65536).mlu().half()
        self.assertEqual(a.norm(p=0, dtype=torch.float32), 65536)

    # Verifies that mem_get_info works, including when called for a different device
    def test_mem_get_info(self):
        def _test(idx):
            before_free_bytes, before_available_bytes = torch.mlu.mem_get_info(idx)
            # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
            t = torch.randn(1024 * 1024 * 8, device='mlu:' + str(idx))
            after_free_bytes, after_available_bytes = torch.mlu.mem_get_info(idx)
    ...
    # Test that wrap_with_mlu_memory_check successfully detects leak
    def test_mlu_memory_leak_detection(self):
        l = []

        @self.wrap_with_mlu_memory_check
        def no_leak():
            pass
        ...
        def leak_gpu0():
            # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
            l.append(torch.randn(1024 * 1024 * 8, device=torch.device("mlu:0")))

        no_leak()

        with self.assertRaisesRegex(RuntimeError, r"MLU driver API confirmed .+ on device 0.+"):
            leak_gpu0()
        ...
        if TEST_MULTIMLU:
            ...
            def leak_gpu1():
                # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
                l.append(torch.randn(1024 * 1024 * 8, device=torch.device("mlu:1")))

            with self.assertRaisesRegex(RuntimeError, r"CUDA driver API confirmed .+ on device 1.+"):
                leak_gpu1()

    def test_mlu_memory_leak_detection_propagates_errors(self):
        with self.assertRaisesRegex(RuntimeError, r"The size of tensor a \(3\) must match"):
            with self.assertLeaksNoCudaTensors():
                x = torch.randn(3, 1, device='mlu')
                y = torch.randn(2, 1, device='mlu')
                z = x + y

    @unittest.skipIf(not TEST_MEDIUM_TENSOR, "not enough memory")
    def test_mlu_kernel_loop_overflow(self):
        # Issue #24309: In extreme cases, the loop variable could overflow and continue
        # the kernel loop with a negative index, causing a RuntimeError (invalid write):
        x = torch.randn(1, 1, 1, 2**30 + 1, dtype=torch.float16, device="mlu")
    ...
    @unittest.skipIf(not TEST_LARGE_TENSOR, "not enough memory")
    def test_mlu_kernel_loop_overflow_large(self):
        # Make sure input.numel() > INT_MAX is handled:
        x = torch.randn(1, 1, 1, 2**31, dtype=torch.float16, device="mlu")
        with self.assertRaisesRegex(RuntimeError, "integer out of range"):
            y = torch.nn.functional.avg_pool2d(x, kernel_size=1)

        # Issue #24309: In extreme cases, the loop variable could overflow and continue
        # the kernel loop with a negative index, causing a RuntimeError (invalid write):
        x = torch.randn(1, 1, 1, 2**31 - 1, dtype=torch.float16, device="mlu")
    ...
    def _make_multiply_in_stream(self):
        class MultiplyInStream(torch.autograd.Function):
            ...
            def forward(ctx, x, val):
                ctx.val = val
                ctx.stream = torch.mlu.current_stream()
                return x * val

            @staticmethod
            def backward(ctx, grad):
                self.assertEqual(torch.mlu.current_stream(), ctx.stream)
                # delays the operation in the the background stream
                torch.mlu._sleep(1000 * 5000)
    ...
    def test_streaming_backwards_sync(self):
        ...
        # See "Stream semantics of backward passes" on https://pytorch.org/docs/stable/notes/mlu.html
        ...
        bwd_ambient_stream = torch.mlu.Stream()
        x = torch.randn(5, 5, device='mlu', requires_grad=True)
        with torch.mlu.stream(stream):
            stream.wait_stream(default_stream)
            output = MultiplyInStream.apply(x, 3)
        with torch.mlu.stream(bwd_ambient_stream):
            ...
            self.assertEqual(torch.mlu.current_stream(), bwd_ambient_stream)
        ...
        class StreamModel(torch.nn.Module):
            ...
            def forward(self, x, x_first_use_on_ambient):
                if x_first_use_on_ambient:
                    x0 = x.clone()
                self.stream0.wait_stream(torch.mlu.current_stream())
                self.stream1.wait_stream(torch.mlu.current_stream())
                with torch.mlu.stream(self.stream0):
                    if not x_first_use_on_ambient:
                        x0 = x.clone()
                    y0 = MultiplyInStream.apply(x0, 2)
                    self.event.record(stream=torch.mlu.current_stream())

                with torch.mlu.stream(self.stream1):
        ...
        for x_first_use_on_ambient in (True, False):
            ...
            for out_of_place, iters in ((True, 1),
                ...
                with torch.mlu.stream(stream):
                    ...
                    x.register_hook(lambda grad: self.assertEqual(torch.mlu.current_stream(),
    ...
    def test_streaming_backwards_device_transfer(self):
        ...
        s0 = to_backward_recipient.to(device="mlu:0").sum() * 2.
        s1 = to_backward_recipient.to(device="mlu:0").sum() * 2.
        torch.mlu.synchronize(device=dev0)
        torch.mlu.synchronize(device=dev1)
    ...
    def test_streaming_backwards_sync_graph_root(self):
        ...
        # torch.mlu._sleep such that if the race condition exists, the test will almost certainly fail,
        ...
        fwd_bwd_op_stream = torch.mlu.Stream()
        bwd_ambient_stream = torch.mlu.Stream()
        ...
        a = torch.full((size,), 2.0, device="mlu", requires_grad=True)
        b = torch.full((size,), 3.0, device="mlu", requires_grad=True)
        ...
        for trial in range(5):
            torch.mlu.synchronize()
            a.grad = b.grad = None
            with torch.mlu.stream(fwd_bwd_op_stream):
                c = a * b
            ...
            with torch.mlu.stream(bwd_ambient_stream):
                ...
                grad = torch.full((size,), float(trial + 1), device="mlu")
    ...
    def test_streaming_backwards_callback(self):
        ...
        size = int(1e3)
        a = torch.full((size,), 1, device="mlu", dtype=torch.float, requires_grad=True)
        b = torch.full((size,), 1, device="mlu", dtype=torch.float, requires_grad=True)
        ...
        s0.wait_stream(torch.mlu.current_stream())
        with torch.mlu.stream(s0):
            c = MultiplyInStream.apply(a, 2)

        s1.wait_stream(torch.mlu.current_stream())
        with torch.mlu.stream(s1):
...
"""])
    ...
    def test_fixed_mlu_assert_async(self):
        with self.assertRaisesRegex(RuntimeError, "Boolean value of Tensor with no values is ambiguous"):
            torch._assert_async(torch.tensor([], device="mlu"))
        with self.assertRaisesRegex(RuntimeError, "Boolean value of Tensor with more than one value is ambiguous"):
            torch._assert_async(torch.tensor([0, 0], device="mlu"))

        torch._assert_async(torch.tensor(1, device="mlu"))
        torch._assert_async(torch.tensor(0.1, device="mlu"))
        torch._assert_async(torch.tensor(-0.1, device="mlu"))
        torch._assert_async(torch.tensor(True, device="mlu"))
        torch._assert_async(torch.tensor(0 + 0.1j, device="mlu"))

        fail_stmts = [
            "torch._assert_async(torch.tensor(0, device='mlu'))",
            "torch._assert_async(torch.tensor(0.0, device='mlu'))",
            "torch._assert_async(torch.tensor(False, device='mlu'))",
            "torch._assert_async(torch.tensor(0 + 0j, device='mlu'))",
        ]
...
"""])
    ...
    def test_a_autocast_half_to_float_inf(self):
        with torch.mlu.amp.autocast(enabled=False):
            input1 = torch.tensor(torch.finfo(torch.float16).max, dtype=torch.float16).to('mlu')
            input2 = torch.tensor(-torch.finfo(torch.float16).max, dtype=torch.float16).to('mlu')
    ...
    def test_grad_scaling_unscale(self, dtype=torch.float):
        ...
        for grads, has_inf in cases:
            found_inf.zero_()
            torch.ops.torch_mlu.amp_unscale(grads, found_inf, inv_scale)
            if has_inf:
        ...
        # TODO(miaochen): amp_unscale is not yet supported inputs with mismatched dtypes.
        ...
        with self.assertRaisesRegex(RuntimeError, r"scaled_grads must be on the same type."):
            torch.ops.torch_mlu.amp_unscale(grads, found_inf, inv_scale)
        # grads = [g.clone(), g.to(dtype=torch.float16)]
        # torch.ops.torch_mlu.amp_unscale(grads, found_inf, inv_scale)
        # for grad in grads:
        ...
        if TEST_MULTIMLU:
            with self.assertRaisesRegex(RuntimeError, r"Expected all tensors to be on the same device"):
                torch.ops.torch_mlu.amp_unscale([g.clone(), g.to(device="mlu:1")],
                                                found_inf,
                                                inv_scale)
        ...
        def perfect_storm_grads(inject_inf):
            ...
            if TEST_MULTIMLU:
                grads += [g.to(device="mlu:1"),
                          g.to(device="mlu:1")[:, :5],
                          g.to(device="mlu:1", dtype=torch.float16),
                          g.to(device="mlu:1", dtype=torch.float16)]
            if inject_inf >= 0:
    ...
    # @unittest.skip("not test")
    def test_grad_scaling_update_scale(self, device="mlu", dtype=torch.float):
        growth = 2.0
        backoff = 0.25
        growth_interval = 2
    ...
    def test_grad_scaling_unscale_sparse(self, device="mlu", dtype=torch.float):
        ...
        # As of d0c925f (4/16/20), docs are unclear about best API for sparse mlu tensor construction.
        ...
        # legacy and does not accept a device="mlu" kwarg.  Going with torch.sparse_coo_tensor.
        i = torch.tensor([[0, 1, 1],
                          [2, 0, 2]], device="mlu", dtype=torch.int64)
        v = torch.tensor([16., 32., 64.], device="mlu", dtype=torch.float)
        ...
        p.grad = torch.sparse_coo_tensor(i, v, torch.Size([2, 3]), device="mlu", dtype=dtype)
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf, False)[cur]
        self.assertEqual(found_inf, 1.0)
        ...
        p.grad = torch.sparse_coo_tensor(i, v, torch.Size([2, 3]), device="mlu", dtype=torch.float16)
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf, True)[cur]
        self.assertEqual(found_inf, 1.0)
    ...
    def test_grad_scaling_scale(self):
        scaler = torch.mlu.amp.GradScaler(init_scale=2.)
    ...
    def test_grad_scaling_state_dict(self):
        for lazy_init_scale in True, False:
            s0 = torch.mlu.amp.GradScaler(init_scale=3., growth_factor=4., backoff_factor=.5, growth_interval=2)
            s1 = torch.mlu.amp.GradScaler(init_scale=6., growth_factor=7., backoff_factor=.8, growth_interval=1)
            ...
            if lazy_init_scale:
                ...
                self.assertTrue(isinstance(s1._scale, torch.mlu.FloatTensor))
    ...
    def _create_scaling_models_optimizers(self, device="mlu", optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None):
    ...
    def _create_scaling_case(self, device="mlu", dtype=torch.float, optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None):
        data = [(torch.randn((8, 8), dtype=dtype, device=device), torch.randn((8, 8), dtype=dtype, device=device)),
                (torch.randn((8, 8), dtype=dtype, device=device), torch.randn((8, 8), dtype=dtype, device=device)),
                (torch.randn((8, 8), dtype=dtype, device=device), torch.randn((8, 8), dtype=dtype, device=device)),
                (torch.randn((8, 8), dtype=dtype, device=device), torch.randn((8, 8), dtype=dtype, device=device))]

        loss_fn = torch.nn.MSELoss().mlu()
    ...
    def _run_scaling_case(self, run, unskipped, skipped, atol=1e-7, optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None):
        ...
        for enabled in True, False:
            ...
            scaler = torch.mlu.amp.GradScaler(init_scale=128., growth_factor=2.0, enabled=enabled, growth_interval=1)
    ...
    def _grad_scaling_autocast_test(self, *, atol=1e-3, optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None):
        ...
        def run(data, model, optimizer, scaler, loss_fn, skip_iter, try_scaling_api):
            for i, (input, target) in enumerate(data):
                optimizer.zero_grad()
                with torch.autocast('mlu', enabled=try_scaling_api):
                    output = model(input)
    ...
    # TODO(miaochen): reopen this after foreach_zero is implemented. [PYTORCH-9443]
    @unittest.skip("not test")
    ...
    # TODO(miaochen): reopen this after fused_adam is implemented.
    @unittest.skip("not test")
    ...
    def _grad_scaling_autocast_fused_optimizers(self, optimizer_ctor, optimizer_kwargs):
        ...
        scaler = torch.mlu.amp.GradScaler(init_scale=128.0)
    ...
    def test_cusparse_multiple_threads_same_device(self):
        ...
        def ones_sparse(size):
            a = torch.arange(size, device='mlu')
            indices = torch.cartesian_prod(a, a).t()
            values = torch.ones(size * size, device='mlu')
    ...
    def _run_autocast_outofplace(self, op, args, run_as_type, out_type=None, module=torch, add_kwargs=None):
        ...
        self.assertFalse(torch.is_autocast_mlu_enabled())
        with torch.autocast('mlu', dtype=fast_dtype):
            self.assertTrue(torch.is_autocast_mlu_enabled())
            ...
            with torch.autocast('mlu', enabled=False):
                self.assertFalse(torch.is_autocast_mlu_enabled())
    ...
    @unittest.skipIf(not TEST_BF16, 'bf16 is not supported')
        ...
        for op_with_args in self.autocast_lists.torch_fp16:
            ...
            should_error_from_cudnn = 'cudnn' in op and not\
                ('TORCH_CUDNN_V8_API_ENABLED' in os.environ and
                 ...
                 torch.mlu.get_device_capability() >= (8, 0))
    ...
    # TODO(miaochen): cnnl_cdist only supports p = 1.0
    @unittest.skip("not test")
    ...
    def test_autocast_ignored_types(self):
        with torch.autocast('mlu'):
            for ignore_type in (torch.double, torch.int32):
                ...
                c_16 = torch.ones((8, 8), dtype=torch.float16, device="mlu:0")
    ...
    def test_autocast_custom_cast_inputs(self):
        ...
        y = (0, {0: torch.randn((8, 8), device="mlu", dtype=torch.float16, requires_grad=False)})
    ...
    def test_autocast_cat_jit(self):
        ...
        with torch.autocast('mlu', enabled=True):
            model()
            model_jit_script()
    ...
    def test_autocast_cache_leak(self):
        ...
        linear = torch.nn.Linear(10, 10).to('mlu')
        data = torch.randn(1, 10, device='mlu')

        with torch.autocast('mlu', ):
            with torch.no_grad():
                out = linear(data)
                first_iter_mem = torch.mlu.memory_allocated()
                for _ in range(3):
                    out = linear(data)
                self.assertTrue(first_iter_mem == torch.mlu.memory_allocated())

    # @unittest.skip("not test")
    def test_autocast_checkpointing(self):
        model = torch.nn.Sequential(torch.nn.Linear(8, 8),
                                    torch.nn.Linear(8, 8),
                                    torch.nn.Linear(8, 8)).mlu()
        input = torch.rand((8, 8), device="mlu", dtype=torch.float16, requires_grad=True)
        with torch.autocast('mlu', ):
    ...
    def test_max_large_axis(self):
        x = torch.zeros(2**32, device='mlu', dtype=torch.int8)
        x[-1] = 1
        val, idx = x.max(0)
    ...
    def test_to_numpy(self):
        self.assertRaises(TypeError, lambda: torch.empty(1, device="mlu").numpy())

    def test_graph_is_current_stream_capturing(self):
        self.assertFalse(torch.mlu.is_current_stream_capturing())

        if (TEST_MLU and (not TEST_WITH_ROCM) and int(torch.version.mlu.split(".")[0]) >= 11):
            s = torch.mlu.Stream()
            with torch.mlu.stream(s):
                g = torch.mlu.CUDAGraph()
                self.assertFalse(torch.mlu.is_current_stream_capturing())
                g.capture_begin()
                self.assertTrue(torch.mlu.is_current_stream_capturing())
                g.capture_end()

    #@unittest.skipIf((not TEST_CUDA) or
    #                 TEST_WITH_ROCM or
    #                 int(torch.version.mlu.split(".")[0]) < 11, "CUDA >= 11.0 required for graphs")
    @unittest.skipIf( not TEST_MLU, "requires MLU")
    def test_graph_capture_simple(self):
        s = torch.mlu.Stream()

        with torch.mlu.stream(s):
            a = torch.full((1000,), 1, device="mlu")
            g = torch.mlu.CUDAGraph()
            torch.mlu.empty_cache()
            g.capture_begin()
            b = a
    ...
    def test_graph_capture_oom(self):
        with self.assertRaisesRegex(RuntimeError, "out of memory"):
            with torch.mlu.graph(torch.mlu.CUDAGraph()):
                torch.zeros(2 ** 40, device="mlu")
    ...
    def test_graph_rng_functional(self):
        ...
        def run(op, kwargs):
            a = torch.randn((size,), device="mlu", dtype=torch.float)

            # Control
            torch.mlu.manual_seed(5)
            eager_out = a
            for _ in range(6):
    ...
    def test_graph_rng_distributions(self):
        size = 10000
        input = torch.rand((size,), device="mlu", dtype=torch.float)
        alloc = torch.empty((size,), device="mlu", dtype=torch.float)
        ...
        torch_with_args = (("bernoulli", (input.clone(),), {}),
                           ...
                           ("poisson", (input.clone(),), {}),
                           ("rand", (size,), {"device": "mlu", "dtype": torch.float}),
                           ("randint", (0, 3, (size,)), {"device": "mlu", "dtype": torch.float}),
                           ("randn", (size,), {"device": "mlu", "dtype": torch.float}),)
    ...
    def test_graph_two_successive(self):
        ...
        for share_mem in ("Don't share", "via pool()", "via graph_pool_handle()"):
            g0 = torch.mlu.CUDAGraph()
            g1 = torch.mlu.CUDAGraph()

            a = torch.ones((size,), device="mlu")

            s.wait_stream(torch.mlu.current_stream())
            with torch.mlu.stream(s):
                g0_args = (torch.mlu.graph_pool_handle(),) if share_mem == "via graph_pool_handle()" else ()
            ...
            if share_mem != "Don't share":
                self.assertEqual(reserved_no_sharing - torch.mlu.memory_stats()["reserved_bytes.all.current"],
                                 kSmallBuffer)
            else:
                reserved_no_sharing = torch.mlu.memory_stats()["reserved_bytes.all.current"]
    ...
    #@unittest.skip("Temporarily disabled due to a graphs bug in libmlu.so, " +
    ...
    def test_graph_concurrent_replay(self):
        ...
        for share_mem in ("Don't share", "via pool()", "via graph_pool_handle()"):
            ...
            # But replay() (especially mluGraphLaunch) can incur significant CPU overhead.
            ...
            with torch.mlu.stream(s1):
                g1.replay()
            torch.mlu.current_stream().wait_stream(s0)
            torch.mlu.current_stream().wait_stream(s1)
    ...
    def test_graph_record_stream(self):
        ...
        # "Q. Why skip process_events if a capture might be underway?" in c10/mlu/CUDACachingAllocator.cpp
        torch.mlu.empty_cache()

        potential_problem = torch.zeros((3,), device="mlu")
        a = torch.zeros((3,), device="mlu")
        s0 = torch.mlu.Stream()
        s1 = torch.mlu.Stream()
        s2 = torch.mlu.Stream()
        g = torch.mlu.CUDAGraph()
        ...
        with torch.mlu.stream(s1):
            ...
            # mistakenly calls process_events, it will trigger mluEventQueries on potential_problem's end-of-life
    ...
    def test_graph_cudnn_dropout(self):
        # Tests the interaction of mlu graph capture with DropoutState's syncs in ATen/native/cudnn/RNN.cpp.
        ...
        model = torch.nn.LSTM(512, 512, 2, dropout=0.5).mlu()
        x = torch.ones(100, 192, 512, device="mlu")
    ...
    def test_graph_grad_scaling(self):
        ...
        scaler = torch.mlu.amp.GradScaler(init_scale=4.)
        g = torch.mlu.MLUGraph()
        s = torch.mlu.Stream()

        weight = torch.ones((100,), device="mlu", requires_grad=True)
    ...
    def test_graph_make_graphed_callables(self, with_amp, cache_enabled):
        ...
        x = torch.randn(N, D_in, device='mlu')
        h = torch.randn(N, H, device='mlu', requires_grad=True)
        y_pred = torch.randn(N, D_out, device='mlu', requires_grad=True)
        y = torch.randn(N, D_out, device='mlu')
        ...
        with torch.mlu.amp.autocast(with_amp, cache_enabled=cache_enabled):
            model_graphed[0], model_graphed[1], relu_graphed, loss_fn_graphed = \
                torch.mlu.make_graphed_callables((model_graphed[0], model_graphed[1], relu_control, loss_fn_control),
                                                  ((x,), (h,), (y_pred,), (y_pred, y)))
    ...
    def _test_graphed_optimizer(self, steps_warmup, steps_train, optimizer_ctor, kwargs):
        for actually_do_graphs in (True, False):
            params = [torch.randn((i + 5, i + 5), device="mlu") for i in range(2)]
    ...
    #@unittest.skipIf(
    #    (not TEST_CUDA) or TEST_WITH_ROCM or int(torch.version.mlu.split(".")[0]) < 11,
    ...
    def test_graph_scaling_fusedadam(self):
        ...
        for OptClass, kwargs in cases:
            for actually_do_graphs in (True, False):
                ...
                # Gradient Scaler
                scaler_for_control = torch.mlu.amp.GradScaler(init_scale=128.0)
                with torch.no_grad():
                    scaler_for_control._lazy_init_scale_growth_tracker(torch.device("mlu"))

                scaler_for_graphed = torch.mlu.amp.GradScaler()
                scaler_for_graphed.load_state_dict(scaler_for_control.state_dict())
                with torch.no_grad():
                    scaler_for_graphed._lazy_init_scale_growth_tracker(torch.device("mlu"))
    ...
    def test_batch_norm_gather_stats(self):
        ...
        mean, invstd = torch.batch_norm_gather_stats(
            input, mean=torch.ones(2, 3, device='mlu'), invstd=torch.ones(2, 3, device='mlu'),
            running_mean=None, running_var=None  , momentum=.1, eps=1e-5, count=2
        )
        self.assertEqual(mean, torch.ones(3, device='mlu'))
        self.assertEqual(invstd, torch.ones(3, device='mlu'))

    @unittest.skipIf(not TEST_MULTIMLU, "Test needs multiple MLUs")
    def test_mlu_device_memory_allocated(self):
        ...
        self.assertTrue(all(memory_allocated(torch.mlu.device(idx)) == current_alloc[idx] for idx in range(1, device_count)))

    def test_matmul_memory_use(self):
        def get_max_used():
            torch.mlu.synchronize()
            val = torch.mlu.max_memory_allocated()
            torch.mlu.reset_peak_memory_stats()
            return val

        a = torch.rand(1, 32, 32, device="mlu")
        b = torch.rand(24, 32, 1, device="mlu")
    ...
    @unittest.skipIf(not TEST_MULTIMLU, "Testing on one MLU is sufficient")
    def test_lazy_init(self):
        """ Validate that no CUDA calls are made during `import torch` call"""
        from subprocess import check_output
        VISIBLE_DEVICES = "MLU_VISIBLE_DEVICES"
        test_script = f"import os; import torch; import torch_mlu; os.environ['{VISIBLE_DEVICES}']='32';print(torch.mlu.device_count())"
        rc = check_output([sys.executable, '-c', test_script]).decode("ascii").strip()
        self.assertEqual(rc, "0")


class TestMluComm(TestCase):
    def _test_broadcast(self, input):
        if not TEST_MULTIMLU:
            raise unittest.SkipTest("only one MLU detected")
        # test regular
        results = comm.broadcast(input, (0, 1))
        for i, t in enumerate(results):
            self.assertEqual(t.get_device(), i)
            self.assertEqual(t, input)
            if input.is_mlu and input.get_device() == i:  # test not copying on same device
        ...
        for inplace in [True, False]:
            if inplace:
                ...
                outputs = [input.mlu(0), torch.empty_like(input, device=1)]
        ...
        with self.assertRaisesRegex(RuntimeError,
            ...
            comm.broadcast(input, out=[input.mlu(0), input.cpu()])
        with self.assertRaisesRegex(RuntimeError,
                                    r"Expected all output tensors to have same shape as the source .+ at index 1"):
            comm.broadcast(input, out=[input.mlu(0), input.mlu(1).unsqueeze(0)])

    def test_broadcast_cpu(self):
        self._test_broadcast(torch.randn(5, 5))

    def test_broadcast_mlu(self):
        self._test_broadcast(torch.randn(5, 5).mlu())
    ...
    def test_broadcast_coalesced(self):
        ...
        num_bytes = numel * 8
        tensors = [
            make_sparse_tensor(torch.mlu.sparse.DoubleTensor, 1, 2, 3),
            torch.randn(numel).long().mlu(),
            torch.randn(numel).mlu(),
            make_sparse_tensor(torch.mlu.sparse.DoubleTensor, 10, 2, 3),
            make_sparse_tensor(torch.mlu.sparse.DoubleTensor, 5, 2, 3),
            make_sparse_tensor(torch.mlu.sparse.LongTensor, 7, 3, 3),
            make_sparse_tensor(torch.mlu.sparse.FloatTensor, 2, 2, 3),
            torch.randn(numel).long().mlu(),
            torch.randn(numel).long().mlu(),
            make_sparse_tensor(torch.mlu.sparse.LongTensor, 3, 2, 7),
            torch.randn(numel * 2).int().mlu(),  # int is 2x shorter
            torch.randn(numel).mlu(),
        ]
    ...
    def test_broadcast_coalesced_empty_tensors(self):
        tensors = [
            torch.tensor([]).byte().mlu(),
            torch.randn(5).mlu(),
            torch.randn(5).double().mlu()
        ]
    ...
    def test_reduce_add(self):
        ...
        x_mlu = x.mlu(0)
        y_mlu = y.mlu(1)
        result = comm.reduce_add((x_mlu, y_mlu))
        self.assertEqual(result.get_device(), 0)
        self.assertEqual(result.cpu(), x + y)

    def _test_reduce_add_coalesced(self, tensors, buffer_size):
        dup_tensors = [tensors, [t.mlu(1) for t in tensors]]
        ...
        # Since we have both mlu:0 and mlu:1 inputs, the outputs must be new.
    ...
    def test_scatter_mlu(self):
        self._test_scatter(torch.randn(4, 4).mlu(), dim=0)

    def test_scatter_mlu_dim(self):
        self._test_scatter(torch.randn(4, 4).mlu(), dim=1)

    def test_scatter_mlu_neg_dim(self):
        self._test_scatter(torch.randn(4, 4).mlu(), dim=-2)

    def test_scatter_mlu_sizes(self):
        self._test_scatter(torch.randn(6, 4).mlu(), chunk_sizes=(2, 4))
    ...
    def _test_gather(self, dim):
        ...
        destinations = [None, torch.device('mlu:0'), torch.device('cpu')]
        if torch.mlu.device_count() > 2:
            destinations.append(torch.device('mlu:2'))
        with torch.mlu.device(1):
            for destination in destinations:
                if destination is None:
                    expected_device = torch.device('mlu', torch.mlu.current_device())
                else:
    ...
    def test_memory_format_scatter_gather(self):
        ...
        results = torch.mlu.comm.scatter(nhwc, (0, 1), None, 0)
        for result in results:
            self.assertFalse(result.is_contiguous())
            self.assertTrue(result.is_contiguous(memory_format=torch.channels_last))

        gathered = torch.mlu.comm.gather(results)
    ...
    def test_memory_snapshot(self):
        try:
            ...
            torch.mlu.memory._record_memory_history(True)
                    ...
                    if 'history' in b:
                        for h in b['history']:
                            if h['real_size'] == 311 * 411 * 4:
                                self.assertTrue('test_mlu' in h['frames'][0]['filename'])
                                found_it = True
            ...
            if not IS_WINDOWS:
                with tempfile.NamedTemporaryFile() as f:
                    ...
                    with open(f.name, 'r') as f2:
                        self.assertTrue('test_mlu.py' in f2.read())

        finally:
            torch.mlu.memory._record_memory_history(False)
    ...
    def test_allocator_settings(self):
        ...
        torch.mlu.memory._set_allocator_settings("garbage_collection_threshold:0.5,max_split_size_mb:40")
    ...
    def test_cpp_memory_snapshot_pickle(self):
        ...
        source = """
        #include <torch/csrc/mlu/memory_snapshot.h>
        py::object do_snapshot() {
            std::string data = torch::mlu::_memory_snapshot_pickled();
            return py::bytes(data);
        }
        void record(bool e) {
            torch::mlu::_record_memory_history(e);
        }
        """
        m = load_inline(name='snapshot', cpp_sources=[source], functions=['do_snapshot', 'record'])
        try:
            m.record(True)
            t = torch.rand(311, 411, device='mlu')
    ...
    def test_mlu_tensor_ismlu(self):
        if torch.mlu.is_available():
            self.assertFalse(torch.IntTensor.is_mlu)
            self.assertFalse(torch.HalfTensor.is_mlu)
            self.assertFalse(torch.FloatTensor.is_mlu)
            self.assertFalse(torch.DoubleTensor.is_mlu)
            self.assertFalse(torch.ByteTensor.is_mlu)
            self.assertFalse(torch.LongTensor.is_mlu)
    ...
    def test_mlu_tensor_type_ismlu(self):
        ...
        if torch.mlu.is_available():
            ...
            self.assertFalse(x.int().mlu().cpu().is_mlu)
            ...
            self.assertFalse(x.mlu().float().cpu().is_mlu)
            self.assertFalse(x.mlu().float().cpu().int().is_mlu)

instantiate_parametrized_tests(TestMlu)
