...
from torch.testing._internal.common_device_type import \
    (PYTORCH_TESTING_DEVICE_EXCEPT_FOR_KEY, PYTORCH_TESTING_DEVICE_ONLY_FOR_KEY,
     get_device_type_test_bases, instantiate_device_type_tests, onlyCUDA, onlyNativeDeviceTypes,
     deviceCountAtLeast, expectedFailureMeta)
from _internal.mlu_common import dtypes, ops, onlyMLU
...
class TestTesting(TestCase):
    ...
    @onlyMLU
    @slowTest
    def test_mlu_assert_should_stop_common_utils_test_suite(self, device):
...
class TestThatContainsMLUAssertFailure(TestCase):
    ...
    @slowTest
    def test_throw_unrecoverable_mlu_exception(self):
        x = torch.rand(10, device='cuda')
        # cause unrecoverable CUDA exception, recoverable on CPU
        y = x[torch.tensor([25])].cpu()

    @slowTest
    def test_trivial_passing_test_case_on_cpu_mlu(self):
...
""")
    ...
    @onlyMLU
    @slowTest
    def test_mlu_assert_should_stop_common_device_type_test_suite(self, device):
...
class TestThatContainsMLUAssertFailure(TestCase):
    ...
    @slowTest
    def test_throw_unrecoverable_mlu_exception(self, device):
        x = torch.rand(10, device=device)
        # cause unrecoverable CUDA exception, recoverable on CPU
        y = x[torch.tensor([25])].cpu()

    @slowTest
    def test_trivial_passing_test_case_on_cpu_mlu(self, device):
        x1 = torch.tensor([0., 1.], device=device)
        x2 = torch.tensor([0., 1.], device='cpu')
        self.assertEqual(x1, x2)

instantiate_device_type_tests(
    TestThatContainsMLUAssertFailure,
    globals(),
    only_for='mlu'
)
...
""")
    ...
    @onlyMLU
    @slowTest
    def test_mlu_assert_should_not_stop_common_distributed_test_suite(self, device):
...
class TestThatContainsMLUAssertFailure(MultiProcessTestCase):
...
instantiate_device_type_tests(TestAssertCloseMultiDevice, globals(), only_for="mlu")
