...
from torch.testing._internal.common_device_type import (
    ...
    onlyNativeDeviceTypes, onlyCUDA, largeTensorTest, precisionOverride)
from _internal.mlu_common import dtypes, dtypesIfMLU, ops, onlyMLU
...
def _generate_input(shape, dtype, device, with_extremal):
    if shape == ():
        ...
        if dtype.is_floating_point or dtype.is_complex:
            ...
            # "random_" not implemented for 'Short', 'Char' and 'Byte'
            #x = torch.randint(15, 100, shape, dtype=dtype, device=device)
            if dtype == torch.int16 or dtype == torch.int8 or dtype == torch.uint8:
                x = torch.randint(15, 100, shape, dtype=dtype, device='cpu').to(device)
            else:
                x = torch.randint(15, 100, shape, dtype=dtype, device=device)
...
class TestReductions(TestCase):
    ...
    @ops(filter(lambda op: op.nan_policy == 'propagate', reduction_ops), dtypes=OpDTypes.supported,
         allowed_dtypes=floating_types_and(torch.bfloat16, torch.float16))
    ...
    @ops(filter(lambda op: op.nan_policy == 'omit', reduction_ops), dtypes=OpDTypes.supported,
         allowed_dtypes=floating_types_and(torch.float16))
         #allowed_dtypes=floating_and_complex_types_and(torch.bfloat16, torch.float16))
    ...
    @expectedFailureMeta  # mode only supports CPU and MLU device type
    ...
    @dtypesIfMLU(torch.half, torch.bfloat16, torch.float, torch.double)
    ...
    def _test_minmax_helper(self, torchfn, reffn, device, dtype, skip_indices=False):
        def create_input(shape, device, dtype):
            if dtype.is_floating_point:
                ...
                if dtype == torch.bool and 'mlu' in device:
                    return torch.randint(low, high, shape, device='cpu', dtype=dtype).to(device)
    ...
    @dtypesIfMLU(torch.half, torch.float, torch.long, torch.bool)
    ...
    @dtypesIfMLU(torch.half, torch.float, torch.int, torch.long, torch.bool)
    @dtypes(torch.half, torch.float, torch.double)
    ...
    @onlyNativeDeviceTypes
    @dtypes(torch.float, torch.double)
    @dtypesIfMLU(torch.half, torch.float, torch.bfloat16)
    ...
    def test_prod_mlu(self, device, dtype):
    ...
    def test_argminmax_multiple(self, device, dtype):
            ...
            for with_extremal in [False, True]:
                for contiguous in [False, True]:
                    ...
                    if dtype != torch.half:
                        ...
                        # cnnl max with dim only support floating dtype, int32 and int64
                        if prims.utils.is_integer_dtype(dtype) and dtype not in [torch.int32, torch.int64]:
                            continue
        ...
        def verify_against_numpy(t):
            ...
            if dtype != torch.half:
                # cnnl max with dim only support floating dtype, int32 and int64
                if prims.utils.is_integer_dtype(dtype) and dtype in [torch.int32, torch.int64]:
    ...
    def test_argminmax_large_axis(self, device):
        ...
        #  mlu max and min not support int8
        #x = torch.zeros(2**31, device=device, dtype=torch.int8)
        x = torch.zeros(2**31, device=device, dtype=torch.int32)
        x[-1] = 1
    ...
    @dtypesIfMLU(torch.int, torch.long, torch.half, torch.float, torch.double)
