...
from _internal import definitions
...
from .mlu_common import skipMLU, onlyMLU
import torch_mlu
...
def error_inputs_cat(op_info, device, **kwargs):
    ...
    # MLU doesn't support stride so expand will create new tensor without overlapping.
...
def sample_inputs_bincount(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for size, weighted in product((S, M), [False, True]):
        # TODO(PYTORCH-10422): MLU Random only support float, half and int.
        # input_tensor = torch.randint(0, size, (size,), dtype=dtype, device=device)
        input_tensor = torch.randint(0, size, (size,), dtype=dtype, device="cpu").to(device)
        weight_tensor = make_arg((size,)) if weighted else None
...
def sample_inputs_interpolate(mode, self, device, dtype, requires_grad, **kwargs):
    ...
    ranks_for_mode = {
        # PYTORCH-9644: nearest3d is not supported
        # 'nearest': [1, 2, 3],
        'nearest': [1, 2],
        'linear': [1],
        'bilinear': [2],
...
def error_inputs_masked_fill(op_info, device, **kwargs):
    ...
    # TODO: uncomment the following 2 test cases after support complex inputs.
...
def sample_inputs_scatter(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for tensor, args in test_cases:
        ...
        # scatter_reduce doesn't support multiply
...
def sample_inputs_embedding_bag(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for generate_per_sample_weight in (True, False):
        for mode in ('sum', 'mean', 'max'):
            ...
            if requires_grad and (generate_per_sample_weight or mode != 'sum'):
                continue
            # TODO(PYTORCH-10088): embedding_bag limitation
            use_mlu=False
            ...
            #if not requires_grad:
            if not requires_grad and not use_mlu:
...
foreach_unary_op_db: List[OpInfo] = [
    ...
    ForeachFuncInfo(
        ...
        dtypesIfCUDA=floating_types_and(torch.half),
        dtypesIfMLU=floating_types_and(torch.half),
        backward_dtypesIfMLU=floating_types_and(torch.half),
    ),
...
foreach_binary_op_db: List[OpInfo] = [
    ...
    ForeachFuncInfo(
        ...
        dtypesIfMLU=all_types_and(torch.bool, torch.float16, torch.complex64, torch.complex128),
        skips=(
    ...
    ForeachFuncInfo(
        ...
        dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.float16),
        dtypesIfCUDA=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.float16),
        dtypesIfMLU=all_types_and(torch.bool, torch.float16),
        backward_dtypesIfMLU=all_types_and(torch.half, torch.bool),
        skips=(
            # Ref: https://github.com/pytorch/pytorch/issues/77946
            DecorateInfo(unittest.skip("Unable to reproduce failure locally"), "TestForeach",
                         "test_binary_op_scalarlist_fastpath",
...
foreach_reduce_op_db: List[ForeachFuncInfo] = [
    ForeachFuncInfo(
        ...
        dtypesIfMLU=floating_types_and(torch.float16),
    ),
]
...
op_db: List[OpInfo] = [
    UnaryUfuncInfo('abs',
                   ...
                   # TODO(PYTORCH-8789): Not support uint8, int8, int16, bool, chalf and bf16
                   dtypesIfMLU=floating_and_complex_types_and(torch.int32, torch.int64, torch.half),
                   backward_dtypesIfMLU=floating_types_and(torch.half),
                   skips=(
    ...
    BinaryUfuncInfo('add',
                    ...
                    # TODO(PYTORCH-9186): bfloat16 and complex is not support.
                    dtypesIfMLU=all_types_and(torch.bool, torch.float16),
                    backward_dtypesIfMLU=all_types_and(torch.bool, torch.float16),
                    assert_autodiffed=True,
    ...
    OpInfo('uniform',
           ...
           # TODO(PYTORCH-10262): not support bf16
           dtypesIfMLU=floating_and_complex_types_and(torch.float16),
           supports_out=False,
           supports_autograd=False,
    ...
    BinaryUfuncInfo('mul',
                    ...
                    # bfloat16 only support on MLU500 and complex half is not supported
                    # TODO(CNNLCORE-14588): backward of mul rely on torch.sum, which not support complex dtypes
                    dtypesIfMLU=all_types_and_complex_and(torch.float16, torch.bool),
                    backward_dtypesIfMLU=all_types_and(torch.float16, torch.bool),
                    assert_autodiffed=True,
                    supports_forward_ad=True,
                    supports_fwgrad_bwgrad=True,
                    supports_two_python_scalars=True),
    ...
    OpInfo('addmm',
           ...
           # PYTORCH-9645
           dtypesIfMLU=floating_types_and(torch.float16, torch.bfloat16),
           backward_dtypesIfMLU=floating_types_and(torch.float16, torch.bfloat16),
           assert_autodiffed=True,
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
    ...
    OpInfo('dot',
           ...
           dtypesIfCUDA=floating_and_complex_types_and(torch.float16,
                                                       *[torch.bfloat16] if (CUDA11OrLater or TEST_WITH_ROCM) else []),
           dtypesIfMLU=floating_types_and(torch.float16),
           backward_dtypesIfMLU=floating_types_and(torch.float16),
           assert_autodiffed=True,
    ...
    OpInfo('addr',
           dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.float16),
           dtypesIfMLU=floating_types_and(torch.bfloat16, torch.float16),
           ...
           backward_dtypesIfMLU=floating_types_and(torch.bfloat16, torch.float16),
    ...
    OpInfo('cdist',
           ...
           dtypesIfMLU=floating_types(),
           backward_dtypesIfMLU=floating_types(),
           supports_out=False,
           supports_gradgrad=False,
           assert_autodiffed=False,
    ...
    OpInfo('chunk',
           ...
           dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16, torch.chalf),
           backward_dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16),
    ...
    UnaryUfuncInfo('conj_physical',
                   ...
                   dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16,
                                                    torch.half, torch.chalf),
                   dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.bfloat16,
                                                    torch.half),
                   supports_forward_ad=True,
                   supports_fwgrad_bwgrad=True,
                   supports_sparse=True,
    ...
    OpInfo('cumsum',
           dtypes=all_types_and_complex_and(torch.bfloat16),
           dtypesIfCUDA=all_types_and_complex_and(torch.half, torch.bfloat16),
           dtypesIfMLU=all_types_and(torch.half),
           backward_dtypesIfMLU=all_types_and(torch.half),
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
           skips=(
    ...
    OpInfo('cumprod',
           dtypes=all_types_and_complex_and(torch.bfloat16),
           dtypesIfCUDA=all_types_and_complex_and(torch.float16, torch.bfloat16),
           dtypesIfMLU=all_types_and(torch.float16),
           backward_dtypesIfMLU=all_types_and(torch.float16),
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
           skips=(
    ...
    UnaryUfuncInfo('exp',
                   ...
                   dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16),
                   dtypesIfCUDA=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16, torch.chalf),
                   dtypesIfMLU=all_types_and(torch.float16, torch.bool),
                   backward_dtypesIfMLU=all_types_and(torch.float16, torch.bool),
                   skips=(
    ...
    OpInfo('expand',
           ...
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16),
           # TODO(PYTORCH-9186): bfloat16 is not support.
           dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.half),
           backward_dtypesIfMLU=all_types_and(torch.bool, torch.half),
    ...
    OpInfo('diag',
           dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16),
           dtypesIfCUDA=all_types_and_complex_and(torch.chalf, torch.bool, torch.half, torch.bfloat16),
           dtypesIfMLU=all_types_and(torch.bool, torch.half),
           backward_dtypesIfMLU=all_types_and(torch.bool, torch.half),
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
    ...
    OpInfo('diag_embed',
           dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.float16, torch.chalf),
           dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16),
           backward_dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16),
           supports_out=False,
           # Runs very slowly on slow gradcheck - alternatively reduce input sizes
           gradcheck_fast_mode=True,
    ...
    OpInfo('stft',
           ...
           dtypes=floating_and_complex_types(),
           dtypesIfMLU=floating_types_and_half(),
    ...
    OpInfo('linspace',
           ...
           dtypesIfMLU=floating_types_and(torch.int, torch.float16, torch.int64),
           backward_dtypesIfMLU=floating_types_and(torch.int, torch.float16, torch.int64),
           is_factory_function=True,
           supports_out=True,
           supports_autograd=False,
    ...
    UnaryUfuncInfo('log',
                   ...
                   # log(z)->-inf for |z|->0
                   reference_numerics_filter=NumericsFilter(condition=lambda x: torch.abs(x.cpu()).mlu() < 0.1, safe_val=1)),
    ...
    OpInfo('masked_fill',
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16, torch.chalf),
           dtypesIfMLU=floating_types_and(torch.half, torch.int8, torch.int16, torch.int32, torch.bool, torch.int64),
           backward_dtypesIfMLU=floating_types_and(torch.half, torch.int8, torch.int16, torch.int32, torch.bool, torch.int64),
    ...
    OpInfo('softmax',
           ...
           dtypes=floating_types_and(torch.bfloat16),
           dtypesIfCUDA=floating_types_and(torch.half, torch.bfloat16),
           # TODO(PYTORCH-9615): softmax not support bf16 and complex.
           dtypesIfMLU=floating_types_and(torch.half),
           backward_dtypesIfMLU=floating_types_and(torch.half),
    ...
    OpInfo('softmax',
           ...
           dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16),
           # TODO(PYTORCH-9615): softmax not support bf16 and complex.
           dtypesIfMLU=all_types_and(torch.half, torch.bool),
           backward_dtypesIfMLU=all_types_and(torch.half, torch.bool),
    ...
    OpInfo('nn.functional.conv_transpose1d',
           ...
           # TODO(PYTORCH-10326): only support half, float and double.
           dtypesIfMLU=floating_types_and(torch.float16),
           backward_dtypesIfMLU=floating_types_and(torch.float16),
    ...
    OpInfo('nn.functional.conv_transpose2d',
           ...
           decorators=[
               DecorateInfo(
                   toleranceOverride({torch.float32: tol(atol=1e-04, rtol=1.3e-06), }),
                   'TestCommon', 'test_variant_consistency_eager', device_type='mlu')],
           skips=(
               # RuntimeError: !lhs.isAliasOf(rhs)INTERNAL ASSERT FAILED at
               # "../torch/csrc/jit/passes/utils/check_alias_annotation.cpp":104, please report a bug to PyTorch.
               DecorateInfo(unittest.skip("Skipped!"), 'TestJit', 'test_variant_consistency_jit'),
               # TODO(CNNLCORE-14562): slow_conv_transposeXd is not support in MLU side.
    ...
    OpInfo('nn.functional.conv_transpose3d',
           ...
           decorators=[
               ...
               DecorateInfo(
                   ...
                   'TestCompositeCompliance', 'test_operator', device_type='mlu'),
               DecorateInfo(
                   toleranceOverride({torch.float32: tol(atol=1.3e-04, rtol=1.3e-06), }),
                   'TestCommon', 'test_noncontiguous_samples', device_type='mlu'),
               DecorateInfo(
                   toleranceOverride({torch.float32: tol(atol=1e-04, rtol=2e-05), }),
                   'TestCompositeCompliance', 'test_forward_ad', device_type='mlu',
                   active_if=torch.is_mlu_available())],
           skips=(
               # RuntimeError: !lhs.isAliasOf(rhs)INTERNAL ASSERT FAILED at
               # "../torch/csrc/jit/passes/utils/check_alias_annotation.cpp":104, please report a bug to PyTorch.
               DecorateInfo(unittest.skip("Skipped!"), 'TestJit', 'test_variant_consistency_jit'),
    ...
    UnaryUfuncInfo(
        ...
        dtypes=floating_types_and(torch.bfloat16),
        dtypesIfCUDA=floating_types_and(torch.float16, torch.bfloat16),
        # MLU silu doesn't support torch.bfloat16
        dtypesIfMLU=floating_types_and(torch.float16),
        backward_dtypesIfMLU=floating_types_and(torch.float16),
        supports_forward_ad=True,
        supports_autograd=True,
    ...
    OpInfo('nn.functional.gelu',
           ...
           dtypes=floating_types_and(torch.bfloat16),
           dtypesIfCUDA=floating_types_and(torch.half, torch.bfloat16),
           dtypesIfMLU=floating_types_and(torch.half, torch.bfloat16),
           supports_gradgrad=True,
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
    ...
    UnaryUfuncInfo('neg',
                   ...
                   dtypes=all_types_and_complex_and(torch.half, torch.bfloat16, torch.chalf),
                   # TODO(PYTORCH-9186): bfloat16 and complex is not support.
                   dtypesIfMLU=floating_types_and(torch.half, torch.int32, torch.int64),
                   backward_dtypesIfMLU=floating_types_and(torch.half),
    ...
    OpInfo('outer',
           ...
           aliases=('ger', ),
           dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16),
           #TODO(PYTORCH-10165): outer not support bf16.
           dtypesIfMLU=all_types_and_complex_and(torch.half, torch.bool),
           backward_dtypesIfMLU=all_types_and(torch.half, torch.bool),
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
           # See https://github.com/pytorch/pytorch/pull/78358
    ...
    OpInfo('permute',
           ...
           # TODO(PYTORCH-9186): bfloat16 is not support.
           # dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16, torch.chalf),
           dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.chalf),
           supports_out=False,
           assert_autodiffed=True,
    ...
    UnaryUfuncInfo('sign',
                   ...
                   # TODO(PYTORCH-8947): not support Integer, bool and bf16
                   dtypesIfMLU=floating_types_and(torch.half),
                   supports_forward_ad=True,
                   supports_fwgrad_bwgrad=True,
                   supports_sparse=True,
    ...
    UnaryUfuncInfo('sgn',
                   ...
                   backward_dtypesIfCUDA=floating_and_complex_types_and(torch.bfloat16, torch.half, torch.chalf),
                   # TODO(PYTORCH-8948): not support Integer, bool, bf16 and complex32
    ...
    OpInfo('split',
           ...
           # TODO(PYTORCH-9186): bfloat16 is not support.
           dtypesIfMLU=all_types_and_complex_and(torch.half, torch.bool, torch.chalf),
           backward_dtypesIfMLU=all_types_and(torch.half, torch.bool),
    ...
    BinaryUfuncInfo('__rmul__',
                    ...
                    # only MLU500 support bfloat16 and complex half is not support
                    # TODO(CNNLCORE-14588): backward of mul rely on torch.sum, which not support complex dtypes
                    dtypes=all_types_and_complex_and(torch.chalf, torch.float16, torch.bfloat16, torch.bool),
                    dtypesIfMLU=all_types_and_complex_and(torch.float16, torch.bool),
                    backward_dtypesIfMLU=all_types_and(torch.float16, torch.bool),
    ...
    UnaryUfuncInfo('angle',
                   ...
                   dtypesIfMLU=all_types_and_complex_and(torch.bool),
                   backward_dtypesIfMLU=all_types_and(torch.bool),
                   decorators=(precisionOverride({torch.float16: 1e-2,
                                                  torch.bfloat16: 1e-2}),),
                   # TODO(PYTORCH-9186): bfloat16 is not support.
                   # backward_dtypes=floating_and_complex_types_and(torch.bfloat16, torch.float16),
                   backward_dtypes=floating_and_complex_types_and(torch.float16),
    ...
    OpInfo('gather',
           ...
           # TODO(PYTORCH-10266): gather not support bf16 and complex.
           dtypesIfMLU=all_types_and(torch.bool, torch.float16),
           backward_dtypesIfMLU=floating_types_and(torch.float16),
    ...
    OpInfo('index_add',
           dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16, torch.chalf),
           dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int16, torch.int64),
           backward_dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int16, torch.int64),
           supports_out=True,
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
           # https://github.com/pytorch/pytorch/issues/66357
    ...
    OpInfo('index_put',
           dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16, torch.chalf),
           dtypesIfMLU=all_types_and(torch.float16, torch.bfloat16),
           backward_dtypesIfMLU=all_types_and(torch.float16, torch.bfloat16),
           supports_out=False,
    ...
    OpInfo('unique',
           ...
           dtypesIfMLU=floating_types_and(torch.int32, torch.long),
           backward_dtypesIfMLU=floating_types_and(torch.int32, torch.long),
    ...
    OpInfo('scatter',
            # remove complex since scatter doesn't support
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16),
           dtypesIfMLU=all_types_and(torch.bool, torch.half),
           backward_dtypesIfMLU=all_types_and(torch.bool, torch.half),
           supports_forward_ad=True,
    ...
    OpInfo('randn',
           ...
           # TODO(PYTORCH-10321): not support bf16 and complex32
           dtypesIfMLU=floating_and_complex_types_and(torch.half),
    ...
    OpInfo('eye',
           ...
           dtypesIfCUDA=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16),
           dtypesIfMLU=all_types_and(torch.bool, torch.half, torch.bfloat16),
           backward_dtypesIfMLU=all_types_and(torch.bool, torch.half, torch.bfloat16),
    ...
    OpInfo('normal',
           ...
           # TODO(PYTORCH-10242): not support bf16
           dtypesIfMLU=floating_types_and(torch.half),
           supports_out=True,
    ...
    OpInfo('bernoulli',
           ...
           dtypes=floating_types_and(torch.bfloat16),
           dtypesIfCUDA=floating_types_and(torch.bfloat16, torch.half),
           # TODO(PYTORCH-10285): not support bf16
           dtypesIfMLU=floating_types_and(torch.half),
           supports_out=True,
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
    ...
    OpInfo('scatter_add',
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16),
           dtypesIfMLU=floating_types_and(torch.int32, torch.half, torch.int64),
           backward_dtypesIfMLU=floating_types_and(torch.int32, torch.half, torch.int64),
    ...
    OpInfo('cat',
           ...
           dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16, torch.complex32),
    ...
    OpInfo('unbind',
           dtypes=all_types_and_complex_and(torch.complex32, torch.bool, torch.float16, torch.bfloat16),
            # TODO(PYTORCH-9186): bfloat16 is not support.
           dtypesIfMLU=all_types_and_complex_and(torch.complex32, torch.bool, torch.float16),
           backward_dtypesIfMLU=all_types_and(torch.bool, torch.float16),
    ...
    OpInfo('unfold',
           ...
           dtypesIfMLU=all_types_and(torch.half, torch.bool, torch.complex64, torch.complex128),
           backward_dtypesIfMLU=all_types_and(torch.half, torch.bool),
           # Runs very slowly on slow gradcheck - alternatively reduce input sizes
           gradcheck_fast_mode=True,
           supports_out=False,
    ...
    ShapeFuncInfo('repeat',
                  ...
                  ref=np.tile,
                  dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16),
                  dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16),
                  # TODO(PYTORCH-10136):repeat need other ops support complex.
                  backward_dtypesIfMLU=all_types_and(torch.half, torch.bool, torch.bfloat16),
                  # https://github.com/pytorch/pytorch/issues/80411
                  gradcheck_fast_mode=True,
                  supports_out=False,
                  supports_forward_ad=True,
    ...
    BinaryUfuncInfo('xlogy',
                    ...
                    supports_forward_ad=True,
                    supports_fwgrad_bwgrad=True,
                    supports_one_python_scalar=False,
                    supports_rhs_python_scalar=False,
    ...
    OpInfo('transpose',
           ...
           # TODO(PYTORCH-9186): bfloat16 is not support.
           # dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.half, torch.chalf),
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.chalf),
           supports_out=False,
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
           # vmap does not support inplace views
    ...
    OpInfo('tril',
           ...
           #TODO: [PYTORCH-10250]
           dtypesIfMLU=floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64, torch.bfloat16, torch.half),
           backward_dtypesIfMLU = floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64, torch.bfloat16, torch.half),
           supports_forward_ad=True,
           supports_fwgrad_bwgrad=True,
    ...
    OpInfo('where',
           ...
           supports_fwgrad_bwgrad=True,
           decorators=(
               DecorateInfo(onlyMLU, "TestCommon", 'test_errors'),),
           skips=(
               # lambda impl
               DecorateInfo(unittest.expectedFailure, "TestNormalizeOperators", "test_normalize_operator_exhaustive"),
               DecorateInfo(unittest.skip("Skipped!"), 'TestJit', 'test_variant_consistency_jit'),
           ...
           dtypes=all_types_and_complex_and(torch.bool, torch.half, torch.bfloat16, torch.chalf),
           dtypesIfMLU=floating_types_and(torch.float16, torch.int8, torch.int16, torch.int32, torch.int64, torch.half),
           backward_dtypesIfMLU=floating_types_and(torch.float16, torch.int8, torch.int16, torch.int32, torch.int64, torch.half)),
    OpInfo('nonzero',
           dtypes=all_types_and_complex_and(torch.bool, torch.bfloat16, torch.float16, torch.chalf),
    ...
    OpInfo(
        ...
        dtypes=all_types_and_complex_and(torch.bool, torch.float16, torch.bfloat16),
        dtypesIfMLU=all_types_and_complex_and(torch.bool, torch.float16),
        backward_dtypesIfMLU=all_types_and_complex_and(torch.half, torch.bool),
        skips=(
            # lambda impl
            DecorateInfo(unittest.expectedFailure, 'TestNormalizeOperators', 'test_normalize_operator_exhaustive'),
            DecorateInfo(unittest.expectedFailure, 'TestJit', 'test_variant_consistency_jit'),),
    ...
    OpInfo(
        ...
        # TODO(PYTORCH-9641): bf16 is not supported
        dtypesIfMLU=floating_types_and(torch.float16),
        backward_dtypesIfMLU=floating_types_and(torch.float16),
        skips=(
            # RuntimeError: input->type()->kind() == TypeKind::OptionalType
    ...
    ReductionOpInfo(
        ...
        dtypes=all_types_and(torch.float16, torch.bfloat16),
        dtypesIfMLU=all_types_and(torch.float16, torch.bfloat16, torch.bool),
    ...
    OpInfo(
        ...
        dtypes=all_types_and(torch.bfloat16),
        dtypesIfCUDA=all_types_and(torch.float16, torch.bfloat16),
        # TODO(PYTORCH-8789): using all_types_and when MLU abs support bfloat16,
        # short, int16, int8, uint8
        dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int64),
        backward_dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int64),
        supports_out=False,
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
    ...
    OpInfo(
        ...
        backward_dtypesIfCUDA=floating_and_complex_types_and(torch.float16, torch.bfloat16, torch.chalf),
        # TODO(PYTORCH-10136):repeat need other ops support complex.
        dtypesIfMLU=all_types_and(torch.bool, torch.float16, torch.bfloat16),
        backward_dtypesIfMLU=all_types_and(torch.half, torch.bool, torch.bfloat16),
    ...
    OpInfo(
        ...
        dtypes=all_types_and(torch.float16, torch.bfloat16, torch.bool),
        dtypesIfMLU=floating_types_and(torch.int32, torch.int64, torch.float16),
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
    ...
    OpInfo(
        ...
        dtypesIfCUDA=all_types_and(torch.float16, torch.bfloat16),
        sample_inputs_func=sample_inputs_scatter_reduce,
        # scatter_reduce just supports add
        decorators=[skipMLU],
        skips=(
            # Pre-existing condition (calls .item); needs to be fixed
            DecorateInfo(unittest.expectedFailure, 'TestCompositeCompliance', 'test_backward'),
...
python_ref_db = [
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        supports_nvfuser=False,
        dtypesIfMLU=integral_types_and(torch.float32, torch.half, torch.bool),
    ),
    ElementwiseUnaryPythonRefInfo(
        "_refs.neg",
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        # _refs.sgn not support complex types since calculation with complex and float is not support
        dtypesIfMLU=floating_types_and(torch.half),
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        # _refs.nn.functional.threshold is torch.where and for mlu it doesn't support bool and uint8.
        dtypesIfMLU=floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64,torch.float16),
        supports_nvfuser=False,
    ),
    PythonRefInfo(
    ...
    PythonRefInfo(
        ...
        # TODO(PYTORCH-8789): delete dtypesIfMLU and backward_dtypesIfMLU 
        # when MLU abs support bfloat16, short, int16, int8, uint8
        dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int64),
        backward_dtypesIfMLU=floating_types_and(torch.float16, torch.int32, torch.int64),
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        # _refs.nn.functional.relu is torch.where and for mlu it doesn't support bool and uint8.
        dtypesIfMLU=floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64,torch.float16),
        supports_nvfuser=False,
        op_db=op_db,
    ),
    ...
    ElementwiseBinaryPythonRefInfo(
        ...
        # _refs.\clamp is torch.where and for mlu it doesn't support bool and uint8.
        dtypesIfMLU=floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64,torch.float16),
        supports_nvfuser=False,
        skips=(
            # test error disabled since rhs non-tensor python scalar is supported
    ...
    PythonRefInfo(
        "_refs.masked_fill",
        dtypesIfMLU=floating_types_and(torch.int8, torch.int16, torch.int32, torch.int64),
...
python_ref_db += definitions.python_ref_db
