...
from .mlu_common import TEST_MLU
from torch.testing._internal.common_dtype import floating_types, floating_types_and, floating_and_complex_types_and
...
lstm_module_info_decorators = (
    ...
    DecorateInfo(
        ...
        active_if=TEST_MLU, device_type='mlu'
    ),
...
rnn_gru_module_info_decorators = (
    # RuntimeError: Batching rule not implemented for aten::_cudnn_rnn_backward.
    # We could not generate a fallback
    DecorateInfo(
        unittest.expectedFailure, "TestModule", "test_grad",
...
module_db: List[ModuleInfo] = [
    ...
    ModuleInfo(torch.nn.Conv1d,
               ...
               # TODO(PYTORCH-10326): Conv only support half, float and double.
               # TODO(PYTORCH-10478): Double precision is not statisfied.
               # CPU is not support half. orignal dtype list are float and double.
               dtypes=(torch.float, ),
               skips=(
    ...
    ModuleInfo(torch.nn.RNN,
               ...
               skips=(
                   DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),),
               decorators=rnn_gru_module_info_decorators
               ),
    ...
    ModuleInfo(torch.nn.LSTM,
               ...
               skips=(
                   DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),),
               decorators=lstm_module_info_decorators)
]
