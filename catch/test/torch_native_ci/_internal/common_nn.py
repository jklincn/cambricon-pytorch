...
from torch.testing._internal.common_utils import TestCase, to_gpu, to_mlu, freeze_rng_state, is_iterable, \
...
new_module_tests = [
    ...
    dict(
        ...
        cudnn=True,
        with_tf32=True,
            # TODO(PYTORCH-10370): fellow pytorch main branch or tf32 precsion is not statisfied.
...
for padding_mode, cpp_padding_mode in zip(
    ...
    for d in (1, 2, 3):
        # TODO(PYTORCH-10477): replication_pad3d is not support in mlu side.
        if d == 3 and padding_mode in ('reflect', "replicate"):
...
class ModuleTest(TestBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.should_test_mlu = kwargs.get('test_mlu', True)
        self.should_test_pickle = kwargs.get('pickle', True)
        self.check_gradgrad = kwargs.get('check_gradgrad', True)
        self.FIXME_no_cuda_gradgrad_comparison = \
            kwargs.get('FIXME_no_cuda_gradgrad_comparison', False)
        self.FIXME_no_mlu_gradgrad_comparison = \
            kwargs.get('FIXME_no_mlu_gradgrad_comparison', False)
    ...
    def test_mlu(self, test_case):
        if not self.should_test_mlu:
            raise unittest.SkipTest('Excluded from MLU tests')

        cpu_input = self._get_input()
        type_map = {torch.double: torch.float}
        cpu_input_tuple = cpu_input if isinstance(cpu_input, tuple) else (cpu_input,)
        mlu_input_tuple = to_mlu(cpu_input_tuple, type_map=type_map)

        cpu_module = self.constructor(*self.constructor_args)
        mlu_module = self.constructor(*self.constructor_args).float().mlu()
        cpu_param = test_case._get_parameters(cpu_module)
        mlu_param = test_case._get_parameters(mlu_module)
        for cpu_p, mlu_p in zip(cpu_param[0], mlu_param[0]):
            mlu_p.data.copy_(cpu_p)

        test_case._zero_grad_input(cpu_input_tuple)
        test_case._zero_grad_input(mlu_input_tuple)
        test_case._zero_grad_parameters(cpu_module)
        test_case._zero_grad_parameters(mlu_module)
        cpu_output = test_case._forward(cpu_module, cpu_input_tuple)
        mlu_output = test_case._forward(mlu_module, mlu_input_tuple)
        if getattr(cpu_module, "return_indices", False):
            cpu_output = cpu_output[0]
            mlu_output = mlu_output[0]
        # TODO(PYTORCH-10370): final solution need discuss with pytorch developer.
        test_case.assertEqual(cpu_output, mlu_output, atol=test_case.precision, rtol=0, exact_dtype=False)

        # Run backwards on CPU and MLU and compare results
        for _ in range(5):
            cpu_gradOutput = cpu_output.clone().normal_()
            mlu_gradOutput = cpu_gradOutput.type_as(mlu_output)
            cpu_gradInput = test_case._backward(cpu_module, cpu_input_tuple, cpu_output, cpu_gradOutput)
            mlu_gradInput = test_case._backward(mlu_module, mlu_input_tuple, mlu_output, mlu_gradOutput)
            test_case.assertEqual(cpu_gradInput, mlu_gradInput, atol=self.precision, rtol=0, exact_dtype=False)
            for cpu_d_p, mlu_d_p in zip(cpu_param[1], mlu_param[1]):
                # TODO(PYTORCH-10370): final solution need discuss with pytorch developer.
                test_case.assertEqual(cpu_d_p, mlu_d_p, atol=test_case.precision, rtol=0)

        # Run double-backwards on CPU and MLU and compare results
        if self.check_gradgrad and not self.FIXME_no_cuda_gradgrad_comparison:
            cpu_output = cpu_module(*cpu_input_tuple)
            mlu_output = mlu_module(*mlu_input_tuple)
            if getattr(cpu_module, "return_indices", False):
                cpu_output = cpu_output[0]
                mlu_output = mlu_output[0]

            cpu_gradOutput = torch.randn_like(cpu_output, requires_grad=True)
            mlu_gradOutput = cpu_gradOutput.type_as(mlu_output).detach()
            mlu_gradOutput.requires_grad = True
            ...
            cpu_gradInputs = torch.autograd.grad(
                ...
                cpu_gradOutput,
                create_graph=True)
            mlu_gradInputs = torch.autograd.grad(
                mlu_output,
                mlu_input_tuple + tuple(mlu_module.parameters()),
                mlu_gradOutput,
                create_graph=True)

            for cpu_d_i, mlu_d_i in zip(cpu_gradInputs, mlu_gradInputs):
                # TODO(PYTORCH-10370): final solution need discuss with pytorch developer.
                test_case.assertEqual(cpu_d_i, mlu_d_i, atol=test_case.precision, rtol=0, exact_dtype=False)
            ...
            cpu_gg = torch.autograd.grad(
                ...
                cpu_input_tuple + (cpu_gradOutput,) + tuple(cpu_module.parameters()),
                retain_graph=True)
            mlu_gg = torch.autograd.grad(
                mlu_output.sum() + sum(x.sum() for x in mlu_gradInputs),
                mlu_input_tuple + (mlu_gradOutput,) + tuple(mlu_module.parameters()),
                retain_graph=True)
            test_case.assertEqual(cpu_gradInput, mlu_gradInput, atol=self.precision, rtol=0, exact_dtype=False)
            for cpu_d_p, mlu_d_p in zip(cpu_gg, mlu_gg):
                test_case.assertEqual(cpu_d_p, mlu_d_p, atol=self.precision, rtol=0, exact_dtype=False)

        self.test_noncontig(test_case, mlu_module, mlu_input_tuple)
...
class NewModuleTest(InputVariableMixin, ModuleTest):  # type: ignore[misc]
    ...
    def _do_test(self, test_case, module, input):
        ...
        if all(isinstance(t, torch.LongTensor) for t in input_tuple) and TEST_CUDA:
            ...
            if self.should_test_mlu:
                # check that mlu() moves module parameters to correct MLU device,
                # and that float() casts parameters correctly

                # to MLU0
                input_tuple = tuple(to_single(t).mlu() for t in input_tuple)
                module.float().mlu()
                module(*input_tuple)
                assert_module_parameters_are(torch.mlu.FloatTensor, 0)  # type: ignore[attr-defined]
                ...
                # back to MLU0
                input_tuple = tuple(t.mlu() for t in input_tuple)
                module.mlu()
                module(*input_tuple)
                assert_module_parameters_are(torch.mlu.FloatTensor, 0)  # type: ignore[attr-defined]

                if torch.mlu.device_count() >= 2:
                    # test cross-MLU transfer works
                    # to mlu1
                    input_tuple = tuple(t.mlu(1) for t in input_tuple)
                    module.mlu(1)
                    with torch.mlu.device(1):
                        module(*input_tuple)
                    assert_module_parameters_are(torch.mlu.FloatTensor, 1)  # type: ignore[attr-defined]

                if not self.skip_double:
                    # test double()
                    input_tuple = tuple(to_double(t).mlu() for t in input_tuple)
                    module.double().mlu()
                    module(*input_tuple)
                    assert_module_parameters_are(torch.mlu.DoubleTensor, 0)  # type: ignore[attr-defined]

                # test half()
                if not self.skip_half:
                    input_tuple = tuple(to_half(t).mlu() for t in input_tuple)
                    module.half().mlu()
                    module(*input_tuple)
                    assert_module_parameters_are(torch.mlu.HalfTensor, 0)  # type: ignore[attr-defined]
...
class CriterionTest(InputVariableMixin, TestBase):  # type: ignore[misc]
    ...
    def test_mlu(self, test_case, dtype, extra_args=None):
        def convert_dtype(obj, dtype, requires_grad=False):
            if isinstance(obj, torch.Tensor):
                return obj.detach().to(dtype=dtype).requires_grad_(requires_grad)
            elif isinstance(obj, tuple):
        ...
        cpu_target = self._get_target()
        cpu_module = self.constructor(*self.constructor_args)
        mlu_module = self.constructor(*self.constructor_args)
        ...
        if cpu_target.is_floating_point() or cpu_target.is_complex():
            cpu_target = convert_dtype(cpu_target, dtype)
        cpu_module.type(dtype)
        mlu_module.type(dtype)

        # MLU setup
        mlu_input = to_mlu(cpu_input)
        mlu_target = to_mlu(cpu_target)
        mlu_module.mlu()
        ...
        cpu_output = test_case._forward_criterion(cpu_module, cpu_input, cpu_target, extra_args=extra_args)
        mlu_output = test_case._forward_criterion(mlu_module, mlu_input, mlu_target, extra_args=extra_args)
        # dtype used to be able to be None, so set precision in this way instead of a precision map
        test_case.assertEqual(cpu_output, mlu_output,
                              atol=1e-1 if dtype in {torch.half, torch.bfloat16} else 4e-4, rtol=0, exact_dtype=False)

        cpu_gradInput = test_case._backward_criterion(
            cpu_module, cpu_input, cpu_output, cpu_target, extra_args=extra_args)
        mlu_gradInput = test_case._backward_criterion(
            mlu_module, mlu_input, mlu_output, mlu_target, extra_args=extra_args)
        # dtype used to be able to be None, so set precision in this way instead of a precision map
        test_case.assertEqual(cpu_gradInput, mlu_gradInput,
                              atol=1e-1 if dtype in {torch.half, torch.bfloat16} else 4e-4, rtol=0, exact_dtype=False)
