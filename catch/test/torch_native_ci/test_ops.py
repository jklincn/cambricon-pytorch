...
class TestCommon(TestCase):
    ...
    def test_python_ref_meta(self, device, dtype, op):
        ...
        if dtype in (torch.bfloat16, torch.complex32):
            return
    ...
    def test_noncontiguous_samples(self, device, dtype, op):
        ...
        for sample_input in sample_inputs:
            ...
            if op.name == "angle" and dtype == torch.complex64:
                continue
    ...
    def test_out_warning(self, device, op):
        ...
        for sample in samples:
            ...
            def _extract_data_ptrs(out):
                if self.device_type != "cpu" and self.device_type != "cuda" and self.device_type != "mlu":
                    return ()
    ...
    def test_out(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            def _case_zero_transform(t):
                try:
                    # changed since mlu doesn't support int64
                    info = torch.iinfo(t.dtype) if t.dtype != torch.int64 else torch.iinfo(torch.int32)
    ...
    def test_non_standard_bool_values(self, device, dtype, op):
        ...
        def convert_boolean_tensors(x):
            ...
            # torch.where for mlu doesn't support bool
            true_vals = torch.randint(2, 255, x.shape, dtype=torch.uint8)
            false_vals = torch.zeros((), dtype=torch.uint8)
            #true_vals = torch.randint(2, 255, x.shape, dtype=torch.uint8, device=x.device)
            #false_vals = torch.zeros((), dtype=torch.uint8, device=x.device)
            x_int = torch.where(x.cpu(), true_vals, false_vals)

            ret = x_int.to(x.device).view(torch.bool)
    ...
    def test_dtypes(self, device, op):
        ...
        allowed_backward_dtypes = floating_and_complex_types_and(
            # torch.bfloat16 is not supported on mlu
            #*((torch.half, torch.bfloat16) + include_complex32)
            *((torch.half,) + include_complex32)
        )
        ...
        for dtype in all_types_and_complex_and(
            ...
            *((torch.half, torch.bool) + include_complex32)
        ):
...
class TestFakeTensor(TestCase):
    ...
    def test_fake_autocast(self, device, dtype, op):
        ...
        if device == "cuda":
            ...
            context = torch.mlu.amp.autocast
        else:
    ...
    def test_fake_crossref_backward_amp(self, device, dtype, op):
        context = torch.mlu.amp.autocast if torch.device(device).type == "mlu" else torch.cpu.amp.autocast
        self._test_fake_crossref_helper(device, dtype, op, context)
