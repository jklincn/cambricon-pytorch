...
from torch.testing._internal.common_device_type import OpDTypes, instantiate_device_type_tests
from _internal.mlu_common import ops, TEST_MLU
from _internal.common_nn import TestBase, module_tests, new_module_tests
from torch.testing._internal.common_utils import TestCase, freeze_rng_state, make_tensor, run_tests, parametrize
from _internal.mlu_common_methods_invocations import op_db
        ...
        for sample_input in supported_inputs(op, sample_inputs):
            if op.name == "nn.functional.embedding":  # embedding flips its argument order for autograd tests
                sample_input = SampleInput(sample_input.args[0].clone(),
                                           ...
                                           kwargs=sample_input.kwargs)
                if "mlu" in device and "max_norm" in sample_input.kwargs and "padding_idx" in sample_input.kwargs:
...
class ContextManagerTests(TestBase):
    def __init__(self, *args, **kwargs):
        ...
        self.test_mlu = kwargs.get('test_mlu', True)
...
for test_param in supported_tests:
    ...
    if TEST_MLU and test.test_mlu:
        # since this checks derivatives, only use double for precision
        setattr(TestExpandedWeightModule, test_name + '_mlu_double',
                lambda self, test=test: test.test_context_manager(self, 'mlu'))
