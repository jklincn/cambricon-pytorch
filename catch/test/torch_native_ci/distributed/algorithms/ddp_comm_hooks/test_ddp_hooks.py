...
class DistributedDataParallelCommHookTest(MultiProcessTestCase):
    ...
    def _get_grads(self, process_group, hook_type=None):
        ...
        if hook_type is not None:
            register_ddp_comm_hook(
                comm_hook_type=hook_type, model=mlu_model, state=process_group
            )

        return self._run_and_get_grads(mlu_model)
    ...
    def test_ddp_comm_hook_allreduce_hook(self):
        ...
        process_group = dist.ProcessGroupCNCL(store, self.rank, self.world_size)
    ...
    def test_is_last_hook(self):
        ...
        mlu_model.register_comm_hook(state=flags, hook=hook)
        input = torch.randn(10, 2)
        mlu_model(input).sum().backward()
