...
def mlus_for_rank(world_size):
    ...
    MLUs on each node. Cncl backend requires equal #MLUs in each process.
...
class DoubleMluNet(nn.Module):
    def __init__(self, mlus):
        super(DoubleMluNet, self).__init__()
        self.fc1 = nn.Linear(2, 10, bias=False).to(mlus[0])
        self.fc2 = nn.Linear(10, 50, bias=False).to(mlus[1])
        self.fc3 = nn.Linear(50, 4, bias=False).to(mlus[1])
        self.relu = nn.ReLU()
...
class QuadraMluNet(nn.Module):
    def __init__(self, mlus):
        super(QuadraMluNet, self).__init__()
        self.fc1 = nn.Linear(2, 10, bias=False).to(mlus[0])
        self.fc2 = nn.Linear(10, 50, bias=False).to(mlus[1])
        self.fc3 = nn.Linear(50, 4, bias=False).to(mlus[2])
        self.fc4 = nn.Linear(4, 4, bias=False).to(mlus[3])
        self.relu = nn.ReLU()
        self.no_grad_param = nn.Parameter(
            torch.tensor([2, 2]).long(), requires_grad=False
        ).to(mlus[0])
...
class ConvNet(nn.Module):
    def __init__(self, mlus, layouts, dtypes):
        super(ConvNet, self).__init__()
        self.dtypes = dtypes
        if isinstance(mlus, list):
            self.layer_mlus = mlus
        else:
            mlus = [mlus] * 4
        self.conv0 = torch.nn.Conv2d(8, 16, (2, 2)).to(
            device=mlus[0], memory_format=layouts[0], dtype=dtypes[0]
        )
        self.conv1 = torch.nn.Conv2d(16, 32, (2, 2)).to(
            device=mlus[1], memory_format=layouts[1], dtype=dtypes[1]
        )
        self.conv2 = torch.nn.Conv2d(32, 16, (2, 2)).to(
            device=mlus[2], memory_format=layouts[2], dtype=dtypes[2]
        )
        self.conv3 = torch.nn.Conv2d(16, 8, (2, 2)).to(
            device=mlus[3], memory_format=layouts[3], dtype=dtypes[3]
        )
    ...
    def forward(self, x):
        ...
        mlus = self.layer_mlus if hasattr(self, "layer_mlus") else [x.device] * 4
        x = self.conv0(x).to(device=mlus[1], dtype=self.dtypes[1])
        x = self.conv1(x).to(device=mlus[2], dtype=self.dtypes[2])
        x = self.conv2(x).to(device=mlus[3], dtype=self.dtypes[3])
        return self.conv3(x)
...
class CommonDistributedDataParallelTest(object):
    ...
    def _prepare_single_device_module(
        ...
        model = Net()
        device = devices[0] if devices else torch.device("mlu:%d" % self.rank)
    ...
    def _prepare_multi_device_module(
        ...
        if len(devices) == 2:
            model = DoubleMluNet(devices)
        elif len(devices) == 4:
            model = QuadraMluNet(devices)
        ...
        input = torch.randn(global_batch_size, 2).mlu(devices[0])
    ...
    def _test_ddp_checkpointing(
        ...
        model = copy.deepcopy(input_model).mlu()
        ddp_model = copy.deepcopy(input_model).mlu()
    ...
    def _prepare_dummy_data(self):
        ...
        input = torch.rand((bs, 20), device="mlu", requires_grad=True)
        target = torch.randn((bs, 20), device="mlu")
    ...
    def _mlu_model_with_builtin_ddp_comm_hook(
        ...
        if hook is not None:
            mlu_model._register_builtin_comm_hook(hook)
...
class AbstractCommTest(object):
    ...
    def _test_sequence_num_incremented(self, process_group, ranks):
        ...
        for i in range(10):
            t = torch.ones(1, device=torch.mlu.current_device())
    ...
    def _test_warn_not_in_group(self, backend):
        ...
        x = torch.zeros(2, 2).mlu(self.rank)
        xs = [torch.zeros(2, 2).mlu(self.rank) for _ in range(len(in_group_ranks))]
...
class ProcessGroupWithDispatchedCollectivesTests(MultiProcessTestCase):
    ...
    def _call_collective_with_varying_tensors(self, backend, collective, *args):
        ...
        device = "mlu" if backend == "cncl" else "cpu"
        # ensure supported devices (cpu, mlu) succeeds during dispatch call
...
class ReduceOpTest(TestCase):
    ...
    def test_op_isinstance_of_reduceop(self):
        ...
        for scale in ([torch.tensor(1.0)], 2.0):
            self.assertTrue(isinstance(dist._make_cncl_premul_sum(scale), c10d.ReduceOp))


if __name__ == "__main__":
    assert (
        not torch.mlu._initialized
    ), "test_distributed must not have initialized mlu context on main process"
