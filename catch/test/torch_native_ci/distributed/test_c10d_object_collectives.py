...
BACKEND = dist.Backend.CNCL if torch.mlu.is_available() else dist.Backend.GLOO
WORLD_SIZE = min(4, max(2, torch.mlu.device_count()))
...
def with_comms(func=None):
    ...
    def wrapper(self, *args, **kwargs):
        if BACKEND == dist.Backend.CNCL and torch.mlu.device_count() < self.world_size:
            sys.exit(TEST_SKIPS[f"multi-mlu-{self.world_size}"].exit_code)
        self.dist_init()
        func(self)
        self.destroy_comms()
    return wrapper
...
class TestObjectCollectives(MultiProcessTestCase):
    ...
    @property
    def device(self):
        return torch.device(self.rank) if BACKEND == dist.Backend.CNCL \
    ...
    def dist_init(self):
        ...
        # set device for CNCL pg for collectives
        if BACKEND == "CNCL":
