...
from test_c10d_common import DoubleMluNet, ConvNet, ModuleForDdpCommHook
...
from _internal.distributed.mlu_common_distributed import (
    init_multimlu_helper,
    # requires_cncl_version,
    requires_cncl,
    skip_if_lt_x_mlu,
    with_cncl_blocking_wait,
    mlus_for_rank
)

from _internal.mlu_common import (
    ops,
    dtypes,
    dtypesIfMLU,
    onlyMLU,
    skipMLU,
    MLU_TEST_LONG_AND_DOUBLE)
...
BFLOAT16_AVAILABLE = (
    ...
    # and
    # (
    #     (torch.version.mlu is not None and int(torch.version.mlu.split('.')[0]) >= 11)
...
class RendezvousEnvTest(TestCase):
    ...
    @requires_cncl()
    @sandcastle_skip_if(
        torch.mlu.device_count() == 0, "No MLUs available, skipping test"
    )
    def test_common_errors(self):
        ...
        with Env(without(vars, "WORLD_SIZE")):
            ...
            with self.assertRaisesRegex(ValueError, "WORLD_SIZE expected"):
                ...
                next(gen)
            c10d.init_process_group(backend="cncl", world_size=1)
        ...
        with Env(without(vars, "RANK")):
            ...
            with self.assertRaisesRegex(ValueError, "RANK expected"):
                gen = c10d.rendezvous("env://")
                next(gen)
            c10d.init_process_group(backend="cncl", rank=0)
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()
        ...
        with Env(withouts(vars, ["RANK", "WORLD_SIZE"])):
            self.assertEqual(None, os.environ.get("RANK"))
            self.assertEqual(None, os.environ.get("WORLD_SIZE"))
            c10d.init_process_group(backend="cncl", rank=0, world_size=1)
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()

        with Env(vars):
            c10d.init_process_group(backend="cncl")
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()
...
class TimeoutTest(test_c10d_common.AbstractTimeoutTest, TestCase):
    ...
    def test_default_store_timeout_cncl(self):
        self._test_default_store_timeout("cncl")


class ProcessGroupCNCLNoMLUTest(TestCase):
    MAIN_PROCESS_RANK = 0
    ...
    @requires_cncl()
    @sandcastle_skip_if(
        torch.mlu.device_count() > 0, "MLUs are available, skipping test"
    )
    def test_init_no_mlus(self):
        store = c10d.FileStore(self.file.name, self.world_size)
        with self.assertRaisesRegex(
            RuntimeError, "ProcessGroupCNCL is only supported with MLUs, no MLUs found!"
        ):
            c10d.ProcessGroupCNCL(store, self.rank, self.world_size)


class ProcessGroupCNCLTest(MultiProcessTestCase):
    def _create_process_group_cncl(self, store, opts):
        # create cncl processgroup with opts
    ...
    def opts(self, high_priority_stream=False):
        opts = c10d.ProcessGroupCNCL.Options()
        opts.is_high_priority_stream = high_priority_stream
        return opts

    def setUp(self):
        super(ProcessGroupCNCLTest, self).setUp()
        # CNCL_BLOCKING_WAIT overrides CNCL_ASYNC_ERROR_HANDLING hence tests
        # that use CNCL_BLOCKING_WAIT will test it as expected.
        os.environ["CNCL_ASYNC_ERROR_HANDLING"] = "1"
        # self.num_mlus = torch.mlu.device_count()
        self._spawn_processes()

    def tearDown(self):
        super(ProcessGroupCNCLTest, self).tearDown()
        try:
    ...
    @property
    def rank_to_MLU(self):
        # return rank to MLU map
        return init_multimlu_helper(self.world_size, "cncl")

    @requires_cncl()
    @sandcastle_skip_if(torch.mlu.device_count() < 2, "CNCL test requires 2+ MLUs")
    def test_empty_tensors(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        pg = self._create_process_group_cncl(store, self.opts())
        local_device_idx = self.rank_to_MLU[self.rank][0]

        xs = [torch.FloatTensor([]).mlu(local_device_idx)]
        ...
        ys = [[torch.FloatTensor([]).mlu(local_device_idx) for _ in range(self.world_size)]]
        pg.allgather(ys, xs).wait()
        for y in ys[0]:
            self.assertEqual(0, y.numel())

        ys = [torch.FloatTensor([]).mlu(local_device_idx)]
        xs = [[torch.FloatTensor([]).mlu(local_device_idx) for _ in range(self.world_size)]]
    ...
    def test_broadcast_ops(self):
        ...
        for i in range(self.world_size):
            ...
            x = torch.tensor([self.rank]).mlu(self.rank_to_MLU[self.rank][0])
            output = broadcast([x], i, 0)
            # TODO(#38095): Replace assertEqualIgnoreType. See issue #38095
            self.assertEqualIgnoreType(torch.tensor([i]), output[0])

            expected_tensor = torch.empty([i + 1, i + 1]).fill_(i + 1)
            xs = [torch.empty([i + 1, i + 1]).fill_(-1).mlu(device=device_idx) for device_idx in self.rank_to_MLU[self.rank]]

            # test with multiple input tensors (multiple mlu in one rank)
            for j in range(len(xs)):
                if self.rank == i:
                    xs[j] = expected_tensor.mlu(device=self.rank_to_MLU[self.rank][j])
    ...
    def test_allreduce_ops(self):
        ...
        pg = self._create_process_group_cncl(store, self.opts())
        local_device_id = self.rank_to_MLU[self.rank][0]
        ...
        # Sum
        tensors = [torch.tensor([self.rank + 1]).mlu(local_device_id)]
        ...
        # Avg (only available for CNCL 2.10+)
        # TODO: [PYTORCH-9271]
        if torch.mlu.cncl.version() >= (math.inf, math.inf, math.inf):
            tensors = [torch.tensor([self.rank + 1.]).mlu(local_device_id)]
        ...
        if torch.mlu.cncl.version() >= (math.inf, math.inf, math.inf):
            for dtype in torch.half, torch.float, torch.double:
                for factor in (3.0,
                    ...
                    tensors = [torch.tensor([self.rank + 1]).mlu(local_device_id).to(dtype=dtype)]

                    allreduce(tensors, c10d._make_cncl_premul_sum(factor))
        ...
        for op, err in zip((c10d.ReduceOp.BAND, c10d.ReduceOp.BOR, c10d.ReduceOp.BXOR),
            ...
            with self.assertRaisesRegex(
                    RuntimeError, "Cannot use " + err + " with CNCL"
            ):
        ...
        for rt in range(self.world_size):
            ...
            if torch.mlu.cncl.version() >= (math.inf, math.inf, math.inf):
                for factor in (3.0, (torch.tensor([5.0], device=local_device_id),)):
                    ...
                    float_tensors = [
                        torch.tensor(
                            [self.rank + 1.0], device=f"mlu:{local_device_id}")
                    ]
                    float_tensors_ref = [
                        torch.tensor(
                            [(self.rank + 1.0) * factor_ref], device=f"mlu:{local_device_id}")
                    ]

                    reduce(float_tensors_ref, rt, 0)
                    reduce(float_tensors, rt, 0, c10d._make_cncl_premul_sum(factor))
                    if self.rank == rt:
    ...
    def test_allgather_ops(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        pg = self._create_process_group_cncl(store, self.opts())
        local_device_ids = self.rank_to_MLU[self.rank]

        def allgather(output_ts, input_ts):
            work = pg.allgather(output_ts, input_ts)
            return work.wait()

        tensors = [torch.empty(2, 2).fill_(2).mlu(device=i) for i in local_device_ids]
        output_tensors = []
        expected_output = []

        output_per_mlu = ([torch.empty(2, 2).fill_(-1)] * len(local_device_ids) * self.world_size)
        expected_per_mlu = ([torch.empty(2, 2).fill_(2)] * len(local_device_ids) * self.world_size)

        for mlu in local_device_ids:
            output_tensors.append([t.mlu(device=mlu) for t in output_per_mlu])
            expected_output.append([t.mlu(device=mlu) for t in expected_per_mlu])
    ...
    def test_allgather_base_ops(self):
        ...
        # allgather_base is MLU number agnostic.
        # Each rank contribute one tensor regardless of MLU counts
        tensor = torch.tensor([self.rank]).mlu(local_device_id)
        # TODO: [PYTORCH-8741]
        output_t = torch.zeros((self.world_size), dtype=tensor.dtype).mlu(local_device_id)
    ...
    def test_allgather_base_basics(self):
        ...
        with self.assertRaisesRegex(
            ...
            tensor = torch.tensor([self.rank], dtype=torch.float).mlu(local_device_id)
            # TODO: [PYTORCH-8741]
    ...
    def test_gather_ops(self):
        ...
        for device_id in local_device_ids:
            tensors.append(torch.tensor([self.rank]).mlu(device_id))

        # init output
        output_ts = []
        for idx in range(num_mlus):
            mlu_idx = local_device_ids[idx]
            output_ts.append([])
            for rank in range(self.world_size):
                output_ts[idx].append(torch.tensor([-1]).mlu(mlu_idx))
    ...
    def test_gather_stress(self):
        ...
        for i in range(stress_length):
            tensors.append([])
            for device_id in local_device_ids:
                tensors[i].append(torch.tensor([self.rank]).mlu(device_id))

        # init output
        output_ts = []
        for i in range(stress_length):
            output_ts.append([[] for _ in range(num_mlus)])
            for idx, ls in enumerate(output_ts[i]):
                mlu_idx = local_device_ids[idx]
                for _ in range(self.world_size):
                    ls.append(torch.tensor([-1]).mlu(mlu_idx))
    ...
    def test_gather_checks(self):
        ...
        with self.assertRaisesRegex(
            ...
            tensors2 = []
            for device_id in local_device_ids:
                tensors2.append(torch.tensor([self.rank]).mlu(device_id))
                tensors2.append(torch.tensor([self.rank]).mlu(device_id))
    ...
    def test_scatter_ops(self):
        ...
        tensors = []
        for device_id in local_device_ids:
            tensors.append(torch.tensor([-1]).mlu(device_id))
        ...
        for idx in range(num_mlus):
            ...
            scatter_list.append([])
            for rank in range(self.world_size):
                scatter_list[idx].append(torch.tensor([rank]).mlu(mlu_idx))
    ...
    def test_scatter_stress(self):
        ...
        for i in range(stress_length):
            tensors.append([])
            for device_id in local_device_ids:
                tensors[i].append(torch.tensor([-1]).mlu(device_id))

        # init input
        scatter_list = []
        for i in range(stress_length):
            scatter_list.append([[] for _ in range(num_mlus)])
            for idx, ls in enumerate(scatter_list[i]):
                mlu_idx = local_device_ids[idx]
                for rank in range(self.world_size):
                    ls.append(torch.tensor([rank]).mlu(mlu_idx))
    ...
    def test_reduce_scatter_base_basics(self):
        ...
        with self.assertRaisesRegex(
            ...
            input_t = torch.tensor([self.rank]).mlu(local_device_id)
            # TODO: [PYTORCH-8741]
            output_t = torch.zeros((self.world_size + 1), dtype=input_t.dtype).mlu(
                local_device_id
            )
            # fails the check because output_t is not correctly sized
    ...
    def test_reduce_scatter_ops(self):
        ...
        output = [torch.tensor([0]).mlu(i) for i in local_device_ids]
        ...
        for i in range(self.world_size):
            input_per_mlu.append(torch.tensor([self.rank + i + 1]))

        for mlu in local_device_ids:
            tensor_lists.append([t.mlu(device=mlu) for t in input_per_mlu])

        reduce_scatter(output, tensor_lists, c10d.ReduceOp.SUM)

        for i in range(num_mlus):
        ...
        # Sum
        # TODO: [PYTORCH-8741]
        output_tensor = torch.zeros_like(input_per_mlu[0][0]).mlu(self.rank)
        input_list = [tensor[0].mlu(self.rank) for tensor in input_per_mlu]
        ...
        if torch.mlu.cncl.version() >= (math.inf, math.inf, math.inf):
            for factor in (3.0, (torch.tensor([5.0], device=self.rank),),):
                ...
                reduce_scatter(output, tensor_lists, c10d._make_cncl_premul_sum(factor))
    ...
    def test_reduce_scatter_base_ops(self):
        ...
        # reduce_scatter_base is MLU number agnostic.
        # Each rank contribute one tensor regardless of MLU counts
        # TODO: [PYTORCH-8741]
        output_t = torch.zeros([1]).mlu(local_device_id)
        tensor = torch.arange(self.world_size, dtype=output_t.dtype).mlu(local_device_id)
    ...
    def test_barrier(self):
        ...
        # 1, 2, 3, 4, .... len(local_device_ids) MLUs
        tensors_list = [[] for _ in range(len(local_device_ids))]

        for i in range(1, len(local_device_ids) + 1):
            for j in range(i):
                tensors_list[i - 1].append(torch.tensor([j + 1]).mlu(local_device_ids[j]))
    ...
    def test_send_recv(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        self._create_process_group_cncl(store, self.opts())
        device = self.rank_to_MLU[self.rank][0]
...
class DistributedDataParallelTest(
    ...
    def _get_process_group(self):
        ...
        return c10d.ProcessGroupCNCL(store, self.rank, self.world_size)

    def _test_cncl_backend(
        self, devices, device_ids, multi_device=False, gradient_as_bucket_view=False
    ):
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_propagate_error_reason(self):
        # Need to use CNCL_BLOCKING_WAIT and not ASYNC_ERROR_HANDLING,
        # otherwise process will be taken down and we can't check for errors.
        os.environ["CNCL_ASYNC_ERROR_HANDLING"] = "0"
        os.environ["CNCL_BLOCKING_WAIT"] = "1"
        # TODO: smaller timeout can fail since PG Cncl does health check in
        # constructor. Look into reducing this test's runtime.
        store = c10d.FileStore(self.file_name, self.world_size)
        # provide sufficient timeout to initialize CNCL comm.
        pg = c10d.ProcessGroupCNCL(store, self.rank, self.world_size, timeout=timedelta(seconds=15))
        pg_gloo = c10d.ProcessGroupGloo(store, self.rank, self.world_size)
        pg.barrier().wait(timedelta(seconds=5))
        # Simulate stuckness in rank 0.
        if self.rank == 0:
            pg_gloo.barrier().wait()
        inp = torch.ones(1).mlu(self.rank)
        ...
        if self.rank != 0:
            ...
            try:
                pg.allreduce([torch.ones(2).mlu(self.rank)]).wait()
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_backend_multi_device_ids_not_allowed(self):
        int_devices = list(range(torch.mlu.device_count()))
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        with self.assertRaisesRegex(
            ValueError, "device_ids can only be None or contain a single element."
        ):
            self._test_cncl_backend(devices, int_devices)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_backend_single_device_module_device_ids_None(self):
        self._test_cncl_backend(None, None)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_backend_single_device_module_empty_device_ids(self):
        ...
        self._test_cncl_backend(None, [])

    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_backend_multi_device_module_device_ids_None(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:2]
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        self._test_cncl_backend(devices, None, multi_device=True)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_backend_1mlu_module_device_ids_integer_list(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:1]
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        self._test_cncl_backend(devices, int_devices)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_backend_1mlu_module_device_ids_torch_device_list(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:1]
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        self._test_cncl_backend(devices, devices)

    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_backend_2mlu_module(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:2]
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        self._test_cncl_backend(devices, None, multi_device=True)
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(8)
    def test_cncl_backend_4mlu_module(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:4]
        devices = [torch.device("mlu:" + str(i)) for i in int_devices]
        self._test_cncl_backend(devices, None, multi_device=True)
    ...
    def test_ddp_multi_device_module_config(self):
        mlus = mlus_for_rank(self.world_size)[self.rank]

        self.assertTrue(len(mlus) >= 2, "expecting at least 2 mlus per process")

        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)

        mlus = mlus[:2]
        model = DoubleMluNet(mlus)
        ...
        with self.assertRaisesRegex(
            ...
            ddp_model = DistributedDataParallel(
                model, output_device=mlus[1], process_group=process_group
            )
        ...
        with self.assertRaisesRegex(
            ...
            ddp_model = DistributedDataParallel(
                model, device_ids=mlus, process_group=process_group
            )
    ...
    def _test_fp16(self, gradient_as_bucket_view=False):
        ...
        mlus = mlus_for_rank(self.world_size)[self.rank]
        model = nn.Linear(1, 1, bias=False).mlu(mlus[0]).half()
        ...
        input = torch.tensor([[2 ** 15]]).mlu(mlus[0]).half()
    ...
    def _test_arbitrary_forward_return_value(self, gradient_as_bucket_view=False):
        ...
        device_id = mlus_for_rank(self.world_size)[self.rank][0]
    ...
    def test_grad_layout_1devicemodule_1replicaperprocess(self):
        dev0 = torch.device("mlu:" + str(mlus_for_rank(self.world_size)[self.rank][0]))
    ...
    def test_grad_layout_2devicemodule(self):
        int_devices = mlus_for_rank(self.world_size)[self.rank][:2]
        dev0 = torch.device("mlu:" + str(int_devices[0]))
        dev1 = torch.device("mlu:" + str(int_devices[1]))
    ...
    def _mlu_model_with_ddp_comm_hook(
        self,
        process_group,
        hook=None,
        ...
        device_id = mlus_for_rank(self.world_size)[self.rank][0]
        mlu_model = DistributedDataParallel(
        ...
        if hook is not None:
            mlu_model.register_comm_hook(state, hook)

        return mlu_model

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_ddp_comm_hook_future_passing_mlu_cncl(self):
        """
        This unit test verifies whether the Future object is passed properly using cncl backend.
        The hook callback function creates a Future object and sets a value to it.
        """
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)

        # Get MLU model with simple_hook registered.
        mlu_model = self._mlu_model_with_ddp_comm_hook(process_group, self._simple_hook)

        # check whether the grads are equal to what simple_hook's then callback returns.
        # without the comm_hook, result would be 0.25 * torch.ones(2, 2).
        self._run_and_verify_hook(mlu_model, 8, 2 * torch.ones(2, 2))

    def _test_ddp_comm_hook_allreduce_hook_cncl(
        ...
        # Get MLU model with allreduce_hook registered.
        mlu_model = self._mlu_model_with_ddp_comm_hook(
            process_group, allreduce_hook, gradient_as_bucket_view, static_graph
        )

        # check whether the grads are equal to what DDP without hook would return.
        self._run_and_verify_hook(mlu_model, 8, 0.25 * torch.ones(2, 2))

    def _test_default_ddp_comm_hooks_cncl(self, gradient_as_bucket_view=False):
        """
        ...
        if (
            ...
            and c10d.is_cncl_available()
            and torch.mlu.cncl.version() >= (math.inf, math.inf)
        ):
            hook_options.append(default.bf16_compress_hook)
        for hook in hook_options:
            # Get MLU model with the hook registered.
    ...
    def _test_powerSGD_ddp_comm_hook_cncl(self, gradient_as_bucket_view=False):
        """
    ...
    def _test_builtin_ddp_comm_hooks_cncl(self, gradient_as_bucket_view=False):
        """
        ...
        for comm_hook_type in [
            ...
            # Get MLU model with the built-in communication hook.
            mlu_model = self._mlu_model_with_builtin_ddp_comm_hook(
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_ddp_comm_hook_allreduce_hook_cncl(self):
        self._test_ddp_comm_hook_allreduce_hook_cncl()

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_default_ddp_comm_hooks_cncl(self):
        self._test_default_ddp_comm_hooks_cncl()

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_fp16_compress_wrapper_cncl(self):
        self._test_fp16_compress_wrapper()

    @requires_cncl()
    # @requires_cncl_version((2, 10), "Need CNCL 2.10+ for BF16_COMPRESS")
    @sandcastle_skip_if(
        not BFLOAT16_AVAILABLE,
        "BFloat16 is only supported by CUDA 11+",
    )
    @skip_if_lt_x_mlu(2)
    def test_bf16_compress_wrapper_cncl(self):
        self._test_bf16_compress_wrapper()

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_builtin_ddp_comm_hooks_cncl(self):
        self._test_builtin_ddp_comm_hooks_cncl()

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_powerSGD_ddp_comm_hook_cncl(self):
        self._test_powerSGD_ddp_comm_hook_cncl()

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_ddp_comm_hook_allreduce_hook_cncl_grad_is_view(self):
        self._test_ddp_comm_hook_allreduce_hook_cncl(gradient_as_bucket_view=True)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_ddp_comm_hook_allreduce_hook_cncl_static_graph(self):
        self._test_ddp_comm_hook_allreduce_hook_cncl(static_graph=True)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_default_ddp_comm_hooks_cncl_is_view(self):
        self._test_default_ddp_comm_hooks_cncl(gradient_as_bucket_view=True)
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_builtin_ddp_comm_hooks_cncl_grad_is_view(self):
        self._test_builtin_ddp_comm_hooks_cncl(gradient_as_bucket_view=True)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_powerSGD_ddp_comm_hook_cncl_grad_is_view(self):
        self._test_powerSGD_ddp_comm_hook_cncl(gradient_as_bucket_view=True)

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_ddp_comm_hook_allreduce_with_then_hook_cncl(self):
        """
        ...
        # Get MLU model with allreduce_with_then_hook registered.
        mlu_model = self._mlu_model_with_ddp_comm_hook(
        ...
        self._run_and_verify_hook(mlu_model, 8, 1.25 * torch.ones(2, 2))
...
class CnclErrorHandlingTest(MultiProcessTestCase):
    def setUp(self):
        super(CnclErrorHandlingTest, self).setUp()
        # Need to skip return code checking for these tests since the child
        # processes don't exit cleanly.
        self.skip_return_code_checks = [
            self.test_cncl_errors_blocking_abort.__wrapped__,
            self.test_cncl_errors_blocking_sigkill.__wrapped__,
            self.test_cncl_errors_blocking_sigterm.__wrapped__,
            self.test_cncl_errors_blocking_nonzero_exit.__wrapped__,
        ]
        # CNCL_BLOCKING_WAIT overrides CNCL_ASYNC_ERROR_HANDLING hence tests
        # that use CNCL_BLOCKING_WAIT will test it as expected.
        os.environ["CNCL_ASYNC_ERROR_HANDLING"] = "1"
    ...
    def tearDown(self):
        super(CnclErrorHandlingTest, self).tearDown()
        try:
            os.remove(self.file_name)
        except OSError:
            pass
    ...
    def _run_all_reduce(self, pg):
        pg.allreduce(torch.rand(10).mlu(self.rank))

    @requires_cncl()
    # @requires_cncl_version((2, 4, 0), "Need CNCL 2.4+ for error checking")
    @skip_if_lt_x_mlu(3)
    @skip_if_rocm
    @sandcastle_skip("Test does not pass when run locally")
    def test_cncl_errors_nonblocking(self):
        # Note: we unset and restore CNCL_ASYNC_ERROR_HANDLING for this test
        # since test_c10d_common runs with async error handling by default, but this
        # tests behavior when it is not enabled.
        prev_cncl_async_error_handling = os.environ.get(
            "CNCL_ASYNC_ERROR_HANDLING", None
        )
        os.environ["CNCL_ASYNC_ERROR_HANDLING"] = "0"
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)
        process_group.allreduce(torch.rand(10).mlu(self.rank))
        if self.rank == 0:
            # This allreduce does not block Python thread as allreduce enqueues
            # the mlu operation, and then wait only blocks the current mlu
            # stream.
            work = process_group.allreduce(torch.rand(10).mlu(self.rank))
            work.wait()
        ...
        if prev_cncl_async_error_handling is not None:
            os.environ["CNCL_ASYNC_ERROR_HANDLING"] = prev_cncl_async_error_handling

    def _test_cncl_errors_blocking(self, func):
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(
            store,
            self.rank,
            self.world_size,
        ...
        if self.rank == 0:
            ...
            # Run some MLU operations to make sure mlu has not gotten stuck.
            # It was observed mlu could get stuck if CNCL communicators were
            # not properly aborted before throwing RuntimeError.
            a = torch.rand(10).mlu(self.rank)
        elif self.rank == 1:
    ...
    @skip_if_lt_x_mlu(3)
    @skip_if_rocm
    def test_cncl_errors_blocking_clean_exit(self):
        self._test_cncl_errors_blocking(lambda: sys.exit(0))
    ...
    @skip_if_lt_x_mlu(3)
    @skip_if_rocm
    def test_cncl_errors_blocking_nonzero_exit(self):
        self._test_cncl_errors_blocking(lambda: sys.exit(1))
    ...
    def test_cncl_errors_blocking_abort(self):
        self._test_cncl_errors_blocking(lambda: os.abort())
    ...
    @skip_if_lt_x_mlu(3)
    @skip_if_rocm
    def test_cncl_errors_blocking_sigkill(self):
        self._test_cncl_errors_blocking(lambda: os.kill(os.getpid(), signal.SIGKILL))
    ...
    @skip_if_lt_x_mlu(3)
    @skip_if_rocm
    def test_cncl_errors_blocking_sigterm(self):
        self._test_cncl_errors_blocking(lambda: os.kill(os.getpid(), signal.SIGTERM))

    @with_cncl_blocking_wait
    @requires_cncl()
    # @requires_cncl_version((2, 4, 0), "Need CNCL 2.4+ for error checking")
    @skip_if_lt_x_mlu(3)
    def test_cncl_blocking_wait_with_barrier(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(
            store,
            self.rank,
    ...
    def _run_invalid_cncl_blocking_wait_env(self, val):
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(3)
    def test_invalid_cncl_blocking_wait_env(self):
        self._run_invalid_cncl_blocking_wait_env("abc")
        self._run_invalid_cncl_blocking_wait_env("-1")
        self._run_invalid_cncl_blocking_wait_env("2147483647")
        self._run_invalid_cncl_blocking_wait_env("4294967295")

    def _check_valid_comm_exception(self, e):
        exception_str = str(e)
        valid_exceptions = [
            "CNCL communicator was aborted",
            "CNCL communicator encountered error",
    ...
    def _wait_for_comm_abort(self, process_group, timeout=None):
        ...
        while True:
            try:
                if not timeout:
                    process_group.allreduce(torch.rand(10).mlu(self.rank)).wait()
                else:
                    assert isinstance(timeout, timedelta)
                    process_group.allreduce(torch.rand(10).mlu(self.rank)).wait(timeout=timeout)
    ...
    def test_cncl_timeout(self):
        ...
        process_group.allreduce(torch.rand(10).mlu(self.rank)).wait(timeout=timedelta(seconds=5))

        if self.rank == 0:
            # This should timeout in about 1 second.
            # Watchdog may abort timed out work resulting in CNCL error instead of operation timed out.
            with self.assertRaisesRegex(RuntimeError, self.blocking_wait_error_msg):
                process_group.allreduce(torch.rand(10).mlu(self.rank)).wait(timeout=failed_collective_timeout)
...
class CommTest(test_c10d_common.AbstractCommTest, MultiProcessTestCase):
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_broadcast_coalesced_cncl(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)
        device = torch.device("mlu:%d" % self.rank)
        ranks = [0, 1]
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_all_reduce_coalesced_cncl(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        process_group = c10d.ProcessGroupCNCL(store, self.rank, self.world_size)
        device = torch.device("mlu:%d" % self.rank)
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_sequence_num_set_default_pg_cncl(self):
        torch.mlu.set_device(self.rank)
        self._test_sequence_num_set_default_pg(backend="cncl")

    @skip_if_lt_x_mlu(2)
    @requires_cncl()
    def test_sequence_num_incremented_cncl_default(self):
        self._test_sequence_num_incremented_default_group("cncl")

    @skip_if_lt_x_mlu(4)
    @requires_cncl()
    def test_sequence_num_incremented_cncl_subgroup(self):
        if self.world_size < 4:
            return sandcastle_skip("Test requires world_size of at least 4")
        self._test_sequence_num_incremented_subgroup("cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_sequence_num_set_cncl_new_group(self):
        torch.mlu.set_device(self.rank)
        self._test_sequence_num_set_new_group(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_pass_cncl_options_high_priority_stream(self):
        pg_opts = c10d.ProcessGroupCNCL.Options()
        ...
        t = torch.tensor([self.rank + 1] * 10).mlu(self.rank)
        pg.allreduce(t).wait()
        expected_tensor = torch.tensor([3] * 10).mlu(self.rank)
        self.assertEqual(expected_tensor, t)

    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_barrier(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="cncl", rank=self.rank, world_size=self.world_size, store=store
        )

        t = torch.tensor([self.rank + 1] * 10).mlu(2 * self.rank)
        c10d.all_reduce(t)
        expected_tensor = torch.tensor([3] * 10).mlu(2 * self.rank)
        self.assertEqual(expected_tensor, t)
        ...
        if self.rank == 0:
            t = torch.tensor([self.rank + 1] * 10).mlu(2 * self.rank)
            expected_tensor = torch.tensor([self.rank + 1] * 10).mlu(2 * self.rank)
            pg.allreduce(t).wait()
            self.assertEqual(expected_tensor, t)
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_barrier_timeout(self):
        os.environ["ENABLE_CNCL_HEALTH_CHECK"] = "1"
        store = c10d.FileStore(self.file_name, self.world_size)
        if self.rank == 0:
            with self.assertRaisesRegex(
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_barrier_timeout_new_group(self):
        os.environ["ENABLE_CNCL_HEALTH_CHECK"] = "1"
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="cncl",
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(4)
    def test_cncl_barrier_timeout_new_group_non_member(self):
        os.environ["ENABLE_CNCL_HEALTH_CHECK"] = "1"
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="cncl",
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_barrier_device_ids(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="cncl", rank=self.rank, world_size=self.world_size, store=store
        )
    ...
    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_cncl_barrier_device_ids_function_argument(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="cncl", rank=self.rank, world_size=self.world_size, store=store
        )
    ...
    @skip_if_lt_x_mlu(2)
    @with_dist_debug_levels(levels=["DETAIL"])
    def test_cncl_warn_not_in_group_debug_detail(self):
        self._test_warn_not_in_group(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    @with_dist_debug_levels(levels=["INFO"])
    def test_cncl_warn_not_in_group_debug_info(self):
        self._test_warn_not_in_group(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    @with_dist_debug_levels(levels=["OFF"])
    def test_cncl_warn_not_in_group_debug_off(self):
        self._test_warn_not_in_group(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_nncl_rank_membership(self):
        self._test_rank_membership(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_tensor_dtype_mismatch(self):
        self._test_tensor_dtype_mismatch(backend="cncl")

    @requires_cncl()
    @skip_if_lt_x_mlu(2)
    def test_tensor_dtype_complex(self):
        self._test_tensor_dtype_complex(backend="cncl")
...
class CompilerTest(test_c10d_common.CompilerTest):
    ...
    @skip_if_lt_x_mlu(2)
    def test_allreduce_work_wait_mlu(self):
        self._test_allgather_work_wait(
            torch.ones(2, 2, device=self.rank) * self.rank,
        )

    @skip_if_lt_x_mlu(2)
    def test_allgather_work_wait_mlu(self):
        self._test_allgather_work_wait(
            torch.ones(2, 2, device=self.rank) * self.rank
        )

    @skip_if_lt_x_mlu(2)
    def test_reduce_scatter_work_wait_mlu(self):
        self._test_reduce_scatter_work_wait(
            torch.ones(2, 2, device=self.rank) * self.rank
        )

    @skip_if_lt_x_mlu(2)
    def test_broadcast_work_wait_mlu(self):
        self._test_broadcast_work_wait(
            torch.ones(2, 2, device=self.rank) * self.rank
        )

    @skip_if_lt_x_mlu(2)
    def test_scatter_work_wait_mlu(self):
    ...
    @skip_if_lt_x_mlu(2)
    def test_consecutive_comm_work_wait_mlu(self):
        self._test_consecutive_comm_work_wait(
            torch.ones(2, 2, device=self.rank) * self.rank
        )

class CnclProcessGroupWithDispatchedCollectivesTests(test_c10d_common.ProcessGroupWithDispatchedCollectivesTests):
    @requires_cncl()
    @skip_if_lt_x_mlu(1)
    def test_collectives(self):
        self._test_collectives(backend="cncl")

if __name__ == "__main__":
    assert (
        not torch.mlu._initialized
