...
from torch.testing._internal.common_device_type import instantiate_device_type_tests, skipMeta
...
NO_CNCL = not hasattr(torch.distributed, "ProcessGroupCNCL")
...
class TestDataParallel(TestCase):
    ...
    @sandcastle_skip_if(not TEST_MULTIMLU, "multi-MLU not supported")
    def test_parallel_apply_autocast(self):
        l1 = nn.Linear(10, 5).to("mlu:0", torch.float)
        l2 = nn.Linear(10, 5).to("mlu:1", torch.float)
        i1 = torch.randn(2, 10, device="mlu:0", dtype=torch.float)
        i2 = torch.randn(2, 10, device="mlu:1", dtype=torch.float)
        with autocast():
            expected1 = l1(i1)
            expected2 = l2(i2)
        modules = (l1, l2)
    ...
    def test_autocast(self):
        ...
        model = dp.DataParallel(Model().mlu().to(dtype=torch.float32))
        input = torch.randn((8, 8), dtype=torch.float32, device="mlu")
