...
class TestWhereOp(TestCase):
    ...
    @testinfo()
    def test_where(self):
        con_shape_list = [(343, 4), (80, 4), (1, 4, 5), (1, 4, 5, 6), (), (0), (1), (343, 4)]
        x_shape_list = [(343, 4), (80, 4), (3, 4, 5), (1, 4, 5, 6), (), (0), (1), (1)]
        y_shape_list = [(343, 4), (80, 4), (3, 4, 5),(3, 4, 5, 6), (), (0), (1), (1)]
        dtype = [torch.float32, torch.int32, torch.int16, torch.int8, torch.long, torch.double]
        cond_dtypes = [torch.bool, torch.uint8]
        for con_shape, x_shape, y_shape in \
                zip(con_shape_list, x_shape_list, y_shape_list):
    ...
    def test_where_condition_tensor(self):
        ...
        dtype = [torch.bool,torch.float32, torch.int32, torch.double, torch.int64, torch.long]
    ...
    def test_where_backward(self):
        ...
        for con_shape, x_shape, y_shape in zip(con_shape_list, x_shape_list, y_shape_list):
    ...
    def test_where_channels_last(self):
        con_shape_list = [(1, 4, 5, 6),]
        x_shape_list = [(1, 4, 5, 6),]
        y_shape_list = [(3, 4, 5, 6),]
        dtype = [torch.float32,]
        for con_shape, x_shape, y_shape, xy_dtype in \
                zip(con_shape_list, x_shape_list, y_shape_list, dtype):
            condition = torch.randint(2, con_shape).to(dtype = torch.bool)
            condition = condition.to(memory_format=torch.channels_last)
            x = torch.randn(x_shape, dtype=xy_dtype).to(memory_format=torch.channels_last)
            y = torch.randn(y_shape, dtype=xy_dtype).to(memory_format=torch.channels_last)
            out_cpu = torch.where(condition, x, y)
            condition_mlu = condition.to('mlu')
            x_mlu = x.to('mlu')
            y_mlu = y.to('mlu')
    ...
    def test_where_one_dim_channels_last(self):
        con_shape_list = [(4, 1, 5, 6),]
        x_shape_list = [(4, 1, 5, 6),]
        y_shape_list = [(4, 1, 5, 6),]
        dtype = [torch.float32,]
        for con_shape, x_shape, y_shape, xy_dtype in \
                zip(con_shape_list, x_shape_list, y_shape_list, dtype):
            condition = torch.randint(2, con_shape).to(dtype = torch.bool)
    ...
    def test_where_scalar_channels_last_contiguous(self):
        con_shape_list = [(2, 1, 2, 2),]
        x_shape_list = [(1, 3, 1, 1),]
        dtype = [torch.float32,]
        for con_shape, x_shape, xy_dtype in \
                zip(con_shape_list, x_shape_list, dtype):
            condition = torch.randint(2, con_shape).to(dtype = torch.bool)
            condition = condition.to(memory_format=torch.channels_last)
            x = torch.randn(x_shape, dtype=xy_dtype).to(memory_format=torch.channels_last)
            y = torch.tensor(1)
    ...
    def test_where_not_dense_channels_last(self):
        ...
        for con_shape, x_shape, y_shape, xy_dtype in \
                zip(con_shape_list, x_shape_list, y_shape_list, dtype):
            condition = torch.randint(2, con_shape).to(dtype = torch.bool, memory_format=torch.channels_last)[..., :3]
            x = torch.randn(x_shape).to(dtype = xy_dtype).to(memory_format=torch.channels_last)[..., :3]
            y = torch.randn(y_shape).to(dtype = xy_dtype).to(memory_format=torch.channels_last)[..., :3]
            out_cpu = torch.where(condition, x, y)
            condition_mlu = condition.to('mlu')
            x_mlu = x.to('mlu')
            y_mlu = y.to('mlu')
    ...
    def test_where_anomalistic_stride(self):
        con_shape_list = [(1, 128, 1, 64),]
        x_shape_list = [(1, 128, 1, 64),]
        y_shape_list = [(1, 128, 1, 64),]
        dtype = [torch.float32,]
        for con_shape, x_shape, y_shape, xy_dtype in \
                zip(con_shape_list, x_shape_list, y_shape_list, dtype):
            condition = torch.randint(2, con_shape).to(dtype = torch.bool)
    ...
    def test_where_large(self):
        ...
        dtype = [torch.int8]
        cond_dtypes = [torch.bool, torch.uint8]
        for con_shape, x_shape, y_shape, xy_dtype, cond_dtype in \
                zip(con_shape_list, x_shape_list, y_shape_list, dtype, cond_dtypes):
