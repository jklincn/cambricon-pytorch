...
from common_utils import testinfo, TEST_BFLOAT16, TestCase  # pylint: disable=C0413, C0411

logging.basicConfig(level=logging.DEBUG)
torch.manual_seed(2)

class TestDropoutOp(TestCase):
    def _test_dropout(self, cls, device, input, p, memory_format=torch.contiguous_format):
    ...
    def test_Fused_Dropout(self):
        ...
        self.assertTrue(fused_res_mlu.is_contiguous(memory_format=torch.contiguous_format))
    ...
    def test_native_dropout(self):
        ...
        native_res_mlu, native_mask_mlu = torch.native_dropout(input_native_mlu, p=0.8, train=True)
        native_res_mlu.backward(grad_native_mlu)
        res_grad = input_native.grad
        self.assertTrue(res_grad.is_contiguous(memory_format=torch.contiguous_format))
        self.assertTrue(native_res_mlu.is_contiguous(memory_format=torch.contiguous_format))
    ...
    def test_Dropout2d_ChannelLast(self):
        ...
        h = random.randint(1, 5)
        input = torch.Tensor(1000, b, w, h)
        self._test_dropout(nn.Dropout2d, 'mlu', input, 0.2, memory_format=torch.channels_last)
    ...
    def test_native_dropout_bfloat16(self):
        input_fused = torch.testing.make_tensor((2, 3, 24, 24), dtype=torch.bfloat16, \
                                            device='cpu')
        grad_fused = torch.testing.make_tensor((2, 3, 24, 24), dtype=torch.bfloat16, \
                                            device='cpu')
        ...
        fused_res_cpu, fused_mask_cpu = torch.native_dropout(input_fused_cpu, p=0.2, train=True)
        fused_res_mlu, fused_mask_mlu = torch.native_dropout(input_fused_mlu, p=0.2, train=True)
        ...
        self.assertTrue(res_grad_cpu.size() == res_grad_mlu.size() and \
                        res_grad_cpu.stride() == res_grad_cpu.stride())
        self.assertTrue(fused_res_cpu.size() == fused_res_mlu.size() and \
                        fused_res_cpu.stride() == fused_res_mlu.stride())
        self.assertTrue(fused_mask_cpu.size() == fused_mask_mlu.size() and \
                        fused_mask_cpu.stride() == fused_mask_mlu.stride())
    ...
    def test_fused_dropout_bfloat16(self):
        ...
        self.assertTrue(res_grad.is_contiguous(memory_format=torch.contiguous_format))
        self.assertTrue(fused_res_mlu.is_contiguous(memory_format=torch.contiguous_format))
        self.assertTrue(fused_mask_mlu.is_contiguous(memory_format=torch.contiguous_format))
