...
class TestReflectionPadOp(TestCase):
    ...
    def test_reflection_pad2d(self):
        shape_list = [(2, 3, 4, 5), (3, 4, 5),
                      (256, 32, 12, 12), (0, 2, 12, 12)]
    ...
    def test_reflection_pad1d_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "negative padding"):
            input = torch.randn((1, 2, 4), dtype=torch.float32, device='mlu').requires_grad_()
            m = torch.nn.ReflectionPad1d(-1)
            res_mlu = m(input)
    ...
    def test_reflection_pad2d_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Padding size should be less than the corresponding input dimension"):
        ...
        with self.assertRaisesRegex(RuntimeError, "negative padding"):
            input = torch.randn((1, 2, 4, 2), dtype=torch.float32, device='mlu').requires_grad_()
            m = torch.nn.ReflectionPad2d(-1)
            res_mlu = m(input)
            res_mlu.sum().backward()
