...
class TestBmmOp(TestCase):
    ...
    def test_bmm_out(self):
        ...
        for (data_type, err), (shape_a, shape_b), shape_o, mode, mode_o\
                in product(dtype_list, shape_ab_list, shape_out_list, mode_list, mode_list):
            a = torch.randn(shape_a, dtype=torch.float)
            b = torch.randn(shape_b, dtype=torch.float)
    ...
    def test_bmm_large(self):
        ...
        for (data_type, err), (shape_a, shape_b)\
                in product(dtype_list, shape_ab_list):
            a = torch.randn(shape_a, dtype=torch.float)
            b = torch.randn(shape_b, dtype=torch.float)
