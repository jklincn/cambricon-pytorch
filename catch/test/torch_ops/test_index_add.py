...
class TestIndexAddOp(TestCase):
    ...
    def test_index_add(self):
        ...
        for i, shape in enumerate(shape_list):
            for channels_last in channels_last_list:
                ...
                out_mlu = torch.index_add(x_mlu, dim_list[i], index_mlu, source.to('mlu'))
    ...
    def test_index_add_not_dense(self):
        ...
        for i, shape in enumerate(shape_list):
            ...
            index_mlu = index.to('mlu')
            source = torch.rand(source_list[i])
            out_cpu = torch.index_add(x[:,:3,...], dim_list[i], index, source[:,:3,...])
            out_mlu = torch.index_add(x_mlu[:,:3,...], dim_list[i],
                      index_mlu, source.to('mlu')[:,:3,...])
            self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.003, use_MSE=True)
    ...
    def test_index_add_inplace_not_dense(self):
        ...
        for i, shape in enumerate(shape_list):
            ...
            x_mlu[:,:3,...].index_add(dim_list[i], index_mlu, source.to('mlu')[:,:3,...])
    ...
    def test_index_add_dtype(self):
        ...
        source_list = [(2, 3, 5), (4, 5, 2, 3), (3, 4, 3, 2, 3), (3, 4)]
        dim_list = [-2, 1, 2, 0]
        type_list = [torch.float, torch.short, torch.int32, torch.int16, torch.double, torch.long]
        for i, shape in enumerate(shape_list):
            for t in type_list:
    ...
    def test_index_add_backward(self):
        ...
        x_mlu = torch.zeros((2, 4, 5), dtype = torch.float, device = 'mlu', requires_grad = True)
