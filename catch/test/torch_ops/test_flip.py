...
class TestFlipOp(TestCase):
    ...
    def test_flip_torch(self):
        def run_test(x, x_mlu, dims, err):
            ...
            self.assertTensorsEqual(
                out_cpu, out_mlu.cpu(),
                err, use_MSE=True)
        type_list = [(torch.float, 0), (torch.half, 0), (torch.int, 0), (torch.short, 0),
                     (torch.long, 0), (torch.uint8, 0), (torch.double, 0), (torch.int8, 0),
                     (torch.bool, 0)]
            ...
            for type_, err in type_list:
                for dims in [(2, 0, 1), ()]:
                    ...
                    run_test(x.to(memory_format = torch.channels_last),
                             x_mlu.to(memory_format = torch.channels_last), dims, err)
    ...
    def test_flip_is_contiguous(self):
        def run_test(x_contiguous, x_mlu_contiguous, dims,
