...
class TestSgnOp(TestCase):
    ...
    def test_sgn_dtype(self):
        ...
        for type_ in dtype_list:
            ...
            if type_ == torch.complex64:
                self.assertTensorsEqual(x.real.float(), x_mlu.real.cpu().float(), 0.0, use_MSE=True)
                self.assertTensorsEqual(x.imag.float(), x_mlu.imag.cpu().float(), 0.0, use_MSE=True)
            else:
    ...
    def test_sgn_backward(self):
        for shape_ in [(1), (10), (2,3), (8, 224, 224), (1, 3, 16, 16), (1, 3, 16, 16, 3)]:
