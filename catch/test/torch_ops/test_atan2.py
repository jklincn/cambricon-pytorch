...
class TestOps(TestCase):
    ...
    def test_atan2_out(self):
        ...
        for shape in shape_list:
            ...
            out_mlu = torch.randn(1, dtype=torch.float).to(ct.mlu_device())
    ...
    def test_atan2_not_dense(self):
        ...
        for shape in shape_list:
            ...
            x_mlu = x.to(ct.mlu_device())[...,:int(shape[-1] / 2)]
    ...
    def test_atan2_channels_last(self):
        shape_list = [(512, 2, 2, 4), (3, 2, 3, 4),
                      (254, 254, 1, 4)]
        for shape in shape_list:
            x = torch.randn(shape, dtype=torch.float)
            x_cpu = x.to(memory_format=torch.channels_last)
            x_mlu = x.to(ct.mlu_device()).to(memory_format=torch.channels_last)
            other = torch.randn(shape, dtype=torch.float)
