...
class TestBitwiseXorOps(TestCase):
    ...
    def test_bitwise_xor(self):
        ...
        for dtype in dtype_lst:
            for shape1, shape2 in [((1, 2,), (1, 1)),
                ...
                self.assertTensorsEqual(result_cpu.float(), result_mlu.cpu().float(), 0.003,
                                        use_MSE = True)
                ...
                # channels last
                if a.dim() == 4:
                    x = self._generate_tensor(shape1, dtype).to(memory_format = torch.channels_last)
                    y = self._generate_tensor(shape2, dtype)
    ...
    def test_bitwise_xor_permute(self):
        ...
        for i in range (4):
            ...
            x, y, out = x.permute(permute_shape[i]), y.permute(permute_shape[i]), out.permute(permute_shape[i])
            x_mlu, y_mlu, out_mlu = x_mlu.permute(permute_shape[i]), y_mlu.permute(permute_shape[i]), \
