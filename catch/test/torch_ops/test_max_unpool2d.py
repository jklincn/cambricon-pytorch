...
class TestMaxUnPoolingOp(TestCase):
    ...
    def test_max_unpool2d_forward(self):
        ...
        for shape, k, s, p, func, dtype in product(*loop_var):
            ...
            indices_mlu_converted = func(indices_mlu).to(torch.int32) if dtype==torch.float else func(indices_mlu).to(torch.int16)
            ...
            output_cpu_F = F.max_unpool2d(func(output_pool_cpu), func(indices_cpu),
                                        kernel_size=k, stride=s, padding=p)
            output_mlu_F = F.max_unpool2d(func(output_pool_mlu), indices_mlu_converted,
                                        kernel_size=k, stride=s, padding=p)
            self.assertTensorsEqual(output_cpu_F, output_mlu_F.cpu(), 3e-3, use_MSE=True)
    ...
    def test_max_unpool2d_backward(self):
        ...
        for shape, k, s, p, func, dtype in product(*loop_var):
            ...
            output_mlu = unpool(func(output_pool_mlu), func(indices_mlu).to(indices_dtype))

            output_mlu.backward(func(grad_mlu), retain_graph = True)
            
            self.assertTensorsEqual(input_cpu.grad.float(), input_mlu.grad.cpu(), 3e-3, use_MSE=True)

            # test torch.nn.functional.max_unpool2d
            input_cpu.grad.zero_()
            input_mlu.grad.zero_()
            out_cpu_F = F.max_unpool2d(func(output_pool_cpu), func(indices_cpu),
                                       k, stride=s, padding=p)
            out_mlu_F = F.max_unpool2d(func(output_pool_mlu), func(indices_mlu).to(indices_dtype),
                                       k, stride=s, padding=p)
    ...
    def test_max_unpool2d_exception(self):
        output, indices = F.max_pool2d(torch.randn([1, 1, 4, 4]).mlu(), 2,
