...
from common_utils import testinfo, TestCase, mlufusion_on_and_off  # pylint: disable=C0413,C0411
...
class TestLstmOp(TestCase):
    ...
    def test_lstm_training_basic(self):
        ...
        bias_v = [True, False]
        for input_size, hidden_size, batch, seq_len, bidirectional, bias in product(
                input_size_v, hidden_size_v, batch_v, seq_len_v,
                bidirectional_v, bias_v):
    ...
    def test_lstm_training_dtype(self):
        for dtype in [torch.half, torch.double]:
            ...
            for w, w_mlu in zip(rnn.parameters(), rnn_mlu.parameters()):
                self.assertTensorsEqual(w.grad,
                                        w_mlu.grad.cpu(),
                                        3e-3,
                                        use_MSE=True)
    ...
    def test_lstm_training(self):
        ...
        for input_size, hidden_size, batch, seq_len, bidirectional, bias in product(
            ...
            h0 = torch.randn((int)(bidirectional) + 1, batch, hidden_size)
            ...
            self.assertTensorsEqual(input.grad,
                                    ...
                                    3e-3,
                                    use_MSE=True)
            self.assertTensorsEqual(h0.grad,
                                    h0_mlu.grad.cpu(),
                                    3e-3,
                                    use_MSE=True)
            self.assertTensorsEqual(c0.grad,
                                    c0_mlu.grad.cpu(),
                                    3e-3,
                                    use_MSE=True)
    ...
    def test_lstm_training_numlayers(self):
        ...
        for input_size, hidden_size, batch, seq_len, bidirectional, bias in product(
            ...
            rnn = nn.LSTM(input_size,
                          ...
                          bidirectional=bidirectional)
            input = torch.randn(seq_len, batch, input_size)
            h0 = torch.randn(((int)(bidirectional) + 1) * num_layers, batch,
                             hidden_size)
            c0 = torch.randn(((int)(bidirectional) + 1) * num_layers, batch,
                             hidden_size)
    ...
    def test_lstm_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            out_mlu, (hn_mlu, cn_mlu) = rnn_mlu(input_mlu, (h0_mlu, c0_mlu))  # pylint: disable=W0612

        ref_msg = "rnn: cx is not contiguous"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            out_mlu, (hn_mlu, cn_mlu) = rnn_mlu(input_mlu, (h0_mlu.contiguous(), c0_mlu))

        ref_msg = r"Expected all tensors to be on the same device, but found at least " + \
          r"two devices, cpu and mlu:0! \(when checking argument for argument hx in method " + \
          r"wrapper_input_lstm\)"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            out_mlu, (hn_mlu, cn_mlu) = rnn_mlu(input_mlu.cpu(), (h0_mlu.contiguous(), c0_mlu))
