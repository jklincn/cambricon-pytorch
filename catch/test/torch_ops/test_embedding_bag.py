...
class TestEmbeddingOp(TestCase):
    ...
    def test_embedding_bag_empty(self):
        es = torch.nn.EmbeddingBag(5, 2, mode='sum', include_last_offset=False)\
            .to(dtype=torch.float32, device='mlu')
        ...
        for c, x_shape in product(c_lists, x_shape_lists):
            for data_type, err in dtype_list:
                for offset_ in offset_lists:
                    ...
                    out_cpu = torch.nn.functional.embedding_bag(x, matrix.float(), offset)
                    out_mlu = torch.nn.functional.embedding_bag(x.to('mlu'),
                        self.to_mlu_dtype(matrix, data_type), offset.to('mlu'))
        ...
        for mode in mode_lists:
            for x_shape in x_shape_lists:
                for data_type, err in dtype_list:
                    ...
                    out_cpu = torch.nn.functional.embedding_bag(x, matrix.float(), mode=mode)
                    out_mlu = torch.nn.functional.embedding_bag(
    ...
    def test_embedding_bag_backward(self):
        ...
        for c, x_shape in product(c_lists, x_shape_lists):
            for data_type, err in dtype_list:
                ...
                matrix.grad.zero_()
                out_mlu = torch.nn.functional.embedding_bag(
                    self.to_device(x), self.to_mlu_dtype(matrix, data_type), mode='sum')
    ...
    def test_embedding_bag_exception(self):
        ...
        ref_msg = "Expected tensor for argument #1 'weight' to have one of the following"
        ref_msg = ref_msg + " scalar types: Float, Half, Double; but got MLUIntType instead"
        ref_msg = ref_msg + r" \(while checking arguments for cnnl__embedding_bag\)"
        ...
        ref_msg_2 = "Expected tensor for argument #1 'indices' to have one of the following"
        ...
        matrix = torch.rand(9796, 511, dtype=torch.float)
        ref_msg_3 = "Expected tensor for argument #1 'indices' to have one of the following"
