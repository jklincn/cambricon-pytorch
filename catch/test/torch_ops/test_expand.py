...
class TestExpandOp(TestCase):
    ...
    def test_expand_bfloat16(self):
        ...
        input_cpu = torch.nn.Parameter(input_t.float())
        input_mlu = torch.nn.Parameter(input_t.mlu())
        out_cpu = input_cpu.expand((2, 4, 24, 5)) * 3.14
        out_mlu = input_mlu.expand((2, 4, 24, 5)) * 3.14
        grad = torch.randn_like(out_cpu)
        out_cpu.backward(grad)
        out_mlu.backward(grad.mlu().bfloat16())
