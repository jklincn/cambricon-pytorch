...
class TestAddrOp(TestCase):
    def run_addr_test(self, m, v1, v2, beta=1, alpha=1, m_transform=lambda x: x):
        ...
        out_cpu = torch.addr(m.cpu().float(), v1.cpu().float(), v2.cpu().float(),
        ...
        self.assertTensorsEqual(out_mlu.float().cpu(),
                                out_cpu, 0.03, use_MSE=True)

    def run_addr_out_test(self, m, v1, v2, dtype, m_transform=lambda x: x, beta=1, alpha=1):
        m = m_transform(m.clone())
        zero_mlu = torch.zeros(v1.size(0), v2.size(0),
                               dtype=dtype, device='mlu')
        zero_cpu = torch.zeros(v1.size(0), v2.size(
            0), dtype=torch.float, device='cpu')
        out_cpu = torch.addr(m.cpu().float(), v1.cpu().float(), v2.cpu().float(),
                             beta=beta, alpha=alpha, out=zero_cpu)
        out_mlu = torch.addr(m, v1, v2, beta=beta, alpha=alpha, out=zero_mlu)
        self.assertTensorsEqual(out_mlu.float().cpu(),
                                out_cpu, 0.03, use_MSE=True)
        self.assertTensorsEqual(zero_mlu.float().cpu(),
                                zero_cpu, 0.03, use_MSE=True)
    ...
    def run_addr__test(self, m, v1, v2, m_transform=lambda x: x):
        ...
        self.assertTensorsEqual(
            m_cpu.float(), m.float().cpu(), 0.03, use_MSE=True)
        ...
        for h, w in [(100, 110), (1, 20), (200, 2)]:
            for beta, alpha in [(0.5, 1), (1, 1), (1, 0.5), (0, 2), (2, 0)]:
                for dtype in [torch.float32, torch.float16]:
                    ...
                    # test transpose
                    self.run_addr_test(m, v2, v1, beta, alpha,
        ...
        for h, w in [(100, 110), (1, 20), (200, 2)]:
            for beta, alpha in [(0.5, 1), (1, 1), (1, 0.5)]:
                for dtype in [torch.float32, torch.float16, torch.double, torch.half]:
                    ...
                    out_cpu = torch.addr(m.float(), v1_cpu.float(),
                                         v2_cpu.float(), beta=beta, alpha=alpha)
                    out_mlu = torch.addr(
                        M, v1_mlu, v2_mlu, beta=beta, alpha=alpha)
                    self.assertTensorsEqual(
            ...
            for beta, alpha in [(0.5, 1), (1, 1), (1, 0.5), (0, 2), (2, 0)]:
                for dtype in [torch.float32, torch.float16]:
                    ...
                    self.run_addr_out_test(
                        m, v1, v2, dtype, beta=beta, alpha=alpha)
    ...
    def test_addr_backward(self):
        def fn(m_0, v1_0, v2_0, dtype, is_grad_conti=True):
            ...
            v1 = v1_0.to("mlu")
            v2 = v2_0.to("mlu")
            out_cpu = torch.addr(m_0.float(), v1_0.float(),
                                 v2_0.float(), beta=1, alpha=1)
    ...
    def test_addr_empty(self):
        ...
        self.assertTensorsEqual(
            out.float(), out_mlu.float().cpu(), 0, use_MSE=True)
    ...
    def test_addr_bfloat16(self):
        ...
        self.assertTensorsEqual(mat_cpu.grad.float(), mat_mlu.grad.cpu().float(), 0.003, use_MSE=True)
        self.assertTensorsEqual(a_cpu.grad.float(), a_mlu.grad.cpu().float(), 0.003, use_MSE=True)
        self.assertTensorsEqual(b_cpu.grad.float(), b_mlu.grad.cpu().float(), 0.003, use_MSE=True)
