...
class TestPreluOp(TestCase):
    ...
    def test_prelu(self):
        ...
        for shape, type, func1, func2 in product(shape_list, type_list, func_list, func_list):
            shape1, shape2 = shape
    ...
    def test_prelu_backward(self):
        shape_list = [[(5), 1], [(3, 2), 2], [(1, 3, 224), 3],
                      [(2, 6, 224, 224), 6], [(1, 3, 224, 224), 3],
                      [(1, 2, 3, 224, 224), 2]]
        type_list = [(torch.float,3e-3), (torch.half, 3e-2)]
        func_list = [self.convert_to_channel_last, self.to_non_dense, lambda x:x]
        for shape_info, type_info, func1, func2 in product(shape_list, type_list,
                                                           func_list, func_list):
            type, err = type_info
            ...
            input.grad.zero_()
            m_mlu = m.to(ct.mlu_device()).to(type)
            out_mlu = m_mlu(func1(self.to_mlu(input)))
            out_mlu.backward(func2(self.to_mlu(grad)))
            x_grad_mlu = input.grad
            self.assertTensorsEqual(x_grad_cpu.float(), x_grad_mlu.cpu().float(), err, use_MSE=True)
    ...
    def test_prelu_exception(self):
        for shape in [(2,3), (2,1)]:
            ...
            msg = r"Mismatch of parameter numbers and input channel size. " \
