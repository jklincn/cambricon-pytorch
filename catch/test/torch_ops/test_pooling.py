...
class TestPoolingOp(TestCase):
    ...
    def test_avgpool1d_non_batch(self):
        ...
        loop_var = [
            shape_list, kernel_v, stride_v, padding_v,
            ceil_mode_v, include_pad_v, dtypes
        ]
    ...
    def test_avgpool2d_non_batch(self):
        shape_list = [(6, 7, 7), (3, 6, 12), (4, 13, 16), (16, 7, 7), (6, 8, 16), (23, 13, 64)]
        kernel_v = [3, 4, 5]
        stride_v = [2, 3, 4]
        padding_v = [0, 1]
        ceil_mode_v = [False, True]
    ...
    def test_avgpool2d_batch(self):
        ...
        loop_var = [
            memory_format_list, shape_list, kernel_v, stride_v, padding_v,
            ceil_mode_v, include_pad_v, dtypes
        ]
        for memory_format, in_shape, kernel, stride, padding, ceil_mode, include_pad, t in product(
                *loop_var):
    ...
    def test_maxpool1d_non_batch(self):
        ...
        loop_var = [kernel_v, stride_v, padding_v, return_indices_v,
            ceil_mode_v, dtypes, in_shapes
        ]
        for kernel, stride, padding, return_indices, ceil_mode, t, in_shape in product(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=t)
            input_cpu = input_t if (t != torch.half) else input_t.to(torch.float)
            output_cpu = F.max_pool1d(input_cpu,
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
            ...
            output_mlu = F.max_pool1d(self.to_mlu(input_t),
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
            ...
            if return_indices == True:
                self.assertTensorsEqual(output_cpu[0],
                                        ...
                                        use_MSE=True)
                if t == torch.half:
                    self.assertIs(output_mlu[1].dtype, torch.short)
                else:
            ...
            # not dense
            output_cpu = F.max_pool1d(input_cpu[:2, ...],
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
            ...
            output_mlu = F.max_pool1d(self.to_mlu(input_t)[:2, ...],
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
    ...
    def test_maxpool1d_index(self):
        ...
        demo_input = torch.Tensor([[1, 3, 4, 2], [2, 5, 6, 7],
                                     [3, 8, 9, 6]]).to(torch.half)
        ...
        output_demo_cpu = F.max_pool1d(demo_input.float(),
                                       kernel_size=2,
                                       stride=1,
                                       padding=0,
                                       dilation=1,
                                       return_indices=True,
                                       ceil_mode=False)
        output_demo = F.max_pool1d(self.to_mlu(demo_input),
                                   kernel_size=2,
                                   stride=1,
                                   padding=0,
                                   dilation=1,
                                   return_indices=True,
                                   ceil_mode=False)
        self.assertEqual(output_demo_cpu[1], out_put_gt.long())
        self.assertEqual(output_demo[1].long().cpu(), out_put_gt_mlu.long())
    ...
    def test_maxpool2d_non_batch(self):
        ...
        for kernel, stride, padding, return_indices, ceil_mode, t, in_shape in product(
            ...
            input_t = torch.randn(in_shape, dtype=t)
            input_cpu = input_t if (t != torch.half) else input_t.to(torch.float)
            output_cpu = F.max_pool2d(input_cpu,
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
            ...
            # not dense
            output_cpu = F.max_pool2d(input_cpu[:2, ...],
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
    ...
    def test_maxpool2d_index(self):
        ...
        output_demo_cpu = F.max_pool2d(demo_input.float(),
                                       kernel_size=2,
                                       stride=1,
                                       padding=0,
                                       dilation=1,
                                       return_indices=True,
                                       ceil_mode=False)
        output_demo = F.max_pool2d(self.to_mlu(demo_input),
                                   kernel_size=2,
                                   stride=1,
                                   padding=0,
                                   dilation=1,
        ...
        self.assertEqual(output_demo[1].long().cpu(), out_put_gt_mlu)
    ...
    def test_maxpool1d_batch(self):
        ...
        loop_var = [
            kernel_v, stride_v, padding_v, return_indices_v,
            ceil_mode_v, dtypes, in_shapes
        ]
        for kernel, stride, padding, return_indices, ceil_mode, t, in_shape in product(
                *loop_var):
    ...
    def test_maxpool2d_batch(self):
        ...
        loop_var = [
            memory_format_list, kernel_v, stride_v, padding_v, return_indices_v,
            ceil_mode_v, dtypes, in_shapes
        ]
        for memory_format, kernel, stride, padding, return_indices, ceil_mode, t, in_shape in product(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=t).to(memory_format=memory_format)
            input_cpu = input_t if (t != torch.half) else input_t.to(torch.float)
            output_cpu = F.max_pool2d(input_cpu,
    ...
    def test_avgpool3d_non_batch(self):
        ...
        for in_shape, kernel, stride, padding, ceil_mode, include_pad, t in zip(
            ...
            self.assertTensorsEqual(output_cpu,
                                    ...
                                    use_MSE=True)
            # test function
            output_cpu = F.avg_pool3d(input_cpu,
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      ceil_mode=ceil_mode,
                                      count_include_pad=include_pad)
            output_mlu = F.avg_pool3d(self.to_mlu(input_t),
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      ceil_mode=ceil_mode,
            ...
            output_cpu = F.avg_pool3d(input_cpu[:2, ...],
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      ceil_mode=ceil_mode,
                                      count_include_pad=include_pad)
            output_mlu = F.avg_pool3d(self.to_mlu(input_t)[:2, ...],
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      ceil_mode=ceil_mode,
    ...
    def test_avgpool3d_batch(self):
        ...
        for memory_format, in_shape, kernel, stride, padding, ceil_mode, include_pad, t in zip(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=t)
            input_cpu = input_t if (t != torch.half) else input_t.to(torch.float)
            # test nn module
    ...
    def test_maxpool3d_non_batch(self):
        ...
        loop_var = [
            shape_list, kernel_v, stride_v, padding_v,
            ceil_mode_v, return_indices_v, dtypes
        ]
        for in_shape, kernel, stride, padding, ceil_mode, return_indices, t in zip(
            ...
            # test function
            output_cpu = F.max_pool3d(input_cpu,
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
                                      return_indices=return_indices,
                                      ceil_mode=ceil_mode)
            output_mlu = F.max_pool3d(self.to_mlu(input_t),
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
    ...
    def test_maxpool3d_index(self):
        # check index value since cnnl return local index.
        demo_input = torch.Tensor([[[[[1, 3, 4, 2], [2, 5, 6, 7], [3, 8, 9, 6]],
                                     [[1, 3, 4, 2], [2, 5, 6, 7], [3, 8, 9, 6]]]]]).to(torch.half)
        out_put_gt = torch.Tensor([[[[[5, 6, 7], [9, 10, 10]]]]]).to(torch.long)
        out_put_gt_mlu = torch.Tensor([[[[[3, 3, 3],[3, 3, 2]]]]]).to(torch.long)
        output_demo_cpu = F.max_pool3d(demo_input.float(),
                                       kernel_size=2,
                                       stride=1,
                                       padding=0,
                                       dilation=1,
                                       return_indices=True,
                                       ceil_mode=False)
        output_demo = F.max_pool3d(self.to_mlu(demo_input),
                                   kernel_size=2,
                                   stride=1,
                                   padding=0,
                                   dilation=1,
    ...
    def test_maxpool3d_batch(self):
        ...
        for memory_format, in_shape, kernel, stride, padding, ceil_mode, return_indices, t in zip(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=t)
            input_cpu = input_t if (t != torch.half) else input_t.to(torch.float)
            # test nn module
    ...
    def test_maxpool2d_exception(self):
        ...
        m = nn.MaxPool2d(kernel_size=(8, 8),
                         stride=(2, 2),
                         padding=0,
                         return_indices=True,
                         dilation=1,
