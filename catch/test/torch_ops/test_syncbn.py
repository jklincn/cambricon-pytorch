...
class TestSyncBatchNormRelatedOp(TestCase):
    ...
    def test_batch_norm_stats(self):
        shape_list = [(4, 32, 257, 257), (4, 256, 1, 1), (3, 4), (10, 3, 12), (2, 6, 8, 11, 5),
                      (3, 1, 4, 5, 8, 2)]
        ...
        for shape in shape_list:
            for dtype in input_dtype_list:
                ...
                self.assertTensorsEqual(mean_ref.float(), mean.cpu().float(), 0.003, use_MSE=True)
                # test channel last input
                mean0, invstd0 = torch.batch_norm_stats(self.convert_to_channel_last(input), 1e-5)
    ...
    def test_batch_norm_gather_stats_with_counts(self):
        ...
        list_list = [shape_list, world_size_list]
        for shape, world_size in product(*list_list):
            ...
            count_all = torch.randn((world_size,)).fill_(input.numel() // input.size(1)).to("mlu")
            # baseline contiguous
            mean, invstd = torch.batch_norm_gather_stats_with_counts(input, mean_all, invstd_all,
                running_mean_, running_var_, 0.1, 0.0001, count_all)
            mean_base = mean.cpu()
            ...
            for f0, f1, f2, f3, f4 in product(*[func_list for i in range(5)]):
                ...
                mean, invstd = torch.batch_norm_gather_stats_with_counts(input, f0(mean_all),
                    f1(invstd_all), running_mean_, running_var_, 0.1, 0.0001, f4(count_all))
    ...
    def test_batch_norm_gather_stats_with_counts_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all.view(-1), invstd_all, running_mean, running_var, 0.1, 0.0001,
                count_all)
        ref_msg = r"running_mean.dim\(\), running_var.dim\(\) and counts.dim\(\) must equal 1"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all, invstd_all, running_mean.unsqueeze(0), running_var, 0.1, 0.0001,
                count_all)
        ref_msg = "data type of mean must equal data type of var"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all.half(), invstd_all, running_mean, running_var, 0.1, 0.0001,
                count_all)
        ref_msg = "data type of counts must equal data type of running_mean, but got Float and Int"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all, invstd_all, running_mean, running_var, 0.1, 0.0001,
                count_all.int())
        ref_msg = "mean and invstd currently only support float data type, but got Half"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all.half(), invstd_all.half(), running_mean, running_var, 0.1, 0.0001,
                count_all)
        ref_msg = "\"batch_norm_update_stats_mlu\" not implemented for \'Int\'"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            mean, invstd = torch.batch_norm_gather_stats_with_counts(  # pylint: disable=W0612
                input, mean_all, invstd_all, running_mean.int(), running_var.int(), 0.1, 0.0001,
                count_all.int())
    ...
    def test_batch_norm_elemt(self):
        ...
        for shape in shape_list:
            ...
            out_base = out.cpu()
            # test not contiguous
            for f0, f1, f2, f3, f4 in product(func_list1, func_list0, func_list0, func_list0,
                func_list0):
                ...
                # test out
                out_in = torch.randn(3,3,3,3).to("mlu")
                need_cmp_ptr = False
    ...
    def test_batch_norm_backward_reduce(self):
        ...
        for shape in shape_list:
            ...
            invstd = torch.randn(shape[1]).to("mlu")
            # baseline contiguous
            o0, o1, dweight, dbias = torch.batch_norm_backward_reduce(doutput, input, mean, invstd,
                weight, True, True, True)
            o0_, o1_, dweight_, dbias_ = o0.cpu(), o1.cpu(), dweight.cpu(), dbias.cpu()
            # test not contiguous (weight is not used in caculation, so we do not need wrap weight)
            for f0, f1, f2, f3 in product(func_list1, func_list1, func_list0, func_list0):
                o0, o1, dweight, dbias = torch.batch_norm_backward_reduce(f0(doutput), f1(input),
                    f2(mean), f3(invstd), weight, True, True, True)
    ...
    def test_batch_norm_backward_reduce_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            out = torch.batch_norm_backward_reduce(  # pylint: disable=W0612
                doutput, torch.randn((0, 4)).to("mlu"), mean, invstd, weight, True, True, True)
    ...
    def test_batch_norm_backward_elemt(self):
        ...
        for shape in shape_list:
            ...
            count = torch.full((4,), input.numel() // input.size(1), dtype=torch.int32).to("mlu")
            # baseline contiguous
            dinput = torch.batch_norm_backward_elemt(doutput, input, mean, invstd,
                weight, sum_dy, sum_dy_xmu, count)
            dinput_base = dinput.cpu()
            for f0, f1, f2, f3, f4, f5, f6, f7 in product(func_list1, func_list1, func_list0,
                func_list0, func_list0, func_list0, func_list0, func_list1):
                dinput = torch.batch_norm_backward_elemt(f0(doutput), f1(input), f2(mean),
                    f3(invstd), f4(weight), f5(sum_dy), f6(sum_dy_xmu), f7(count))
    ...
    def test_batch_norm_backward_elemt_exception(self):
        ...
        sum_dy = torch.randn(shape[1]).to("mlu")
        sum_dy_xmu = torch.randn(shape[1]).to("mlu")
        count = torch.full((2,), input.numel() // input.size(1), dtype=torch.int32).to("mlu")
        ref_msg = "data type of sum_dy and sum_dy_xmu must be float, but got Half and Float"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            out = torch.batch_norm_backward_elemt(  # pylint: disable=W0612
                doutput, torch.randn((0, 4)).to("mlu"), mean, invstd, weight,
