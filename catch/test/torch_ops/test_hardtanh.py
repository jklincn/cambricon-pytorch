    ...
    def test_hardtanh_permute(self):
        ...
        for in_shape in [(8, 224, 224), (1, 1, 1, 1), (1, 3, 16, 16, 4),
            ...
            for min_v, max_v in minmax_list:
                ...
                F.hardtanh(input_inplace_, min_v, max_v, inplace=True)# test inplace operation
                F.hardtanh(input_mlu_inplace_, min_v, max_v, inplace=True)
                self.assertTensorsEqual(output_cpu, output_mlu.cpu(), 0.003, use_MSE=True)
                self.assertTensorsEqual(input_inplace_, input_mlu_inplace_.cpu(), 0.003, use_MSE=True)
