...
class TestOps(TestCase):
    ...
    def test_l1l2norm(self):
        ...
        for shape, scalar_op, dim, keep in product(*loop_var2):
            x = torch.rand(shape, dtype=torch.float)
            out_cpu = x.norm(scalar_op, dim, keepdim = keep)
            out_mlu = self.to_mlu_dtype(x, torch.half).norm(scalar_op, dim, keepdim = keep)
            self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(), 0.003, use_MSE = True)
    ...
    def test_norm_out(self):
        ...
        keepdims = [False, True]
        for shape, dim, input_dtype, keepdim in product(input_shape_list, dim_list, input_dtypes, keepdims):
            ...
            self.assertTensorsEqual(out.float(), out_mlu.cpu().float(), 0.003, use_MSE=True)
    ...
    def test_norm_backward(self):
        ...
        for shape, dim, input_dtype, keepdim in product(input_shape_list, dim_list, input_dtypes, keepdims):
            ...
            self.assertTensorsEqual(x_grad_cpu.float(), x.grad.cpu().float(), 0.003, use_MSE=True)
    ...
    def test_norm_out_no_contiguous(self):
        ...
        self.assertTensorsEqual(y[:, :, 1, :].cpu(), y_expected.cpu(), 0.0, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_norm_exceptions(self):
        x = torch.randn(4, 4, dtype=torch.float)
        msg = "torch_mlu only support L1-Norm as p=1.0 and L2-Norm as p=2.0, but got p=3."
