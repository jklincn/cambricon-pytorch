...
class TestRemainderOp(TestCase):    # pylint: disable=R0904
    ...
    def test_remainder_tensor_tensor(self):
        dtype_list = [(torch.double, 3e-3), (torch.float, 3e-3), (torch.half, 3e-3),
                      (torch.int32, 0), (torch.short, 0)]
        for data_type, err in dtype_list:
            for shape1, shape2 in [((), ()),
                ...
                if data_type == torch.half:
                    ...
                    out_cpu = x % y
                out_mlu = x_mlu % y_mlu
                self.assertEqual(out_cpu.dtype if data_type != torch.half \
                        else torch.half, out_mlu.dtype)
    ...
    def test_remainder_backward(self):
        parameter_list = [(torch.double, 3e-3, False), (torch.float, 3e-3, True), \
        ...
        for parameter, shape in product(parameter_list, shape_list):
            ...
            x = (20 * torch.randn(shape1, dtype=torch.float)).to(data_type)
            y = (20 * torch.rand(shape2, dtype=torch.float)).to(data_type) + 1 \
                    if other_is_scalar is False else 2.0
            ...
            self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(),
                                    err, use_MSE=True)
            self.assertTensorsEqual(x_cpu.grad.float(), x_mlu.grad.cpu().float(),
                                    err, use_MSE=True)
    ...
    def test_remainder_bfloat16(self):
        # TODO(guwei): Add test after CNNL supports bf16
        pass
    ...
    def test_remainder_tensor_tensor_channels_last(self):
        ...
        for data_type, err in dtype_list:
            for shape1, shape2 in [((1, 3, 224, 224), (1, 3, 224, 1)),
                                   ((1, 3, 16, 1), (1, 1, 1, 16))]:
                data1 = torch.randn(shape1, dtype=torch.float).to(memory_format=torch.channels_last)
                data2 = torch.rand(shape2, dtype=torch.float).to(memory_format=torch.channels_last)
                x = (20 * data1).to(data_type)
                y = (20 * data2).to(data_type) + 1
                out_cpu = x % y
                out_mlu = self.to_mlu_dtype(x, data_type) % self.to_mlu_dtype(y, data_type)
                self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(),
                                        err, use_MSE=True)
    ...
    def test_remainder_tensor_tensor_not_dense(self):
        ...
        for data_type, err in dtype_list:
            for shape1, shape2 in [((1, 3, 224, 224), (1, 3, 224, 1)),
                ...
                y = (20 * data2).to(data_type) + 1
                out_cpu = x[:, :, :, :1] % y[:, :, :1, :]
                out_mlu = self.to_mlu_dtype(x, data_type)[:, :, :, :1] % \
    ...
    def test_remainder_tensor_scalar(self):
        dtype_list = [(torch.float, 3e-3), (torch.int32, 0)]
        for data_type, err in dtype_list:
            shape_list = [(), (6, 0), (5), (7, 9), (9, 8, 7), (1, 2, 3, 4),
                          (5, 6, 7, 8, 9), (9, 2, 12, 1, 2, 16)]
    ...
    def test_remainder_inplace_tensor_tensor(self):
        ...
        for data_type, err in dtype_list:
            for shape1, shape2 in [((), ()),
                ...
                self.assertTensorsEqual(x.float(), x_mlu.cpu().float(),
                                        err, use_MSE=True)
    ...
    def test_remainder_out_tensor_scalar(self):
        dtype_list = [(torch.float, 3e-3), (torch.int32, 0)]
        for data_type, err in dtype_list:
            shape_list = [(), (5), (6, 0), (7, 9), (9, 8, 7), (1, 2, 3, 4),
                          (5, 6, 7, 8, 9), (9, 2, 12, 1, 2, 16)]
            for shape in shape_list:
    ...
    def test_remainder_mixed_memory_format(self):
        x = 20 * torch.randn((3, 1, 5, 1), dtype=torch.float).to(memory_format=torch.channels_last)
        ...
        y = 20 * torch.randn((3, 1, 5, 1), dtype=torch.float).to(memory_format=torch.channels_last)
    ...
    def test_remainder_permute(self):
        ...
        for i in range (4):
            ...
            x, y, out = x.permute(permute_shape[i]), \
                        y.permute(permute_shape[i]), \
                        out.permute(permute_shape[i])
            x_mlu, y_mlu, out_mlu = x_mlu.permute(permute_shape[i]), \
    ...
    def test_remainder_exception(self):
        ...
        ref_msg = r"^output with shape \[1, 16\] doesn't match the broadcast shape \[16, 16\]"
