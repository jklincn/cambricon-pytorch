...
class TestBinCountOp(TestCase):
    ...
    def test_all_type_bincount(self):
        input_size = (5000,)
        weight_type_list = [torch.half, torch.float, torch.double,
                             torch.int, torch.short, torch.int8, torch.bool,
                             torch.long, torch.uint8]
        input_type_list = [torch.int, torch.short, torch.int8,
                            torch.long, torch.uint8]
        func_list = [lambda x:x, self.to_non_dense, self.convert_to_channel_last]
        for input_type, weight_type, func in product(input_type_list, weight_type_list, func_list):
            input = torch.randint(50, input_size, dtype=input_type, device='cpu')
            weight = torch.testing.make_tensor(input_size, dtype=weight_type, device='cpu')
            input_mlu = input.mlu()
