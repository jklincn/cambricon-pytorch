...
class TestPoolbpOp(TestCase):
    ...
    def test_avgpool1d_backward_non_batch(self):
        ...
        loop_var = [
            ...
            ceil_mode_v, include_pad_v, dtype_list
        ]
    ...
    def test_avgpool1d_backward_batch(self):
        shape_list = [(16, 7, 7), (16, 8, 16), (23, 13, 64),
                      (0, 128, 128)]
        kernel_v = [3, 4, 5]
        stride_v = [2, 3, 4, None]
        padding_v = [0, 1]
    ...
    def test_avgpool2d_backward_batch(self):
        ...
        for memory_format, in_shape, kernel, stride, padding, ceil_mode, include_pad, dtype in product(
                *loop_var):
            ...
            o_shape = output_cpu.size()
            grad = torch.randn(o_shape[0], o_shape[1], o_shape[2],
                               o_shape[3] + 1).to(memory_format=memory_format)
            output_cpu.backward(grad[..., :-1])
            output_mlu.backward(self.to_mlu_dtype(grad, dtype)[..., :-1])
            self.assertTensorsEqual(input_t.grad.float(),
                                    input_mlu_t.grad.float(),
    ...
    def test_maxpool1d_backward_non_batch(self):
        ...
        loop_var = [
            kernel_v, stride_v, padding_v, ceil_mode_v,
            return_indices_v, in_shapes, dtype_list
        ]
        for kernel, stride, padding, ceil_mode, return_indices, in_shape, dtype in product(
                *loop_var):
            input_t = torch.randn(
                in_shape, dtype=dtype, requires_grad=True)
            input_cpu = input_t if dtype != torch.half else input_t.to(torch.float)
            input_mlu = copy.deepcopy(input_t)
            ...
            output_mlu = F.max_pool1d(self.to_device(input_mlu),
                                      kernel,
                                      stride=stride,
                                      padding=padding,
                                      dilation=1,
    ...
    def test_maxpool2d_backward_batch(self):
        ...
        loop_var = [
            memory_format_list, kernel_v, stride_v, padding_v, ceil_mode_v,
            return_indices_v, in_shapes, dtype_list
        ]
        for memory_format, kernel, stride, padding, ceil_mode, return_indices, in_shape, dtype in product(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=dtype,
                      requires_grad=True)
            input_mlu_t = copy.deepcopy(input_t)
            ...
            if return_indices == True:
                output_cpu = output_cpu[0]
            grad = torch.ones(output_cpu.shape, dtype=torch.float).to(memory_format=memory_format)
            output_cpu.backward(grad, retain_graph=True)
            grad_cpu = input_t.grad
    ...
    def test_avgpool3d_backward_batch(self):
        ...
        for memory_format, in_shape, kernel, stride, padding, ceil_mode, include_pad, dtype in zip(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=dtype,
                requires_grad=True)
            input_mlu_t = copy.deepcopy(input_t)
            ...
            input_mlu_t.grad.zero_()
            o_shape = output_cpu.size()
            grad = torch.randn(o_shape[0], o_shape[1], o_shape[2], o_shape[3],
                        o_shape[4] + 1).to(memory_format=memory_format)
            output_cpu.backward(grad[..., :-1])
            output_mlu.backward(self.to_mlu_dtype(grad, dtype)[..., :-1])
            self.assertTensorsEqual(input_t.grad.float(),
                                    input_mlu_t.grad.float(),
    ...
    def test_maxpool3d_backward_non_batch(self):
        ...
        loop_var = [
            shape_list, kernel_v, stride_v, padding_v,
            ceil_mode_v, return_indices_v, dtype_list
        ]
    ...
    def test_maxpool3d_backward_batch(self):
        ...
        loop_var = [
            shape_list, memory_format_list, kernel_v, stride_v, padding_v,
            ceil_mode_v, return_indices_v, dtype_list
        ]
        for in_shape, memory_format, kernel, stride, padding, ceil_mode, return_indices, dtype in zip(
                *loop_var):
            input_t = torch.randn(in_shape, dtype=dtype,
                requires_grad=True)
            input_mlu_t = copy.deepcopy(input_t)
