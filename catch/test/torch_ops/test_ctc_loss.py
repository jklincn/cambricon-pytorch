...
class TestCTCLossOp(TestCase):
    ...
    def test_warp_ctc_loss(self):
        ...
        probs.grad.zero_()
        out_mlu = ctc_loss(probs.log_softmax(2).to('mlu'), target.to('mlu'), input_lengths, target_lengths)
        out_mlu.backward()
        ...
        out_mlu = torch.ops.torch_mlu.warp_ctc_loss(
            probs.to('mlu'), target.to('mlu'), input_lengths.to('mlu'),
            target_lengths.to('mlu'), 0, 1, True, 0)
        out_mlu.backward()
        grad_mlu = copy.deepcopy(probs.grad)
        probs.grad.zero_()
        self.assertTensorsEqual(out_cpu.unsqueeze(0).float(), out_mlu.cpu().float(), 0.003, use_MSE=True)
        self.assertTensorsEqual(grad_cpu, grad_mlu, 0.003, use_MSE=True)

    def compare_ctc_loss(self, log_probs, target, input_lengths, target_lengths, reduction="mean", blank=0, zero_infinity=False, use_MSE=True, diff_f=0.003, diff_b=0.003):
        ctc_loss = nn.CTCLoss(
        ...
        out_mlu = ctc_loss(log_probs.to('mlu'), target.to('mlu'), input_lengths, target_lengths)
        ...
        if use_MSE:
            ...
            self.assertTensorsEqual(
                grad_cpu, grad_mlu, diff_b, use_MSE=True)
        else:
            self.assertEqual(
    ...
    def test_ctc_loss(self):
        ...
        target_lengths_list = [
            ...
            torch.randint(low=S_min, high=S, size=(N,), dtype=torch.long).numpy().tolist(),
        ]
        reduction_list = ['none', 'mean', 'sum']
        zero_infinity_list = [True, False]
        for reduction, zero_infinity in itertools.product(reduction_list, zero_infinity_list):
            for input_lengths, target_lengths in zip(input_lengths_list, target_lengths_list):
                # target shape = (N, S)
                target = torch.randint(
                    low=1, high=C, size=(N, S), dtype=torch.long)
                ...
                target = torch.randint(
                    low=0, high=blank, size=(N, S), dtype=torch.long)
                self.compare_ctc_loss(
                    log_probs, target, input_lengths, target_lengths,
                    reduction=reduction, blank=blank, zero_infinity=zero_infinity)
                ...
                blank = random.randint(int((C - 1) / 2), C - 1)
                # target should not include blank
                target = torch.randint(low=0, high=blank, size=(
                    sum(target_lengths),), dtype=torch.long)
                self.compare_ctc_loss(log_probs, target, input_lengths, target_lengths,
                                      reduction=reduction, blank=blank, zero_infinity=zero_infinity)
    ...
    def test_ctc_loss_zero_infinity(self):
        ...
        for reduction, zero_infinity in itertools.product(reduction_list, zero_infinity_list):
            for input_lengths, target_lengths in zip(input_lengths_list, target_lengths_list):
                self.compare_ctc_loss(log_probs, target, input_lengths, target_lengths,
                                      reduction=reduction, zero_infinity=zero_infinity, use_MSE=False)
    ...
    def test_ctc_loss_exceptions(self):
        ...
        ref_msg = r"Expected tensor to have size at least 30 at dimension 1, but got size 29"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.nn.functional.ctc_loss(log_probs, targets, input_lengths, target_lengths)
    ...
    def test_ctc_loss_custom_case(self):
        ...
        for reduction, zero_infinity in itertools.product(reduction_list, zero_infinity_list):
            for input_lengths, target_lengths in zip(input_lengths_list, target_lengths_list):
                ...
                self.compare_ctc_loss(
                    log_probs, target, input_lengths, target_lengths,
                    reduction=reduction, zero_infinity=zero_infinity, diff_b=0.05)
    ...
    @testinfo()
    @unittest.skipUnless(TEST_LARGETENSOR, "run largeTensorCases by `TEST_LARGETENSOR=TRUE` or `--large`")
    @largeTensorTest('56GB', device='mlu')
    def test_ctc_loss_large_exceptions(self):
        torch.manual_seed(0)
        device = 'mlu'
        T = 2
        N = 1
        C = 4096*48*13725   # Number of classes (including blank)
        S = 8000      # Target sequence length of longest target in batch (padding length)
        S_min = 10  # Minimum target length, for demonstration purposes
        ref_msg = r"datacast fail! expected smaller than 2147483647 and greater than -2147483648but got 2437421847"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            log_probs_large = torch.randn(T, N, C).log_softmax(2).to(device)
            input_lengths = torch.full(size=(N,), fill_value=T, dtype=torch.long)
            target_lengths = torch.randint(low=S_min, high=S, size=(N,), dtype=torch.long)
            target = torch.randint(
                low=1, high=C, size=(sum(target_lengths),), dtype=torch.long).to(device)
            torch.nn.functional.ctc_loss(log_probs_large, target, input_lengths, target_lengths)
