...
class TestLstmOp(TestCase):
    ...
    def test_packed_lstm_training(self):
        ...
        amp_v = [False, True]
        mse_limited = 3e-3
        for num_layers, input_size, hidden_size, batch, seq_len, \
                bidirectional, bias, is_amp, func, is_grad_conti in product(
                num_layers_v, input_size_v, hidden_size_v, batch_v, seq_len_v,
                bidirectional_v, bias_v, amp_v, func_list, grad_contiguous_v):

            cpu_rnn = PackedLSTM(input_size, hidden_size, num_layers, bias, bidirectional)
            ...
            input_length_list = [np.random.randint(seq_len / 2, seq_len) for i in range(batch)]
            ...
            output_cpu, hn_cpu, cn_cpu = cpu_rnn(func(input), input_length, h0, c0, self.assertTrue)
            with torch.autocast("mlu", enabled=is_amp):
                output_mlu, hn_mlu, cn_mlu = mlu_rnn(func(input_mlu), input_length, \
                                                     h0_mlu, c0_mlu, self.assertTrue)
            ...
            for w, w_mlu in zip(cpu_rnn.parameters(), mlu_rnn.parameters()):
                self.assertTensorsEqual(w.grad.float(),
                                        w_mlu.grad.cpu().float(),
                                        mse_limited,
                                        use_MSE=True)
