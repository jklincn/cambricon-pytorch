...
class TestCumminOp(TestCase):
    ...
    def test_cummin(self):
        ...
        for memory_format, shape, dim in product(*loop_var):  # pylint: disable=C0200
            ...
            self.assertTensorsEqual(out_cpu[0],
                                    out_mlu[0].cpu(),
                                    0,
                                    use_MSE=True)  # float type precision : 0
            self.assertTensorsEqual(out_cpu[1],
                                    out_mlu[1].cpu(),
                                    0,
                                    use_MSE=True)  # float type precision : 0
            self.assertTensorsEqual(y_value_cpu,
                                    y_value_mlu.cpu(),
                                    0,
                                    use_MSE=True)  # float type precision : 0
            self.assertTensorsEqual(y_indice_cpu,
                                    y_indice_mlu.cpu(),
                                    0,
                                    use_MSE=True)  # float type precision : 0
    ...
    def test_cummin_not_dense(self):
        shape_list = [(12, 10, 2, 2, 4), (10, 3, 32, 32), (2, 3, 4),
                      (20, 15, 12, 1, 1, 3)]
    ...
    def test_cummin_support_dtype(self):
        support_dtype = [
            torch.uint8, torch.int8, torch.int16, torch.int32, torch.long,
            torch.float16, torch.bool, torch.float, torch.double
        ]
        for dtype in support_dtype:
            input_c = torch.randn(size=(13, 24), dtype=torch.float32).to(dtype)
            out_values_c = torch.zeros(size=(13, 24),
                                       dtype=torch.float32).to(dtype)
            out_indices_c = torch.zeros(size=(13, 24),
                                        dtype=torch.float32).to(dtype)
            ...
            if str(dtype) != "torch.float16":
                self.assertTensorsEqual(
                    out_values_c, out_values_m.cpu(), 0,
                    use_MSE=True)  # float type precision : 0
                self.assertTensorsEqual(
                    out_indices_c, out_indices_m.cpu(), 0,
                    use_MSE=True)  # float type precision : 0
    ...
    def test_cummin_zero(self):
        ...
        self.assertTensorsEqual(out_cpu[0], out_mlu[0].cpu(), 0,
                                use_MSE=True)  # float type precision : 0
        self.assertTensorsEqual(out_cpu[1], out_mlu[1].cpu(), 0,
                                use_MSE=True)  # float type precision : 0
