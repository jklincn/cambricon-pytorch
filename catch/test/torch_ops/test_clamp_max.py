...
class TestClampMaxOp(TestCase):
    ...
    def test_clamp_max(self):
        ...
        product_list = product(shape_list,
                               dtype_list,
                               max_list)
        for shape, dtype, max_, in product_list:
            if dtype == torch.half:
            ...
            out_cpu = torch.clamp_max(x, max_,)
            out_mlu = torch.clamp_max(self.to_mlu_dtype(x, dtype), max_,)
    ...
    def test_clamp_max_tensor(self):
        ...
        for (shape1, shape2) in shape_list:
            ...
            out_cpu = torch.clamp_max(x, max_,)
            out_mlu = torch.clamp_max(x.to('mlu'), max_.to('mlu'),)
            self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0)
