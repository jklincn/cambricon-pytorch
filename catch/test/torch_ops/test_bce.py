        ...
        for i in range (4):
            for reduct in reduct_list:
                for weight_flag in weight_flag_list:
                    ...
                    output =F.binary_cross_entropy(x, target, reduction=reduct, weight=weight_cpu)
                    if (reduct == "none"):
                    ...
                    self.assertTensorsEqual(
                            grad_input.float(), grad_input_mlu.cpu().float(), 0.003, use_MSE=True)

    # @unittest.skip("not test")
    @testinfo()
    def test_bceloss_dtype(self):
        reduct_list = ["none", "mean", "sum"]
        dtype_list = [torch.double, torch.float]
        weight_flag_list = [True, False]
        for dtype , reduct, weight_flag in product(dtype_list, reduct_list, weight_flag_list):
            x = torch.rand((2,3,4,5,8)).to(torch.half).to(torch.float)
            target = torch.rand((2,3,4,5,8)).to(torch.half).to(torch.float)
            weight = torch.rand((2,3,4,5,8)).to(torch.half).to(torch.float)
            x_mlu, target_mlu = self.to_mlu_dtype(x, dtype), self.to_mlu_dtype(target, dtype)
            x.requires_grad=True
            x_mlu.requires_grad=True
            if weight_flag:
                weight_cpu = weight
            ...
            self.assertTensorsEqual(grad_input, grad_input_mlu.float().cpu(),
                0.003, use_MSE=True, message = str(dtype) +" "+ str(reduct))
