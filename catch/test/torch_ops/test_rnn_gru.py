            ...
            for x_layer, y_layer in zip(rnn1.all_weights, rnn2.all_weights):
                for x, y in zip(x_layer, y_layer):
                    if y.dtype == torch.half:
                        self.assertTensorsEqual(x.grad, y.grad.cpu().float(), 0.03, use_MSE=True)
                    else:
            ...
            for module in (nn.RNN, nn.GRU):
                for bias, inp_f, hx_f, grado_f, gradh_f in product(
                  biass, funcs, funcs, funcs, funcs):
                    ...
                    output1, hy1 = rnn(inp_f(inp.float() if dtype == torch.half else inp),
                    hx_f(hx.float() if dtype == torch.half else hx))
                    output2, hy2 = rnn_device(inp_f(inp_cu), hx_f(hx_device))
                    torch.autograd.backward([output1, hy1], [grado_f(grad_output), gradh_f(grad_hy)])
                    torch.autograd.backward([output2, hy2], [grado_f(grad_output.to(device)),
                      gradh_f(grad_hy.to(device))])
                    if dtype == torch.half:
                        self.assertTensorsEqual(output1, output2.cpu().float(), 0.03, use_MSE=True)
                        self.assertTensorsEqual(hy1, hy2.cpu().float(), 0.03, use_MSE=True)
                    else:
                    ...
                    if dtype == torch.half:
                        self.assertTensorsEqual(inp.grad, inp_cu.grad.cpu().float(), 0.03, use_MSE=True)
                        self.assertTensorsEqual(hx.grad, hx_device.grad.cpu().float(), 0.03, use_MSE=True)
                    else:
