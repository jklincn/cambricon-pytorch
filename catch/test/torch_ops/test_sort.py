...
class TestOps(TestCase):
    ...
    @testinfo()
    def test_sort(self):
        shape_list = [(64,), (76, 102), (32, 43, 54),
                      (2, 3, 4, 5), (32, 1, 51, 43),
                      (2, 1, 4, 5, 6), (32, 16, 51, 43, 52)]
        type_list = [torch.long, torch.int, torch.short, torch.float,
                     torch.half, torch.double, torch.int8, torch.uint8]
        order = [True, False]
    ...
    def test_sort_stable(self):
        ...
        for i in shape_list:
            for typeId in type_list:
                ...
                dim = random.randint(-len(i), len(i) - 1)
                out_cpu = torch.sort(x, stable=is_stable, dim=dim, descending=is_descending)
                if channel is False:
                    x = self.convert_to_channel_last(x)
                out_mlu = torch.sort(x.to("mlu"), stable=is_stable,
                                       dim=dim, descending=is_descending)
                self.assertTensorsEqual(
                    out_cpu[0].float(), out_mlu[0].cpu().float().contiguous(), \
                    0.0, use_MSE=True)
                self.assertTrue(out_cpu[1].dtype, out_mlu[1].cpu().dtype)
