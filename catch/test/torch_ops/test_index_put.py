...
class TestIndexPut(TestCase):
    ...
    def test_index_put_empty_indices(self):
        ...
        x_mlu = x.mlu()
        x_mlu_ptr = x_mlu.data_ptr()
        indices = (torch.tensor([], dtype=torch.int64, device = 'cpu'), torch.tensor([], dtype=torch.int64, device = 'cpu'))
        indices_mlu = (torch.tensor([], dtype=torch.int64, device = 'mlu'), torch.tensor([], dtype=torch.int64, device = 'mlu'))
        x[indices] = False
        ...
        reference = torch.arange(0., 160, dtype=torch.long, device='cpu').view(4, 8, 5)
    ...
    def test_index_put_empty_input(self):
        ...
        ref_msg_3 = "Index put requires the source and destination dtypes match, " \
    ...
    def test_index_put_high_dim_backward(self):
        input = torch.nn.parameter.Parameter(torch.rand(16, 3, 80, 20, 20), requires_grad=True)
        input_mlu = input.mlu()
