...
from common_utils import (
    testinfo, run_tests, TestCase, TEST_LARGETENSOR, largeTensorTest, TEST_BFLOAT16)
...
class TestAnyOp(TestCase):
    ...
    def test_any_dim(self):
        shape_list = [(10,), (3, 5), (4, 5, 8), (8, 10, 12, 14), (8, 0, 12, 14),
                      (0,), (0, 5), (4, 5, 0), (8, 0, 12, 14)]
        dim_list = [0, 1, -1, 3, 3,
                    0, 1, -1, 3]
        dtype_list = [torch.bool, torch.int8, torch.uint8, torch.int16, torch.int32, torch.int64, \
                      torch.float, torch.half, torch.double]
    ...
    def test_any_out(self):
        ...
        dtype_list = [(torch.bool, torch.uint8), (torch.int8, torch.bool),
                      (torch.uint8, torch.uint8), (torch.int32, torch.bool),
                      (torch.int64, torch.uint8), (torch.float, torch.bool),
                      (torch.half, torch.uint8), (torch.double, torch.bool),
        ...
        for dim_, list_ in zip(dim_list, shape_list):
            for dtype in dtype_list:
                for keep in keep_type:
                    ...
                    self.assertTrue(out_mlu.dtype == out_cpu.dtype, "any out dtype is not right")
    ...
    def test_any_out_not_contiguous(self):
        ...
        dim_list_nc = [-2, -1, 3, 1]
        dtype_list_nc = [(torch.bool, torch.bool), (torch.int8, torch.uint8),
                         (torch.uint8, torch.bool), (torch.int32, torch.uint8),
                         (torch.int64, torch.bool), (torch.float, torch.uint8),
                         (torch.half, torch.bool), (torch.double, torch.uint8),
        ...
        for dim_, list_ in zip(dim_list_nc, shape_list_nc):
            for dtype in dtype_list_nc:
                for keep in keep_type_nc:
                    x_cpu = (torch.rand(list_, dtype=torch.float) > 0.5).to(dtype[0])
                    out_cpu = (torch.rand(list_, dtype=torch.float) > 0.5).to(dtype[1])[:,1:]
                    x_mlu = x_cpu.to('mlu')
                    out_mlu = (torch.rand(list_, dtype=torch.float) > 0.5).mlu().to(dtype[1])[:,1:]
    ...
    def test_all_bfloat16(self):
        left_cpu = torch.testing.make_tensor((2, 10, 24), dtype=torch.bfloat16, device='cpu')
        ...
        self.assertTensorsEqual(out_cpu.float(),
                                ...
                                0.0,
                                use_MSE=True)
        output_cpu = torch.testing.make_tensor(out_cpu.shape, dtype=torch.bool, device='cpu')
        ...
        self.assertTensorsEqual(output_mlu.cpu().float(),
                                output_cpu.float(),
                                0.0,
                                use_MSE=True)
        self.assertTensorsEqual(output_mlu.cpu().float(),
