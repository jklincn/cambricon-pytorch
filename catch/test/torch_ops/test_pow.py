...
class TestOps(TestCase):
    ...
    def test_pow(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in data_types:
                ...
                input1 = input1.to(data_type) if isinstance(shape_list[i], tuple) else input1
    ...
    def test_pow_out(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in data_types:
                ...
                torch.pow(self.to_mlu_dtype(input1, data_type), \
                          self.to_mlu_dtype(exp1, data_type), out = out_mlu)
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), err, use_MSE=True)
    ...
    def test_pow_permute(self):
        ...
        for i in range (4):
            ...
            out_mlu = copy.deepcopy(out).to(ct.mlu_device())
    ...
    def test_pow_bfloat16(self):
        left = torch.testing.make_tensor((3, 4, 6), dtype=torch.bfloat16, \
                                         device='cpu')
        left_cpu = torch.nn.Parameter(left)
        left_mlu = torch.nn.Parameter(left.mlu())
        out_cpu = torch.pow(left_cpu, 3)
        out_mlu = torch.pow(left_mlu, 3)
        grad = torch.randn_like(out_cpu)
        out_cpu.backward(grad)
        out_mlu.backward(grad.to('mlu'))
        self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(), 3e-3, use_MSE=True)
        self.assertTensorsEqual(left_cpu.grad.float(), left_mlu.grad.cpu().float(), 3e-3, use_MSE=True)
        left_cpu.grad.zero_()
        left_mlu.grad.zero_()
        exponent = torch.testing.make_tensor((3, 4, 6), dtype=torch.short, \
                                         device='cpu')
        out_cpu = torch.pow(left_cpu, exponent)
        out_mlu = torch.pow(left_mlu, exponent.mlu())
        # TODO(CNNLCORE-14205): backward is not support bfloat16
        # grad = torch.randn_like(out_cpu)
        # out_cpu.backward(grad)
        # out_mlu.backward(grad.to('mlu'))
        self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(), 3e-3, use_MSE=True)
