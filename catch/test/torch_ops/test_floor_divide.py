...
class TestFloorDivideOp(TestCase):
    ...
    def test_floor_divide_tensor_scalar(self):
        ...
        for t in [torch.float, torch.double, torch.float, torch.int]:
    ...
    def test_floor_divide_mixed_memory_format(self):
        x = torch.randn((3, 1, 5, 1), dtype=torch.float).to(memory_format=torch.channels_last)
    ...
    @testinfo()
    def test_floor_divide_bfloat16(self):
