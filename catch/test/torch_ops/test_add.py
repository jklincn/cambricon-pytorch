...
class TestAddOp(TestCase):      # pylint: disable=R0904
    ...
    def test_add(self):
        dtype_list = [(torch.float, 3e-3), (torch.half, 3e-3),
                      ...
                      (torch.bool, 0)]
        shape_list = [((1, 3, 224, 224), (1, 3, 224, 1)),
                      ((2, 30, 80), (2, 30, 80)),
                      ((3, 20), (3, 20)),
                      ((10), (10)),
                      ((2, 1, 2, 4), (1, 2, 4)),
    ...
    def test_add_inplace(self):
        ...
        for shape, dtype_err, func in product(shape_list, dtype_list, func_list):
            ...
            self.assertTensorsEqual(x_left_cpu.float(), x_left_mlu.cpu().float(), \
                                    dtype_err[1], use_MSE=True)
    ...
    def test_add_inplace_floatscalar(self):
        ...
        for input_t in type_list:
            if input_t is torch.half:
                ...
                input_self_cpu = torch.normal(mean=20,
                                              std=torch.randn(20, dtype=torch.float).abs()).to(input_t)
            input_self_mlu = self.to_mlu_dtype(copy.deepcopy(input_self_cpu), input_t)
            input_ptr = input_self_mlu.data_ptr()
            input_self_cpu += 2.3
            input_self_mlu += 2.3
    ...
    def test_add_high_diemension_after_permute(self):
        for i in range(5):
            ...
            a = torch.randn(shape)
            b = torch.randn(shape)
            ouput_ture = torch.add(torch.permute(a, tuple(permute_size)),
            ...
            self.assertTensorsEqual(ouput_ture,ouput_ture_mlu.cpu(), 3e-3, use_MSE=True)
