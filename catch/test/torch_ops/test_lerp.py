...
class TestLerpOp(TestCase):
    ...
    def test_lerp_tensor(self):
        ...
        for dtype_err, shape, func in product(dtype_list, shape_list, func_list):
            ...
            a_mlu, b_mlu, w_mlu = func(a.to('mlu')), func(b.to('mlu')), func(w.to('mlu'))
    ...
    def test_lerp_out_tensor(self):
        ...
        for dtype_err, shape in product(dtype_list, shape_list):
            ...
            a_mlu, b_mlu, w_mlu, output_mlu = a.to('mlu'), b.to('mlu'), w.to('mlu'), output.to('mlu')
            output = output.float()
    ...
    def test_lerp_scalar_permute(self):
        ...
        for i in range (4):
            ...
            a_mlu, b_mlu, output_mlu = a.to('mlu'), b.to('mlu'), output.to('mlu')
            a, b, output = a.permute(permute), b.permute(permute), output.permute(permute)
            a_mlu, b_mlu, output_mlu = a_mlu.permute(permute), b_mlu.permute(permute), output_mlu.permute(permute)
    ...
    def test_lerp_scalar_dtype(self):
        ...
        for dtype in dtype_list:
            ...
            self.assertTensorsEqual(output, output_mlu.cpu().float(), 3e-3, use_MSE=True)
