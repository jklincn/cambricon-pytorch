...
def training_batchnormNd_impl(self, layer, shape):
    for param in product(*loop_val_train):
        ...
        out_cpu = model_cpu(x_cpu.half().float() if data_type == torch.half else x_cpu)
...
class TestBatchNormOp(TestCase):
    ...
    def test_training_torch_batchnorm(self):
        ...
        for func_i, func_g in list_list:
            ...
            output_cpu = torch.batch_norm(func_i(input_cpu), m_cpu.weight,
              m_cpu.bias, m_cpu.running_mean, m_cpu.running_var,
              True, 0.1, 1e-5, True)
            output_mlu = torch.batch_norm(func_i(input_mlu.mlu()), m_mlu.weight,
              m_mlu.bias, m_mlu.running_mean, m_mlu.running_var,
              True, 0.1, 1e-5, True)
            ...
            self.assertTensorsEqual(input_cpu.grad, input_mlu.grad.cpu(), 0.003, use_MSE=True)
            self.assertTensorsEqual(m_cpu.weight.grad, m_mlu.weight.grad.cpu(), 0.003, use_MSE=True)
            self.assertTensorsEqual(m_cpu.bias.grad, m_mlu.bias.grad.cpu(), 0.003, use_MSE=True)
    ...
    def test_batchnorm_exception(self):
        ...
        ref_msg = r"Expected tensor for argument #2 'weight' " + \
