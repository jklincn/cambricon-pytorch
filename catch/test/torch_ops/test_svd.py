...
class TestSvdOp(TestCase):
    ...
    def _test_svd_helper(self, shape, some, device, dtype):
        ...
        for x, y in zip(cpu_result, device_result):
            ...
            self.assertTensorsEqual(x[..., :m].abs(), y[..., :m].abs().cpu(),
                                        0.003, use_MSE=True)

    def _test_svd_helper_not_dense(self, shape, some, device, dtype):
        cpu_tensor = torch.empty(0)
        if len(shape) == 2:
            cpu_tensor = torch.randn(shape, device='cpu').to(dtype)[:, :int(shape[-1] / 2)]
        elif len(shape) == 3:
            cpu_tensor = torch.randn(shape, device='cpu').to(dtype)[:, :, :int(shape[-1] / 2)]
        elif len(shape) == 4:
            cpu_tensor = torch.randn(shape, device='cpu').to(dtype)[:, :, :, :int(shape[-1] / 2)]
    ...
    def test_svd_out(self):
        ...
        for shape in shape_list:
            for channel in channel_first:
                ...
                m = min(x.shape[-2:])
                self.assertEqual(resu[..., :m].abs(), outu[..., :m].abs(), atol=3e-3, rtol=0)
                self.assertEqual(ress[..., :m].abs(), outs[..., :m].abs(), atol=3e-3, rtol=0)
                self.assertEqual(resv[..., :m].abs(), outv[..., :m].abs(), atol=3e-3, rtol=0)
    ...
    def test_svd_backward(self):
        ...
        for shape in shape_list:
            ...
            self.assertTensorsEqual(U[..., :m], U_mlu.cpu()[..., :m], 0.003, use_MSE=True)
            self.assertTensorsEqual(S[..., :m], S_mlu.cpu()[..., :m], 0.003, use_MSE=True)
            self.assertTensorsEqual(V[..., :m], V_mlu.cpu()[..., :m], 0.003, use_MSE=True)
            ...
            self.assertTensorsEqual(input_U_grad[..., :m], input_U_grad_mlu.cpu()[..., :m],
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(input_S_grad[..., :m], input_S_grad_mlu.cpu()[..., :m],
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(input_V_grad[..., :m], input_V_grad_mlu.cpu()[..., :m],
                                    0.003, use_MSE=True)
