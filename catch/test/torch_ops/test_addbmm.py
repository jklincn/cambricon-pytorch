...
class TestAddBmmOp(TestCase):
    def _test_addbmm(self, func, b1, b2, ref, out_tensor):
        ...
        self.assertEqual(out_tensor.cpu(), getattr(torch, func)(1, out_tensor, 0, b1, b2).cpu())

        res = getattr(torch, func)(out_tensor, b1, b2, beta=1, alpha=.5)
        ...
        res = getattr(torch, func)(out_tensor, b1, b2, beta=.1, alpha=.5)
        self.assertEqual(res.cpu(), out_tensor.cpu() * .1 + .5 * ref)
        ...
        def generate_tensor(dtype, device):
            ...
            for perm1, perm2 in itertools.product(itertools.permutations((0, 1, 2)), repeat=2):
                for perm3 in itertools.permutations((0, 1)):
                    b1 = make_tensor((num_batches, M, N), device=device, dtype=dtype, low=-1, high=1)
                    b2 = make_tensor((num_batches, N, O), device=device, dtype=dtype, low=-1, high=1)
                    b1 = b1.permute(perm1).contiguous().permute(invert_perm(perm1))
                    b2 = b2.permute(perm2).contiguous().permute(invert_perm(perm2))
                    ref = torch.from_numpy(b1.cpu().to(dtype).numpy() @ b2.cpu().to(
                      dtype).numpy()).to(dtype).sum(0)
                    out_tensor = torch.zeros_like(ref, device=device).permute(
                      perm3).contiguous().permute(perm3)
            ...
            for s1, s2, s3, s4, s5, s6 in itertools.product((True, False), repeat=6):
                ...
                b1 = make_tensor(shape1, device=device, dtype=dtype, low=-1, high=1).expand(num_batches, M, N)
                b2 = make_tensor(shape2, device=device, dtype=dtype, low=-1, high=1).expand(num_batches, N, O)
                ref = torch.from_numpy(b1.cpu().to(dtype).numpy() @ b2.cpu().to(dtype).numpy()).to(
                    dtype).sum(0)
