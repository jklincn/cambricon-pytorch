...
class TestGerOp(TestCase):
    def run_ger_test(self, v0, v1):
        for dtype in [torch.float32, torch.float16]:
            out_mlu = torch.ger(v0.to(dtype=dtype).to("mlu"), v1.to(dtype=dtype).to("mlu"))
    ...
    def run_ger_out_test(self, v0, v1):
        for dtype in [torch.float32, torch.float16]:
            ...
            zero_cpu = torch.zeros(v0.size(0), v1.size(0), dtype=torch.float, device='cpu')
            out_mlu = torch.ger(v0.to(dtype=dtype).to("mlu"),
                                v1.to(dtype=dtype).to("mlu"), out=zero_mlu)
            out_cpu = torch.ger(v0.float(), v1.float(), out=zero_cpu)
            self.assertTensorsEqual(out_mlu.cpu().float(), out_cpu, 0.003, use_MSE=True)
            self.assertTensorsEqual(zero_mlu.cpu().float(), zero_cpu, 0.003, use_MSE=True)
    ...
    def run_ger_out_not_contiguous_test(self, v0, v1):
        ...
        for v0_func, v1_func, out_func in product(*list_list):
            ...
            out_mlu = torch.ger(v0_func(v0.to("mlu")), v1_func(v1.to("mlu")), out=zero_mlu)
