...
from common_utils import testinfo, TestCase, TEST_BFLOAT16  # pylint: disable=C0411, C0413
...
class TestLtOp(TestCase):
    ...
    def test_lt_exception(self):
        device = 'mlu'
        ref_msg = r"datacast fail! expected smaller than 2147483647 and greater than" + \
                  r" -2147483648but got 1099511627776"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            (torch.tensor(1 << 40, dtype=torch.int64, device=device) < # pylint: disable=W0106
            torch.tensor([1 << 30], dtype=torch.int32, device=device))
