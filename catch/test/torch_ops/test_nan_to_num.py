...
class TestNantonumOp(TestCase):
    ...
    def test_nan_to_num_contiguous(self):
        ...
        for shape, dtype in zip(shape_list, dtype_list):
            ...
            if dtype.is_floating_point:
                ...
                for idx, extremal in zip(torch.randint(0, shape[0], (3, )),
                                         extremals):
                    x[idx] = extremal
            x_mlu = x.to('mlu')
    ...
    def test_nan_to_num_out_not_dense(self):
        ...
        for dtype in dtype_list:
            ...
            if dtype.is_floating_point:
                extremals = [float('nan'), float('inf'), -float('inf')]
                for idx, extremal in zip(torch.randint(0, 3, (3, )),
                                         extremals):
                    x[idx] = extremal
            x_mlu = self.to_mlu(x)
    ...
    def test_nan_to_num_permute(self):
        ...
        dtype_list = [torch.float, torch.half, torch.bool]
        for shape, p_shape, dtype in zip(shape_list, permute_shape,
                                         dtype_list):
            x = make_tensor(shape,
                            low=0.,
                            high=100.,
    ...
    def test_nan_to_num_exception(self):
        x_mlu = torch.tensor([float('nan'),
                              float('inf'), -float('inf'), 3.14]).to('mlu')
        y_out = make_tensor((1, 4),
                            low=0.,
                            high=100.,
                            dtype=torch.half,
                            device='mlu')
