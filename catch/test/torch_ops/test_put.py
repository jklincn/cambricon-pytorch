...
class TestOps(TestCase):
    ...
    def test_put__not_dense(self):
        ...
        for items in product(shapes, input_dtypes, indices_shapes, accumulate_list):
            ...
            input_mlu = input.mlu()
            idx = torch.linspace(-input[...,2].numel(), input[...,2].numel()-1, np.prod(items[2]))
            idx = idx.to(torch.long).reshape(items[2])
    ...
    def test_put__backward(self):
        input_cpu = torch.randn((2,5,3), dtype = torch.float, requires_grad = True)
        ...
        self.assertTensorsEqual(
            input_cpu, input_mlu.cpu(), 0.0, use_MSE=True)
