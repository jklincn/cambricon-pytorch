...
class TestOps(TestCase):
    ...
    def test_fill_(self):
        ...
        dtype_list = [(torch.uint8, 0), (torch.int8, 0),
                      (torch.int16, 0), (torch.int32, 0),
                      (torch.float, 0), (torch.half, 0),
                      (torch.bool, 0), (torch.double, 0),
                      (torch.long, 0), (torch.complex64, 0),
                      (torch.complex128, 0)]
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in dtype_list:
                input1 = torch.testing.make_tensor(shape_list[i], \
                                                   dtype=data_type, device='cpu')
                out_cpu = torch.fill_(input1, value_list[i])
                out_mlu_1 = torch.fill_(self.to_mlu_dtype(
                    input1, data_type), value_list[i])
        ...
        for shape in shape_list:
            for data_type, err in dtype_list:
                tensor_1 = torch.testing.make_tensor(shape, \
                                                     dtype=data_type, device='cpu')
                tensor_1_int = torch.tensor(2.55, dtype=torch.int32)
                tensor_2 = torch.testing.make_tensor(shape, \
                                                     dtype=data_type, device='cpu')
    ...
    def test_fill_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.fill_(a, value)
        ref_msg = "value cannot be converted to type int without overflow"
        a = torch.randn((2,)).long().mlu()
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.fill_(a, 0x7fffffffffffffff)
    ...
    def test_fill_large(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in dtype_list:
                ...
                out_cpu = torch.fill_(input1, value_list[i])
                out_mlu = torch.fill_(self.to_mlu_dtype(input1, data_type), value_list[i])
                self.assertTensorsEqual(
    ...
    def test_fill_bfloat16(self):
        ...
        for value in value_list:
            x_cpu = torch.testing.make_tensor((1, 3, 224, 224), dtype=torch.bfloat16, \
                                              device='cpu')
