...
class TestGeluOp(TestCase):
    ...
    def test_gelu_backward(self):
        def run_test(x_0, dtype):
            ...
            self.assertTensorsEqual(
                out_grad_cpu,
                out_grad_mlu.cpu().float() if dtype == torch.half else out_grad_mlu.cpu(),
                0.003,
                use_MSE=True)
        ...
        for shape in in_shape:
            for typeId in type_list:
                ...
                # not-dense input
                run_test(x_0[..., :2].detach(), typeId)

    #@unittest.skip("not test")
    @testinfo()
    def test_gelu_permute(self):
        import random
        for in_shape in [(8, 224, 224),
                         (1, 3, 16, 16, 4),
                         (1, 3, 16, 16, 3, 6),
                         (1, 3, 16, 16, 4, 15, 8)]:
    ...
    def test_gelu_approximate(self):
        def run_test(x_0, dtype):
            ...
            out_cpu = F.gelu(x_0, approximate="none")
            out_mlu = F.gelu(x_mlu, approximate="none")
