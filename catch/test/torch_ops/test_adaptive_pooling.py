...
class TestAdaptivePoolingOp(TestCase):
    ...
    def test_adaptive_avg_pool1d(self):
        in_shape_list = [(0, 2), (1, 4), (16, 8),
                         (0, 4, 15), (2, 3, 7), (8, 14, 14),
                         (4, 23, 64),(64, 128, 128)]
    ...
    def test_adaptive_avg_pool2d(self):
        in_shape_list = [(0, 2, 3), (1, 4, 4), (16, 6, 8),
                         (0, 4, 13, 15), (2, 3, 3, 7), (8, 16, 14, 14),
                         (4, 23, 13, 64),(4, 64, 128, 128)]
    ...
    def test_adaptive_avg_pool3d(self):
        in_shape_list = [(0, 2, 3, 6), (1, 2, 3, 4), (6, 8, 16, 16), (16, 6, 8, 8),
                         (0, 6, 4, 13, 15), (4, 8, 16, 14, 14), (4, 4, 23, 13, 64),
                         (4, 4, 64, 128, 128)]
        ...
        for in_shape, out_shape, dtype in product(*list_list):
            ...
            output_mlu = self.to_device(torch.randn(output_cpu.size(), dtype=dtype))
            with torch.no_grad():
                torch._C._nn.adaptive_avg_pool3d(self.to_device(input_mlu), out_shape,
                  out=output_mlu)
                self.assertTensorsEqual(output_cpu.float(),
                                        output_mlu.cpu().float(),
                                        err, use_MSE=True)
    ...
    def test_adaptive_avg_pool3d_not_contiguous(self):
        ...
        for in_shape, out_shape, in_func, out_func in product(*list_list):
            ...
            output_mlu_ptr = output_mlu.data_ptr()
            with torch.no_grad():
                torch._C._nn.adaptive_avg_pool3d(in_func(self.to_device(input_mlu)), out_shape,
                  out=output_mlu)
                self.assertTensorsEqual(output_cpu.float(),
                                        output_mlu.cpu().float(),
                                        3e-3, use_MSE=True)
    ...
    def test_adaptive_max_pool1d(self):
        ...
        self.assertTensorsEqual(torch.tensor([[[1, 1],[1, 1],[1, 1],[1, 1]]]),
                                indices_mlu.cpu(), 0)
    ...
    def test_adaptive_avg_pool2d_exception(self):
        ...
        ref_msg = r"^adaptive_avg_pool2d\(\): Expected 3D or 4D tensor,"\
                   " but got: \[2, 3\]$"
        ...
        inputs = torch.randn((32, 16, 120, 120), dtype=torch.float, 
                    requires_grad=True).to("mlu")
    ...
    def test_adaptive_avg_pool3d_exception(self):
        ...
        ref_msg = r"^cnnl_adaptive_avg_pool3d\(\): Expected 4D or 5D tensor,"\
                   " but got \[2, 3, 4\]$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            output = m(input)
        ...
        m = nn.AdaptiveAvgPool3d(7)
        ref_msg = r"^cnnl_adaptive_avg_pool3d\(\): Expected input to have"\
        ...
        ref_msg = "^The internal kernel size for cnnl_adaptive_avg_pool3d_backward_out"\
    ...
    def test_adaptive_max_pool2d_exception(self):
        ...
        ref_msg = r"adaptive_max_pool2d\(\): Expected 3D or 4D tensor, but got: \[2, 3\]"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
        ...
        ref_msg = r"^adaptive_max_pool2d\(\): internal error:"\
        ...
        ref_msg = "^The internal kernel size for adaptive_max_pool2d_out_mlu should be smaller than 3582, " \
