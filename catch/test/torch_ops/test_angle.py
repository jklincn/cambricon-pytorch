...
from common_utils import (
    testinfo, run_tests, TestCase, TEST_LARGETENSOR, largeTensorTest)  # pylint: disable=C0413,C0411
...
class TestAngleOp(TestCase):
    ...
    def test_angle_complex_to_float(self, dtype=torch.complex64):
        ...
        for vals in [random_vals, []]:
            ...
            float_dtype = (torch.float32
                           if dtype is torch.complex64 else torch.float64)
            np_float_out = np_fn(a).astype(
                torch_to_numpy_dtype_dict[float_dtype])
            float_out = torch.empty_like(t,
                                         dtype=float_dtype).float().to("mlu")
            ...
            torch_fn(t.to("mlu"), out=float_out)
            self.assertEqual(torch.from_numpy(np_float_out),
                             float_out.cpu(),
                             rtol=self.rtol,
                             atol=self.atol)
            ...
            torch_fn(t.to("mlu"), out=complex_out)
            self.assertEqual(torch.from_numpy(np_complex_out),
                             complex_out.cpu(),
                             rtol=self.rtol,
                             atol=self.atol)
            ...
            if fn_name == "angle":
                with self.assertRaises(AttributeError):
                    torch_inplace_method = getattr(torch.Tensor, fn_name + "_") # NOSONAR

    # @unittest.skip("not test")
    @testinfo()
    def test_angle(self):
        shape_list = [(512, 1024, 2, 2), (2, 3, 4), (254, 254, 112, 1, 1, 3)]
        dtype_list = [torch.double, torch.float, torch.complex64, torch.complex128, \
                      torch.int64, torch.int, torch.int16, torch.uint8, torch.int8]
    ...
    def test_angle_permute(self):
        ...
        for i in range (4):
            x = torch.testing.make_tensor(shape_list[i], dtype=torch.float, device='cpu')
            out = torch.testing.make_tensor(shape_list[i], dtype=torch.float, device='cpu')
    ...
    def test_exception(self):
        mlu_x = torch.testing.make_tensor((2, 3, 4), \
                                          dtype=torch.half, \
                                          device='cpu').mlu()
        with self.assertRaisesRegex(RuntimeError,
                                    r"MLU angle don't support tensor dtype Half."):
            torch.angle(mlu_x)
