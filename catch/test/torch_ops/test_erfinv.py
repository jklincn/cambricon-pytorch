...
from common_utils import (run_tests, testinfo, TestCase, TEST_LARGETENSOR, largeTensorTest)   # pylint: disable=C0413, C0411
...
class TestErfinvOp(TestCase):
    ...
    def test_erfinv_contiguous(self):
        for func in [torch.erfinv, torch.special.erfinv]:
            for shape in [(), (2, 3), (2, 3, 4), (1, 1, 1, 1), (1, 3, 16, 16), (1, 3, 16, 16, 3)]:
                x = torch.randn(shape, dtype=torch.float)
                out_cpu = func(x)
                out_mlu = func(x.to('mlu'))
                self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.003, use_MSE=True, allow_inf=True)
                ...
                x_cpu.erfinv_()
                x_mlu.erfinv_()
                self.assertTensorsEqual(x_cpu, x_mlu.cpu(), 0.003, use_MSE=True, allow_inf=True)
    ...
    def test_erfinv_backward(self):
        for func in [torch.erfinv, torch.special.erfinv]:
            for shape in [(2, 3), (1, 1, 1, 1), (1, 3, 16, 16), (1, 3, 16, 16, 3)]:
                ...
                self.assertTensorsEqual(grad_cpu, grad_mlu.cpu(), 0.003,
                                        use_MSE=True, allow_inf=True)
    ...
    def test_erfinv_permute(self):
        shape_list = [(12, 24, 2, 2, 4), (10, 3, 2, 2), (2, 3, 4),
                (25, 25, 12, 1, 1, 3)]
        ...
        for func in [torch.erfinv, torch.special.erfinv]:
            for i in range (4):
                ...
                self.assertTensorsEqual(out, out_mlu.cpu(), 0.003, use_MSE=True, allow_inf=True)
            ...
            for shape in shape_list:
                for type in type_list:
                    ...
                    self.assertTensorsEqual(out_cpu, out_mlu.cpu(),
                                            0.003, use_MSE=True, allow_inf=True)
                    if type == torch.half:
    ...
    @testinfo()
    @unittest.skipUnless(TEST_LARGETENSOR,
                         "run largeTensorCases by `TEST_LARGETENSOR=TRUE` or `--large`")
