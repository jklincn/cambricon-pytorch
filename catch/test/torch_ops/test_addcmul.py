...
class TestAddcMulOp(TestCase):
    ...
    def test_addcmul_contiguous(self):
        ...
        for shape_a, shape_b, shape_c in [((128,64,0,7), (128,64,0,7), (128,64,0,1)),
            ...
            for data_type in data_types:
                ...
                if data_type == torch.int:
                    ...
                    with self.assertRaisesRegex(RuntimeError, ref_msg):
                        out_mlu = torch.addcmul(a_mlu, self.to_mlu_dtype(b, data_type),
    ...
    def test_addcmul_channel_last(self):
        ...
        for shape_a, shape_b, shape_c in [((128,64,7,7), (128,64,1,7), (128,64,7,1)),
                                          ((1024,512,3,3), (1024,512,1,3), (1024,512,3,3)),
                                          ((512,256,3,3), (1,256,1,3), (512,1,3,1))]:
            for data_type, only_one in product(data_types, only_one_tensor_channle_last):
                a = torch.rand(shape_a, dtype=torch.float)
                b = torch.rand(shape_b, dtype=torch.float)
                c = torch.rand(shape_c, dtype=torch.float) + 1
                if not only_one:
    ...
    def test_addcmul_not_dense(self):
        data_types = [torch.half, torch.float, torch.double]
        for shape_a, shape_b, shape_c in [((128,64,7,14), (128,64,1,14), (128,64,7,2)),
            ...
            for data_type in data_types:
                ...
                a_mlu = self.to_mlu_dtype(copy.deepcopy(a),
                                          data_type)[:, :, :, :int(shape_a[-1] / 2)]
                b_mlu = self.to_mlu_dtype(copy.deepcopy(b),
                                          data_type)[:, :, :, :int(shape_b[-1] / 2)]
                c_mlu = self.to_mlu_dtype(copy.deepcopy(c),
                                          data_type)[:, :, :, :int(shape_c[-1] / 2)]
    ...
    def test_addcmul_contiguous_(self):
        ...
        for shape_a, shape_b, shape_c in [((128,64,7,7), (128,64,1,7), (128,64,7,1)),
            ...
            for data_type in data_types:
                ...
                a_mlu.addcmul_(self.to_mlu_dtype(b, data_type),
                               self.to_mlu_dtype(c, data_type), value = 0.35)
                self.assertEqual(a_ptr, a_mlu.data_ptr())
                self.assertTensorsEqual(a.float(), a_mlu.cpu().float(),
                                        0.003, use_MSE=True)
    ...
    def test_addcmul_channel_last_not_dense(self):
        data_types = [torch.float, torch.half, torch.double]
        only_one_tensor_channle_last = [False, True]
        for shape_a, shape_b, shape_c in [((2,2,2,4), (2,2,1,4), (2,2,1,4)),
            ...
            for data_type, only_one in product(data_types, only_one_tensor_channle_last):
                ...
                self.assertEqual(a_mlu.is_contiguous(memory_format = torch.preserve_format),
                                 a_cpu.is_contiguous(memory_format = torch.preserve_format))
                self.assertEqual(a_mlu.is_contiguous(memory_format = torch.channels_last),
                                 a_cpu.is_contiguous(memory_format = torch.channels_last))
                self.assertEqual(a_mlu.is_contiguous(memory_format = torch.contiguous_format),
                                 a_cpu.is_contiguous(memory_format = torch.contiguous_format))
                self.assertTensorsEqual(a_cpu.float(), a_mlu.cpu().float(),
                                        0.003, use_MSE=True)
    # @unittest.skip("not test")
    @testinfo()
    ...
    def test_addcmul_out_contiguous(self):
        ...
        for shape_a, shape_b, shape_c in [((128,64,7,7), (128,64,1,7), (128,64,7,1)),
            ...
            for data_type in data_types:
                ...
                torch.addcmul(a_mlu, self.to_mlu_dtype(b, data_type),
                              self.to_mlu_dtype(c, data_type), value = 0.35, out=out_mlu)
                self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(),
                                        0.003, use_MSE=True)
    ...
    def test_addcmul_out_not_dense(self):
        data_types = [torch.half, torch.float, torch.double]
        for shape_a, shape_b, shape_c in [((128,64,7,14), (128,64,1,14), (128,64,7,14)),
                                          ((1024,512,3,6), (1024,512,1,6), (1024,512,3,6)),
                                          ((512,256,3,6), (1,256,1,6), (512,1,3,2))]:
