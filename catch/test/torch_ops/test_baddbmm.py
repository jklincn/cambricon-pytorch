...
def shape_generator(broadcast=False, zerodim=False):
    ...
    basic_shape = [randint(min_shape, max_shape)
                   for i in range(4)]  # basic_shape is BNMP
    B, N, M, P = basic_shape
...
class TestBaddbmm(TestCase):
    ...
    def test_baddbmm(self):
        convert_func = [self.to_non_dense,
                        self.convert_to_channel_last, lambda x:x]
        for convert1, convert2, convert3 in product(*([convert_func] * 3)):
            for broadcast, zerodim, has_inf_nan, dtype_err in product(*test_list):
                ...
                for input_shape, mat1_shape, mat2_shape in shape_table:
                    input_cpu = add_inf_nan(torch.randn(
                        input_shape).to(dtype), has_inf_nan)
                    ...
                    result_cpu = torch.baddbmm(input_cpu, mat1_cpu, mat2_cpu,
                                               beta=beta, alpha=alpha)
                    result_mlu = torch.baddbmm(input_mlu, mat1_mlu, mat2_mlu,
                                               beta=beta, alpha=alpha)
                    self.assertTensorsEqual(result_cpu.float(),
                                            result_mlu.cpu().contiguous().float(), err, use_MSE=True)
    ...
    def test_baddbmm_beta_not_zero(self):
        ...
        out_cpu = torch.baddbmm(
            input_cpu, batch1, batch2, beta=beta, alpha=alpha)
        out_device = torch.baddbmm(input_cpu.to('mlu'), batch1.to(
            'mlu'), batch2.to('mlu'), beta=beta, alpha=alpha)
        self.assertTensorsEqual(out_cpu.float(), out_device.cpu(
        ).contiguous().float(), 3e-3, use_MSE=True)
        ...
        for convert1, convert2, convert3 in product(*([convert_func] * 3)):
            for broadcast, zerodim, has_inf_nan, dtype_err in product(*test_list):
                ...
                for input_shape, mat1_shape, mat2_shape in shape_table:
                    ...
                    input_cpu.baddbmm_(mat1_cpu, mat2_cpu,
                                       beta=beta, alpha=alpha)
                    input_mlu.baddbmm_(mat1_mlu, mat2_mlu,
                                       beta=beta, alpha=alpha)
                    self.assertTensorsEqual(input_cpu.float(),
                                            input_mlu.cpu().contiguous().float(), err, use_MSE=True)
        ...
        for convert1, convert2, convert3 in product(*([convert_func] * 3)):
            for broadcast, zerodim, has_inf_nan, dtype_err in product(*test_list):
                ...
                for input_shape, mat1_shape, mat2_shape in shape_table:
                    ...
                    result_cpu = torch.baddbmm(input_cpu, mat1_cpu, mat2_cpu,
                                               beta=beta, alpha=alpha)
                    torch.baddbmm(input_cpu, mat1_cpu, mat2_cpu, beta=beta,
                                  alpha=alpha, out=result_cpu)
                    result_mlu = torch.randn(result_cpu.size()).to(
                        input_mlu.dtype).to('mlu')
                    result_mlu_ptr = result_mlu.data_ptr()
                    torch.baddbmm(input_mlu, mat1_mlu, mat2_mlu, beta=beta, alpha=alpha,
                                  out=result_mlu)
                    self.assertTensorsEqual(result_cpu.float(),
                                            result_mlu.cpu().contiguous().float(), err, use_MSE=True)
    ...
    def test_baddbmm_permute(self):
        ...
        out_cpu = torch.baddbmm(input, batch1.permute(1,0,2), batch2.permute(1,0,2), beta=1.1)
        out_mlu = torch.baddbmm(input.mlu(), 
                                batch1.mlu().permute(1,0,2), 
                                batch2.mlu().permute(1,0,2), beta=1.1)
    ...
    def test_baddbmm_bfloat16(self):
        ...
        out_device = torch.baddbmm(input_mlu, batch1_mlu, batch2_mlu, beta=beta, alpha=alpha)
        grad = torch.randn(out_cpu.shape).bfloat16().float()
        grad_mlu = grad.mlu().bfloat16()
        out_cpu.backward(grad)
        out_device.backward(grad_mlu)
        self.assertTensorsEqual(out_cpu.float(), out_device.cpu().float(),
                                3e-3, use_MSE=True)
        self.assertTensorsEqual(input_cpu.grad.float(), input_mlu.grad.cpu().float(), 0.003, use_MSE=True)
        self.assertTensorsEqual(batch1_cpu.grad.float(), batch1_mlu.grad.cpu().float(), 0.003, use_MSE=True)
        self.assertTensorsEqual(batch2_cpu.grad.float(), batch2_mlu.grad.cpu().float(), 0.003, use_MSE=True)
