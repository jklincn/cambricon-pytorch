...
class TestFmodOp(TestCase):
    ...
    def test_fmod(self):
        ...
        for dtype_err, in_shape, func in product(
    ...
    def test_fmod_permute(self):
        ...
        for i in range (4):
            ...
            x, y, out = x.permute(permute_shape[i]), y.permute(permute_shape[i]), \
                        out.permute(permute_shape[i])
            x_mlu, y_mlu, out_mlu = x_mlu.permute(permute_shape[i]), \
                             y_mlu.permute(permute_shape[i]), out_mlu.permute(permute_shape[i])
    ...
    def test_fmod_stride_channle_last_tensor(self):
        ...
        for dtype_err, in_shape, stride_shape in product(dtype_list, shape_list, stride_list):
            dtype, err = dtype_err
            x_cpu = (20 * torch.randn(in_shape)).to(dtype)
            y_cpu = (20 * torch.rand(in_shape)).to(dtype) + 1
            x_mlu = x_cpu.to('mlu').to(memory_format=torch.channels_last)
            y_mlu = y_cpu.to('mlu').as_strided(in_shape, stride_shape)
            out_cpu = torch.fmod(x_cpu.to(memory_format=torch.channels_last),
