...
class TestRandpermOps(TestCase):
    ...
    def test_randperm(self):
        for dtype in [torch.int8, torch.uint8, torch.int16, torch.half, torch.int32,
                      torch.float32, torch.long, torch.double]:
            for n in [1, 10, 100]:
                ...
                self.assertLess(abs(out_mlu.cpu().float().sum().item() -
                    out_cpu.float().sum().item()), 0.003)
