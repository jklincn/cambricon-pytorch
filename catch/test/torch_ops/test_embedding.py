...
class TestEmbeddingOp(TestCase):
    ...
    def test_embedding(self):
        x_shape_lists = [(51, 77), (32,), (16, 7200, 2), (16, 7200, 1, 1), (2, 3, 4, 5, 6), (0,), ()]
        c_lists = [9796, 1234]
        dtype_list = [(torch.float, 0.0), (torch.half, 3e-3)]
        func_list = [lambda x:x, self.convert_to_channel_last, self.get_not_contiguous_tensor]
    ...
    def test_embedding_backward(self):
        x_shape_lists = [(2, 4), (32,), (16, 7200, 2), (16, 7200, 1, 1), (2, 3, 4, 5, 6), (0,), ()]
        c_lists = [8, 16]
    ...
    def test_embedding_bfloat16(self):
        ...
        self.assertTensorsEqual(matrix_cpu.grad.float(), matrix_mlu.grad.cpu().float(), 0.003, use_MSE=True)
    ...
    def test_embedding_transpose_backward(self):
        ...
        out_mlu = torch.nn.functional.embedding(
            self.to_device(x), self.to_device(matrix)).transpose(0, 1).contiguous()
        out_mlu.backward(torch.ones_like(out_mlu))
        g2 = copy.deepcopy(matrix.grad)
        self.assertTensorsEqual(
            out_cpu, out_mlu.cpu().float(), 0.0001, use_MSE=True)
        self.assertTensorsEqual(g1, g2.cpu(), 0.003, use_MSE=True)
    ...
    def test_embedding_backward_with_mix_memory_format(self):
        ...
        out_mlu = model(self.to_device(grad_output), self.to_device(indices), 16, -1, False)
        self.assertTensorsEqual(
