...
class TestAddmvOp(TestCase):
    ...
    def test_addmv_empty(self):
        def fn(torchfn, *args, test_out=False, **kwargs):
            def call_torch_fn(*args, **kwargs):
                return torchfn(*tuple(torch.randn(shape, device=device)
            ...
            if not test_out:
                ...
                out1 = call_torch_fn(
                    *args, **kwargs, out=out)  # pylint: disable=W0612
                return out
        # mv, addmv
        device = 'mlu'
    ...
    def _test_addmm_addmv(self, f, t, m, v, *, alpha=None, beta=None,
                          transpose_out=False, err=3e-3):
        dtype = t.dtype
        numpy_dtype = dtype
        ...
        res3 = alpha * (m.to(numpy_dtype).cpu().numpy() @
    ...
    def test_addmv_ori(self):
        ...
        for dtype, err in dtype_list:
            ...
            ms = [
                ...
                torch.randint(3, (10, 1), dtype=torch.float).to(
                    device).to(dtype).expand(10, 100),
                # 2d
            ...
            s = 3
            a_data = torch.arange(
                1, o * s + 1, dtype=dtype).to(device).view(o, s)
            x_data = torch.arange(1, s + 1, 1, dtype=dtype).to(device)
            y_data = torch.ones(o, device=device, dtype=dtype)
            control = torch.tensor([15., 33., 51., 69., 87.],  # pylint: disable=W0612
                                   device=device, dtype=dtype)

            def _test(row_major, incx, incy, lda_tail):
                if row_major:
                    a_storage = torch.full((o, s + lda_tail), float('nan'), device=device,
                                           dtype=dtype)
                else:
                    a_storage = torch.full((s, o + lda_tail), float('nan'), device=device,
                                           dtype=dtype).permute(1, 0)
                a = a_storage[:o, :s].copy_(a_data)

                x_storage = torch.full((s, incx), float(
                    'nan'), device=device, dtype=dtype)
                x = x_storage[:, 0].copy_(x_data)

                y_storage = torch.full((o, incy), float(
                    'nan'), device=device, dtype=dtype)
                y = y_storage[:, 0].copy_(y_data)

                self._test_addmm_addmv(torch.addmv, y, a, x)

            for row_major, incx, incy, lda_tail in itertools.product((False, True),
                                                                     (1, 2), (1, 2), (0, 1)):
    ...
    def test_addmv_alpha_beta_empty(self):
        ...
        for dtype, _ in dtype_list:
            ...
            self.assertEqual(torch.full((2,), beta * value, dtype=dtype, device=device),
                             torch.addmv(input=input, mat=mat, vec=vec, alpha=alpha,
                                         beta=beta, out=out))
    ...
    def test_addmv(self):
        ...
        for dtype_err, func in product(
                dtype_list, func_list):
            dtype, err = dtype_err
            ...
            out_cpu = torch.addmv(func(M_cpu).float(), func(mat_cpu).float(),
                                  func(vec_cpu).float())
