    ...
    def test_full(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in dtype_list:
                out_cpu = torch.full(shape_list[i], value_list[i], dtype=data_type, device='cpu')
                out_mlu_1 = torch.full(shape_list[i], value_list[i], dtype=data_type, device='mlu')
                out_mlu_2 = torch.full(
                    shape_list[i], self.to_mlu(torch.tensor(value_list[i])),
                    dtype=data_type, device='mlu')
...
class TestFullLikeOp(TestCase):
    ...
    def test_full(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in dtype_list:
                ...
                out_cpu = torch.full_like(x, value_list[i], dtype=data_type, device='cpu')
                out_mlu_1 = torch.full_like(x, value_list[i], dtype=data_type, device='mlu')
                out_mlu_2 = torch.full_like(
                    x, self.to_mlu(torch.tensor(value_list[i])), dtype=data_type, device='mlu')
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu_1.cpu().float(), err, use_MSE=True)
                self.assertTensorsEqual(
                    out_cpu.float(), out_mlu_2.cpu().float(), err, use_MSE=True)
    ...
    def test_full_channels_last(self):
        ...
        for shape in shape_list:
            ...
            out_cpu = torch.full_like(x, value, device='cpu', memory_format=torch.channels_last)
            out_mlu = torch.full_like(x, value, device='mlu', memory_format=torch.channels_last)
...
class TestNewFullOp(TestCase):
    ...
    def test_full(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for data_type, err in dtype_list:
                tmp_out = torch.full(shape_list[i], value_list[i], dtype=data_type, device='cpu')
                tmp_out_mlu = tmp_out.mlu()
                out_cpu = tmp_out.new_full(new_shape, new_value, dtype=data_type, device='cpu')
                out_mlu = tmp_out_mlu.new_full(new_shape, new_value, dtype=data_type, device='mlu')
                self.assertTensorsEqual(
