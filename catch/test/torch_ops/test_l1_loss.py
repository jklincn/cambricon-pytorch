...
class TestOps(TestCase):
    ...
    def test_l1_loss_forward_not_dense(self):
        ...
        for item in product(shape_list, reduct_list, dtype_list):
            ...
            layer_mlu = torch.nn.L1Loss(reduction=item[1])
            out_mlu = layer_mlu(self.to_mlu_dtype(x, item[2]),
                    self.to_mlu_dtype(target, item[2]))
    ...
    def test_l1_loss_backward(self):
        ...
        for item in product(shape_list, reduct_list, dtype_list):
            ...
            if item[2] == torch.half:
                ...
                self.assertTensorsEqual(
                    grad_input_cpu.float(), grad_input_mlu.cpu().float(), 0.003, use_MSE=True)
    ...
    def test_l1_loss_backward_not_dense(self):
        ...
        for item in product(shape_list, reduct_list, dtype_list):
            ...
            if item[2] == torch.half:
                self.assertTensorsEqual(
                    grad_input_cpu.float(), grad_input_mlu.cpu().float(), 0.09, use_MSE=True)
            else:
                self.assertTensorsEqual(
                    grad_input_cpu.float(), grad_input_mlu.cpu().float(), 0.003, use_MSE=True)
    ...
    def test_l1_loss_permute(self):
        ...
        for i in range (4):
            for reduct in reduct_list:
                ...
                self.assertTensorsEqual(grad_input.float(), grad_input_mlu.cpu().float(), 0.003,
                            use_MSE=True, message = str(shape_list[i]) +" "+ str(reduct))
    ...
    def test_l1_loss_dtype(self):
        ...
        for dtype , reduct in product(dtype_list, reduct_list):
            ...
            self.assertTensorsEqual(output.float(), output_mlu.cpu().float(),
                0.003, use_MSE=True)
            if dtype == torch.half:
                self.assertEqual(grad_input, grad_input_mlu.cpu(),
                    msg = str(dtype) +" "+ str(reduct))
            else:
                self.assertTensorsEqual(grad_input.float(), grad_input_mlu.float().cpu(),
