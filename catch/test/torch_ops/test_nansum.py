...
class TestOps(TestCase):
            ...
            for i in range(1, dim_len+1):
                ...
                for test_dim in dim_lists:
                    for test_type in type_list:
                        ...
                        x_mlu = self.to_mlu(x)
                        torch.nansum(x.double(), test_dim, keepdim=test_type, out=out_cpu)
                        ...
                        try:
                            ...
                            out_sum = np.nansum(x_numpy, axis=test_dim, keepdims=test_type)
                            self.assertTensorsEqual(
    ...
    def test_nansum_dtype(self):
        ...
        out_dtype_list = [torch.int8, torch.int16, torch.int, torch.half, torch.float, torch.double]
        for t in type_list:
            for out_dtype in out_dtype_list:
                ...
                if t is not torch.int16 and out_dtype is not torch.int8:
                    self.assertTensorsEqual(
                    out_cpu.float(), out_mlu.cpu().float(), 0.003, use_MSE=True, allow_inf=True)
    ...
    def test_nansum_bfloat16(self):
        left = torch.testing.make_tensor((2, 10, 104), dtype=torch.bfloat16, device='cpu')
