...
from torch._six import inf  # pylint: disable=C0411, C0413
from torch.testing._internal.common_utils import TestCase, run_tests, set_rng_seed, \
    TEST_WITH_UBSAN, load_tests, gradcheck  # pylint: disable=C0411, C0413
from torch.autograd import grad  # pylint: disable=C0411, C0413
from torch.distributions import (Bernoulli, Categorical,
                                 Cauchy, ContinuousBernoulli,
                                 Distribution, Exponential, ExponentialFamily,
                                 Geometric, Gumbel,
                                 HalfCauchy, HalfNormal, Independent,
                                 Laplace, LogisticNormal,
                                 ...
                                 Pareto, RelaxedBernoulli,
                                 TransformedDistribution, Uniform,
                                 VonMises, kl_divergence)  # pylint: disable=C0411, C0413
...
ERR_MSG_OP_NOT_IMPLEMENTED_ON_MLU = "Could not run '.*' with arguments from the 'MLU' backend."
...
EXAMPLES = [
    Example(Bernoulli, [
        ...
        {'logits': torch.tensor([0.], requires_grad=True).to('mlu')},
    ]),
    Example(Geometric, [
        {'probs': torch.tensor([0.7, 0.2, 0.4], requires_grad=True).to('mlu')},
        {'probs': torch.tensor([0.3], requires_grad=True).to('mlu')},
    ...
    Example(Categorical, [
        {'probs': torch.tensor([[0.1, 0.2, 0.3], [0.5, 0.3, 0.2]], requires_grad=True).to('mlu')},
        {'probs': torch.tensor([[1.0, 0.0], [0.0, 1.0]], requires_grad=True).to('mlu')},
        {'logits': torch.tensor([[0.0, 0.0], [0.0, 0.0]], requires_grad=True).to('mlu')},
    ]),
    Example(Exponential, [
    ...
    Example(Independent, [
        {
            'base_distribution': Normal(torch.randn(2, 3, requires_grad=True).to('mlu'),
        ...
        {
            'base_distribution': Normal(torch.randn(2, 3, 5, requires_grad=True).to('mlu'),
    ...
    Example(TransformedDistribution, [
        ...
        },
        {
            'base_distribution': Normal(torch.randn(2, 3, 5, requires_grad=True).to('mlu'),
                                        torch.randn(2, 3, 5).to('mlu').abs().requires_grad_()),
            'transforms': [AffineTransform(torch.randn(3, 5).to('mlu'),
                                           torch.randn(3, 5).to('mlu')),
                           ExpTransform()],
        },
        {
            'base_distribution': Normal(torch.randn(2, 3, 5, requires_grad=True).to('mlu'),
    ...
    Example(VonMises, [
        ...
        },
        {
            ...
            'concentration': torch.tensor([1.0, 10.0], requires_grad=True).to('mlu')
        },
    ]),
...
BAD_EXAMPLES = [
    ...
    Example(Categorical, [
        {'probs': torch.tensor([[-0.1, 0.2, 0.3], [0.5, 0.3, 0.2]], requires_grad=True).to('mlu')},
        {'probs': torch.tensor([[-1.0, 10.0], [0.0, -1.0]], requires_grad=True).to('mlu')},
    ]),
    Example(Exponential, [
        {'rate': torch.tensor([0., 0.], requires_grad=True).to('mlu')},
        {'rate': torch.tensor([-2.], requires_grad=True).to('mlu')}
    ]),
    Example(Gumbel, [
        {
            'loc': torch.tensor([1., 1.], requires_grad=True).to('mlu'),
            'scale': torch.tensor([0., 1.], requires_grad=True).to('mlu'),
        },
        {
            'loc': torch.tensor([1., 1.], requires_grad=True).to('mlu'),
            'scale': torch.tensor([1., -1.], requires_grad=True).to('mlu'),
        },
    ]),
    Example(HalfNormal, [
        {'scale': torch.tensor([0., 1.], requires_grad=True).to('mlu')},
    ...
    Example(OneHotCategorical, [
        {'probs': torch.tensor([[0.1, 0.2, 0.3], [0.1, -10.0, 0.2]], requires_grad=True).to('mlu')},
        {'probs': torch.tensor([[0.0, 0.0], [0.0, 0.0]], requires_grad=True).to('mlu')},
    ]),
    Example(OneHotCategoricalStraightThrough, [
        {'probs': torch.tensor([[0.1, 0.2, 0.3], [0.1, -10.0, 0.2]], requires_grad=True).to('mlu')},
        {'probs': torch.tensor([[0.0, 0.0], [0.0, 0.0]], requires_grad=True).to('mlu')},
...
class TestDistributions(TestCase):
                ...
                try:
                    self.assertTrue(type(dist.sample()) is type(dist.enumerate_support()),
                                    msg=('{} example {}/{}, return type mismatch between ' +
                                         'sample and enumerate_support.').format(
                                                           Dist.__name__, i + 1, len(params)))
    ...
    def test_bernoulli(self):
        ...
        self.assertEqual(Bernoulli(r_mlu).sample((3, 2)).size(), (3, 2,))
        ...
        self._check_log_prob(Bernoulli(logits=p_mlu.log() - (-p_mlu).log1p()), ref_log_prob)
        self.assertRaises(NotImplementedError, Bernoulli(r_mlu).rsample)

        # check entropy computation
        self.assertEqual(Bernoulli(p).entropy(),
                         torch.tensor([0.6108, 0.5004, 0.6730]), atol=1e-4, rtol=0)
        self.assertEqual(Bernoulli(torch.tensor([0.0]).to('mlu')).entropy(), torch.tensor([0.0]))
    ...
    def test_geometric_log_prob_and_entropy(self):
        ...
        self._check_log_prob(Geometric(logits=p_mlu.log() - (-p_mlu).log1p()), ref_log_prob)

        # check entropy computation
        self.assertEqual(Geometric(p_mlu).entropy(),
    ...
    def test_geometric_sample(self):
        ...
        for prob in [0.01, 0.18, 0.8]:
            ...
            self._check_sampler_discrete(Geometric(prob_mlu),
    ...
    def test_categorical_2d(self):
        ...
        self.assertEqual(Categorical(p_mlu).sample(sample_shape=(3, 4)).size(), (3, 4, 2))
        self.assertEqual(Categorical(p_mlu).sample((6,)).size(), (6, 2))
        self._gradcheck_log_prob(Categorical, (p_mlu,))

        # sample check for extreme value of probs
        set_rng_seed(0)
        self.assertEqual(Categorical(s_mlu).sample(sample_shape=(2,)),
        ...
        # check entropy computation
        self.assertEqual(Categorical(p_mlu).entropy(),
                         torch.tensor([1.0114, 1.0297]), atol=1e-4, rtol=0)
    ...
    def test_one_hot_categorical_2d(self):
        ...
        self.assertEqual(OneHotCategorical(p_mlu).sample(sample_shape=(3, 4)).size(), (3, 4, 2, 3))
    ...
    def test_relaxed_bernoulli(self):
        ...
        self.assertEqual(RelaxedBernoulli(temp_mlu, r_mlu).sample((3, 2)).size(), (3, 2,))
    ...
    def test_rounded_relaxed_bernoulli(self):
        ...
        for probs, temp in product([0.1, 0.2, 0.8], [0.1, 1.0, 10.0]):
            ...
            self._check_sampler_discrete(Rounded(RelaxedBernoulli(temp_mlu, probs_mlu)),
    ...
    def test_halfnormal(self):
        ...
        set_rng_seed(1)
        self.assertEqual(HalfNormal(std_delta_mlu).sample(sample_shape=(1, 2)),
    ...
    def test_halfnormal_sample(self):
        ...
        for std in [0.1, 1.0, 10.0]:
            ...
            self._check_sampler_sampler(HalfNormal(std_mlu),
    ...
    def test_lognormal(self):
        ...
        set_rng_seed(1)
        self.assertEqual(LogNormal(mean_delta_mlu, std_delta_mlu).sample(sample_shape=(1, 2)),
    ...
    def test_lognormal_sample(self):
        ...
        for mean, std in product([-1.0, 0.0, 1.0], [0.1, 1.0, 10.0]):
            ...
            std_mlu = torch.tensor(std).to('mlu')
            self._check_sampler_sampler(LogNormal(mean_mlu, std_mlu),
    ...
    def test_logisticnormal(self):
        ...
        self.assertEqual(LogisticNormal(mean_mlu, std_mlu).sample((7,)).size(), (7, 5, 6))
        self.assertEqual(LogisticNormal(mean_1d_mlu, std_1d_mlu).sample((1,)).size(), (1, 2))
        self.assertEqual(LogisticNormal(mean_1d_mlu, std_1d_mlu).sample().size(), (2,))

        # sample check for extreme value of mean, std
        set_rng_seed(1)
        self.assertEqual(LogisticNormal(mean_delta_mlu, std_delta_mlu).sample(),
        ...
        self._gradcheck_log_prob(lambda m, s: LogisticNormal(m, s, validate_args=False),
                                 (mean_mlu, std_mlu))
        self._gradcheck_log_prob(lambda m, s: LogisticNormal(m, s, validate_args=False),
                                 (mean_mlu, 1.0))
        self._gradcheck_log_prob(lambda m, s: LogisticNormal(m, s, validate_args=False),
    ...
    def test_mixture_same_family_shape(self):
        ...
        normal_case_2d_batch = MixtureSameFamily(
            Categorical(torch.rand(3, 5).mlu()),
            Independent(Normal(torch.randn(3, 5, 2).mlu(), torch.rand(3, 5, 2).mlu()), 1))
        normal_case_2d_multi_batch = MixtureSameFamily(
            Categorical(torch.rand(4, 3, 5).mlu()),
            Independent(Normal(torch.randn(4, 3, 5, 2).mlu(), torch.rand(4, 3, 5, 2).mlu()), 1))
    ...
    def test_mixture_same_family_log_prob(self):
        ...
        self._check_log_prob(
            MixtureSameFamily(Categorical(probs=probs_mlu),
                              Normal(loc_mlu, scale_mlu)), ref_log_prob)
    ...
    def test_normal(self):
        ...
        # sample check for extreme value of mean, std
        set_rng_seed(1)
        self.assertEqual(Normal(loc_delta_mlu, scale_delta_mlu).sample(sample_shape=(1, 2)),
    ...
    def test_normal_sample(self):
        ...
        for loc, scale in product([-1.0, 0.0, 1.0], [0.1, 1.0, 10.0]):
            ...
            self._check_sampler_sampler(Normal(loc_mlu, scale_mlu),
    ...
    def test_exponential_sample(self):
        ...
        for rate in [1e-5, 1.0, 10.]:
            ...
            self._check_sampler_sampler(Exponential(rate_mlu),
    ...
    def test_laplace(self):
        ...
        # sample check for extreme value of mean, std
        set_rng_seed(0)
        self.assertEqual(Laplace(loc_delta_mlu, scale_delta_mlu).sample(sample_shape=(1, 2)),
                         torch.tensor([[[1.0, 0.0], [1.0, 0.0]]]),
                         atol=1e-4, rtol=0)
        ...
        set_rng_seed(1)
        eps = torch.ones_like(loc_mlu).uniform_(-.5, .5)
        set_rng_seed(1)
        z = Laplace(loc_mlu, scale_mlu).rsample()
        z.backward(torch.ones_like(z))
        self.assertEqual(loc.grad, torch.ones_like(loc))
        self.assertEqual(scale.grad, -eps.cpu().sign() * torch.log1p(-2 * eps.cpu().abs()))
        loc.grad.zero_()
        scale.grad.zero_()
        self.assertEqual(z.size(), (5, 5))
    ...
    def test_pareto_sample(self):
        ...
        for scale, alpha in product([0.1, 1.0, 5.0], [0.1, 1.0, 10.0]):
            ...
            self._check_sampler_sampler(Pareto(scale_mlu, alpha_mlu),
    ...
    def test_continuous_bernoulli(self):
        ...
        self.assertEqual(ContinuousBernoulli(r_mlu).sample((3, 2)).size(), (3, 2,))
        ...
        self._check_log_prob(ContinuousBernoulli(logits=p_mlu.log() - (-p_mlu).log1p()),
                             ref_log_prob)

        # check entropy computation
        self.assertEqual(ContinuousBernoulli(p_mlu).entropy(),
                         torch.tensor([-0.02938, -0.07641, -0.00682]), atol=1e-4, rtol=0)
        # entropy below corresponds to the clamped value of prob when using float 64
        # the value for float32 should be -1.76898
        self.assertEqual(ContinuousBernoulli(torch.tensor([0.0]).mlu()).entropy(),
                         torch.tensor([-1.76898]), atol=1e-5, rtol=0)
    ...
    def test_continuous_bernoulli_3d(self):
        ...
        self.assertEqual(ContinuousBernoulli(p_mlu).sample(sample_shape=(2, 5)).size(),
                         (2, 5, 2, 3, 5))
        ...
        for Dist, params in EXAMPLES:
            for param in params:
                ...
                for reinterpreted_batch_ndims in range(len(base_dist.batch_shape) + 1):
                    ...
                    indep_log_prob_shape = base_log_prob_shape[:len(base_log_prob_shape)
                    ...
                    if indep_dist.has_rsample:
                        self.assertEqual(indep_dist.sample().shape, base_dist.sample().shape)
                    try:
                        self.assertEqual(indep_dist.enumerate_support().shape,
    ...
    def test_cdf_icdf_inverse(self):
        ...
        for Dist, params in EXAMPLES:
            for i, param in enumerate(params):
                ...
                self.assertLess(rel_error.max(), 1e-3, msg='\n'.join([
        ...
        for Dist, params in EXAMPLES:
            for i, param in enumerate(params):
                ...
                try:
                    ...
                    self.assertNotRegex(str(e), ERR_MSG_OP_NOT_IMPLEMENTED_ON_MLU)
                    continue
                cdfs_derivative = grad(cdfs.sum(),
                                       [samples])[0]  # this should not be wrapped in torch.abs()
    ...
    def test_valid_parameter_broadcasting(self):
        ...
        valid_examples = [
            (Normal(loc=torch.tensor([0., 0.]).to('mlu'), scale=1),
             (2,)),
            (Normal(loc=0, scale=torch.tensor([1., 1.]).to('mlu')),
             (2,)),
            (Normal(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([1.]).to('mlu')),
             (2,)),
            (Normal(loc=torch.tensor([0., 0.]).to('mlu'),
                    scale=torch.tensor([[1.], [1.]]).to('mlu')),
             (2, 2)),
            (Normal(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 2)),
            (Normal(loc=torch.tensor([0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 1)),
            (Gumbel(loc=torch.tensor([0., 0.]).to('mlu'), scale=1),
             (2,)),
            (Gumbel(loc=0, scale=torch.tensor([1., 1.]).to('mlu')),
             (2,)),
            (Gumbel(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([1.]).to('mlu')),
             (2,)),
            (Gumbel(loc=torch.tensor([0., 0.]).to('mlu'),
                    scale=torch.tensor([[1.], [1.]]).to('mlu')),
             (2, 2)),
            (Gumbel(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 2)),
            (Gumbel(loc=torch.tensor([0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 1)),
            (Laplace(loc=torch.tensor([0., 0.]).to('mlu'), scale=1),
             (2,)),
            (Laplace(loc=0, scale=torch.tensor([1., 1.]).to('mlu')),
             (2,)),
            (Laplace(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([1.]).to('mlu')),
             (2,)),
            (Laplace(loc=torch.tensor([0., 0.]).to('mlu'),
                     scale=torch.tensor([[1.], [1.]]).to('mlu')),
             (2, 2)),
            (Laplace(loc=torch.tensor([0., 0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 2)),
            (Laplace(loc=torch.tensor([0.]).to('mlu'), scale=torch.tensor([[1.]]).to('mlu')),
             (1, 1)),
            (Pareto(scale=torch.tensor([1., 1.]).to('mlu'), alpha=1),
             (2,)),
            (Pareto(scale=1, alpha=torch.tensor([1., 1.]).to('mlu')),
             (2,)),
            (Pareto(scale=torch.tensor([1., 1.]).to('mlu'), alpha=torch.tensor([1.]).to('mlu')),
             (2,)),
            (Pareto(scale=torch.tensor([1., 1.]).to('mlu'), alpha=torch.tensor([[1.], [1.]]).to('mlu')),
             (2, 2)),
            (Pareto(scale=torch.tensor([1., 1.]).to('mlu'), alpha=torch.tensor([[1.]]).to('mlu')),
             (1, 2)),
            (Pareto(scale=torch.tensor([1.]).to('mlu'), alpha=torch.tensor([[1.]]).to('mlu')),
             (1, 1))
        ]
...
class TestDistributionShapes(TestCase):
    ...
    def test_bernoulli_shape_tensor_params(self):
        bernoulli = Bernoulli(torch.tensor([[0.6, 0.3], [0.6, 0.3], [0.6, 0.3]]).to('mlu'))
        ...
        self.assertEqual(bernoulli.log_prob(torch.ones(3, 1, 1).mlu()).size(),
                         torch.Size((3, 3, 2)))
    ...
    def test_geometric_shape_tensor_params(self):
        geometric = Geometric(torch.tensor([[0.6, 0.3], [0.6, 0.3], [0.6, 0.3]]).to('mlu'))
        ...
        self.assertEqual(geometric.log_prob(torch.ones(3, 1, 1).mlu()).size(),
                         torch.Size((3, 3, 2)))
    ...
    def test_categorical_shape(self):
        ...
        self.assertEqual(dist.log_prob(torch.ones(3, 1).mlu()).size(), torch.Size((3, 1)))
        # batched
        ...
        self.assertEqual(dist.log_prob(self.tensor_sample_2).size(), torch.Size((3, 2, 3)))
        self.assertEqual(dist.log_prob(torch.ones(3, 1).mlu()).size(), torch.Size((3, 3)))
    ...
    def test_one_hot_categorical_shape(self):
        ...
        self.assertEqual(dist.sample((3, 2)).size(), torch.Size((3, 2, 3)))
        self.assertRaises(ValueError, dist.log_prob, self.tensor_sample_1)
        sample = torch.tensor([0., 1., 0.]).to('mlu').expand(3, 2, 3)
        ...
        # batched
        dist = OneHotCategorical(torch.tensor([[0.6, 0.3], [0.6, 0.3], [0.6, 0.3]]).mlu())
        self.assertEqual(dist._batch_shape, torch.Size((3,)))
        self.assertEqual(dist._event_shape, torch.Size((2,)))
        self.assertEqual(dist.sample().size(), torch.Size((3, 2)))
        self.assertEqual(dist.sample((3, 2)).size(), torch.Size((3, 2, 3, 2)))
        sample = torch.tensor([0., 1.]).to('mlu')
        self.assertEqual(dist.log_prob(sample).size(), torch.Size((3,)))
        self.assertRaises(ValueError, dist.log_prob, self.tensor_sample_2)
        self.assertEqual(dist.log_prob(dist.enumerate_support()).size(), torch.Size((2, 3)))
        sample = torch.tensor([0., 1.]).to('mlu').expand(3, 1, 2)
        self.assertEqual(dist.log_prob(sample).size(), torch.Size((3, 3)))

    def test_mixture_same_family_shape(self):
        dist = MixtureSameFamily(Categorical(torch.rand(5).to('mlu')),
    ...
    def test_vonmises_shape_tensor_params(self):
        von_mises = VonMises(torch.tensor([0., 0.]).to('mlu'), torch.tensor([1., 1.]).to('mlu'))
        ...
        self.assertEqual(von_mises.log_prob(torch.ones(2, 1).to('mlu')).size(), torch.Size((2, 2)))

    def test_normal_shape_tensor_params(self):
        normal = Normal(torch.tensor([0., 0.]).to('mlu'), torch.tensor([1., 1.]).to('mlu'))
        ...
        self.assertEqual(normal.log_prob(torch.ones(2, 1).to('mlu')).size(), torch.Size((2, 2)))

    def test_uniform_shape_tensor_params(self):
        uniform = Uniform(torch.tensor([0., 0.]).to('mlu'), torch.tensor([1., 1.]).to('mlu'))
        ...
        self.assertEqual(uniform.log_prob(torch.ones(2, 1).to('mlu')).size(), torch.Size((2, 2)))
    ...
    def test_exponential_shape_tensor_param(self):
        expon = Exponential(torch.tensor([1., 1.]).to('mlu'))
        ...
        self.assertEqual(expon.log_prob(torch.ones(2, 2).to('mlu')).size(), torch.Size((2, 2)))

    def test_laplace_shape_tensor_params(self):
        laplace = Laplace(torch.tensor([0., 0.]).to('mlu'), torch.tensor([1., 1.]).to('mlu'))
        ...
        self.assertEqual(laplace.log_prob(torch.ones(2, 1).to('mlu')).size(), torch.Size((2, 2)))
    ...
    def test_continuous_bernoulli_shape_tensor_params(self):
        ...
        self.assertEqual(continuous_bernoulli.log_prob(self.tensor_sample_1).size(),
                         torch.Size((3, 2)))
        self.assertRaises(ValueError, continuous_bernoulli.log_prob, self.tensor_sample_2)
        self.assertEqual(continuous_bernoulli.log_prob(torch.ones(3, 1, 1).to('mlu')).size(),
                         torch.Size((3, 3, 2)))
...
class TestKL(TestCase):
    ...
    def setUp(self):
        ...
        onehotcategorical = pairwise(OneHotCategorical, [[0.4, 0.3, 0.3],
                                                         ...
                                                         [0.33, 0.33, 0.34],
                                                         [0.2, 0.2, 0.6]])
        pareto = (Pareto(torch.tensor([2.5, 4.0, 2.5, 4.0]).to('mlu').expand(4, 4),
                         torch.tensor([2.25, 3.75, 2.25, 3.75]).to('mlu').expand(4, 4)),
                  Pareto(torch.tensor([2.25, 3.75, 2.25, 3.8]).to('mlu').expand(4, 4),
                         torch.tensor([2.25, 3.75, 2.25, 3.75]).to('mlu').expand(4, 4)))
        ...
        self.infinite_examples = [
            (Categorical(torch.tensor([0.9, 0.1]).to('mlu')),
             Categorical(torch.tensor([1., 0.]).to('mlu'))),
            (Categorical(torch.tensor([[0.9, 0.1], [.9, .1]]).to('mlu')),
             Categorical(torch.tensor([1., 0.]).to('mlu')))
        ]
        ...
        for (p, _), (_, q) in self.finite_examples:
            ...
            while denominator < self.max_samples:
                ...
                if error[~torch.isnan(error)].max() < self.precision:
                    break
            self.assertLess(error[~torch.isnan(error)].max(), self.precision, '\n'.join([
    ...
    def test_kl_edgecases(self):
        self.assertEqual(kl_divergence(Categorical(torch.tensor([0., 1.]).to('mlu')),
                                       Categorical(torch.tensor([0., 1.]).to('mlu'))), 0)
        ...
        for Dist, params in EXAMPLES:
            for i, param in enumerate(params):
                ...
                try:
                    ...
                    self.assertNotRegex(str(e), ERR_MSG_OP_NOT_IMPLEMENTED_ON_MLU, str(dist))
                    continue
                expected_shape = dist.batch_shape if dist.batch_shape else torch.Size()
...
class TestNumericalStability(TestCase):
        ...
        for tensor_type in [torch.FloatTensor]:
            ...
            self._test_pdf_score(dist_class=Bernoulli,
                                 ...
                                 expected_value=tensor_type(
                                    [torch.finfo(tensor_type([]).dtype).eps]).log(),
                                 expected_gradient=tensor_type([0]))
    ...
    def test_categorical_log_prob_with_logits(self):
        for dtype in ([torch.float]):
            ...
            log_pdf_prob_1 = categorical.log_prob(torch.tensor([0, 1], dtype=dtype).mlu())
            self.assertEqual(log_pdf_prob_1.item(), 0)
            log_pdf_prob_0 = categorical.log_prob(torch.tensor([1, 0], dtype=dtype).mlu())
    ...
    def test_continuous_bernoulli_gradient(self):
        ...
        def expec_val(x, probs=None, logits=None):
            ...
            if probs < 0.499 or probs > 0.501:  # using default values of lims here
                log_norm_const = math.log(
                    math.fabs(math.atanh(1. - 2. * probs))) - \
        ...
        def expec_grad(x, probs=None, logits=None):
            ...
            if probs < 0.499 or probs > 0.501:  # using default values of lims here
                grad_log_c = 2. * probs - 4. * (probs - 1.) * probs * \
                    math.atanh(1. - 2. * probs) - 1.
                grad_log_c /= 2. * (probs - 1.) * probs * (2. * probs - 1.) * \
                    math.atanh(1. - 2. * probs)
            else:
        ...
        for tensor_type in [torch.FloatTensor]:
            ...
            self._test_pdf_score(dist_class=ContinuousBernoulli,
                                 ...
                                 expected_gradient=tensor_type(
                                     tensor_type([expec_grad(1, probs=1e-4)])),
                                 atol=1e-3)
            ...
            self._test_pdf_score(dist_class=ContinuousBernoulli,
                                 ...
                                 expected_gradient=tensor_type(
                                     [expec_grad(0, logits=math.log(9999))]),
                                 atol=1e-3)
...
class TestLazyLogitsInitialization(TestCase):
    def setUp(self):
        ...
        self.examples = [e for e in EXAMPLES if e.Dist in
                         (Categorical, OneHotCategorical, Bernoulli)]
    ...
    def setUp(self):
        ...
        self.distribution_pairs = [
            (
                ...
                scipy.stats.bernoulli(simplex_tensor.cpu())
            ),
            (
            ...
            (
                ...
                scipy.stats.geom(simplex_tensor.cpu(), loc=-1)
            ),
            (
            ...
            (
                ...
                scipy.stats.lognorm(s=positive_var.cpu().clamp(max=3),
                                    scale=random_var.cpu().exp())
            ),
            (
    ...
    def test_mean(self):
        for pytorch_dist, scipy_dist in self.distribution_pairs:
            if isinstance(pytorch_dist, (Cauchy, HalfCauchy)):
                ...
                self.assertEqual(pytorch_dist.mean, scipy_dist.mean().astype('float32'), msg=pytorch_dist)
    ...
    def test_variance_stddev(self):
        for pytorch_dist, scipy_dist in self.distribution_pairs:
            if isinstance(pytorch_dist, (Cauchy, HalfCauchy, VonMises)):
                ...
                self.assertEqual(pytorch_dist.variance,
                                 np.diag(scipy_dist.cov().astype('float32')), msg=pytorch_dist)
                self.assertEqual(pytorch_dist.stddev,
                                 np.diag(scipy_dist.cov().astype('float32')) ** 0.5, msg=pytorch_dist)
            elif isinstance(pytorch_dist, (LowRankMultivariateNormal, MultivariateNormal)):
                self.assertEqual(pytorch_dist.variance, np.diag(scipy_dist.cov), msg=pytorch_dist)
                self.assertEqual(pytorch_dist.stddev,
                                 np.diag(scipy_dist.cov) ** 0.5, msg=pytorch_dist)
            else:
                self.assertEqual(pytorch_dist.variance, scipy_dist.var().astype('float32'), msg=pytorch_dist)
                self.assertEqual(pytorch_dist.stddev, scipy_dist.var().astype('float32') ** 0.5, msg=pytorch_dist)
    ...
    def test_cdf(self):
        for pytorch_dist, scipy_dist in self.distribution_pairs:
            ...
            try:
                ...
                self.assertNotRegex(str(e), ERR_MSG_OP_NOT_IMPLEMENTED_ON_MLU)
                continue
            self.assertEqual(cdf, scipy_dist.cdf(samples.cpu()).astype('float32'), msg=pytorch_dist)
    ...
    def test_icdf(self):
        for pytorch_dist, scipy_dist in self.distribution_pairs:
            ...
            try:
                ...
                self.assertNotRegex(str(e), ERR_MSG_OP_NOT_IMPLEMENTED_ON_MLU)
                continue
            self.assertEqual(icdf, scipy_dist.ppf(samples).astype('float32'), atol=1e-4, rtol=0, msg=pytorch_dist)
...
class TestFunctors(TestCase):
    ...
    def test_cat_transform_non_uniform(self):
        x1 = -1 * torch.arange(1, 101, dtype=torch.float).mlu().view(-1, 100)
        x2 = torch.cat([(torch.arange(1, 101, dtype=torch.float).mlu().view(-1, 100) - 1) / 100,
        ...
        self.assertEqual(expected, actual)
        y1 = torch.arange(1, 101, dtype=torch.float).mlu().view(-1, 100)
        y2 = torch.cat([torch.arange(1, 101, dtype=torch.float).mlu().view(-1, 100),
                        torch.arange(1, 101, dtype=torch.float).mlu().view(-1, 100)])
        ...
        for Dist, params in BAD_EXAMPLES:
            for i, param in enumerate(params):
                try:
                    ...
                    raise AssertionError(fail_string.format(Dist.__name__, i + 1,
                                                            len(params))) from e
