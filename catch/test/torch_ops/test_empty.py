...
class TestEmpty(TestCase):
    ...
    @testinfo()
    def test_empty(self):
        shape_memory_format_list = [((2, 3, 4, 5), torch.channels_last, False),
                                    ...
                                    ((2, 3, 0, 5), torch.channels_last, True),
                                   ]
        for shape, memory_format, pin_memory in shape_memory_format_list:
            x_mlu = torch.empty(shape, memory_format=memory_format, \
                                device='mlu')
            x_mlu_to_cpu = torch.empty(shape, memory_format=memory_format, \
                                       device='mlu').cpu()
            x_cpu = torch.empty(shape, memory_format=memory_format, pin_memory=pin_memory)
            ...
            if pin_memory is True:
                self.assertTrue(x_cpu.is_pinned(), "create cpu pin memory tensor failed.")
