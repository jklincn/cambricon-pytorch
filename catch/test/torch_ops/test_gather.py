...
class TestGatherOp(TestCase):
    ...
    def test_gather(self):
        shapes = [(32, 12, 24, 32, 16), (32, 3, 224, 224),
                  (2, 100, 56), (234, 32), (0, 32), (24,)]
        for shape in shapes:
        ...
        for shape in shapes:
            for dim in dims:
                ...
                run_test(x.to(memory_format = torch.channels_last),
                         x_mlu.to(memory_format = torch.channels_last), dim, index)
    ...
    def test_gather_large(self):
        ...
        for shape, dtype in product(*list_list):
            ...
            index_mlu = self.to_device(index)
            out_mlu = torch.gather(x_mlu, dim, index_mlu)
            self.assertTensorsEqual(out.float(), out_mlu.cpu().float(),
                              0.003, use_MSE = True)
