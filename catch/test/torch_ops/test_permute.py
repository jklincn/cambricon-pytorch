...
class TestPermuteOps(TestCase):
    ...
    def test_permute(self):
        shape_permute_list = [[(3, 224, 224), (0, 2, 1)],
                              [(2, 3, 224, 224), (0, 3, 1, 2)],
                              [(2, 3, 224, 224), (-4, -1, -3, -2)],
                              [(2, 3, 224, 224), (0, 1, 2, 3)],
        ...
        for shape_permute, dtype_err, func in \
          product(shape_permute_list, data_types, func_list):
            ...
            self.assertTensorsEqual(output_cpu.float(), output_mlu.cpu().float(), err, use_MSE=True)
    ...
    def test_permute_bfloat16(self):
        ...
        self.assertTensorsEqual(
            out_cpu, out_mlu.cpu(), 0, use_MSE=True)
        self.assertTensorsEqual(
            input_cpu.grad, input_mlu.grad.cpu(), 0, use_MSE=True)
