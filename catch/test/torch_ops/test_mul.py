...
from common_utils import (
    testinfo, run_tests, TestCase, TEST_LARGETENSOR, TEST_BFLOAT16, largeTensorTest)
...
class TestMulOps(TestCase):
    ...
    def test_mul(self):
        dtype_list = [(torch.float, 3e-3), (torch.half, 3e-3),
                      (torch.int, 3e-3), (torch.short, 3e-3),
                      (torch.int8, 3e-3), (torch.uint8, 3e-3),
                      (torch.double, 3e-3), (torch.long, 3e-3),
                      (torch.bool, 0)]
        shape_list = [((1, 3, 224, 224), (1, 3, 224, 224)),
                       ((1, 3, 224, 224), (1, 3, 1, 1)),
        ...
        for shape, dtype_err, func in product(shape_list, dtype_list, func_list):
            x_left = torch.testing.make_tensor(shape[0], dtype=dtype_err[0], device='cpu')
            x_right = torch.testing.make_tensor(shape[0], dtype=dtype_err[0], device='cpu')
            ...
            self.assertTensorsEqual(out_cpu.float(), out_mlu1.cpu().float(), \
                                    dtype_err[1], use_MSE=True)
            self.assertTensorsEqual(out_cpu.float(), out_mlu2.cpu().float(), \
                                    dtype_err[1], use_MSE=True)
    ...
    def test_mul_inplace(self):
        ...
        for shape, dtype_err, func in product(shape_list, dtype_list, func_list):
            ...
            self.assertTensorsEqual(x_left_cpu.float(), x_left_mlu1.cpu().float(), \
                                    dtype_err[1], use_MSE=True)
            self.assertTensorsEqual(x_left_cpu.float(), x_left_mlu2.cpu().float(), \
                                    dtype_err[1], use_MSE=True)
    ...
    def test_mul_inplace_intscalar(self):
        ...
        for input_t in type_list:
            if input_t is torch.half:
                ...
                input_self_cpu = torch.normal(
                    mean=20, std=torch.randn(20, dtype=torch.float).abs()).to(input_t)
    ...
    def test_mul_inplace_floatscalar(self):
        ...
        for data_type in data_types:
            input_self_cpu = torch.normal(mean=20,
                                          std=torch.randn(20, dtype=torch.float).abs())
        ...
        for scalar, type_t in product(scalar_list, type_list):
            if type_t is torch.half:
                input_self_cpu = torch.normal(mean=5,
                    std=torch.randn(5, dtype=torch.float).abs())
              ...
              # prevent data overflow
                ...
                input_self_cpu = torch.normal(mean=5,
                    std=torch.randn(5, dtype=torch.float).abs()).to(type_t)
    ...
    def test_mul_high_diemension_after_permute(self):
        ...
        for i in range(5):
            ...
            a = torch.randn(shape)
            b = torch.randn(shape)
            ouput = torch.mul(torch.permute(a, tuple(permute_size)),
    ...
    def test_mul_bfloat16(self):
        dtype_list = [(torch.float, torch.bfloat16, 3e-3), (torch.half, torch.bfloat16, 3e-3),
                      (torch.int, torch.bfloat16, 3e-3), (torch.short, torch.bfloat16, 3e-3),
                      (torch.int8, torch.bfloat16, 3e-3), (torch.uint8, torch.bfloat16, 3e-3),
                      (torch.double, torch.bfloat16, 3e-3), (torch.long, torch.bfloat16, 3e-3),
                      (torch.bool, torch.bfloat16, 3e-3), (torch.bfloat16, torch.bfloat16, 0)]
        for dtype_err in dtype_list:
            x_left = torch.testing.make_tensor((1, 3, 224, 224), dtype=dtype_err[0], device='cpu')
            x_right = torch.testing.make_tensor((1, 3, 224, 1), dtype=dtype_err[1], device='cpu')
            x_left_mlu = x_left.mlu()
            x_right_mlu = x_right.mlu()
    ...
    def test_mul_scalar_bfloat16(self):
        input = torch.testing.make_tensor((1, 3, 224, 224), dtype=torch.bfloat16, device='cpu')
        ...
        self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(), \
                                0.003, use_MSE=True)
        self.assertTensorsEqual(input_cpu.grad.float(), input_mlu.grad.cpu().float(), \
                                0.003, use_MSE=True)

    @testinfo()
    def test_mul_tensor_complex(self):
        dtype_list = [(torch.complex64, torch.complex64),
                      (torch.complex64, torch.complex128),
                      (torch.complex128, torch.complex128)]
        for dtype_err in dtype_list:
            x_left = torch.testing.make_tensor((1, 3, 224, 224), dtype=dtype_err[0], device='cpu')
            x_right = torch.testing.make_tensor((1, 3, 224, 224), dtype=dtype_err[1], device='cpu')
            x_left_mlu = x_left.mlu()
            x_right_mlu = x_right.mlu()
            out_cpu = torch.mul(x_left, x_right)
            out_mlu = torch.mul(x_left_mlu, x_right_mlu)
    ...
    def test_mul_tensor_complex_with_broadcast(self):
        ...
        for shape in shape_list:
            x_left = torch.testing.make_tensor(shape[0], dtype=torch.complex64, device='cpu')
            x_right = torch.testing.make_tensor(shape[1], dtype=torch.complex64, device='cpu')
            x_left_mlu = x_left.mlu()
            x_right_mlu = x_right.mlu()
            out_cpu = torch.mul(x_left, x_right)
            out_mlu = torch.mul(x_left_mlu, x_right_mlu)
    ...
    def test_mul_scalar_complex(self):
        ...
        for dtype_err in dtype_list:
            input_cpu = torch.testing.make_tensor((1, 3, 224, 224), dtype=dtype_err, device='cpu')
