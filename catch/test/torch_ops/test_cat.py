...
from common_utils import (
    testinfo, run_tests, TestCase, TEST_LARGETENSOR, TEST_BFLOAT16, largeTensorTest)  # pylint: disable=C0413,C0411
...
class TestCatOp(TestCase):
    ...
    def test_cat_type(self):
        ...
        for dtype in dtypes:
            ...
            inputs_mlu = [self.to_mlu_dtype(
                input1, dtype), self.to_mlu_dtype(input2, dtype)]
        ...
        for [in_shape1, in_shape2] in [in_shapes1, in_shapes2]:
            for dtype, err in dtypes:
                ...
                self.assertTensorsEqual(
                    output_cpu, output_mlu.cpu().float(), err)
    ...
    def test_cat_channel_last(self):
        ...
        for [in_shape1, in_shape2] in [in_shapes1]:
            for dtype, err in dtypes:
                ...
                self.assertTensorsEqual(
                    output_cpu, output_mlu_channels_first, err)
    ...
    def test_cat_out(self):
        ...
        for data_type, err in dtypes:
            ...
            out_mlu = self.to_mlu_dtype(torch.randn(
                (4, ), dtype=torch.float), data_type)
    ...
    def test_cat_promote(self):
        ...
        in_shapes4 = [(0), (0), (0)]
        dtypes1 = [torch.double, torch.int8, torch.float, torch.half,
                   torch.int, torch.short, torch.uint8]
        list_list = [dtypes1, dtypes1, dtypes1]
        for [in_shape1, in_shape2, in_shape3] in [in_shapes1,
                                                  in_shapes2, in_shapes3, in_shapes4]:
    ...
    def test_cat_zero_dim(self):
        ...
        input1_mlu = torch.randn(
            0, dtype=torch.float32).mlu().requires_grad_(True)
        input2_mlu = torch.randn(
            0, dtype=torch.float32).mlu().requires_grad_(True)
        ...
        self.assertTensorsEqual(
            output_cpu.float(), output_mlu.cpu().float(), 0.0)
