...
class TestDivideOp(TestCase):
    ...
    def test_divide_tensor_tensor(self):
        ...
        for data_type, channel, mode in itertools.product(*param_list):
            ...
            for shape1, shape2 in [((1, 3, 224, 224), (1, 3, 224, 1)),
                ...
                out_mlu = torch.divide(x_cpu.to("mlu"), y_cpu.to("mlu"), rounding_mode=mode)
    ...
    def test_divide_not_contiguous_tensor_tensor(self):
        ...
        for data_type, err in dtype_list:
            for shape1, shape2 in [((1, 10, 224, 224), (1, 10, 224, 1)),
                ...
                out_mlu = torch.divide(self.to_mlu_dtype(x_, data_type)[:,2:8,5:60],
                ...
                torch.divide(self.to_mlu_dtype(x_, data_type)[:,2:8,5:60],
                          self.to_mlu_dtype(y_, torch.float)[:,2:8,5:60],
                          out=output_mlu)
    ...
    def test_divide_tensor_tensor_channel_last(self):
        ...
        for data_type, func_x, func_y in itertools.product(*param_list):
            for shape1, shape2 in [((224, 224), (1, 10, 224, 1)),
                ...
                out_mlu = torch.divide(func_x(self.to_mlu_dtype(x__, data_type)),
    ...
    def test_divide_high_diemention_after_permute(self):
        ...
        for i in range(5):
            ...
            ouput_ture = torch.divide(torch.permute(a, tuple(permute_size)),
                                                            torch.permute(b, tuple(permute_size)))
            output_floor = torch.divide (torch.permute(a, tuple(permute_size)),
                                                                torch.permute(b, tuple(permute_size)), # pylint: disable=C0301
                                                                rounding_mode="floor")
            output_trunc = torch.divide (torch.permute(a, tuple(permute_size)),
                                                                torch.permute(b, tuple(permute_size)), # pylint: disable=C0301
                                                                rounding_mode="trunc")
    ...
    def test_divide_backward(self):
        ...
        for shape in shape_list:
            for data_type in type_list:
                ...
                out_cpu = torch.divide(x_0, y_0)
                out_mlu = torch.divide(x_mlu, y_mlu)
    ...
    def test_floor_divide_large(self):
        ...
        for rounding_mode in rounding_modes:
            ...
            out_mlu = torch.divide(x_cpu.to("mlu"), y_cpu.to("mlu"), rounding_mode=rounding_mode)
            self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float().contiguous(),
                                        3e-3, use_MSE=True)
