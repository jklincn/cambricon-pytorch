...
class TestTrigonOps(TestCase):
    ...
    def test_trigon(self):
        ...
        for shape, type_err, func, trigon in product(shape_list, type_list, func_list, trigon_list):
            ...
            self.assertTensorsEqual(x_cpu, x_mlu.cpu().float(), type_err[1], use_MSE=True,
                                    message = trigon[3])
    ...
    def test_trigon_inplace(self):
        ...
        type_list = [[torch.float, 3e-3], [torch.half, 1e-2], [torch.double, 3e-3]]
        func_list = [self.convert_to_channel_last, self.to_non_dense, lambda x:x]
        for shape, type_err, func, trigon_ in product(shape_list, type_list,
                                                      func_list, trigon_list_):
            if trigon_[1] == trigon_[2] == -1:
                x_cpu = torch.randn(shape, dtype=type_err[0])
            else:
                x_cpu = torch.empty(shape, dtype=type_err[0]).uniform_(trigon_[1], trigon_[2])
            ...
            self.assertTensorsEqual(x_cpu.float(), x_mlu.cpu().float(), type_err[1], use_MSE=True,
                                    message = trigon_[3])
            self.assertEqual(x_mlu.is_contiguous(memory_format = torch.preserve_format),
                            x_cpu.is_contiguous(memory_format = torch.preserve_format),
                            msg = trigon_[3])
            self.assertEqual(x_mlu.is_contiguous(memory_format = torch.channels_last),
                            x_cpu.is_contiguous(memory_format = torch.channels_last),
                            msg = trigon_[3])
            self.assertEqual(x_mlu.is_contiguous(memory_format = torch.contiguous_format),
    ...
    def test_trigon_out(self):
        ...
        type_list = [[torch.float, 3e-3], [torch.half, 1e-2], [torch.double, 3e-3]]
        func_list = [self.convert_to_channel_last, self.to_non_dense, lambda x:x]
        for shape, out_shape, type_err, func, func_out, trigon in product(shape_list,
                        out_shape_list, type_list, func_list, func_list, trigon_list):
            if trigon[1] == trigon[2] == -1:
                x = torch.randn(shape, dtype=type_err[0])
            else:
                x = torch.empty(shape, dtype=type_err[0]).uniform_(trigon[1], trigon[2])
            ...
            x = func(x)
            x_mlu = func(x_mlu)
            out_mlu = torch.randn(out_shape, dtype=type_err[0]).to(ct.mlu_device())
            out_cpu = func_out(out_cpu)
            out_mlu = func_out(out_mlu)
            trigon[0](x.float(), out=out_cpu)
            trigon[0](x_mlu, out=out_mlu)
            self.assertTensorsEqual(out_cpu.float(), out_mlu.cpu().float(), type_err[1],
                                    use_MSE=True, message = trigon[3])
    ...
    def test_trigon_special_case(self):
        ...
        shape_out_list = [(), (5,0)]
        shape_list = [(2), (0, 6), ()]
        for shape, shape_out, type_err, trigon in product(shape_list, shape_out_list,
                                                          type_list, trigon_list):
            if trigon[1] == trigon[2] == -1:
                x = torch.randn(shape, dtype=type_err[0])
            else:
                x = torch.empty(shape, dtype=type_err[0]).uniform_(trigon[1], trigon[2])
            out_cpu = torch.randn(shape_out)
            out_mlu = torch.randn(shape_out, dtype=type_err[0]).to(ct.mlu_device())
            trigon[0](x.float(), out = out_cpu)
            trigon[0](self.to_mlu(x), out = out_mlu)
            self.assertTrue(out_cpu.stride() == out_mlu.stride(), trigon[3])
            self.assertTrue(out_mlu.storage_offset() == out_cpu.storage_offset(), trigon[3])
            self.assertTensorsEqual(out_cpu, out_mlu.cpu().float(), type_err[1], use_MSE=True,
                                    message = trigon[3])
            self.assertEqual(out_mlu.is_contiguous(memory_format = torch.preserve_format),
                            out_cpu.is_contiguous(memory_format = torch.preserve_format),
                            msg = trigon[3])
            self.assertEqual(out_mlu.is_contiguous(memory_format = torch.channels_last),
                            out_cpu.is_contiguous(memory_format = torch.channels_last),
                            msg = trigon[3])
            self.assertEqual(out_mlu.is_contiguous(memory_format = torch.contiguous_format),
    ...
    def test_trigon_permute(self):
        type_list = [[torch.float, 3e-3], [torch.half, 1e-2], [torch.double, 3e-3]]
        shape_list = [[(512, 1024, 2, 2, 4),(2, 4, 3, 1, 0)],
                                    [(2, 3, 4), (2, 1, 0)],
        ...
        for shape, type_err, trigon in product(shape_list, type_list, trigon_list):
            if trigon[1] == trigon[2] == -1:
                ...
                x = torch.empty(shape[0], dtype=type_err[0]).uniform_(trigon[1],
                                                                      trigon[2]).permute(shape[1])
