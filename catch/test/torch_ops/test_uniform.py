...
class TestUniformOp(TestCase):
    ...
    def test_uniform_orig(self):
        ...
        for layout in layout_list:
            for data_type in data_types:
                ...
                self.assertLessEqual(torch.max(m.to(torch.float)), 1,
                                    "cpu uniform are not right")
                # MLU cast is not support complex.
                self.assertLessEqual(torch.max(m_mlu.cpu().float()), 1,
                                    "mlu uniform are not right")
                self.assertLessEqual(0, torch.min(m.to(torch.float)),
                                    "cpu uniform are not right")
                self.assertLessEqual(0, torch.min(m_mlu.cpu().float()),
                                    "mlu uniform are not right")
    ...
    def test_uniform_range(self):
        ...
        for shape in shape_list:
            for scope in range_list:
                ...
                self.assertLessEqual(torch.max(m), scope[1], "cpu uniform are not right")
                self.assertLessEqual(torch.max(m_mlu.cpu()), scope[1],
					"mlu uniform are not right")
                self.assertLessEqual(scope[0], torch.min(m), "cpu uniform are not right")
                self.assertLessEqual(scope[0], torch.min(m_mlu.cpu()),
					"mlu uniform are not right")
                self.assertEqual(mlu_data_ptr_orig, m_mlu.data_ptr())
        ...
        for _ in range(TEST_NUM):
            for shape in shape_list:
                for scope in range_list:
                    ...
                    p_value = stats.kstest(m_mlu.cpu().numpy().reshape(-1), 'uniform', args=scope,
                                           N=w.numel(), alternative='less')
                    if p_value[1] <= 0.005:
        ...
        for _ in range(TEST_NUM):
            for layout in layout_list:
                for scope in range_list:
                    ...
                    # use kstest to test the data is uniform distribution or not
                    p_value = stats.kstest(m_mlu.cpu().numpy().reshape(-1), 'randint', args=scope,
                                           N=w.numel(), alternative='less')
                    if p_value[1] <= 0.005:
                        failure_count = failure_count + 1
        self.assertTrue(failure_count < total_count * 0.1)
    ...
    def test_uniform_exceptions(self):
        ...
        msg = "uniform_ expects to return a [from, to) range, "\
        ...
        msg = "uniform_ expects to-from <= std::numeric_limits<Float>::max(), "\
