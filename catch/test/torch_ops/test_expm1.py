...
class TestExpm1Op(TestCase):
    ...
    def test_expm1(self):
        ...
        shape_list = [(10, 12, 10, 13), (2, 10, 15), (0, 3, 2, 1), (2, 0, 1),
                      (1, 3, 6, 2, 4)]
        func_list = [self.convert_to_channel_last, self.to_non_dense, lambda x:x]
        for (dtype, err), shape, func in itertools.product(dtype_list, shape_list, func_list):
    ...
    def test_expm1_permute(self):
        ...
        for i in range(4):
            ...
            x_mlu, out_mlu = x.mlu().permute(permute_shape[i]), out.mlu().permute(permute_shape[i])
            torch.expm1(x_cpu, out=out_cpu)
            torch.expm1(x_mlu, out=out_mlu)
        ...
        for shape in shape_list:
            for data_type in type_list:
                ...
                self.assertTensorsEqual(
                    out_grad_cpu,
                    out_grad_mlu.cpu().float() if data_type == torch.half else out_grad_mlu.cpu(),
                    0.003,
                    use_MSE=True)
