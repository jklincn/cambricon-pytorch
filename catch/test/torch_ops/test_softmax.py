...
class TestOps(TestCase):
    ...
    def test__softmax_half_to_float(self):
        ...
        shapes = [(2, 3, 5), (7, 8, 9, 10), (2, 0, 3, 5), (10,), (10, 15), (10, 20, 30, 40, 50)]
        for shape, option in product(*[shapes, [True, False]]):
            for dim in range(len(shape)):
                ...
                self.assertTensorsEqual(grad_cpu, grad_mlu.cpu().float(), 0.003, use_MSE=True)

    #@unittest.skip("not test")
    @testinfo()
    def test__softmax_half_to_float_out(self):
        _softmax = torch._softmax
        shapes = [[(2, 3, 5), (2, 3, 5)], [(7, 8, 9, 10), (9, 4)],
                  [(10,), (3, 5, 7)], [(2, 0, 3, 5), (9, 4)], [(10, 15), (10, 15)]]
    ...
    def test_softmax(self):
        shapes = [(2, 3, 4, 5, 7, 8, 9, 11), (2, 3, 4, 5), (2, 0, 3, 5),
                  (2, 3, 4), (2, 3), (2, )]
    ...
    def test__log_softmax_half_to_float_out(self):
        ...
        shapes = [[(2, 3, 5), (2, 3, 5)], [(7, 8, 9, 10), (9, 4)],
                  [(10,), (3, 5, 7)], [(2, 0, 5, 3), (9, 4)], [(10, 15), (10, 15)]]
        for [shape, out_shape], option in product(*[shapes, [True, False]]):
            for dim in range(len(shape)):
                x = torch.randn(shape, dtype=torch.float)
                out_cpu = torch.randn(out_shape, dtype=torch.float)
    ...
    def test_logsoftmax_and_softmax_backward_non_dense(self):
        ...
        for func in [torch.ops.aten._log_softmax_backward_data, torch.ops.aten._softmax_backward_data]:
            ...
            device_out = func(to_non_dense(grad_output.mlu(), grad_output.dim() // 2),
                              to_non_dense(output.mlu(), output.dim() // 2), 1, torch.float32)
    ...
    def test_softmax_bfloat16(self):
        # log softmax is not support now.
        softmax_list = [torch._softmax, torch.special.softmax]
        #               torch._log_softmax, torch.special.log_softmax]
