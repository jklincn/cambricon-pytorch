...
class TestOps(TestCase):
    ...
    def test_repeat_interleave(self):
        ...
        # test output_size
        repeats_out_size_list = [(torch.tensor([1, 2]), 3), (torch.tensor(
            [1, 4]), 5), (torch.tensor([2, 4]), 6), (torch.tensor([3, 5]), 8)]
        for repeats, output_size in repeats_out_size_list:
            y3_cpu = torch.repeat_interleave(
                y, repeats, dim=0, output_size=output_size)
        ...
        self.assertTensorsEqual(
            y_cpu.float(), y_mlu.cpu().float(), 3e-4, use_MSE=True)
    ...
    def test_repeat_interleave_shape_limit(self):
        ...
        output_mlu = self.to_mlu(
            channels_last_input1).repeat_interleave(repeats.to('mlu'))
