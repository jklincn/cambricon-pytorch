...
class TestLayerNormOp(TestCase):
    ...
    def test_layer_norm_backward(self):
        ...
        for param in gen_params:
            ...
            layer_norm_mlu = LayerNormModel(
                param[0], weight_mlu, bias_mlu).mlu()
    ...
    def test_layer_norm_backward_not_dense(self):
        # init: param | input: tensor
        gen_params = [((1024*2), (37, 104, 1024*2)),
                      ((500*2), (32, 100, 500*2)), ((4*2), (1, 4, 4*2)),
                      ((1024*2), (32, 1, 1024*2)),
        ...
        for param in gen_params:
            ...
            weight_cpu = copy.deepcopy(weight)\
              [...,:int(param[0]/2)].requires_grad_()
            bias_cpu = copy.deepcopy(bias)\
              [...,:int(param[0]/2)].requires_grad_()
            input_cpu = copy.deepcopy(input_)\
              [...,:int(param[1][-1]/2)].requires_grad_()
            grad_cpu = copy.deepcopy(grad)[...,:int(param[1][-1]/2)]
            weight_mlu = copy.deepcopy(weight).mlu()\
              [...,:int(param[0]/2)].requires_grad_()
            bias_mlu = copy.deepcopy(bias).mlu()\
              [...,:int(param[0]/2)].requires_grad_()
            ...
            layer_norm_mlu = LayerNormModel(
                [int(param[0]/2)], weight_mlu, bias_mlu).mlu()
            ...
            self.assertTrue(input_grad_cpu.storage_offset() == input_grad_mlu.storage_offset())
    ...
    def test_layer_norm_backward_module(self):
        ...
        gen_params = [((1024), (37, 104, 1024), True, False),
        ...
        for param in gen_params:
            for t in dtypes:
                ...
                layer_norm = torch.nn.LayerNorm(param[0], elementwise_affine=param[3]).to(t)
                out_cpu = layer_norm(input_cpu)
    ...
    def test_layer_norm_backward_large(self):
        ...
        for param in gen_params:
            weight = torch.randn(
                param[0], dtype=dtype, requires_grad=True)
            bias = torch.randn(param[0], dtype=dtype, requires_grad=True)
            weight_mlu = copy.deepcopy(weight)
            bias_mlu = copy.deepcopy(bias)
            input_ = torch.randn(
                param[1], dtype=dtype, requires_grad=True)
            input_mlu = copy.deepcopy(input_)
