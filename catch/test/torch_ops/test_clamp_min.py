...
class TestClampMinOp(TestCase):
    ...
    def test_clamp_min(self):
        ...
        product_list = product(shape_list,
                               dtype_list,
                               min_list)
        for shape, dtype, min_, in product_list:
            if dtype == torch.half:
            ...
            out_cpu = torch.clamp_min(x, min_,)
            out_mlu = torch.clamp_min(self.to_mlu_dtype(x, dtype), min_,)
    ...
    def test_clamp_min_tensor(self):
        ...
        for (shape1, shape2) in shape_list:
            ...
            out_cpu = torch.clamp_min(x, min_,)
            out_mlu = torch.clamp_min(x.to('mlu'), min_.to('mlu'),)
            self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0)
