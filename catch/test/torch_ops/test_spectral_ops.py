...
class TestSpectralOps(TestCase):
    def _test_stft(self):
        def librosa_stft(x, n_fft, hop_len, center, pad_mode, normalized):
            ...
            result = []
            for xi in x:
                ri = librosa.stft(xi.cpu().numpy(), n_fft=n_fft, hop_length=hop_len,
                                  window=np.ones(n_fft), center=center, pad_mode=pad_mode)
        ...
        list_list = [in_sizes_list, dtype_list, n_fft_list, hop_length_list,
                     center_list, pad_mode_list, normalized_list, onesided_list,
                     return_complex_list]
        for in_sizes, dtype, n_fft, hop_len, center, pad_mode, normalized, \
            onesided, return_complex in product(*list_list):
            ...
            result0 = x.stft(n_fft, hop_len, center=center, pad_mode=pad_mode,
                             normalized=normalized, onesided=onesided,
                             return_complex=return_complex)
            if result0.is_complex():
                result0 = torch.view_as_real(result0)
            self.assertTensorsEqual(result0.float().cpu(), ref_result.float(), 0.003, use_MSE=True)

            # test not contiguous
            result1 = self.to_non_dense(x).stft(n_fft, hop_len, center=center, pad_mode=pad_mode,
                            normalized=normalized, onesided=onesided,
                            return_complex=return_complex)
    ...
    def test_stft_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            o = torch.stft(x, 7, pad_mode="constant", onesided=False)   # pylint: disable=W0612
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            o = torch.stft(x, 4097, pad_mode="constant")   # pylint: disable=W0612
        x = torch.randn(0, 16).to("mlu")
        ref_msg = "currently do not support empty Tensor input"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            o = torch.stft(x, 8, center=False)   # pylint: disable=W0612
        x = torch.randn(10).to('mlu').half()
        ref_msg = r"CNNL FFT only supports dimensions whose sizes are powers of " + \
    ...
    def test_cnfft_plan_cache(self):
        ...
        with plan_cache_max_size(0, max(1, torch.backends.mlu.cnfft_plan_cache.size - 10)):
            self._test_stft()
        ...
        with self.assertRaisesRegex(RuntimeError, r"but got device with index"):
            torch.backends.mlu.cnfft_plan_cache[torch.mlu.device_count() + 10]  # pylint: disable=W0106
        ...
        if len(devices) > 1:
            ...
            x1 = x0.to(devices[1])
            self.assertTensorsEqual(torch.stft(x0, 7, pad_mode="constant").cpu(),
                                    torch.stft(x1, 7, pad_mode="constant").cpu(), 0)
            x0.copy_(x1)
            ...
            with plan_cache_max_size(devices[0], 10):
                with plan_cache_max_size(devices[1], 11):
                    ...
                    self.assertEqual(torch.backends.mlu.cnfft_plan_cache[1].max_size, 11)

                    self.assertEqual(torch.backends.mlu.cnfft_plan_cache.max_size, 10)  # default is mlu:0
                    with torch.mlu.device(devices[1]):
                        self.assertEqual(torch.backends.mlu.cnfft_plan_cache.max_size, 11)  # default is mlu:1
        ...
        for dtype in [torch.float, torch.double, torch.complex64]:
            ...
            for fwd, bwd in fft_functions:
                for x, dim, norm, func in test_args:
                    o = getattr(torch.fft, fwd)(func(x.mlu()), n=x.size(dim), dim=dim, norm=norm)
                    # The “backward”, “forward” values were added after numpy 1.20.0
                    if norm is None or norm == "ortho":
                        o_np = torch.from_numpy(getattr(np.fft, fwd)(func(x).numpy(),
                            n=x.size(dim), axis=dim, norm=norm))
                        self.assertTensorsEqual(torch.view_as_real(o).cpu(),
                            torch.view_as_real(o_np), 0.003, use_MSE=True)
                    y = getattr(torch.fft, bwd)(o, n=x.size(dim), dim=dim, norm=norm)
                    # The “backward”, “forward” values were added after numpy 1.20.0
                    if norm is None or norm == "ortho":
                        o_np = torch.from_numpy(getattr(np.fft, bwd)(o.cpu().numpy(),
                            n=x.size(dim), axis=dim, norm=norm))
                        y_real = torch.view_as_real(y) if y.is_complex() else y
                        o_np_real = torch.view_as_real(o_np) if o_np.is_complex() else o_np
                        self.assertTensorsEqual(y_real.cpu(), o_np_real, 0.003, use_MSE=True)
