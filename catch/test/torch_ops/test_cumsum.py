...
class TestCumsumOp(TestCase):
    ...
    def test_cumsum(self):
        shape_list = [(1, 2, 3, 4), (10, 10, 10, 10),
                      (100, 200), (3, 40, 32), (1111),
                      (99, 30, 40), (34, 56, 78, 90)]
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
              ...
              self.assertTensorsEqual(out_cpu, out_mlu_1.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
              self.assertTensorsEqual(out_cpu, out_mlu_2.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
              self.assertTensorsEqual(out_cpu, out_mlu_3.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
              self.assertTensorsEqual(y_cpu, y_mlu.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
              self.assertTensorsEqual(out_cpu_1, out_mlu_4.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
              self.assertTensorsEqual(out_cpu_1, out_mlu_5.cpu(), 0.003,
                                      use_MSE=True)  # float type precision : 0.003
    ...
    def test_cumsum_inplace(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            ...
            self.assertTensorsEqual(x, x_mlu.cpu(), 0.003,
                                      use_MSE=True)
    ...
    def test_cumsum_inplace_no_contiguous(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            ...
            x_mlu.cumsum_(dim_list[i])
            self.assertEqual(mlu_ptr, x_mlu.data_ptr())
            self.assertTensorsEqual(x_cpu, x_mlu.cpu(), 0.003,
                                      use_MSE=True)
    ...
    def test_cumsum_permute(self):
        ...
        for i in range(len(shape_list)):  # pylint: disable=C0200
            for t in dtypes:
                ...
                self.assertTrue(x_cpu_permute.storage_offset() == x_mlu_permute.storage_offset())
                self.assertTensorsEqual(out_cpu, out_mlu.cpu(), 0.003,
                                        use_MSE=True)
                x_cpu_permute.cumsum_(0)
                x_mlu_permute.cumsum_(0)
                self.assertTensorsEqual(x_cpu_permute, x_mlu_permute.cpu(), 0.003,
                                        use_MSE=True)
    ...
    def test_cumsum_channels_last(self):
        shape_list = [(1, 2, 3, 4), (10, 10, 10, 10),
                      (100, 200, 5,7), (3, 40, 32, 111),
                      (99, 30, 40, 45), (34, 56, 78, 90)]
        dim_list = [0, 1, -2, 2, -1, 1, 2]
        for i in range(len(shape_list)):  # pylint: disable=C0200
            x = torch.rand(shape_list[i], dtype=torch.float).to(memory_format = torch.channels_last)
    ...
    def test_cumsum_backward(self):
        ...
        for memory_format, shape, dim in product(*loop_var):  # pylint: disable=C0200
            x_cpu = torch.randn(shape, dtype=torch.float).to(memory_format=memory_format)
