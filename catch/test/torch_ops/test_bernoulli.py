...
class TestBenoulliOp(TestCase):
    ...
    def test_bernoulli(self):  # pylint: disable=R0201
        ...
        for j in range(3):
            ...
            for shape, pred, memory_format in product(*param):
                ...
                a = torch.randn(shape, dtype=torch.float).to(memory_format = memory_format)
                ...
                sucesses = a_cpu.sum()
                p_value = stats.binom_test(sucesses, n=count, p=pred, alternative='less')
    ...
    def test_bernoulli_tensor(self):  # pylint: disable=R0201
        ...
        for j in range(3):
            ...
            for shape, memory_format in product(*param):
                ...
                count = reduce(mul, shape)
                sucesses = a_cpu.sum()
                p_value = stats.binom_test(sucesses, n=count,
                                    p=torch.mean(a).item(), alternative='less')
                self.assertTrue(torch.ne(b, 0).mul_(torch.ne(b, 1)).sum().item() == 0)
                if p_value < 0.005:
                    failure_count += 1
        self.assertTrue(failure_count < 3)
    ...
    def test_bernoulli_p_broadcast(self):
        device = 'mlu'
        a = torch.rand((12, 3, 226, 226),
            dtype=torch.float).to(device).to(memory_format=torch.channels_last)
