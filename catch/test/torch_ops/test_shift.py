...
class TestShiftOps(TestCase):
    ...
    def test_left_and_right_shift(self):
        ...
        dtype_lst = (torch.uint8, torch.int8, torch.int16, torch.int32,
                     torch.int64)
        for dtype in dtype_lst:
            for shape1 in [
                ...
                for shift_num in shift_nums:
                    ...
                    shift_num_tensor = torch.Tensor([shift_num]).type(
                        torch.int32).cpu()
                    ...
                    result_mlu = torch.bitwise_left_shift(self.to_device(input_data), self.to_device(
                        shift_num))
                    self.assertTensorsEqual(result_cpu, result_mlu.cpu(), 0)
                    shift_num_tensor = torch.Tensor([shift_num]).type(
                        torch.int32).cpu()
                    ...
                    result_mlu = torch.bitwise_right_shift(self.to_device(input_data), self.to_device(
                        shift_num))
                    self.assertTensorsEqual(result_cpu, result_mlu.cpu(), 0)
                    shift_num_tensor = torch.Tensor([shift_num]).type(
                        torch.int32).cpu()
                    ...
                    self.assertTensorsEqual(result_data_cpu, data_on_mlu.cpu(),
                                            0)
                    ...
                    self.assertTensorsEqual(result_data_cpu_t,
                                            result_data_mlu_t.cpu(), 0)
                    # irshift
                    input_data = self._generate_tensor(shape1, dtype, 1, 16)
                    result_data_cpu = input_data.clone()
                    result_data_cpu >>= shift_num
                ...
                for shape1 in [
                    ...
                    for shift_num in shift_nums:
                        ...
                        result_cpu = self._shift_with_cpu(
                            input_data, shift_num)
                        result_mlu = self.to_device(
                            input_data) << self.to_device(shift_num)
                        self.assertTensorsEqual(result_cpu, result_mlu.cpu(),
                                                0)
                        shift_num_tensor = torch.Tensor([shift_num]).type(
                            torch.int32).cpu()
                        result_cpu = self._shift_with_cpu(
                            input_data, shift_num)
                        result_mlu = self.to_device(
                            input_data) << self.to_device(shift_num_tensor)
                        self.assertTensorsEqual(result_cpu, result_mlu.cpu(),
                                                0)
                        # rshift
                        result_cpu = self._shift_with_cpu(
                        ...
                        self.assertTensorsEqual(result_data_cpu,
                                                result_data_mlu.cpu(), 0)
