...
class TestQrOp(TestCase):
    ...
    def _test_qr(self, shape, some, device, dtype):
        qr_funcs = [(torch.qr, some), (torch.linalg.qr, "reduced" if some else "complete")]
        for qr_func in qr_funcs:
            cpu_dtype = dtype
            if dtype == torch.half:
            ...
            self.assertEqual(resq[..., :m].abs(), outq[..., :m].abs(), atol=3e-2, rtol=0)
            self.assertEqual(resr[..., :m].abs(), outr[..., :m].abs(), atol=3e-2, rtol=0)
    ...
    def _test_qr_not_dense(self, shape, some, device, dtype):
        ...
        for qr_func in qr_funcs:
            ...
            if len(shape) == 2:
                cpu_tensor = torch.randn(shape, device='cpu', dtype=torch.half)[:, :int(shape[-1] / 2)]
            elif len(shape) == 3:
                cpu_tensor = torch.randn(shape, device='cpu', dtype=torch.half)\
                        [:, :, :int(shape[-1] / 2)]
            elif len(shape) == 4:
    ...
    def test_linalg_qr(self):
        ...
        for mode in mode_list:
            ...
            out = (torch.empty((0), dtype=torch.float, device='mlu'),
