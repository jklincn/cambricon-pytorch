...
class ConvCat(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.cm = torch.nn.Conv2d(384, 128, 1,
                                  bias=False,
                                  stride=1,
                                  padding=0)
...
def get_tensor_cl_stride(input: torch.Tensor, replace_stride_value = False):
    ...
    flag = input.is_contiguous(memory_format=torch.channels_last) or \
    ...
    if dim == 4:
        stride1 = 1
        stride3 = input.size()[1] if (input.size()[3] != 1 or \
                replace_stride_value is False) else 1
        stride2 = stride3 * input.size()[3]
        stride0 = stride2 * input.size()[2] if stride2 != 1 else \
                input.size()[1]
        result = [stride0, stride1, stride2, stride3]
    else:
        stride1 = 1
        stride4 = input.size()[1] if (input.size()[4] != 1 or \
                replace_stride_value is False) else 1
...
def get_tensor_stride(input: torch.Tensor, replace_stride_value = False):
    ...
    if input.is_contiguous(memory_format=torch.channels_last) or \
            input.is_contiguous(memory_format=torch.channels_last_3d):
...
class TestOps(TestCase):
    ...
    def test_view_channels_last_unsafe(self):
        for in_shape, out_shape in [((64, 3, 24, 24), (1, -1)),
            ...
            self.assertTensorsEqual(y_cpu.float(), y_mlu.cpu().float(), 0)
            self.assertTrue(x_cpu.size() == x_mlu.size())
            self.assertTrue(get_tensor_stride(x_mlu) == x_mlu.stride() or \
                    get_tensor_stride(x_mlu, True) == x_mlu.stride())
            self.assertTensorsEqual(x_cpu.float(), x_mlu.cpu().float(), 0)
    ...
    def test_conv2d_and_view(self):
        ...
        for func in func_list:
            ...
            self.assertTrue(get_tensor_stride(x_mlu.grad) == x_mlu.grad.stride() or \
                    get_tensor_stride(x_mlu.grad, True) == x_mlu.grad.stride())
            self.assertTrue(w.grad.size() == w_mlu.grad.size())
            self.assertTrue(get_tensor_stride(w_mlu.grad) == w_mlu.grad.stride() or \
                    get_tensor_stride(w_mlu.grad, True) == w_mlu.grad.stride())
            self.assertTrue(output_cpu.size() == output_mlu.size())
            self.assertTrue(get_tensor_stride(output_mlu) == output_mlu.stride() or \
                    get_tensor_stride(output_mlu, True) == output_mlu.stride())
