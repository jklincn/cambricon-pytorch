...
class TestOps(TestCase):
    ...
    def test_kthvalue(self):
        ...
        shape_list = [(2, 3, 4, 2, 1, 7, 8), (2, 3, 4), (7, 300), (20, 26258), (3, 4, 5)]
        k_list = [5, 2, 4, 3, 2]
        dim_list = [6, 1, -1, 1, 0]
        ...
        for t in type_list:
            for i in range(len(shape_list)):  # pylint: disable=C0200
                ...
                out_mlu = torch.kthvalue(self.to_mlu(x), k_list[i], dim_list[i], keepdim[i])
    ...
    def test_kthvalue_out(self):
        ...
        for i in range(len(shape_list)): # pylint: disable=C0200
            ...
            if i != len(shape_list) - 1:
                ...
                torch.kthvalue(x_mlu, k_list[i], dim_list[i], keepdim[i], out=(y_mlu, index_mlu))
            else:
                torch.kthvalue(x, k_list[i], dim_list[i], keepdim[i], out=(y[:, 1], index[:, 1]))
                torch.kthvalue(x_mlu, k_list[i], dim_list[i], keepdim[i], out=(y_mlu[:, 1], index_mlu[:, 1]))
            self.assertTensorsEqual(y, y_mlu.cpu(), 0.0, use_MSE=True)
            self.assertTensorsEqual(index.float(), index_mlu.cpu().float(), 0.0, use_MSE=True)
