...
class TestAddmmOp(TestCase):
    ...
    def test_addmm_contiguous(self):
        ...
        res_cpu = torch.addmm(input=M, beta=beta, mat1=m1,
                              mat2=m2, alpha=alpha)
        res_mlu = torch.addmm(input=M.mlu(), beta=beta, mat1=m1.mlu(),
                              mat2=m2.mlu(), alpha=alpha)
        self.assertTensorsEqual(res_cpu[1], res_mlu.cpu().float(),
                                0.003, use_MSE=True)
            ...
            for M_shape, m1_shape, m2_shape in shape_list:
                for beta in beta_list:
                    for alpha in alpha_list:
                        ...
                        M_mlu.addmm_(mat1=m1_mlu, mat2=m2_mlu,
    ...
    def test_addmm_bfloat16(self):
        ...
        out_cpu = torch.addmm(input=mat_cpu, beta=0.4, mat1=a_cpu, mat2=b_cpu, alpha=1.7)
        out_mlu = torch.addmm(input=mat_mlu, beta=0.4, mat1=a_mlu, mat2=b_mlu, alpha=1.7)
