...
class TestTransposeOps(TestCase):
    ...
    def test_transpose(self):
        shape_list = [(126, 24, 1024), (4, 12, 45, 100), (4, 5, 6, 7, 8), (3, 4, 10, 200, 10, 20)]
        dim0_lst = [0, 1, 2]
        dim1_lst = [0, 1, 2]
        data_types = [(torch.float, 0.0), (torch.half, 0.0)]
        func_list = [lambda x:x, self.to_non_dense, self.convert_to_channel_last]
        for shape, dtype_err, func, dim0, dim1 in \
          product(shape_list, data_types, func_list, dim0_lst, dim1_lst):
            dtype, err = dtype_err
