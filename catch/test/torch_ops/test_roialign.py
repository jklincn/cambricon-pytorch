...
class TestRoiAlignOp(TestCase):
    ...
    def test_roi_align(self):
        input_shape = [(16,256,200,304), (16,256,100,152), (16,256,50,76), (16,256,25,38),
                       (16,256,200,304), (16,256,100,152), (16,256,50,76), (16,256,25,38)]
        boxes_shape = [(150,5), (740,5), (179,5), (20,5), (81,5), (1,5), (0,5), (0,5)]
        ...
        param = zip(input_shape, boxes_shape, output_size, spatial_scale, sampling_ratio)
        for i_shape, b_shape, o_size, s_scale, s_ratio in param:
            ...
            boxes_ = torch.tensor(list(zip(boxes_index, boxes_x1, boxes_y1,
                                           boxes_x2, boxes_y2))).float()
            if b_shape[0]==0:
                boxes_ = torch.randn((b_shape), dtype=torch.float)
            out = torchvision.ops.roi_align(a, boxes=boxes_, output_size=o_size,
                                            spatial_scale=s_scale, sampling_ratio=s_ratio)
            grad = torch.randn(out.shape, dtype=torch.float)
            out.backward(grad)
            a_mlu = a_.to('mlu')
            boxes_mlu = copy.deepcopy(boxes_).to('mlu')
            grad_mlu = copy.deepcopy(grad).to('mlu')
            out_mlu = torchvision.ops.roi_align(a_mlu, boxes=boxes_mlu, output_size=o_size,
                                                spatial_scale=s_scale, sampling_ratio=s_ratio)
    ...
    def test_roi_align_boxes_list(self):
        ...
        for i_shape, b_shape, o_size, s_scale, s_ratio in param:
            ...
            boxes_x2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
            boxes_y2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
            boxes1 = torch.tensor(list(zip(boxes_x1, boxes_y1, boxes_x2, boxes_y2))).float()
    ...
    def test_roi_align_channels_last(self):
        ...
        for i_shape, b_shape, o_size, s_scale, s_ratio in param:
            ...
            boxes_x2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
            boxes_y2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
            boxes = torch.tensor(list(zip(boxes_index, boxes_x1, boxes_y1,
                                          boxes_x2, boxes_y2))).float()
            out = torchvision.ops.roi_align(a_, boxes=boxes, output_size=o_size,
                                            spatial_scale=s_scale, sampling_ratio=s_ratio)
            grad = torch.randn(out.shape, dtype=torch.float)
            out.backward(grad)
    ...
    def test_roi_align_no_dense(self):
        ...
        for i_shape, b_shape, o_size, s_scale, s_ratio in param:
            ...
            boxes = torch.tensor(list(zip(boxes_index, boxes_x1, boxes_y1,
                                          boxes_x2, boxes_y2))).float()
            out = torchvision.ops.roi_align(a[..., ::2], boxes=boxes, output_size=o_size,
                                            spatial_scale=s_scale, sampling_ratio=s_ratio)
            grad = torch.randn(out.shape, dtype=torch.float)
            out.backward(grad)
            grad_cpu = copy.deepcopy(a.grad)
            ...
            out_mlu = torchvision.ops.roi_align(
                a_mlu[..., ::2], boxes=boxes_mlu, output_size=o_size,
                spatial_scale=s_scale, sampling_ratio=s_ratio)
            out_mlu.backward(grad_mlu)
            self.assertTensorsEqual(out, out_mlu.cpu(), 0.003, use_MSE=True)
            self.assertTensorsEqual(grad_cpu, a.grad, 0.003, use_MSE=True)
    ...
    def test_roialign_exception(self):
        ...
        ref_msg = ref_msg + " tensor for argument #2 'rois'; but type MLUFloatType does not equal"
        ref_msg = ref_msg + r" MLUDoubleType \(while checking arguments for cnnl_roi_align\)"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.roi_align(a, boxes=boxes_, output_size=1,
                                      spatial_scale=0.125, sampling_ratio=2)
    ...
    def test_roi_align_with_amp(self):
        ...
        boxes_x2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
        boxes_y2 = np.random.randint(low=15, high=32, size=(b_shape[0],))
        boxes = torch.tensor(list(zip(boxes_index, boxes_x1, boxes_y1, boxes_x2, boxes_y2))).float()
        ...
        with torch.amp.autocast(device_type = 'mlu'):
            out_cpu = torchvision.ops.roi_align(a, boxes=boxes, output_size=o_size,
                                    spatial_scale=s_scale, sampling_ratio=s_ratio)

            out_mlu = torchvision.ops.roi_align(a_mlu, boxes=boxes_mlu.half(), output_size=o_size,
                                    spatial_scale=s_scale, sampling_ratio=s_ratio)
