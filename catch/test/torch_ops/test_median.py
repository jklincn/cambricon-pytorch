...
class TestMedianOps(TestCase):
    ...
    def test_median_dim(self):
        ...
        shape_list = [(1, 32, 5, 12, 8), (2, 32, 10, 6),
                      (2, 32, 8), (1, 100), (24,)]
        func_list = [lambda x:x, self.convert_to_channel_last,
                     lambda x:x[..., ::2]]
        ...
        for test_type, dtype, shape, func in itertools.product(*param_list):
            ...
            for dim in range(0, dim_len):
                ...
                out_cpu = func(x).float().median(
                    dim, keepdim=test_type)
                out_mlu = func(self.to_mlu(x)).median(
                    dim, keepdim=test_type)
        ...
        torch.median(y, 1, keepdim=False, out=(
            values[:, 1], indices[:, 1]))
        values_expected, indices_expected = torch.median(
            y, 1, keepdim=False)

        self.assertTensorsEqual(
            values[:, 1].float().cpu(), values_expected.float().cpu(),
            0.003, use_MSE=True)
        self.assertTensorsEqual(
    ...
    def test_median_exception(self):
        ...
        ref_msg = f"MLU median: elements number of input tensor should be less than 167936"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.median(a)
        ref_msg = f"MLU median: elements number of input tensor should be less than 335872"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.median(b)
