...
class TestAllOp(TestCase):
    ...
    def test_all_dim(self):
        ...
        dim_list_nc = [-2, -1, 3, 1]
        dtype_list_nc = [torch.bool, torch.int8, torch.uint8, torch.int16, torch.int32, torch.int64, \
                      torch.float, torch.half, torch.double]
    ...
    def test_all_out(self):
        ...
        shape_list_nc = [(10,11,20),(2,3,4,8,10), (34, 56, 78, 90), (0, 6)]
        dim_list_nc = [-2, -1, 3, 1]
        dtype_list_nc = [(torch.bool, torch.uint8), (torch.int8, torch.bool),
                      (torch.uint8, torch.uint8), (torch.int32, torch.bool),
                      (torch.int64, torch.uint8), (torch.float, torch.bool),
                      (torch.half, torch.uint8), (torch.double, torch.bool),
                      (torch.int16, torch.uint8)]
        ...
        for dim_, list_ in zip(dim_list_nc, shape_list_nc):
            for dtype in dtype_list_nc:
                for keep in keep_type_nc:
                    ...
                    x_mlu = x_cpu.to('mlu')
                    out_mlu = (torch.rand(list_, dtype=torch.float) > 0.5).to(dtype[1])[:,1:]
                    out_mlu = out_mlu.to('mlu')
    ...
    def test_all_bfloat16(self):
        left = torch.testing.make_tensor((2, 10, 24), dtype=torch.bfloat16, device='cpu')
        left_mlu = left.mlu()
