...
class TestDeformConv2dOps(TestCase):
    ...
    def test_deform_conv2d(self):
        ...
        groups = [1, 2]
        offset_groups = [1, 2]
        product_list = product(input_shapes, kernel_sizes, strides, paddings, dilations,
                               groups, offset_groups)
        for shape, k_size, stride, padding, dilation, group, offset_group in product_list:
            ...
            out_cpu = deform_conv2d(input, offset, weight, bias=bias, stride=stride,
                                    padding=padding, dilation=dilation, mask=mask)
            out_mlu = deform_conv2d(input_mlu, offset_mlu, weight_mlu, bias=bias_mlu,
                        stride=stride, padding=padding, dilation=dilation, mask=mask_mlu)
    ...
    def test_deform_conv2d_channels_last_and_no_dense(self):
        ...
        for func in func_list:
            ...
            out_cpu = deform_conv2d(func(input), func(offset), func(weight),
                                    bias=func(bias), mask=func(mask))
            out_mlu = deform_conv2d(func(input_mlu), func(offset_mlu), func(weight_mlu),
                                    bias=func(bias_mlu), mask=func(mask_mlu))
    ...
    def test_deform_conv2d_type(self):
        ...
        for type in dtype_list:
            ...
            out_cpu = deform_conv2d(input.float(), offset.float(), weight.float(),
                                    bias=bias.float(), mask=mask.float())
            out_mlu = deform_conv2d(input_mlu, offset_mlu, weight_mlu,
                                    bias=bias_mlu, mask=mask_mlu)
            ...
            self.assertTensorsEqual(out_cpu, out_mlu.float().cpu(),
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(input.grad.float(), input_.grad.float(),
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(weight.grad.float(), weight_.grad.float(),
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(offset.grad.float(), offset_.grad.float(),
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(mask.grad.float(), mask_.grad.float(),
                                    0.003, use_MSE=True)
            self.assertTensorsEqual(bias.grad.float(), bias_.grad.float(),
                                    0.003, use_MSE=True)
    ...
    def test_deform_conv2d_spec(self):
        ...
        out_cpu = deform_conv2d(input, offset, weight,
                                bias=bias, mask=mask)
        out_mlu = deform_conv2d(input_mlu, offset_mlu, weight_mlu, bias=bias_mlu,
                                mask=mask_mlu)
    ...
    def test_deform_conv2d_with_amp(self):
        ...
        with torch.autocast(enabled=True, device_type='mlu'):
            ...
            out_cpu = deform_conv2d(
                input, offset, weight,
                bias, stride=(1, 1), padding=(1, 1), mask=mask).half()

            out_mlu = deform_conv2d(
                input.half().mlu(), offset.half().mlu(), weight.half().mlu(),
                bias.mlu(), stride=(1, 1), padding=(1, 1), mask=mask.mlu())
