...
class TestAbsOp(TestCase):
    ...
    def test_abs_contiguous(self):
        ...
        dtype_list = [torch.float,torch.complex64,torch.int]
        ...
        for shape in shape_list:
            for dtype in dtype_list:
                ...
                y = torch.randn(shape, dtype=torch.float32) \
                        if dtype != torch.int else \
                        torch.randn(shape, dtype=torch.float32).to(torch.int)
    ...
    def test_abs_t_channel_last(self):
        shape_list = [(512, 1024, 2, 2, 4), (10, 3, 32, 32), (2, 3, 4),
                      (254, 254, 112, 1, 1, 3)]
        dtype_list = [torch.float,torch.complex64,torch.int32]
        for shape in shape_list:
            for dtype in dtype_list:
                x = torch.randn(shape, dtype=torch.float32).to(dtype)
                x = self.convert_to_channel_last(x)
    ...
    def test_abs_exception(self):
        x = torch.randn((2,3,4,10), dtype=torch.complex64)
        out_mlu = x.clone().to('mlu')
        ref_msg = "Output tensors can't be complex type."
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.abs(x.to('mlu'), out=out_mlu)
