...
class TestOps(TestCase):
    ...
    def test_nll_loss1d_non_batch(self):
        ...
        dtype_err_lst = [(torch.float, 0),
                         (torch.half, 3e-2), (torch.double, 0)]
        ...
        for reduct, C, ignore, weight_flag, dtype_err in product_lst:
            ...
            # when all the elements in output was omited by ignore_index and reduction
            # is mean, cpu will return NaN (0/0), while mlu and gpu return zero.
            if target.item() == ignore and reduct == "mean":
                self.assertTrue(torch.isnan(out_cpu))
                self.assertTensorsEqual(out_mlu.cpu().float(),
                        torch.tensor(0).float(), err, use_MSE=True)
            else:
    ...
    def test_nll_loss_backward(self):  # pylint: disable=R0912
        ...
        for reduct, N, C, ignore, weight_flag, dtype_err in product_lst:
            ...
            layer_mlu = torch.nn.NLLLoss(weight_mlu,
                                         reduction=reduct,
                                         ignore_index=ignore)
            out_mlu = layer_mlu(self.to_mlu_dtype(x_mlu, dtype), target.to(ct.mlu_device()))
            ...
            layer2 = torch.nn.NLLLoss(weight,
                                      reduction=reduct,
                                      ignore_index=ignore)
    ...
    def test_nll_loss_2d_empty(self):
        ...
        for N, H, W, reduction in product_lst:
            ...
            if N != 0 and H != 0 and W != 0:
                ...
                mse_flag = True
            self.assertTensorsEqual(
                output, output_mlu.cpu(), err, use_MSE=mse_flag)
