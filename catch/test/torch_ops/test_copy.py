...
class TestCopyOp(TestCase):
    ...
    def test_copy_H2D_long_double(self):
        ...
        for shape in shape_list:
            for dtype in dtype_list:
                ...
                self.assertTensorsEqual(result_cpu.float(), result_mlu.cpu().float(),
                                        0.0, use_MSE = True)
                # without input slice.
    ...
    def test_copy_complex(self):
        ...
        for dtype in dtype_list:
            ...
            self.assertTensorsEqual(torch.view_as_real(a).float(),
                torch.view_as_real(bm.cpu()).float(), 0.0)
    ...
    def test_device_to_device(self):
        ...
        self.assertTensorsEqual(y_mlu1[:, :, 7:996, :].cpu(),
                                x_mlu[:, :, 7:996, :].cpu(), 0.0,
                                use_MSE = True)
    ...
    def test_copy_not_contiguous_D2H(self):
        shape = (1, 3, 24, 24)
        dtype_list = [torch.float, torch.half, torch.int, torch.short,
                      torch.int8, torch.uint8, torch.double, torch.long,
        ...
        for dtype, func in product(dtype_list, func_list):
            ...
            self.assertTensorsEqual(func(a).float(), a_cpu.float(),
                                    err, use_MSE = True)

    # @unittest.skip("not test")
    @testinfo()
    def test_copy_expand(self):
        shape_list = [((16, 3, 22, 22), (22, 22)),
                      ((3, 4, 8), (1, 8))]
    ...
    def test_non_overlapping_and_dense_H2D(self):
        ...
        for shape, order in shape_order_list:
            ...
            b.copy_(input_cpu)
            self.assertEqual(b.dtype, input_cpu.dtype)
            self.assertTensorsEqual(input_cpu.float(), b.cpu().float(),
                                    0.0, use_MSE = True)
