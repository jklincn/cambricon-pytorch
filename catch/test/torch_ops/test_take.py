...
class TestOps(TestCase):
    ...
    def test_take(self):
        ...
        indices = [torch.tensor([0, 1, 2]), torch.tensor([10, 20 ,40]),
                   torch.tensor([1, 2, 3, 4, 5, 6]), torch.tensor([0, 1, 3, 3]),
    ...
    def test_take_idx(self):
        ...
        self.assertTensorsEqual(
            idx_cpu.float(), idx_mlu.cpu().float(), 0, use_MSE = True)

    # @unittest.skip("not test")
    @testinfo()
    def test_take_backward(self):
        input_cpu = torch.randn((3,4), dtype = torch.float, requires_grad = True)
    ...
    def test_take_exception(self):
        ...
        for idx in indices:
            if (idx >= numel or idx < -numel):
                err_idx = idx
                break
        ref_msg = "out of range: tried to access index "+ str(err_idx.item())\
                  + " on a tensor of " + str(numel) + " elements"
