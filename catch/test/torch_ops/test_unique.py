...
class TestUniqueOp(TestCase):
    def _test_unique(self, input_cpu, input_mlu, sort, inverse, counts, dim=None):
        if inverse and counts:
            output_cpu, inverse_indices_cpu, counts_cpu = torch.unique(input_cpu,
                    sorted=sort, return_inverse=inverse, return_counts=counts, dim=dim)
            ...
            output_cpu, inverse_indices_cpu = torch.unique(input_cpu,
                    sorted=sort, return_inverse=inverse, dim=dim)
            ...
            output_cpu, counts_cpu = torch.unique(input_cpu, sorted=sort, return_counts=counts, dim=dim)
            output_mlu, counts_mlu = torch.unique(input_mlu, sorted=sort, return_counts=counts, dim=dim)
            self.assertTensorsEqual(output_cpu, output_mlu.cpu(), 0)
            self.assertTensorsEqual(counts_cpu, counts_mlu.cpu(), 0)
            self.assertEqual(output_cpu.dtype, output_mlu.dtype)
            self.assertEqual(counts_cpu.dtype, counts_mlu.dtype)
    ...
    def test_unique(self):
        ...
        dim_list = [None, 0]
        loop_var = [type_list, shape_list, sort_list, inverse_list, counts_list, dim_list]
    ...
    def test_unique_exception(self):
        ...
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.unique(x_mlu, dim=0, sorted=True, return_inverse=True, return_counts=True)

        shape = (0, 2, 1, 3)
        x_mlu = torch.randint(0, 64, shape).mlu()
        ref_msg = "There are 0 sized dimensions, and they aren't selected, so unique cannot be applied"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torch.unique(x_mlu, dim=1, sorted=True, return_inverse=True, return_counts=True)
    ...
    def test_unique_spec(self):
        ...
        _, inverse, _ = torch.unique(x, sorted=True,
