...
class TestArgminOp(TestCase):
    ...
    def test_argmin(self):
        dtype_list = [torch.uint8, torch.int8, torch.int16, torch.int32, torch.long, torch.float, torch.half, torch.double]
        shape_list = [(2, 3, 4), (10, 11, 9, 8), (32,),
                      (15, 16, 8, 9, 10, 11), (2, 3, 4, 5, 6, 7, 8), (1, 256), (1, 1, 1)]
    ...
    def test_argmin_out(self):
        input_dtype_list = [torch.uint8, torch.int8, torch.int16, torch.int32, torch.half, torch.float, torch.double]
        outptu_dtype_list = [torch.uint8, torch.int8, torch.int16, torch.int32, torch.half, torch.float, torch.double]
        # [CNNLCORE-13417]dim value is none, cnnlGetReduceOpWorkspaceSize/cnnlReduce output dim <= input dim check error
        # pt1.9 output scalar, but pt1.13 output shape dependency keepdim parameter.
        #dim_list = [1, -1, 0, 2, 3, 6, 7, None]
        dim_list = [1, -1, 0, 2, 3, 6, 7]
        ...
        keepdims = [False, True]
        params_list = [input_dtype_list, outptu_dtype_list, dim_list, keepdims, input_shapes]
        for in_dtype, out_dtype, dim, keepdim, input_shape in product(*params_list):
            ...
            output_mlu = torch.argmin(input = x.mlu(), dim = dim, keepdim=keepdim, out = y_mlu)
    ...
    def test_argmin_bfloat16(self):
        ...
        self.assertTensorsEqual(out_cpu,
                                ...
                                0.0,
                                use_MSE=True)
        output_cpu = torch.testing.make_tensor(out_cpu.shape, dtype=torch.long, device='cpu')
        ...
        self.assertTensorsEqual(output_mlu.cpu(),
                                output_cpu,
                                0.0,
                                use_MSE=True)
        self.assertTensorsEqual(output_mlu.cpu(),
