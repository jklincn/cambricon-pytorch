...
class TestConstantPadNdOp(TestCase):
    ...
    def test_constant_pad_nd_limit_test(self):
        ...
        for dtype_value in dtype_value_list:
            ...
            out_mlu = torch.constant_pad_nd(
                x.mlu(), (1, 2), dtype_value[1])
    ...
    def test_constant_pad_nd_issue(self):
        ...
        device_out = torch.ops.aten.constant_pad_nd(to_non_dense(input.mlu().to(memory_format=torch.channels_last), 2), pad, value)
        torch.testing.assert_close(cpu_out, device_out.cpu())

    # @unittest.skip("not test")
    @testinfo()
    def test_constant_pad_nd_exception(self):
        with self.assertRaisesRegex(RuntimeError, "Length of pad must be even but instead it equals"):
            x = torch.randn((1, 2, 4), dtype=torch.float32).mlu()
            out_mlu = torch.constant_pad_nd(x, (1,), 0.1)

        with self.assertRaisesRegex(RuntimeError, "Length of pad should be no more than twice the number of"):
