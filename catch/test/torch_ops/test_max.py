...
class TestMaxOp(TestCase):
    ...
    def test_max_dim_and_out(self):
        ...
        for func in func_list:
            ...
            for i, _ in enumerate(shape_list):
                ...
                torch.max(func(self.to_device(x)), dim_list[i],keepdim=type_list[i],
                          out=(out_mlu_value, out_mlu_index))
                out_mlu = torch.max(func(self.to_device(x)), dim_list[i],keepdim=type_list[i])
                self.assertTensorsEqual(out_cpu[0].float(), out_mlu_value.cpu().float(),
                                        0.0, use_MSE=True)
    ...
    def test_max(self):
        ...
        func_list = [lambda x:x, self.convert_to_channel_last, lambda x:x[..., ::2]]
        dtype_list = [torch.bool, torch.int8, torch.int16, torch.int, torch.int64, torch.half, torch.float, torch.double]
        for func in func_list:
            for shape in shape_list:
                for dtype in dtype_list:
                    ...
                    self.assertTensorsEqual(out_cpu.float(), out_mlu.float().cpu(), 0.0, use_MSE=True)
        # test scalar
        x = torch.randn(())
        out_cpu = torch.max(x)
    ...
    def test_max_permute(self):
        ...
        for i in range (4):
            ...
            x, y, out = x.permute(permute_shape[i]), \
                        y.permute(permute_shape[i]), out.permute(permute_shape[i])
    ...
    def test_max_no_contiguous_tensor(self):
        ...
        self.assertTensorsEqual(values[:, 1].cpu(), values_expected.cpu(), 0.0, use_MSE=True)
        self.assertTensorsEqual(indices[:, 1].cpu(), indices_expected.cpu(), 0.0, use_MSE=True)
    ...
    def test_max_bfloat16(self):
        left = torch.testing.make_tensor((1, 10, 24), dtype=torch.bfloat16, device='cpu')
        right = torch.testing.make_tensor((64, 10, 1), dtype=torch.bfloat16, device='cpu')
