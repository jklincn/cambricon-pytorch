...
from common_utils import testinfo, TestCase, TEST_BFLOAT16  # pylint: disable=C0413,C0411
...
class Storage(TestCase):
    ...
    def test_storage_clone(self):
        if torch.mlu.device_count()>1:
            ...
            x = torch.randn(4, 4, device='mlu:1')._storage()
            y = x.clone()
            self.assertTrue(x.get_device()==y.get_device())
    ...
    def test_serialization_array_with_storage(self):
        ...
        self.assertTrue(isinstance(q_copy[3]._storage, torch.mlu.UntypedStorage))
        q_copy[1].fill_(10)
    ...
    def test_has_storage(self):
        ...
        self.assertIsNotNone(torch.tensor([]).mlu()._storage())
        self.assertIsNotNone(torch.empty(0).mlu()._storage())
        self.assertIsNotNone(torch.tensor([]).clone().mlu()._storage())
        self.assertIsNotNone(torch.tensor([0, 0, 0]).nonzero().mlu()._storage())
        self.assertIsNotNone(torch.tensor([]).new().mlu()._storage())
    ...
    def test_storage_casts(self):
        ...
        self.assertEqual(storage.type(), 'torch_mlu.IntStorage')
        ...
        self.assertEqual(floatStorage.type(), 'torch_mlu.FloatStorage')
        ...
        self.assertEqual(halfStorage.type(), 'torch_mlu.HalfStorage')
        ...
        self.assertEqual(longStorage.type(), 'torch_mlu.LongStorage')
        ...
        self.assertEqual(shortStorage.type(), 'torch_mlu.ShortStorage')
        ...
        self.assertEqual(doubleStorage.type(), 'torch_mlu.DoubleStorage')
        ...
        self.assertEqual(charStorage.type(), 'torch_mlu.CharStorage')
        ...
        self.assertEqual(byteStorage.type(), 'torch_mlu.ByteStorage')
        ...
        self.assertEqual(complexdouble_storage.type(), 'torch_mlu.ComplexDoubleStorage')
    ...
    def test_sizeof(self):
        ...
        sizeof_empty = torch.randn(0).mlu()._storage().__sizeof__()
        sizeof_10 = torch.randn(10).mlu()._storage().__sizeof__()
        sizeof_100 = torch.randn(100).mlu()._storage().__sizeof__()
        self.assertEqual((sizeof_100 - sizeof_empty) // (sizeof_10 - sizeof_empty), 10)
        self.assertEqual((sizeof_100 - sizeof_empty) % (sizeof_10 - sizeof_empty), 0)

        sizeof_empty = torch.randn(0).to(torch.uint8).mlu()._storage().__sizeof__()
        sizeof_10 = torch.randn(10).to(torch.uint8).mlu()._storage().__sizeof__()
        sizeof_100 = torch.randn(100).to(torch.uint8).mlu()._storage().__sizeof__()
        self.assertEqual((sizeof_100 - sizeof_empty) // (sizeof_10 - sizeof_empty), 10)
        self.assertEqual((sizeof_100 - sizeof_empty) % (sizeof_10 - sizeof_empty), 0)
    ...
    def test_storage(self):
        element_size_dtypes = {torch.int64 : torch.int32,
                               torch.float64 : torch.float32,
                               torch.complex128 : torch.complex64}
        dtypes = [torch.int8, torch.uint8, torch.int16, torch.int32,
                  torch.bool, torch.float32, torch.complex64]
        for dtype in dtypes:
            v = make_tensor((3, 5), dtype=dtype, device='mlu', low=-9, high=9)
            ...
            for el_num in range(v.numel()):
                ...
                dim1 = el_num % v.size(1)
                self.assertEqual(
                    bytes_to_scalar(v_s_byte[start:end], dtype, 'mlu'),
                    v[dim0][dim1])
    ...
    def test_has_storage_numpy(self):
        for dtype in [np.float32, np.float64, np.int64,
            ...
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.float32).storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.double).storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.int).storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.long).storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.uint8).storage())

            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.float32)._storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.double)._storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.int)._storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.long)._storage())
            self.assertIsNotNone(torch.tensor(arr, device="mlu", dtype=torch.uint8)._storage())
    ...
    def test_storage_pin_memory_and_is_pinned(self):
        ...
        y_UntypedStorage= x_storage.pin_memory()
    ...
    def test_storage_resize(self):
        ...
        self.assertEqual(x[0], x_clone[0], atol=1.e-12, rtol=1.e-12)
        self.assertEqual(x[1], x_clone[1], atol=1.e-12, rtol=1.e-12)

        x = torch.tensor([2.2, 3.1], device="mlu")._storage()
        self.assertEqual(x.size(), 16)
        x_clone = x.clone()
        x.resize_(32)
        self.assertEqual(x.size(), 32)
        self.assertEqual(x[0], x_clone[0], atol=1.e-12, rtol=1.e-12)
        self.assertEqual(x[15], x_clone[15], atol=1.e-12, rtol=1.e-12)

        x.resize_(0)
        self.assertEqual(x.size(), 0, atol=1.e-12, rtol=1.e-12)
    ...
    def test_storage_new_with_weak_ptr(self):
        for device in ['cpu', 'mlu']:
            tmp = torch.randn(2,2, dtype=torch.float32).storage()._storage.tolist()
    ...
    def test_storage_new_shared_fd_cpu(self):
        a = torch.mlu.UntypedStorage([189, 0, 89, 63,
                                      227, 64, 194, 61,
                                      7, 234, 129, 63,
                                      29, 147, 120, 63], device='cpu')
    ...
    def test_tensor_set_errors(self):
        ...
        if torch.mlu.is_available():
            ...
            self.assertRaises(RuntimeError, lambda: f_mlu.set_(f_cpu.storage(), 0, f_cpu.size(), f_cpu.stride()))
    ...
    def test_from_buffer(self):
        for order in ['big', 'little', 'native']:
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order=order, dtype=torch.uint8)
            untyped_cpu = torch.UntypedStorage.from_buffer(a, byte_order=order, dtype=torch.uint8)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order=order, dtype=torch.short)
            untyped_cpu = torch.UntypedStorage.from_buffer(a, byte_order=order, dtype=torch.short)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x40, 0x10, 0x00, 0x00])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float32)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float32)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.bool)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.bool)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float64)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float64)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.int32)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.int32)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.int64)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.int64)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float16)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.float16)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.bfloat16)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.bfloat16)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
            f = bytearray([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10, 0x40])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.complex64)
            untyped_cpu = torch.UntypedStorage.from_buffer(f, byte_order=order, dtype=torch.complex64)
            self.assertEqual(untyped_mlu.tolist(), untyped_cpu.tolist())
    ...
    def test_pynew(self):
        ...
        self.assertEqual(floatStorage.type(), 'torch_mlu.FloatStorage')
        self.assertIs(floatStorage.dtype, torch.float32)
        self.assertIs(floatStorage.element_size(), torch.mlu.FloatTensor().element_size())
        ...
        self.assertEqual(complexfloatStorage.type(), 'torch_mlu.ComplexFloatStorage')
        self.assertIs(complexfloatStorage.dtype, torch.complex64)
        self.assertIs(complexfloatStorage.element_size(),torch.tensor([], dtype=torch.complex64).element_size())

        complexdoubleStorage = torch.mlu.ComplexDoubleStorage([1+1j,2-1j,3,4])
        self.assertEqual(complexdoubleStorage.size(), 4)
        self.assertEqual(complexdoubleStorage.type(), 'torch_mlu.ComplexDoubleStorage')
        self.assertIs(complexdoubleStorage.dtype, torch.complex128)
        self.assertIs(complexdoubleStorage.element_size(),torch.tensor([], dtype=torch.complex128).element_size())
    ...
    def test_copy_(self):
        ...
        self.assertEqual(a, b.reshape([2,2]), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.char(), b.char(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.byte(), b.byte(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.double(), b.double(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.long(), b.long(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.short(), b.short(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.half(), b.half(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(a.bool(), b.bool(), atol=1.e-12, rtol=1.e-12)
    ...
    def test__write_and_new_with_file(self):
        ...
        self.assertEqual(tensor_a, tensor_c.reshape(4,5), atol=1.e-12, rtol=1.e-12)
        with open('_write_file.pt', 'wb') as writer:
            tensor_a._storage()._write_file(writer, True, True, torch._utils._element_size(tensor_a.dtype))
        with open('_write_file.pt', 'rb') as reader:
            storage_b = torch.mlu.UntypedStorage._new_with_file(reader, torch._utils._element_size(tensor_a.dtype))
        tensor_b = torch.mlu.FloatTensor(storage_b)
        self.assertEqual(tensor_a, tensor_b.reshape(4,5), atol=1.e-12, rtol=1.e-12)

        untyped_a = torch.mlu.UntypedStorage([1,2,3,4], device='cpu')
        with open('_write_file_1.pt', 'wb') as writer:
            untyped_a._write_file(writer, False, True, torch._utils._element_size(torch.float16))
        with open('_write_file_1.pt', 'rb') as reader:
            untyped_b = torch.mlu.UntypedStorage._new_with_file(reader, torch._utils._element_size(torch.float16))
        self.assertEqual(untyped_a.tolist(), untyped_b.tolist(), atol=1.e-12, rtol=1.e-12)

        untyped_a = torch.mlu.UntypedStorage([1,2,3,4], device='cpu')
        with open('_write_file_1.pt', 'wb') as writer:
            untyped_a._write_file(writer, True, True, torch._utils._element_size(torch.float16))
        with open('_write_file_1.pt', 'rb') as reader:
            untyped_b = torch.mlu.UntypedStorage._new_with_file(reader, torch._utils._element_size(torch.float16))
        self.assertEqual(untyped_a.tolist(), untyped_b.tolist(), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(tensor_a, tensor_b.double().reshape(4,5), atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(tensor_a, tensor_b.reshape(4,5), atol=1.e-12, rtol=1.e-12)
        self.assertEqual(tensor_a.storage(), typedstorage_b, atol=1.e-12, rtol=1.e-12)
        ...
        self.assertEqual(tensor_a.storage(), typedstorage_b, atol=1.e-12, rtol=1.e-12)
        self.assertEqual(typedstorage_b.type(), 'torch_mlu.ComplexDoubleStorage')
    ...
    def test__set_from_file(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            tensor_a._storage()._write_file(f, True, True, torch._utils._element_size(tensor_a.dtype))
            tensor_b._storage()._write_file(f, True, True, torch._utils._element_size(tensor_b.dtype))
            tensor_c._storage()._write_file(f, True, True, torch._utils._element_size(tensor_c.dtype))
            ...
            tensor_a_shadow._storage()._set_from_file(f, 0, True, torch._utils._element_size(tensor_a.dtype))
            tensor_b_shadow._storage()._set_from_file(f, None, True, torch._utils._element_size(tensor_b.dtype))
            tensor_c_shadow._storage()._set_from_file(f, None, True, torch._utils._element_size(tensor_c.dtype))

            self.assertEqual(tensor_a, tensor_a_shadow)
            self.assertEqual(tensor_b, tensor_b_shadow)
            self.assertEqual(tensor_c, tensor_c_shadow)
    ...
    def test_new_with_file1(self):
        ...
        y = torch.tensor([3., 4.], dtype = torch.float)
        ...
        with tempfile.NamedTemporaryFile() as f:
            x._storage()._write_file(f, True, True, torch._utils._element_size(x.dtype))
            y._storage()._write_file(f, True, True, torch._utils._element_size(y.dtype))
            z._storage()._write_file(f, True, True, torch._utils._element_size(z.dtype))
            w._storage()._write_file(f, True, True, torch._utils._element_size(w.dtype))
            f.seek(0)
            x_untypedstorage = torch.mlu.UntypedStorage._new_with_file(f, torch._utils._element_size(x.dtype))
            y_untypedstorage = torch.mlu.UntypedStorage._new_with_file(f, torch._utils._element_size(y.dtype))
            z_untypedstorage = torch.mlu.UntypedStorage._new_with_file(f, torch._utils._element_size(z.dtype))
            w_untypedstorage = torch.mlu.UntypedStorage._new_with_file(f, torch._utils._element_size(w.dtype))
            ...
            z_typedstorage = torch.mlu.ComplexDoubleStorage(wrap_storage = z_untypedstorage)
    ...
    def test_new_with_file2(self):
        x = torch.tensor([1., 2.], dtype = torch.double, device="mlu")
        y = torch.tensor([3., 4.], dtype = torch.float, device="mlu")
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            w._storage()._write_file(f, True, True, torch._utils._element_size(w.dtype))
            f.seek(0)
            x_untypedstorage = torch.UntypedStorage._new_with_file(f, torch._utils._element_size(x.dtype))
            y_untypedstorage = torch.UntypedStorage._new_with_file(f, torch._utils._element_size(y.dtype))
            z_untypedstorage = torch.UntypedStorage._new_with_file(f, torch._utils._element_size(z.dtype))
            w_untypedstorage = torch.UntypedStorage._new_with_file(f, torch._utils._element_size(w.dtype))
    ...
    def test_save_load_1(self):
        for is_new in [False, True]:
            ...
            y = torch.tensor([3., 4.], dtype = torch.complex128)
    ...
    def test_save_load_2(self):
        for is_new in [False, True]:
            ...
            y = torch.tensor([3., 4.], dtype = torch.complex128, device='mlu')
            w = torch.tensor([3., 4.], dtype = torch.float, device="mlu")
            q = [x.storage(), y.storage(), w.storage()]
            with tempfile.NamedTemporaryFile() as f:
                torch.save(q, f, _use_new_zipfile_serialization=is_new)
                f.seek(0)
    ...
    def test_storage_get_set(self):
        ...
        a_s[4] = 6.0
        self.assertEqual(a[1][1], 6.0, atol=1.e-12, rtol=1.e-12)
    ...
    def test_storage_copy_(self):
        ...
        b = torch.zeros(2, 2, device="mlu")
        b.storage().copy_(a.storage())
        self.assertTensorsEqual(a, b.cpu(), 1.e-12, use_MSE=True)
    ...
    def test_untyped_get_set(self):
        ...
        self.assertEqual(a[0], b[0], atol=1.e-12, rtol=1.e-12)
        self.assertEqual(a[1], b[1], atol=1.e-12, rtol=1.e-12)
        self.assertEqual(a[0:2].tolist(), b[0:2].tolist(), atol=1.e-12, rtol=1.e-12)
        self.assertEqual(a[2:-1].tolist(), b[2:-1].tolist(), atol=1.e-12, rtol=1.e-12)
        b[4] = 6
        self.assertEqual(b[4], 6, atol=1.e-12, rtol=1.e-12)

    # @unittest.skip("not test")
    @testinfo()
    def test_storage_exception(self):
        ref_msg = r"torch.mlu.UntypedStorage\(\): This Storage is only suggested applicable to MLU devices, but gives cuda"
        ...
        with self.assertRaisesRegex(ValueError, ref_msg):
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order='native', offset=-1, dtype=torch.uint8)

        ref_msg = r"invalid byte_order 'error' \(expected 'big', 'little', or 'native'\)"
        with self.assertRaisesRegex(ValueError, ref_msg):
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order='error', dtype=torch.float32)

        ref_msg = r"buffer size \( 4\) must be a multiple of element size \(4\)"
        with self.assertRaisesRegex(ValueError, ref_msg):
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order='native', count=-1, offset=3, dtype=torch.float32)

        ref_msg = r"buffer has only 1 elements after offset 3, but specified a size of 5"
        with self.assertRaisesRegex(ValueError, ref_msg):
            a = bytearray([1, 2, 3, 4])
            untyped_mlu = torch.mlu.UntypedStorage.from_buffer(a, byte_order='native', count=5, offset=3, dtype=torch.float32)
    ...
    def test_storage_64bit_raise_warn(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            self.assertNotWarnRegex(lambda: hi.load_pickle("obj", "obj.pkl"), warning_msg)
