...
class TestDataLoader(TestCase):
    ...
    def _test_generator(self, num_workers, batch_size, pin_memory, pin_memory_device, non_blocking):
        loader = DataLoader(self.dataset, batch_size=batch_size, num_workers=num_workers, \
        ...
        for input_t, target in loader:
            ...
            self.assertTensorsEqual(input_t.cpu(), input_cpu, 0., use_MSE=True)
            self.assertTensorsEqual(target.cpu(), target_cpu, 0., use_MSE=True)
                ...
                for num_workers in [0, 2]: #0 is single process; 1 is multi process
                    for non_blocking in [True, False]:
                        for batch_size in [1, 2]:
                            self._test_generator(num_workers, batch_size, \
                                                 pin_memory, pin_memory_device, non_blocking)
