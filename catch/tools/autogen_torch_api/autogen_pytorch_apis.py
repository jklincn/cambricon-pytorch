...
SECTION_MAP_PATTERN = re.compile(r"class=\"reference internal\" href=\"(.*)\.html\">([^<]*)<")
HREF_PATTERN = re.compile(r"<li><a href=\"(.*?)\"")
CURRENT_SECTION_PATTERN = re.compile(r"\"toctree-l1 current\"><a class=\"reference internal\" href=\"[\./]*([^\n]*).html\"")

parser = argparse.ArgumentParser(
    description='Automatically output Pytorch Python API list in reST format to the screem '
                'from official docs of Pytorch.'
)
parser.add_argument('pytorch_version', help='the version of Pytorch you want to generate API list.')
...
def section_search(line, matched_api, section2apis):
    ...
    if section_match:
        ...
        logging.warning("Can not find which section this api " + matched_api +
    ...
    if api_match:
        ...
        if line.startswith("<li><a href=\"generated/"):
            ...
            if not section_match:
                logging.error("Can not find section name, please check this line: " + line)
                sys.exit(1)
...
for k, v in section2apis.items():
    ...
    if k == "Others":
        ...
        elif val == "torch.distributed.elastic" or val == "torch.distributions" or \
             val == "DDP Communication Hooks" or val == "Quantization":
            print("    :widths: 8 2 4")
        else:
            print("    :widths: 8 2 8")
    print("    :header-rows: 1")
