FROM yellow.hub.cambricon.com/pytorch/base/x86_64/pytorch:v0.1-x86_64-ubuntu18.04
...
RUN apt-get install -y wget aptitude vim make git tcl-dev tk-dev software-properties-common libsqlite3-dev libopenblas-dev \
...
RUN cd /tmp/extra-dependencies && \
    ...
    cmake -DGFLAGS_NAMESPACE=google -DCMAKE_CXX_FLAGS=-fPIC -DBUILD_SHARED_LIBS=BOOL=ON -DCMAKE_CXX_FLAGS="-std=c++14 -D _GLIBCXX_USE_CXX11_ABI=0" .. && \
	make -j && make install
## glog
RUN cd /tmp/extra-dependencies && \
...
RUN wget -qO - https://www.mellanox.com/downloads/ofed/RPM-GPG-KEY-Mellanox | apt-key add - && \
    ...
    wget -q -nc --no-check-certificate -P /etc/apt/sources.list.d https://linux.mellanox.com/public/repo/mlnx_ofed/5.3-1.0.0.1/ubuntu18.04/mellanox_mlnx_ofed.list && \
    apt-get update -y && \
...
RUN cd /etc/libibverbs.d && \
    find * | grep -v mlx5.driver | xargs rm
